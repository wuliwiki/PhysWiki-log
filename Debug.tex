\documentclass[zihao=-4,openany,fancyhdr,UTF8]{ctexbook}
\usepackage{amsmath,amssymb,amsthm}
\usepackage{physics} % bra ket 符号等
\usepackage{graphicx}
\usepackage{wrapfig}
\usepackage{xcolor}
\usepackage[framemethod=TikZ]{mdframed}
\usepackage[final]{pdfpages}
\usepackage{listings} % 代码排版
\usepackage{bm} % 用于矢量的粗体
\input{./others/format}
\usepackage{etoolbox} % 新增
\setboolean{@twoside}{false} % 封面只占一面
\usepackage[capitalize,nameinlink,noabbrev]{cleveref} % 新增
\usepackage{enumitem}  % \begin{enumerate}[resume] 可以接着之前的编号
\begin{document}

\input{./others/MatlabStyle} % 设置 Matlab 颜色
\frontmatter % 开始罗马数字页码
\setcounter{tocdepth}{1}
\tableofcontents % 生成目录
\mainmatter % 开始阿拉伯数字页码

\part{创作中}
\chapter{创作中}
%====================================
%\entry{矢量的导数\ 偏导数\ 求导法则}{DerV}\newpage



%\entry{}{}\newpage
%\Entry{}{}\newpage

%============================
\chapter{修改审阅中}
\entry{一阶齐次线性微分方程}{ODE1}\newpage
\entry{常微分方程}{ODE}\newpage
\entry{一元函数的微分}{Diff}\newpage
\entry{积分表}{ITable}\newpage
\entry{质心\ 质心系}{CM}\newpage
\entry{二体系统}{TwoBD}\newpage
\entry{受阻落体}{RFall}\newpage

%\entry{Matlab编程基础}{Matlab}\newpage
%\entry{弹簧振子受迫运动的简单数值计算}{SHOFN}\newpage
%\entry{位置矢量}{Disp}\newpage
%\entry{势能}{V}\newpage
%\entry{简谐振子}{SHO}\newpage
%\entry{基本初等函数的导数}{FunDer}\newpage
%\entry{二项式定理（非整数幂）}{BiNorR}\newpage
%\entry{不定积分}{Int}\newpage
%\entry{积分表}{ITable}\newpage

\entry{本书编写规范}{Sample}\newpage


%\entry{角动量定理\ 角动量守恒}{AMLaw}\newpage
%\entry{梯度\ 梯度定理}{Grad}\newpage
%\entry{力场\ 势能}{V}\newpage
%\entry{全微分}{TDiff}\newpage
%\entry{功\ 功率}{Fwork}\newpage
%\entry{方向导数}{DerDir}\newpage
%\entry{牛顿--莱布尼兹公式}{NLeib}\newpage
%\entry{线积分}{IntL}\newpage
%\entry{动能\ 动能定理（单个质点）}{KELaw1}\newpage
%\entry{动量\ 动量定理（单个质点）}{PLaw1}\newpage


% 小时物理笔记


%%%%%%%%%%%%%    Phys Wiki      %%%%%%%%%%%%%%

% \part{数学}
%=======================================
%\chapter{数学拾遗}
%\entry{二项式定理}{BiNor}
%\entry{充分必要条件}{SufCond}
%\entry{二项式定理（非整数幂）}{BiNorR}\newpage
%\Entry{选择的展开定理}{ChExpn}\newpage
%\entry{极坐标系}{Polar}\newpage
%\entry{柱坐标系}{Cylin}\newpage
%\entry{球坐标系}{Sph}\newpage
%\entry{球坐标与直角坐标的转换}{SphCar}\newpage
%\entry{圆锥曲线的极坐标方程}{Cone}\newpage
%\entry{椭圆}{Elips3}\newpage
%\entry{双曲线}{Hypb3}\newpage
%\entry{复变函数}{Cplx}
%\entry{指数函数（复数）}{CExp}
%\entry{三角函数（复数）}{CTrig}\newpage

%\chapter{微积分}
%%------------------------------------------------------------------------------
%\entry{微积分导航}{Calc}
%\entry{极限}{Lim}\newpage
%\entry{小角正弦极限}{LimArc}
%\entry{自然对数底}{E}
%\entry{切线与割线}{TanL}\newpage
%\entry{导数}{Der}\newpage
%\entry{求导法则}{DerRul}\newpage
%\entry{反函数求导}{InvDer}\newpage
%\entry{基本初等函数的导数}{FunDer}\newpage
%\entry{导数与函数极值}{DerMax}\newpage
%\entry{用极值点确定函数图像}{DerImg}\newpage
%\entry{一元函数的微分}{Diff}\newpage
%\entry{一元复合函数求导（链式法则）}{ChainR}\newpage
%% 未完成 \entry{复合函数求导（链式法则）}{ChainR}\newpage
%\entry{泰勒展开}{Taylor}\newpage
%\entry{不定积分}{Int}\newpage
%\entry{积分表}{ITable}\newpage
%\entry{定积分}{DefInt}\newpage
%\entry{牛顿--莱布尼兹公式}{NLeib}\newpage
%\entry{换元积分法}{IntCV}\newpage
%\entry{分部积分法}{IntBP}\newpage
%\entry{多维球体的体积}{NDimSphV}\newpage
%\entry{二阶常系数齐次微分方程}{Ode2}
%\entry{二阶常系数非齐次微分方程}{Ode2N}
%\entry{正交函数系}{Fbasis}\newpage
%\entry{傅里叶级数（三角）}{FSTri}\newpage
%\entry{傅里叶级数（指数）}{FSExp}\newpage
%\entry{偏导数}{ParDer}\newpage
%\entry{全微分}{TDiff}\newpage
%\entry{复合函数的偏导\ 链式法则}{PChain}\newpage
%\entry{全导数}{TotDer}\newpage

%\chapter{线性代数}
%%------------------------------------------------------------------------------
%% 未完成: 高中矢量内容回顾
%\entry{线性代数导航}{Vector}\newpage
%\entry{矢量点乘}{Dot}
%\entry{矢量叉乘}{Cross}\newpage
%\entry{矢量叉乘分配律的几何证明}{CrossP}
%\entry{连续叉乘的化简}{TriCro}\newpage
%\entry{三矢矢积}{TriVM}
%
%\entry{正交归一基}{OrNrB}
%\entry{平面旋转变换}{Rot2DT}
%\entry{线性变换}{LTrans}\newpage
%\entry{矩阵}{Mat}\newpage
%\entry{平面旋转矩阵}{Rot2D}
%\entry{行列式}{Deter}\newpage

%\chapter{多元微积分}
%%------------------------------------------------------------------------------
%\entry{方向导数}{DerDir}\newpage
%\entry{矢量求导}{DerV}
%\entry{矢量场}{Vfield}
%\entry{极坐标中单位矢量的偏导}{Dpol1}
%\entry{极坐标中的矢量偏导}{Dpol}
%\entry{线积分}{IntL}


%\chapter{计算物理}
%%------------------------------------------------------------------------------
%\entry{计算物理导航}{NumPhy}\newpage
%\entry{Matlab 编程基础}{Matlab}\newpage
%\entry{Matlab 数值验证非整数二项式定理}{BiNorM}\newpage
%\entry{弹簧振子受迫运动的简单数值计算}{SHOFN}\newpage
%% 未完成 Mathematica
%% 未完成 Wolfram Alpha
%% 未完成 Wikipedia



%\part{力学}
%%=======================================

%\chapter{质点}
%%------------------------------------------------------------------------------
%\entry{位置矢量}{Disp}
%\entry{速度\ 加速度（矢量）}{VnA}
%\entry{匀速圆周运动的速度（几何法）}{CMVG}
%\entry{匀速圆周运动的速度（求导法）}{CMVD}\newpage
%\entry{匀速圆周运动的加速度（几何法）}{CMAG}
%\entry{匀速圆周运动的加速度（求导法）}{CMAD}\newpage
%\entry{牛顿运动定律\ 惯性系}{New3}
%\entry{功\ 功率}{Fwork}\newpage
%\entry{动能\ 动能定理（单个质点）}{KELaw1}\newpage
%\entry{动量\ 动量定理（单个质点）}{PLaw1}\newpage
%\entry{简谐振子}{SHO}
%% 未完成: 单摆
%\entry{受阻简谐振子}{SHOf}\newpage
%\entry{惯性力}{Iner}
%\entry{离心力}{Centri}\newpage
%\entry{科里奥利力}{Corio}\newpage
%\entry{地球表面的科里奥利力}{ErthCf}\newpage

%\chapter{质点系与刚体}
%%------------------------------------------------------------------------------
%
%% 未完成: 质点系
%\entry{质心\ 质心系}{CM}\newpage
%\entry{二体系统}{TwoBD}\newpage
%% 未完成: 质心系中的碰撞
%\entry{质点系的动量}{SysP}\newpage
%\entry{动量定理\ 动量守恒}{PLaw}\newpage
%\entry{质点系的动能\ 柯尼西定理}{Konig}
%\entry{刚体}{RigBd}\newpage 未完成(包括转动方程, 类比平动方程) (包括平衡点)
%\entry{力矩}{Torque}\newpage
%\entry{刚体的静力平衡}{RBSt}\newpage %(强调这是动力学的结果, 并不是公理)
%\entry{角动量}{AngMom}\newpage
%\entry{角动量定理\ 角动量守恒（单个质点）}{AMLaw1}\newpage
%\entry{角动量定理\ 角动量守恒}{AMLaw}\newpage
%%\entry{刚体\ 刚体的转动}{}\newpage
%% 介绍刚体的定义, 质点系, 角加速度, 等
%\entry{刚体的转动\ 转动惯量}{RigRot}\newpage
%\entry{常见几何体的转动惯量}{ExMI}\newpage
%\entry{刚体的运动方程}{RBEM}\newpage
%\entry{浮力}{Buoy}

%\chapter{振动与波动}
%%------------------------------------------------------------------------------
%% 未完成：应该把简谐运动放到这里
%\entry{平面波的复数表示}{CPWave}
%% 未完成 \entry{一维波动方程}
%% 未完成 边界条件 (两条密度不同的绳子)

%\chapter{万有引力与天体运动}
%%----------------------------------------------------------------------
%\entry{万有引力\ 引力势能}{Gravty}\newpage
%% 未完成: 质心系, 约化质量
%\entry{开普勒问题}{CelBd}\newpage
%\entry{反开普勒问题}{InvKep}\newpage
%% 散射, 定义微分截面 % d sigma/d Omega 真的就是面积元比对应的立体角元
%\entry{罗瑟福散射}{RuthSc}\newpage
%% 未完成: 开普勒三定律
%\Entry{开普勒第一定律的证明}{Keple1}\newpage
%\Entry{开普勒第二定律的证明}{Keple2}\newpage
%\Entry{开普勒第三定律的证明}{Keple3}\newpage
%\Entry{比耐公式}{Binet}\newpage
%% 未完成: 三体问题

%\chapter{分析力学}
%%------------------------------------------------------------------------------
%\Entry{拉格朗日方程}{Lagrng}\newpage
%\Entry{哈密顿原理}{HamPrn}\newpage



%%=======================================
%\part{其他分册预览}
%
%\chapter{数学}
%\Entry{堆放排列组合}{StackC}\newpage
%\Entry{雅可比行列式}{JcbDet}\newpage
%\Entry{$\Gamma$ 函数}{Gamma}\newpage
%\Entry{高斯分布（正态分布）}{GausPD}\newpage
%\Entry{散度\ 散度定理}{Divgnc}\newpage
%\Entry{柱坐标系中的拉普拉斯方程}{CylLap}\newpage
%\Entry{球坐标系中的梯度散度旋度及拉普拉斯算符}{SphNab}\newpage
%\Entry{勒让德多项式的生成函数}{Legen}\newpage
%\Entry{球谐函数}{SphHar}\newpage
%\Entry{贝赛尔函数}{Bessel}\newpage
%\Entry{球贝塞尔函数}{SphBsl}\newpage
%\Entry{傅里叶变换（指数）}{FTExp}\newpage
%\Entry{离散傅里叶变换}{DFT}\newpage
%\Entry{证明闭合曲面的法向量面积分为零}{CSI0}\newpage
%\Entry{平均值的不确定度}{MeanS}\newpage
%%------------------------------------------------------------------------------
%\chapter{理论力学}
%\Entry{经典力学笔记}{ClsMec}\newpage
%
%%------------------------------------------------------------------------------
%\chapter{电动力学}
%\Entry{电流}{I}
%\Entry{电荷守恒\ 电流连续性方程}{ChgCsv}\newpage
%\Entry{LC 振荡电路}{LC}\newpage
%\Entry{比奥萨伐尔定律}{BioSav}\newpage
%\Entry{洛伦兹力}{Lorenz}\newpage
%\Entry{磁场的能量}{BEng}\newpage
%\Entry{磁通量的定义}{BFlux}\newpage
%\Entry{磁旋比 \  玻尔磁子}{BohMag}\newpage
%\Entry{安培力}{FAmp}\newpage
%\Entry{磁场中闭合电流的合力}{EBLoop}\newpage
%\Entry{闭合电流在磁场中的力矩}{EBTorq}\newpage
%\Entry{电磁场的动量守恒 \  动量流密度张量}{EBP}\newpage
%\Entry{电场的高斯定理}{EGauss}\newpage
%\Entry{法拉第电磁感应定律}{FaraEB}\newpage
%\Entry{电磁场的能量守恒\ 坡印廷矢量}{EBS}\newpage
%\Entry{非齐次亥姆霍兹方程\ 推迟势}{RetPot}\newpage
%\Entry{电场波动方程}{EWEq}\newpage
%\Entry{介质中的波动方程}{MedWF}\newpage
%\Entry{菲涅尔公式}{Fresnl}\newpage
%\Entry{盒中的电磁波}{EBBox}\newpage
%\Entry{拉格朗日电磁势}{EMLagP}\newpage
%
%
%%------------------------------------------------------------------------------
%\chapter{量子力学}
%% 考虑一下选取什么样的讲解顺序？ 大概就是先介绍线性代数（包括离散和连续的矢量空间，%狄拉克符号），介绍量子力学的基本公设（本征方程，算符，时间演化三步法），德布罗意波，%然后推导平均值公式，位置动量表象的变换，束缚态，散射，等等
%
%% 未完成 讲讲为什么定态波函数一定是实数函数？ 实数波函数为什么会有动量平均值为零？
%% 未完成 x,p不确定原理，一般的不确定原理, 高斯波包时取等号．
%% 未完成 束缚态的一般性质: 节点数, 对称性 (偶势能的基态是偶函数), 简并性(一维情况不简并)
%
%\Entry{原子单位}{AU}\newpage
%\Entry{电子轨道与元素周期表}{Ptable}\newpage
%\Entry{玻尔原子模型}{BohrMd}\newpage
%\Entry{类氢原子的约化质量}{HRMass}\newpage
%\Entry{康普顿散射}{Comptn}\newpage
%\Entry{概率流密度}{PrbJ}\newpage
%\Entry{能量归一化}{EngNor}\newpage
%\Entry{氢原子基态的波函数}{HWF0}\newpage
%\Entry{算符对易与共同本征函数}{Commut}\newpage
%\Entry{算符的矩阵表示}{OpMat}\newpage
%\Entry{无限深势阱}{ISW}\newpage
%\Entry{有限深球势阱}{FiSph}\newpage
%\Entry{升降算符}{RLop}\newpage
%\Entry{简谐振子（升降算符）}{QSHOop}\newpage
%\Entry{简谐振子升降算符归一化}{RLNor}\newpage
%\Entry{简谐振子升降算符归一化}{QSHOnr}\newpage
%\Entry{简谐振子（级数）}{QSHOxn}\newpage
%\Entry{高斯波包}{GausWP}\newpage
%\Entry{轨道角动量}{QOrbAM}\newpage
%\Entry{轨道角动量升降算符归一化}{QLNorm}\newpage
%\Entry{自旋角动量}{Spin}\newpage
%\Entry{直积空间}{DirPro}\newpage
%\Entry{角动量加法}{AMAdd}\newpage
%\Entry{球坐标和柱坐标中的径向方程}{RadSE}\newpage
%\Entry{数值解薛定谔方程}{NSES}\newpage
%\Entry{氢原子的波函数}{HWF}\newpage
%\Entry{含时微扰理论}{TDPT}\newpage
%\Entry{几种含时微扰}{TDPEx}\newpage
%\Entry{含连续态的微扰理论}{PTCont}\newpage
%\Entry{量子散射\ 分波展开}{ParWav}\newpage
%\Entry{波恩近似（散射）}{BornSc}\newpage
%
%
%\chapter{热学与统计力学}%%\Entry{统计力学公式大全}{StatEq}\newpage
%\Entry{分子平均碰壁数}{AvgHit}\newpage
%\Entry{相空间}{PhSpace}\newpage
%\Entry{理想气体的状态密度（相空间）}{IdSDp}\newpage
%\Entry{理想气体单粒子能级密度}{IdED1}\newpage
%\Entry{理想气体（微正则系综法）}{IdNCE}\newpage
%\Entry{理想气体（正则系宗法）}{IdCE}\newpage
%\Entry{理想气体（巨正则系综法）}{IdMCE}\newpage
%\Entry{巨正则系综法}{MCEsb}\newpage
%\Entry{量子气体（单能级巨正则系综法）}{QGs1ME}\newpage
%\Entry{量子气体（巨正则系宗）}{QGsME}\newpage
%
%\Entry{物理学常数定义}{Consts}\newpage
%
%\part{小时物理笔记}
%\Entry{电磁场角动量分解}{EMAMSp}\newpage
%\Entry{晶体衍射}{CrysDf}\newpage
%\Entry{Hartree-Fork 方法}{HarFor}\newpage
%%=======================================
%\entry{本书格式规范}{Sample}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%未完成, 未放入 wiki
%% \Entry{正则系宗法}{CEsb}\newpage   % 未完成
%
%
%% \entry{动能定理（刚体）}{KELaw}\newpage
%%\Entry{黑体辐射定律}{BBdLaw}\newpage
%%\Entry{电场波动方程}{EWEq}\newpage
%%\Entry{质心系中的多粒子问题}{SECM}\newpage
%%\Entry{麦克斯韦方程组（介质）}{MWEq1}\newpage
%%\Entry{FEDVR 算法}{FEDVR}\newpage
%%\Entry{Lanczos 算法}{Lanc}\newpage
%%\Entry{等间隔能级系统（正则系宗）}{EqCE}\newpage
%%\Entry{傅里叶变换（三角）}{FTTri}\newpage
%
%
%\entry{本书编写规范}{Sample}\newpage

%
%%====================================
%\entry{本书编写规范}{Sample}
%
\end{document}



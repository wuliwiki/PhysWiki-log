% 悬链线
% keys 微分方程|受力分析|绳子|悬链线|力学|微积分
% license Xiao
% type Tutor

\pentry{双曲函数\nref{nod_TrigH}， 常微分方程\nref{nod_ODE}}{nod_e4bf}
...
\end{equation}
其中 $\cosh$ 是\enref{双曲余弦函数}{TrigH}，常数 $k$ 满足
...
\subsection{推导（受力分析法）}
\pentry{曲线的长度\nref{nod_CurLen}， 最小作用量 哈密顿原理\nref{nod_HamPrn}}{nod_8a8c}
...
现在，函数 $\phi(y,x')$ 包含自变量 $y$，我们就可以使用拉格朗日方程求解。回想拉格朗日方程的推导，不难看出，$\phi(y,x')$ 代表拉格朗日函数 $L(t,q,q')$, $y$ 代表拉格朗日方程中的 $t$，而 $x,x'$ 分别代表 $q,q'$。代入拉格朗日方程\autoref{eq_Lagrng_1}，由于 $\phi(y,x')$ 不显含 $x$，有
\begin{equation}
\pdv{\phi}{x'}=C~,
\end{equation}

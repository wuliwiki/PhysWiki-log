...
\subsection{推导（受力分析法）}
\pentry{曲线的长度\upref{CurLen}， 常微分方程\upref{ODE}，最小作用量 哈密顿原理\upref{HamPrn}}
...
\end{equation}
现在，问题就转化为求 $y(x)$，使得 $y_C$最小．这个问题可用欧拉-拉格朗日方程\autoref{Lagrng_eq1}~\upref{Lagrng}求解．
...
现在，函数 $\phi(y,x')$ 包含自变量 $y$，我们就可以使用拉格朗日方程求解．回想拉格朗日方程的推导，不难看出，$\phi(y,x')$代表拉格朗日函数$L(t,q,q')$, $y$代表拉格朗日方程中的 $t$，而$x,x'$ 分别代表 $q,q'$．代入拉格朗日方程\autoref{Lagrng_eq1}~\upref{Lagrng}，由于 $\phi(y,x')$ 不显含 $x$，有
\begin{equation}
\pdv{\phi}{x'}=C
\end{equation}

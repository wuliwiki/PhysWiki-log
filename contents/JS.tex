...
\item 一些开源项目也会同时提供一个 source map， 文件名例如 \verb`xxx.js.map`。浏览器的开发者工具可以根据该文件自动把 minify 的 js 显示为原来的源码。 要使用 source map， 一般 js 文件的最后会有一行类似于 \verb`//# sourceMappingURL=app.min.js.map` 的注释，指定 source map 的位置，浏览器会根据这个读取 map 文件。

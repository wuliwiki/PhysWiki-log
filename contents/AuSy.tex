...
y'=f(y)~,
\end{equation}
其中 $y=(y_1,\cdots,y_n),f=(f_1,\cdots,f_n)$。与“\enref{基本知识（常微分方程）}{ODEPr}”中类似，我们总假定 $f_i(y_1,\cdots,y_n)$ 及其一阶偏导数在其定义区间（记为 $\Delta$）上连续。约定：当出现关于指标 $i$ 的表达式而不指出其取值范围时，就代表对每个 $i$ 所能取的值表达式均成立。
...
\end{theorem}
\textbf{证明：}由\enref{复合函数求导法则}{ChainR}，成立
...
显然，若解 $\varphi_1(x)$ 有\enref{最大存在区间}{ODEPr} $(m_1,m_2)$，则 $\varphi^*(x)$ 有最大存在区间 $(m_1-c,m_2-c)$.

...
\pentry{电场的高斯定律\nref{nod_EGauss}，磁场的高斯定律\nref{nod_MagGau}，安培环路定律（静磁学）\nref{nod_AmpLaw}}{nod_109e}
...
当（大量）电荷定向运动起来，就会产生\textsl{电流} \footnote{有时，我们假定电流伴随着异号静止电荷，因此电流总体上不带静电荷，也不产生电场。}。\enref{电流}{I}可由电流强度 $I$ 与\enref{电流密度}{Idens} $\bvec j$ 表述：
...
同质量一样\footnote{防杠声明：与电动力学紧密结合的狭义相对论告诉我们，这个表述是不严谨的}，\enref{电荷是守恒的}{ChgCsv}。也就是说，如果一个区域内有电荷的流出(电流)，那么这个区域内的电荷量就会相应地减小。
$$
...
\begin{tabular}{|c|c|c|}
\hline
 & \enref{电场}{Efield} $\bvec E$  & \enref{磁场}{MagneF} $\bvec B$ \\
\hline
场源 & \enref{电荷}{Efield} $q$ & \enref{电流}{I} (运动的电荷) $I$ \\
\hline
...
$$\div \bvec E = \frac{\rho}{\epsilon_0}~$$ \enref{电场的高斯定律}{EGauss}
&
$$\oint \bvec B \vdot \dd{\bvec s} = 0~$$
$$\div \bvec B = 0~$$ \enref{磁场的高斯定律}{MagGau}\\
\hline
旋度方程 & 
$$ \oint \bvec E \vdot \dd{\bvec l} = \bvec 0~$$
$$ \curl \bvec E = \bvec 0 ~$$ \enref{静电场的环路定理}{ELECLD}，\enref{基尔霍夫第二定律}{Kirch} 的一种表述。
 &
...
\hline
势与场 & $$\bvec E = -\grad \varphi~$$ $$ \varphi(\bvec r) = \int^{\bvec r_0}_{\bvec r} \bvec E \cdot \dd {\bvec l}~ $$ ($\bvec r_0$是零势参考点，一般选取无穷远处势为0，具体参考下文的“\enref{规范}{QEng}”) & $\bvec B = \curl \bvec A~$ \upref{BvecA} \\
\hline
...
\subsubsection{势的定义}
引入势的数学考量是（详见 \cite{GriffE} 与“\enref{矢量分析总结}{VecAnl} ”）：
\begin{itemize}
...
磁势也是同理，但是需要更巧妙地运用\enref{数学技巧}{VecAnl} 与规范，\textsl{具体证明过程按惯例留给读者}：
$$
\left \{
\begin{aligned}
\bvec B &= \curl \bvec A\\
...
\subsection{洛伦兹力}
一个点电荷在电磁场中的受力由\enref{洛伦兹力}{Lorenz}公式给出：
$$

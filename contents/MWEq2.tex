...
\subsection{前两个方程}
\pentry{外导数\upref{ExtDer}}{nod_3349}
...
\pentry{霍奇星算子\upref{HodgeO}}{nod_fa01}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
所以差异是因为，$\star F$相比$F$，在形式上是把$B$替换为$E$，而把$E$替换为$-B$，多且仅仅多了一个负号。而仔细看计算过程会发现，这个负号是来自Minkowski度规的，如果换用欧几里得度量就会出现两个负号，负负得正，导致$\dd \star F=0$对应的方程形式上就是$\dd F=0$对应的方程中直接调换$\bvec{E}$和$\bvec{B}$的位置而已\footnote{这也反映出电动力学与经典力学不适配，应使用相对论时空观。}。

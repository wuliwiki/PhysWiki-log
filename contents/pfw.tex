...
\pentry{流形\upref{Manif}，光滑映射（流形）\upref{DiffTg}，切向量场\upref{Vec}}
\begin{issues}
\issueTODO缺图
\end{issues}
...
\begin{theorem}{}
设$F:M\rightarrow N$是微分同胚。对于\textbf{任意光滑切场}$X\in \mathfrak{X}(M)$，总存在$N$上的一个\textbf{光滑切场}与之F关联。
\end{theorem}
proof.
...
\pentry{仿射联络\upref{affcon}，协变导数\upref{CoDer}}

对于定义在整个流形的光滑切场，前推一般要求流形之间的映射是微分同胚。而如果只是前推沿着道路的切场，对映射的要求就能降低到具备光滑性即可。

具体来说，设$f:M\rightarrow N$是光滑映射，$c(t):[0,1]\rightarrow M$是光滑道路。$V$是$M$上沿着该道路的任意切场：$V(t)=v^i(t)e_{i,c(t)}$，其中$\{e_{i,c(t)}\}$为点$c(t)$邻域上的一组基。为方便计，重新表示$V(t)=v^ie_i$。根据前推的定义，我们可以定义切场的推前为$f_*V(t)=a^i\widetilde e_{i,f\circ c(t)}$，其中$\widetilde e_i=f_*e_i$。由于前推是线性映射，因此${\widetilde e_i}$线性无关，是$f\circ c(t)$上的一组基。




设$f:(M,\nabla)\rightarrow(N,\widetilde {\nabla})$是\textbf{保联络的微分同胚映射}。$c(t)$是$M$上的光滑道路，$\frac{D}{dt}$和$\frac{\widetilde D}{dt}$分别是$M$上沿$c(t)$与$N$上沿$f\circ  c(t)$的协变导数。如果$V(t)$是$M$上沿$c$定义的光滑切向量场（即每一点的切向量不一定是道路上该点的切向量）。那么我们有：
\begin{equation}

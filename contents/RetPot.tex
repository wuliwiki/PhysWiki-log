...
\end{equation} 
\autoref{RetPot_eq5} 与静电场的势能公式很像，但是场源的时间做了修正．其中 $t - \abs{\bvec r - \bvec r'}/c \equiv t_{ret}$ 定义为\bb{推迟时间（ retarded time ）}， $\abs{\bvec r - \bvec r'}/c$ 是波从 $\bvec r'$ 到 $\bvec r$ 所需的时间．也就是说，$f(\bvec r, t)$ 并不能马上影响 $\Psi(\bvec r, t)$，而是需要一个“信号传播时间” 才行．在静电场中，由于场源不随时间变化，所以不需要考虑时间延迟．
...
\end{equation} 
（解方程提示： $\frac{1}{R^2}\dv{R} \qty(R^2 \dv{A}{R}) = \frac1R \dv[2]{R} (RA)$， 令 $\frac{1}{R^2} \dv{R} (R^2 \dv{A}{R}) = \frac1R \dv[2]{R}\dd{(RA)}$ ）\\

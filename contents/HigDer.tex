...
\pentry{导数\upref{Der},求导法则\upref{DerRul}}
...
若把$y=f(x)$的导数$y'=f'(x)$称为$y=f(x)$的\bb{一阶导数},那么,一阶导数的导数就称为\bb{二阶导数}.若二阶导数$y''=f''(x)$仍然可导，我们就把二阶导数的数$y''=f''(x)$的导数称为\bb{三阶导数},记为

\begin{equation}
y''',\quad f'''(x),\quad \frac{\dd{^3y}}{\dd{x^3}},\quad \frac{\dd{^3f}}{\dd{x^3}},\quad y'''(x)
\end{equation}

一般地，如果$y=f(x)$的$n-1$阶导数是可导的，我们就把$n-1$阶导数的导数称为果$y=f(x)$的\bb{n阶导数}，记为

\begin{equation}
y^{(n)},\quad f^{(n)}(x),\quad \frac{\dd{^ny}}{\dd{x^n}},\quad \frac{\dd{^nf}}{\dd{x^n}},\quad y^{(n)}(x)
\end{equation}

二阶和二阶以上的导数统称为\bb{高阶导数}.利用求导公式和求导法则就可以求出高阶导数.

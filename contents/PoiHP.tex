...
\pentry{联络形式与结构定理\nref{nod_ConFom}}{nod_f522}

本小节先计算 $\mathbb{H}^2$ 上的一个\textbf{黎曼}联络形式。

由\textbf{反对称定理}\autoref{the_ConFom_1}~\upref{ConFom}，计算联络形式时选标准正交基来进行计算最为方便，因为 $\mathbb{H}^2$ 是二维的，标准正交基下的联络形式矩阵是一个反对称的 $2$ 阶方阵，也就是说可以被一个分量唯一确定。
...
第三步，\textbf{设挠率}为 $0$，应用\textbf{结构定理}（\autoref{the_ConFom_2}~\upref{ConFom}）和\textbf{反对称定理}（\autoref{the_ConFom_1}~\upref{ConFom}）得：
...
最后，联立\autoref{eq_PoiHP_1} 、\autoref{eq_PoiHP_2} 和\autoref{eq_PoiHP_3} ，得到：
...
\pentry{高斯绝妙定理\nref{nod_Egreg}}{nod_18d5}

由\enref{高斯绝妙定理}{Egreg}，如果要计算 $\mathbb{H}^2$ 的高斯曲率，我们就要去计算某个基下的曲率形式 $\Omega^i_j$，从而有 $K=\Omega^1_2(\uvec{e}_1, \uvec{e}_2)$。

由上一小节计算出来的联络形式，结合\textbf{结构定理}\autoref{the_ConFom_2}~\upref{ConFom}，注意 $\omega^1_1=\omega^2_2=0$，我们有：
\begin{equation}
\begin{aligned}
...
\pentry{Christoffel符号\nref{nod_CrstfS}}{nod_ae4f}
...
\begin{figure}[ht]
\centering
\includegraphics[width=5cm]{./figures/e3efd1d75bd08408.pdf}
\caption{庞加莱半平面在通常的 $\mathbb{R}^2$ 坐标系中的Christoffel符号} \label{fig_PoiHP_1}
\end{figure}

% \begin{table}[ht]
% \centering
% \caption{庞加莱半平面在通常的 $\mathbb{R}^2$ 坐标系中的Christoffel符号}\label{tab_PoiHP_1}
...
\pentry{测地线\nref{nod_geodes}}{nod_48d3}

有了Christoffel符号，我们就可以列出测地线方程了。

根据\autoref{eq_geodes_1}~\upref{geodes}，誊抄如下：
...
\autoref{eq_PoiHP_9} 的第二式较为难解，为了简化解答，将其用测地线的另一个性质，匀速性（\autoref{the_geodes_1}~\upref{geodes}），来代替：
\begin{equation}\label{eq_PoiHP_8}
\frac{\dot{x}^2+\dot{y}^2}{y^2}=1~.
\end{equation}
注意，这里我们设定测地线的速度总是 $1$，这是因为任意非零测地线都可以重新参数化，将速度变成 $1$。

\autoref{eq_PoiHP_8} 没有提供超出\autoref{eq_PoiHP_9} 的信息，但是它可以简化解答。

...
\item 如果本地的公钥和私钥不匹配， ssh 会明确说
\item 如果 ssh 看起来进去了，有欢迎提示，但是卡住了，很可能是卡在了 \verb`~/.bashrc` 或 \verb`~/.profile` 上面。 可以用以下命令绕过： \verb`ssh 用户名@主机名 -t "bash --noprofile --norc"` 登进去以后可以调试修改这两个文件， 再退出再正常 ssh。

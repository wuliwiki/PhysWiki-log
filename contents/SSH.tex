...
\item \verb|sudo service ssh --full-restart| 也可以重启（WSL1\upref{WSLnt} 上没有 \verb|systemctl| 或者 \verb|systemd|）
...
\begin{itemize}
\item 用途： 例如校内网的服务器想让外网访问， 那就把外网一台有公共 ip 的转发服务器的某个端口和内网的服务器的某个端口链接。 “内网穿透笔记\upref{NATthr}” 中会提供更常用的办法。

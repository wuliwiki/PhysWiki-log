...
\pentry{多变量分布函数\upref{MulPdf}}{nod_9be6}
...
\subsection{速率分布}
若把速度（矢量）的模长 $v = \abs{\bvec v}$ 叫做速率， 则类比\autoref{eq_MulPdf_5}~\upref{MulPdf}， 速率的分布函数为
\begin{equation}
F(v) = 4\pi v^2 f(v)~.
\end{equation}

令气体分子的数量为 $N$， 则随机一个分子速度绝对值落在 $[v_a, v_b]$ 范围的概率为（类比\autoref{eq_MulPdf_4}~\upref{MulPdf}）
\begin{equation}

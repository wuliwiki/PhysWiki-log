...
\item 是否可以在编译时使用 rpath 把依赖库的相对目录直接写进 exe 中（这样就可以把库放到子文件夹）？ GPT 说这是不行的。 要么修改系统路径，要么创建一个 XML\upref{XML} 文件指定依赖路径，命名为 \verb`可执行文件名.exe.manifest`，和 exe 文件放在一起即可（文件具体内容可以问 GPT）。

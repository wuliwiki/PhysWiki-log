...
\begin{Command}
>> y1 = cos(x);\\
>> hold {\color{string}on}; plot(x, y1);
\end{Command}
...
\begin{Command}
>> x2 = cos(x); y2 = sin(x);\\
>> figure; plot(x2, y2, {\color{string}'r'});
\end{Command}
...
\begin{Command}
>> plot(x2, y2, {\color{string}'.-r'});\\
>> axis([-1.2, 1.2, -1.2, 1.2]);\\
>> xlabel({\color{string}'x'}); ylabel({\color{string}'y'}); title({\color{string}'unit circle'});
\end{Command}
...
\begin{Command}
>> hold on; scatter(0, 0, {\color{string}'b'});
\end{Command}

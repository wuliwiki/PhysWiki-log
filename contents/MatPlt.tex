...
>> figure; plot(x2, y2, 'r');
\end{Command}
在新增的窗口中， 结果如\autoref{MatPlt_fig2} （左）所示． 注意根据窗口尺寸的不同， $x$ 轴和 $y$ 轴的单位长度一般不同， 若要使其相同， 可以在 \x{plot} 后面用 \x{'axis equal'} 命令（其中 \x{equal} 是 \x{axis} 函数的输入变量）， 得到\autoref{MatPlt_fig2} （右）．
\begin{figure}[ht]
\centering

% 算符的指数函数、波函数传播子
% keys 算符|量子力学|指数函数|波函数|传播子
% license Xiao
% type Tutor

\pentry{一阶线性常微分方程组\upref{ODEsys}}{nod_7467}

\subsection{不含时， 有限维的情况}
%这里的“传播子propagator”应该是“时间演化算符time evolution operator”。
...
将 “一阶线性常微分方程组\upref{ODEsys}” 中\autoref{eq_ODEsys_1} 拓展成偏微分方程\footnote{形象理解： 将矢量 $\bvec v$ 看作有无穷多个元， 且取值连续， 就成了 $f(x, t)$}， 令 $A$ 为算符。
\begin{equation}
...
\end{equation}
若 $A$ 含有 $t$， 形式解\autoref{eq_ODEsys_3}~\upref{ODEsys} 变为
\begin{equation}
...
我们把哈密顿算符 $H$ 看作是无穷维矩阵， 薛定谔方程可记为与一阶线性常微分方程组（\autoref{eq_ODEsys_1}~\upref{ODEsys}）相同的形式
\begin{equation}
\dv{t} \ket{\psi(t)} = -\I H \ket{\psi(t)}~.
\end{equation}
当哈密顿算符 $H$ 不含时， 解为（根据\autoref{eq_ODEsys_2}~\upref{ODEsys}）
\begin{equation}

...
\pentry{正交曲线坐标系\upref{CurCor}， 重积分\upref{IntN}}
...
\begin{figure}[ht]
\centering
\includegraphics[width=4cm]{./figures/19febbf45ee70d2d.pdf}
\caption{极坐标中的面积元} \label{fig_CrIntN_1}
\end{figure}

我们来看如何在极坐标系\upref{Polar}中进行二重积分。 我们先把积分区域划分为无数个小\textbf{面积元}， 点 $\bvec r$ 处面元的形状如\autoref{fig_CrIntN_1} 所示， 即把两个坐标 $r, \theta$ 坐标分别在原来的基础上增加一个微小值，并围成一块小区域。 由于 $\dd{r}, \dd{\theta}$ 都是无穷小， 该面元的形状趋近于长方形， 其面积为两边长相乘
...
\autoref{eq_CrIntN_4} 也是柱坐标系\upref{Cylin}中 $r$、$\theta$ 曲坐标面上的面积元。 对于球坐标， 我们可得到 $\theta$、 $\phi$ 曲坐标面上的面积元
\begin{equation}
...
\begin{figure}[ht]
\centering
\includegraphics[width=9.5cm]{./figures/2733184958781c64.pdf}

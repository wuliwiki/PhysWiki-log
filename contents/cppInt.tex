...
\begin{itemize}
\item 当较短的整型（\verb|bool, char, signed char, unsigned char, short, unsigned
short| 等， 包括 11 以后的新标准加入的一些类型）参与二元运算如 \verb`+, -, *, /, %, &, |, ^, <<, >>, ==, !=, <, >, <=, >=, +=, -=, *=, /=, %=, &=, |=, ^=, <<=, >>=`， 以及 \verb`:?` 的第二和第三个参数， 会先转换为较大的整型， 这就叫 \textbf{integral promotion} 或 \textbf{integer promotion}

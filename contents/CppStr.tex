...
可以用 C++ 解释器 Cling\upref{Cling} 方便测试。 \verb|printf| 参考 C 语言笔记\upref{CNote}。
...
\item C++ 对 UTF-8 的支持非常有限， 甚至没有一个 iterator 可以对每个字符进行循环。 见 SLISC\upref{SLISC} 中的 \verb|u8_iter|。
...
\item 无法替换字符串中的所有 \verb|key|，见 SLISC\upref{SLISC} 的 \verb|replace(str,key,new_str)|。
...
\item SLISC\upref{SLISC} 提供一个 UTF-8 iterator， 也包含 utfcpp 库

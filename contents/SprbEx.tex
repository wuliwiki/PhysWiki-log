% 可分扩张
% 可分多项式|完全域|完备域|形式微商|微商|导数|形式导数|separable polynomial|重根|自同构|形式微分|Frobenius同态


\pentry{分裂域\upref{SpltFd}}

\addTODO{画一个“各部分逻辑关系”的示意图．}
...
考虑$\mathbb{Z}_p$的单超越扩域$\mathbb{Z}_p(\pi)$．则$x^p-\pi$是$\mathbb{Z}_p(\pi)$上的不可约多项式\footnote{考虑到$\mathbb{Z}_p(\pi)$是整环，且$\pi$是其素元素，应用\textbf{爱森斯坦判别式}\upref{EsstCr}即可得证．}，但$\opn{D}(x^p-\pi)=0$，从而是一个不可分的不可约多项式．因此，$\mathbb{Z}_p(\pi)$不是完备域．

\end{example}



在域$\mathbb{F}$上，任取$a\in\mathbb{F}$，如果存在$b\in\mathbb{F}$使得$a=b^p$，那么据\autoref{SprbEx_lem2} ，$x^p-a$一定是有重根的：$x^p-a=(x-b)^p$\footnote{和\autoref{SprbEx_ex2} 比起来，这里爱森斯坦判别式无效了，因为任何元素都不是素元素了．理由很简单，$a=b^p$，所以$a\mid b\cdot b^{p-1}$．于是，要么$a$是\textbf{单位}（\autoref{FctTre_def3}~\upref{FctTre}），这样$a\mid 1$；要么$a$不是素元素．总之$x^p-a$没法符合爱森斯坦判别式中素元素或素理想的要求．}．所以为了构造完备域，我们必须要求所有这类多项式都是可约的，这样才不违反“不存在不可约不可分多项式”的规定．
...
% 由于扩域不改变域的特征以及特征为$0$的域都是完备域，我们只需要讨论特征为素数$p$的情况．又因为我们只需要证明，如果$x_0$是$\mathbb{F}$的代数元，那必存在$y_0\in\mathbb{F}(x_0)$使得$y_0^p=x_0$，故只需要考虑单代数扩张的情况．

% 设$x_0$在$\mathbb{F}$上的最小多项式为$f(x)=\sum_{i=0}^n a_ix^i$．

% 由于$\mathbb{F}$是完备域，所以可以取$b_i\in\mathbb{F}$使得$b_i^p=a_i$．定义$h(x) = \sum_{i=0}^n b_ix^i$．由于$\opn{ch}\mathbb{F}=p$，据\autoref{SprbEx_lem1} 知
% \begin{equation}
% \begin{aligned}

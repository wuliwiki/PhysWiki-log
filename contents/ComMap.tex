...
% license Xiao
% type Tutor
\pentry{映射\nref{nod_map}，度量空间\nref{nod_Metric}}{nod_82c7}
在正式定义之前，我们先讨论根据“压缩映射”这一名词应该怎么定义它。首先，压缩映射是集合到自身的一个映射（因为“压缩”就应该是种包含关系）。其次，“压缩”是指对集合中任意两点，在压缩映射下两点的像点的“距离”比两原像更小。这就是说，压缩映射是定义在带有“距离”的集合上的，这样的集合便是\enref{度量空间}{Metric}。容易验证，满足上面定义的压缩映射 $A$ 的定义集合 $X$ 是个无限集。事实上，若 $X$ 有限，且 $d:X\times X\rightarrow \mathbb R$ 是距离函数\footnote{表示"距离"的集合的元素应都是可比较的，就是说它是个\enref{有序集}{OrdRel}，不妨将这个集合取作实数集。}。我们构建两个序列
\begin{equation}
...
\end{aligned}
\end{equation}
但是 $A$ 是压缩映射要求任意 $n>0$，都有 $d(A^n x,A^n y)<d(x,y)$\footnote{$A^n x$表示把$x$压缩 $n$ 次}。\autoref{eq_ComMap_1} 与这是矛盾的，这表明压缩映射必须定义在无限集上。上面的论证中用到了这样的事实，即\enref{映射}{map}可作用在其定义集合的所有元上。
...
压缩映射是个连续映射。
\end{theorem}
\textbf{证明：}类比\enref{函数的连续性}{contin}，度量空间 $(M,d)$ 到度量空间 $(M',d')$ 上的连续映射可以描述为：对任意正数 $\epsilon$ ，若存在数 $\delta$，只要 $x$ 满足 $d(x,x_0)<\delta$，就有 $d'(f(x),f(x_0))<\epsilon$，则称映射 $f$ 在点 $x_0$ 连续。下面我们来证明压缩映射确是在度量空间上为连续。

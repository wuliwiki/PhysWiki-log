...
\end{theorem}
\textbf{证明：}\textbf{必要性:}假设 $v_1\overset{U}{\sim} v_2$，于是存在 $x\in V$，使得 $v_1,v_2\in x+U$，于是可记 $v_1=x+\bvec{u}_1,v_2=x+\bvec{u}_2$，其中 $\bvec{u}_1,\bvec{u}_2\in U$。那么 $v_1-v_2=\bvec{u}_1-\bvec{u}_2\in U$。

\textbf{充分性：}假设 $v_1-v_2\in U$。设 $v_1=x+\bvec{u}_1,v_1-v_2=u$，其中 $\bvec{u}_1,u\in U$，那么 $v_2=v_1-u=x+\bvec{u}_1-u\in x+U$。即 $v_1\overset{U}{\sim} v_2$。
...
% 下面我们再来分析一下 $V, W$ 和 $V/W $ 三者的基底之间的关系。设 $\{\bvec{u}_1, \bvec{u}_2, \cdots, u_m\}$ 是 $W $ 的一个基底。我们再补充 $n - m$ 个向量 ${u}_{m+1}, \cdots {u}_n$ 使 $\{\bvec{u}_1, \bvec{u}_2, \cdots, u_n\}$ 成为 $V$ 的一个基底。由于 $\bvec{u}_1, \bvec{u}_2, \cdots, u_m \in W$，我们有 $[\bvec{u}_1], [\bvec{u}_2], \cdots, [u_n] = [0]$，因此我们考虑 $V / W$ 的向量组 $\mathcal{B}: = \{[u_{m + 1}], \cdots, [u_n]\}$，我们要证明 $\mathcal{B}$
% \begin{itemize}
% \item 是张成 $V / W$的，
% \item 是线性无关的。
% \end{itemize}
...
\begin{theorem}{第一同构定理}\label{the_QuoSpa_4}
...
设任意$\bvec x\in U_1+U_2$可以分解为$\bvec x=\bvec{u}_1+\bvec{u}_2$，映射$f$为
\begin{equation}
f(\overline {\bvec x})=f(\bvec x+U_2)=\bvec{u}_1~,
\end{equation}
...
单射是合理的，即使$\bvec x$有两种分解，也会映射到一个同余类。比如设$\bvec x=\bvec{u}_1+\bvec{u}_2=\bvec{u'}_1+\bvec{u'}_2$，则$f(\bvec{u}_1+\bvec{u}_2+U_2)-f(\bvec{u'}_1+\bvec{u'}_2+U_2)=\bvec{u}_1+U_1\cap U_2=\bvec{u'}_1+U_1\cap U_2$。
...
建立线性映射$f:V/U_2\rightarrow V/U_1$为$f(\bvec x+U_2)=\bvec x+U_1$，显然这是线性映射。因为映射的核为$U_1/U_2$。结合\autoref{the_QuoSpa_4} 可得证。
\begin{corollary}{}
设$V=V_1\oplus V_2$，$U=U_1\oplus U_2\subset V$，则有自然同构
\begin{equation}
\frac{V_1\oplus V_2}{U_1\oplus U_2}\cong \frac{V_1}{U_1}\oplus \frac{V_2}{U_2}~.
\end{equation}
\end{corollary}

...
\end{aligned}
\end{equation}
其中用到了指数函数的泰勒展开（\autoref{eq_Taylor_11}~\upref{Taylor}）。
...
另外， 想象在巨正则系综的物理情景中， 变化 $T$ 和 $\mu $，  从而使\autoref{eq_IdMCE_7} 中的粒子数保持不变， 则 $N$ 不变时 可以看成 $T$ 的函数（而这个函数应该与正则系宗所得到的一样）。由\autoref{eq_IdMCE_7} 得

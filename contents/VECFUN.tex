...
\footnote{本文参考了\cite{Thomas}。}我们在一元函数\enref{函数}{functi}与多元函数中已经学会了函数的映射含义：一元函数就是把定义域中的一个数（\enref{实数或复数}{CplxNo}）通过一定的方式转换为另一个数：
\begin{equation}
y=f_1(x)~.
\end{equation}
...
如今，我们要面对向量值函数。他的含义也完全类似：向量函数把定义域中的一个数或\enref{向量}{GVec}通过一定的方式转换为\textbf{另一个向量}。
\begin{equation}
\bvec y = \bvec f_3 (x)~.
\end{equation}
\begin{equation}

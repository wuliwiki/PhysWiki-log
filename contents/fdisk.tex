...
\item \verb|mklabel| 修改分区的 label， \verb|ntfslabel /dev/nvme0n1p3 win| 改变 ntfs 分区的 label， 还有 \verb|e2label| 用法一样。 注意用 \verb|gparted| 里面的 label 功能似乎并不是真正的 label， \verb|/etc/fstab| 识别不出后者。

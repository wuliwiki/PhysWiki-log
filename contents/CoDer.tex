...
不过，在\textbf{Christoffel符号}\upref{CrstfS}文章中我们看到，在具体图中计算联络时，只用到了几个定义在欧几里得空间的函数，也就是向量场的坐标值函数和Christoffel符号。欧几里得空间上函数的求导是可以考虑局部的，也就是我们可以计算一个点上函数的导数，而不必像联络的定义那样要考虑整个场的变换。这就意味着我们有可能局部地计算联络。
...
% 根据\autoref{eq_CrstfS_4}~\upref{CrstfS}，誊抄如下：
% \begin{equation}
...
将\autoref{eq_CoDer_5} 与\textbf{Christoffel符号}\upref{CrstfS}的概念结合，我们还可以进一步写出给定图中协变导数的计算公式：
...
这是由“度量相容性（见\autoref{def_RieCon_1}~\upref{RieCon}的第二条）”推论得出的。

...
\begin{issues}
\issueTODO
\issueOther{可以对照\enref{线性算子的张量积}{TPofLO}进行阅读}
\end{issues}

\pentry{向量空间的张量积\nref{nod_TPofSp}}{nod_3034}
...
\end{equation}
根据\enref{秩-零化度定理}{MatLS2}可得 $\iota$ 是一个同构映射。
...
\addTODO{Kronecker product，可以从\enref{线性算子的张量积}{TPofLO}的第二部分改编过来。}


\subsubsection{线性形式和对偶空间}
\addTODO{要不要线性形式单独成章节？}

\begin{theorem}{}
全体 $k$-线性形式构成的向量空间与 $(V^*)^{\otimes k}$ （参考\enref{对偶空间}{DualSp}）自然同构（不依赖基的选取）。
\end{theorem}
% Giacomo：如果把对偶空间的定义移动到《线性映射》的话，这里就不需要引用了
\textbf{证明：} $k$-线性形式构成的向量空间自然同构于 $(V^{\otimes k})^*$，它自然同构于 $(V^*)^{\otimes k}$。
\textbf{证毕。}

\subsubsection{线性映射的张量幂}

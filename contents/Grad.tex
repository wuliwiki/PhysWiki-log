...
\end{equation}
由内积的几何定义\upref{Dot}可知，从某点出发，若微位移 $\dd{\bvec r}$ 的大小不变，那么当其方向与梯度方向相同时函数增量 $\dd{f}$ 最大；二者方向垂直时，函数增量为零；二者夹角为 $\theta$ 时，函数增量等于最大值乘以 $\cos \theta$． 所以梯度矢量的方向是函数 $f$ 增加最快的方向， 梯度的大小等于该方向的方向导数．注意\autoref{Grad_eq5} 和\autoref{Grad_eq1} 的关系可以类比一元函数的导数\upref{Der}和微分\upref{Diff} 的关系， 当函数可微时， 二者等效．
...
先以 $xy$ 平面的曲线为例，任意曲线可以用函数 $f(x,y)$ 的\bb{等值线}来表示，即 $f(x,y) = C$（ $C$ 为常数）．若从曲线上的某点出发，沿曲线的切线方向取一个微位移 $\dd{\bvec r} = \dd{x}\uvec x + \dd{y} \uvec y$，由于 $(x+\dd{x}, y+\dd{y})$ 仍然在等值线上，函数增量 $\dd{f} = 0$． 代入\autoref{Grad_eq5} 得
\begin{equation}
...
同理， 柱坐标中的微位移（\autoref{CurCor_eq7}\upref{CurCor}） 与函数 $f(r,\theta, z)$ 的全微分可以分别表示为
\begin{equation}
...
球坐标也类似， 球坐标中的微位移（\autoref{CurCor_eq12}\upref{CurCor}） 与 $f(r,\theta,\phi)$ 的全微分可以分别表示为
\begin{equation}
...
\bb{梯度定理} ：一个标量函数 $f(\bvec r)$ 的梯度延任何路径从起点 $\bvec r_i$ 到终点 $\bvec r_f$（角标 $i$ 表示 initial， $f$ 表示 final）  线积分的结果等于该函数在末位置的函数值减去初位置的函数值．可以用下式表示
...
\subsection{由梯度求势函数}
我们通常把上面的标量函数 $f(\bvec r)$ 叫做\bb{势函数}， 其地位相当于牛顿—莱布尼兹公式中的原函数． 在这个类比中， 既然“对原函数求导” 对应“对势函数求梯度”， 那么不定积分对应的“通过梯度函数求势函数”又该如何实现呢？

...
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/QCC_1.pdf}
...
\end{equation}
 该结论由\autoref{QCU_the3}~\upref{QCU} 直接得到．
\end{example}
\subsubsection{数乘}
\autoref{QCU_the2}~\upref{QCU} 使得我们能够在量类当中定义数乘．
\begin{definition}{}
...
\end{equation}
\end{example}
可以验证，这样定义的加法、数乘、零元满足矢量空间\upref{LSpace}的定义．所以每个量类都是（实数域上的）一个矢量空间．又因为它只有一个线性独立的元素（不妨就选取作为单位的量），即只有一个基矢，所以是1维矢量空间．

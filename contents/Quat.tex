...
现代四元数的定义是把每一个四元数看成一个实数和一个三维实向量的组合：$(a_0, \bvec{a})$．在这个语境下，$\I$，$\mathrm{j}$，$\mathrm{k}$被看成是一个标准正交基中的向量，为了避免将来的混淆，把这三个基向量重新表示成$1, \hat{\I}, \hat{\mathrm{j}}$和$\hat{\mathrm{k}}$后，四元数的运算可以定义如下：

\begin{definition}{四元数的四则运算}
记四元数集合为$\mathbb{H}=\{(a_0, \bvec{a})|a_0\in\mathbb{R}, \bvec{a}\in\mathbb{R}^3\}$，则其运算为：
\begin{itemize}
\item $\forall(a_0, \bvec{a}), (b_0, \bvec{b})\in\mathbb{H}$，有$(a_0, \bvec{a})+ (b_0, \bvec{b})=(a_0+b_0, \bvec{a}+\bvec{b})$．
\item $\forall(a_0, \bvec{a}), (b_0, \bvec{b})\in\mathbb{H}$，有$(a_0, \bvec{a})\times (b_0, \bvec{b})=(a_0b_0-\bvec{a}\cdot\bvec{b}, a_0\bvec{b}+b_0\bvec{a}+\bvec{a}\times\bvec{b})$．

\end{itemize}

\end{definition}

注意，由于一般来说向量的叉积不可交换，因此一般来说四元数不具有交换律．只有$\bvec{a}\parallel\bvec{b}$的时候，才会有$\bvec{a}\times\bvec{b}=\bvec{0}=\bvec{b}\times\bvec{a}$，这个时候两个四元数交换．

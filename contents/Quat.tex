...
两个复数相乘， 就是把它们的模长相乘， 幅角相加（\autoref{eq_CplxNo_10}~\upref{CplxNo}）， 类似地两个四元数相乘的模长也等于各自的模长相乘（证明留做习题）
\begin{equation}
\abs{AB} = \abs{A}\abs{B}
\end{equation}
...
令四元数为 $A=a_0+\bvec{a}$，则使用四元数的乘法（\autoref{eq_Quat_1}），定义（类比复数的指数函数定义\upref{CExp}）

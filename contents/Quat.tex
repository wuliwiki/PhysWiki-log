...
\item $\forall(a_0, \bvec{a}), (b_0, \bvec{b})\in\mathbb{H}$，有$(a_0, \bvec{a})\times (b_0, \bvec{b})=(a_0b_0-\bvec{a}\cdot\bvec{b}, a_0\bvec{b}+b_0\vec{a}+\bvec{a}\times\bvec{b})$．

\end{itemize}

\end{definition}

注意，由于一般来说向量的叉积不可交换，因此一般来说四元数不具有交换律．只有$\vec{a}\parallel\bvec{b}$的时候，才会有$\bvec{a}\times\bvec{b}=\bvec{0}=\bvec{b}\times\vec{a}$，这个时候两个四元数交换．

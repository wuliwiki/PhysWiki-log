...
Shakeup 就是说总波函数中一个电子处于某个基态， 另一个电子处于散射态的部分． 令归一化的基态为
\begin{equation}
\phi_{n_1, l_1, m_1}(\bvec r_1) = R_{n_1,l_1}(r_1)Y_{l_1,m_1}(\uvec r_1)
\end{equation}
归一化的散射态为
\begin{equation}
C_{l_2}(k_2, \bvec r_2) = \sqrt{\frac{2}{\pi}} F_{l_2}(k_2, r_2) Y_{l_2, m_2} (\uvec r_2)
\end{equation}
以下的计算和单电子概率分布很类似， 我们同样不需要太在意粒子交换对称的问题， 把内积乘以 $\sqrt{2}$， 概率乘以 2 即可． 同样先把总波函数转换为\autoref{HeAnal_eq6} 的形式， 投影得到
\begin{equation}
\ali{
&\bra{\phi_{n_1, l_1, m_1}}\braket{C_{l_2}(k_2)}{\Psi} = \sqrt{\frac{2}{\pi}} \iint r_1 R_{n_1,l_1}(r_1) F_{l_2}(k_2, r_2) \psi_{l_1, l_2}^{m_1, m_2}(r_1, r_2) \dd{r_1} \dd{r_2}\\
& = \sqrt{\frac{2}{\pi}} \sum_{L,M} \bmat{l_1 & l_2 & L\\ m_1 & M - m_1 & M} \iint r_1 R_{n_1,l_1}(r_1) F_{l_2}(k_2, r_2) \psi_{l_1, l_2}^{L,M}(r_1, r_2) \dd{r_1} \dd{r_2}
}\end{equation}
\begin{equation}
P(n_1, l_1, m_1, k_2) = \sum_{l_2, m_2}\abs{\bra{\phi_{n_1, l_1, m_1}}\braket{C_{l_2}(k_2)}{\Psi}}^2
\end{equation}
注意这里对 $m_2$ 求和其实没有必要， 因为上式只有 $m_2 = M - m_1$ 时才不为零．

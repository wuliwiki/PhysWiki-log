...
库仑波函数（见\autoref{eq_CulmWf_9}~\upref{CulmWf}， 我们只需要平面波出射）为（$\eta = -mZ/k$， $Z = 2$）
\begin{equation}
...
\end{equation}
类比氢原子的情况（\autoref{eq_HWF_4}~\upref{HWF}）， 令 $\lambda$ 代表 $l_1, l_2, L, M$
\begin{equation}
\ali{
...
\end{equation}
那么由\autoref{eq_HeTDSE_2}~\upref{HeTDSE} 和\autoref{eq_GenYlm_1}~\upref{GenYlm} 得
...
\subsection{单电子位置分布}
由\autoref{ex_IdPar_1}~\upref{IdPar}， 在 $\bvec r_2$ 位置发现任意一个粒子的概率密度为
...
\end{equation}
然而 streaking trace 需要的不是上式的概率而是某个方向的概率。 我们使用\autoref{eq_CulmWf_6}~\upref{CulmWf} 做线性组合即可
...
\end{equation}
然而， streaking trace 需要的不是上式的概率而是某个方向的概率。 我们只需使用\autoref{eq_CulmWf_6}~\upref{CulmWf} 做线性组合即可
\begin{equation}
...
\end{equation}
中括号中的内容逐项投影到库仑平面波 $\ket{\bvec k_2}$ 上得 $\psi_{l_1,m_1}(r_1)$， 按照\autoref{eq_CulmWf_3}~\upref{CulmWf} 的方法即可。 于是
...
\begin{figure}[ht]
\centering
\includegraphics[width=3.5cm]{./figures/d297f14ba447f52b.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/90b870927476d6f8.png}

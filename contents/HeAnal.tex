...
\subsection{径向概率分布}
\begin{equation}\label{HeAnal_eq21}
...
\begin{equation}\label{HeAnal_eq22}
\begin{aligned}
...
\end{equation}
三重微分截面可以定义为
\begin{equation}\label{HeAnal_eq23}
...
\subsection{单电子位置分布}
由\autoref{IdPar_ex1}~\upref{IdPar}， 在 $\bvec r_2$ 位置发现任意一个粒子的概率密度为
\begin{equation}\label{HeAnal_eq24}
...
\end{equation}
单电子概率为
\begin{equation}\label{HeAnal_eq25}
\begin{aligned}
...
最后一种方案是：
\begin{equation}\label{HeAnal_eq26}
\begin{aligned}

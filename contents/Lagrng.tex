...
以上几个例子中，每个质点的坐标都是独立的，在没有约束时，$D$ 维空间中的 $N$ 质点有 $D\times N$ 个独立的广义坐标． 在一些问题中，我们往往会遇到不同形式的\textbf{约束}，例如当两个质点之间由轻棍连接时，约束方程为 $(x_1-x_2)^2+(y_1-x_2)^2+(z_1-z_2)^2=L^2$ 这时两个质点的自由度\upref{DoF}就由 6 个变为 5 个．约束可以是时间的函数，例如细棍的长度可以是关于时间的函数 $L(t)$．这里我们只讨论\textbf{完整约束（holonomic constraint）}，即可以表示成以下形式
\begin{equation}
...
\addTODO{讨论含时势能， $V$ 若含时， 有可能只是含时约束带来的， 也有可能 $V(\bvec r_i)$ 本来就含时， 例如变化的匀强电场中的带电粒子． 只要能保证\autoref{dAlbt_eq2}~\upref{dAlbt} 即可．}

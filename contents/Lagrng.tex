...
\end{equation}
其中 $T$ 是系统的动能， $V$ 是势能。 注意我们为了简单起见暂且假设 $V$ 不显含\footnote{“$V$ 不显含 $\dot q$” 就是指函数 $V(q,t)$ 的自变量中不包含 $\dot q$， 详见\autoref{sub_PChain_1}。 所以此时可以把\autoref{eq_Lagrng_1} 左边的 $L$ 替换为动能 $T$。} $\dot q$， 但在讨论广义势能（例如\enref{拉格朗日电磁势}{EMLagP}）时是可以显含的。

...
记为 $\lambda_i$ （$i = 1, 2\dots N$）。 将它们依次代入\autoref{eq_MatEig_2}， 就可以分别解出对应的本征矢。 考虑到\autoref{eq_MatEig_2} 是一个齐次方程， 所以 $\mat A - \lambda_i\mat I$ 的零空间（\autoref{the_LinMap_1}~\upref{LinMap}）中所有矢量都是本征矢， 且零空间至少是一维的。 我们把这个空间叫做 $\lambda_i$ 的\textbf{本征矢空间}， 是 $\bvec v$ 所在的 $N$ 维矢量空间的子空间。 所以 1. 任何本征矢乘以非零常数都是本征矢； 2. 本征值相同的一组本征矢的任意线性组合都仍然是本征矢。

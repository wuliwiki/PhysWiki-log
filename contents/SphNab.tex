...
\end{equation}
\addTODO{该式的推导应该在\enref{正交曲线坐标系}{CurCor}中完成}。
...
\end{equation}
其中 $\laplacian_r$ 只对 $r$ 求偏导（\autoref{eq_SphNab_4} 右边第一项）， $\laplacian_\Omega$ 只对 $\theta,\phi$ 求偏导（\autoref{eq_SphNab_4} 右边后两项）。 该分解有助于使用分离变量法求解\enref{球坐标系中的拉普拉斯方程}{SphLap}。
...
\begin{equation}
\pdv{\uvec{r}}{r} = 0, \qquad
\pdv{\uvec{r}}{\theta} = \uvec{\theta}, \qquad
\pdv{\uvec{r}}{\phi} = \sin\theta\uvec{\phi}~.
\end{equation}
\begin{equation}
\pdv{\uvec{\theta}}{r} = 0, \qquad
\pdv{\uvec{\theta}}{\theta} = -\uvec{r}, \qquad
\pdv{\uvec{\theta}}{\phi} = \cos\theta\uvec{\phi}~.
\end{equation}
\begin{equation}
\pdv{\uvec{\phi}}{r} = 0, \qquad
\pdv{\uvec{\phi}}{\theta} = 0, \qquad
\pdv{\uvec{\phi}}{\phi} = -\sin\theta\uvec{r} - \cos\theta\uvec{\theta}~.
\end{equation}

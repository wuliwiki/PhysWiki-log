...
\end{equation}
该方程可以通过下面变分得到
\begin{equation}\label{eq_CanTra_1}
\delta\int\qty(\sum_iP_i\dd Q^i-H'\dd t)=0~,
\end{equation}
而旧变量的哈密顿正则方程是通过变分
\begin{equation}\label{eq_CanTra_2}
\delta\int\qty(\sum_ip_i\dd q^i-H\dd t)=0~
\end{equation}
得到的。要使 \autoref{eq_CanTra_1} ，\autoref{eq_CanTra_2} 描述的是同一物理系统，那么它们的被积函数只能相差某个关于相空间坐标的函数 $F$ 的全微分，因为这是在变分时不起作用的常数。因此
\begin{equation}
...
\begin{equation}\label{eq_CanTra_3}
Q^i=Q^i(p,q,t),\quad P_i=P_i(p,q,t)~
\end{equation}
满足关系
\begin{equation}\label{eq_CanTra_4}
\sum_ip_i\dd q^i-H\dd t=\sum_iP_i\dd Q^i-H'\dd t+\dd F~,
\end{equation}
其中 $H,H'$ 分别是以旧坐标 $(p,q)$ 和新坐标 $(P,Q)$ 描述的哈密顿量。则称坐标变换\autoref{eq_CanTra_3} 为对应物理系统的\textbf{正则变换}，函数 $F$ 称为该正则变换的\textbf{生成函数}或\textbf{母函数}。
\end{definition}
...
改写 \autoref{eq_CanTra_4} 为：
\begin{equation}\label{eq_CanTra_5}
...
用 $F_2+\sum_iP_iQ^i$ 代替\autoref{eq_CanTra_6} 中的 $F_2$，就有
\begin{equation}
...
\end{aligned}
\end{equation}
若将这一变换视作 $p_t,q_t$ 到 $p_{t+\tau},q_{t+\tau}$ 的坐标变换，则这即是正则变换，因为由\autoref{eq_EPAct_1}~\upref{EPAct}
\begin{equation}
\dd S=\sum \qty(p_{t+\tau}\dd {q_{t+\tau}}-p_{t}\dd {q_{t}})-(H_{t+\tau}-H_t)\dd t~.
\end{equation}
于\autoref{eq_CanTra_5} 对比，可知 $-S$ 是变换的母函数。 
...
%首先， 根据勒让德变换\autoref{eq_HamCan_1}~\upref{HamCan}， 拉格朗日量可以表示为
%\begin{equation}

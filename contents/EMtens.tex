...
\pentry{电磁场的作用量\upref{ElecS}， 电磁场的能量守恒、坡印廷矢量\upref{EBS}，电磁场的动量守恒、动量流密度张量\upref{EBP}}
...
\end{aligned}
\end{equation}
这与麦克斯韦应力张量 \autoref{EBP_eq2}~\upref{EBP} 的形式是一致的．
...
现在考虑场源的影响．根据 \autoref{ElecS_eq2}~\upref{ElecS}，作用量的完整形式为
\begin{equation}
...
进一步，根据洛伦兹力公式\autoref{EMFT_eq3}~\upref{EMFT}，
\begin{equation}
\epsilon \frac{\dd U^\nu}{\dd t}=\rho F^\mu{}_\nu U^\nu \frac{\dd \tau}{\dd t}=\rho F^\mu{}_\nu\frac{\dd x^\nu}{\dd t}=F^\mu{}_\nu J^\nu 
\end{equation}
代入 \autoref{EMtens_eq5} 将得到
\begin{equation}\label{EMtens_eq6}
\partial_\mu T_{\rm{e.m.}}^{\mu\nu}=-F^{\mu\nu}J^\nu
\end{equation}
等式左边包含两部分：一项是场的能量动量密度随时间变化率，另一项是能量动量流的散度；等式右侧代表的是电磁场对电荷作的功．因此 \autoref{EMtens_eq6} 相当于电动力学的能量动量守恒律．
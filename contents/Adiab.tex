...
\pentry{热容量\upref{ThCapa}}{nod_9178}

在系统状态变化过程中， 如果和外界没有热量和粒子交换， 这个过程就叫做\textbf{绝热过程（adiabatic process）}
\begin{equation}
\Delta Q = 0~.
\end{equation}
根据熵增（\autoref{eq_Entrop_1}~\upref{Entrop}） 的定义， 绝热过程是一个等熵过程。

由热力学第一定律\autoref{eq_Th1Law_1}~\upref{Th1Law}，
...
\gamma = \frac{i+2}{i}~.
\end{equation}
绝热指数也可以用等体热容（\autoref{eq_ThCapa_1}~\upref{ThCapa}）和等压热容（\autoref{eq_ThCapa_2}~\upref{ThCapa}）表示为
\begin{equation}
\gamma = \frac{C_P}{C_V}~.
\end{equation}
...
\dd{W} + \dd{E} = 0~,
\end{equation}
其中（\autoref{eq_PVgraf_1}~\upref{PVgraf}）
\begin{equation}\label{eq_Adiab_2}
\dd{W} = P\dd{V}~.
\end{equation}
将理想气体状态方程（\autoref{eq_PVnRT_1}~\upref{PVnRT}）两边微分得
\begin{equation}
\dd{P}V + P\dd{V} = nRdT~,
\end{equation}
将气体的内能公式（\autoref{eq_IdgEng_1}~\upref{IdgEng}）两边微分得
...
\end{aligned}
\end{equation}
那么\autoref{eq_atmDen_5}~\upref{atmDen}变为
\begin{equation}
\frac{\gamma}{\gamma-1}T'(z)=-\frac{\mu g}{R}~.
\end{equation}
大气温度 $T$ 总是 $>0$ 的， 且大气的绝热指数 $\gamma>1$，上式右侧 $<0$，左侧 $<0$，这意味着温度随高度的增加而降低。 再根据气体热容量公式\autoref{eq_ThCapa_4}~\upref{ThCapa}约去 $\gamma$，积分得：

...
\item \enref{柯西序列（Cauchy sequence）}{cauchy}： 对任意 $\varepsilon > 0$， 存在 $N$， 当 $n, m \geqslant N$ 就有 $\norm{u_n - u_m} < \varepsilon$
...
\begin{itemize}
\item 赋范空间 $X$ 是 \enref{Banach 空间}{banach}当且仅当每个柯西数列都收敛（到 $X$ 的元素）

\item 在 Banach 空间中， 收敛序列都是柯西序列\footnote{在完备空间中，收敛序列与柯西序列等同。}

\item 空间 $X := C[a, b]$ 是实数 Banach 空间， 定义范数为 $\norm{u} := \max_{a \leqslant x \leqslant b} \abs{u(x)}$。 $u_n \to u$ 意味着 $\norm{u_n - u} = \max_{a \leqslant x \leqslant b}  \abs{u_n(x) - u(x)} \to 0$。 也就是 $u_n(x)$ \enref{一致收敛}{UniCnv}到 $u$

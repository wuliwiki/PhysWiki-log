...
要证明反对称性很简单， 任意交换算符 $P_{i,j}$ 作用在\autoref{symetr_eq3} , 相当于把行列式的两行置换（这里其实交换的是行列式的两列，但是矩阵的行列式等于矩阵转置的行列式）， 而根据行列式的性质（\autoref{DetPro_the6}~\upref{DetPro}）， 这会使结果取相反数． 证毕．

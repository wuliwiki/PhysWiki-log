% 重积分、面积分、体积分（简明微积分）
% keys 多元微积分|积分|定积分|重积分|面积分|二重积分|双重极限|体积分|三重积分|三重极限
% license Xiao
% type Tutor
...
\pentry{定积分\upref{DefInt}}{nod_6152}
\subsection{面积分}
\textbf{面积分}也叫\textbf{二重积分}， 可以看做一元函数定积分\upref{DefInt} 的一种拓展。 从几何上来理解， 如果一元函数的定积分是计算一定区间内被积函数曲线与坐标轴之间的面积， 那么二重积分就是计算一定二维区域内被积函数曲面与坐标轴平面之间的体积（\autoref{eq_IntN_1} ）。

\begin{figure}[ht]
\centering
\includegraphics[width=7cm]{./figures/5bea347882213c85.png}
\caption{$f(x, y)$ 曲面下的体积} \label{fig_IntN_1}
\end{figure}

对一元函数的定积分， 我们需要指定积分的下限和上限， 那么对于面积分， 我们需要指定 $x$-$y$ 平面上的一块定积分的区域 $\mathcal S$。 例如\autoref{eq_IntN_1} 表示的面积分的区域就是一个正方形。 与一元函数的情况相同， 我们把积分区域划分成许多小长方形， 每个小长方形的面积为 $\Delta x_i \Delta y_j$。 把二重积分对应的体积用一个个柱体来近似， 每个柱体的体积为 $\Delta x_i \Delta y_j f(x_i, y_i)$ 其中 $(x_i, y_i)$ 是长方形中任意一点。 当这些小长方形趋近无穷小时， 就得到了精确的体积。 用双重极限表示为
% 未完成：双重极限，引用
\begin{equation}\label{eq_IntN_1}
...
\begin{example}{抛物线下的体积}\label{ex_IntN_1}
...
\begin{equation}\label{eq_IntN_8}
...
\begin{equation}\label{eq_IntN_2}
...
与二重积分类似， 要从定义上来证明这种算法可行， 就把\autoref{eq_IntN_8} 改写成
\begin{equation}
...
在柱坐标和球坐标下，函数 $f(x,y,z)$ 变为 $f(r,\theta,z)$ 和 $f(r,\theta,\phi)$，而体积元 $\dd x\dd y\dd z$ 变为 $r\dd r\dd\theta\dd z$ \autoref{eq_CrIntN_1}~\upref{CrIntN}和 $r^2\sin\theta\dd r\dd\theta\dd\phi$ \autoref{eq_CrIntN_3}~\upref{CrIntN}.故三重积分\autoref{eq_IntN_2} 在柱坐标和球坐标下分别表示为
\begin{equation}

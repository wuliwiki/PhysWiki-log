...
直角坐标是我们最常用的坐标之一。 在三维向量空间中， 首先确定三个互相垂直的单位向量（通常还要求符合\enref{右手定则}{RHRul}），这里分别记为 $\uvec x, \uvec y, \uvec z$ （也有教材记为 $\uvec i, \uvec j, \uvec k$）。 显然， 它们是线性无关的， 可以作为基底。 要确定从坐标原点到空间中任意一点的向量 $\bvec v$， 我们可以画出一个长方体（\autoref{fig_GVec_3}）， 它沿三个方向的边长 $x, y, z$ 就是向量 $\bvec v$ 的三个坐标（三个有序实数）， 满足线性组合

...
三个球坐标分别有对应的单位矢量 $\uvec r, \uvec\theta, \uvec\phi$ （如\autoref{fig_Sph_1} ）。定义它们的方向分别指向对应坐标增加的方向，例如 $r$ 增加时，点 $P(r,\theta ,\phi )$ 就向 $\uvec r$ 的方向移动， $\theta$ 增加时 $P(r,\theta ,\phi )$ 就向 $\uvec \theta$ 方向移动。三个单位矢量两两垂直，形成一组\enref{正交归一基底}{OrNrB}，任意三维矢量都可以表示成它们的线性组合（\autoref{eq_OrNrB_5}~\upref{OrNrB}）
...
对于球坐标为 $(r, \theta , \phi )$ 的任意点， 球坐标和直角坐标间的转换以及单位矢量 $\uvec r, \uvec \theta, \uvec \phi$  与 $\uvec x, \uvec y, \uvec z$ 之间的转换见详见 “\enref{球坐标与直角坐标的转换}{SphCar}”， 这里引用（\autoref{eq_SphCar_1}~\upref{SphCar} 和\autoref{eq_SphCar_2}~\upref{SphCar}）：
\begin{equation}
\begin{cases}

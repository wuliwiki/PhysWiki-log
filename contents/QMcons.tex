...
答案是否定的， 在量子力学中， 若对系统的任意状态测量某个物理量得到任意本征值的概率都不随时间改变， 这个量就是守恒的． 例如在无限深势阱中随时间变化的波包， 测得每个能量的概率都不随时间变化， 这就说明能量守恒． 由这个定义可知， 守恒量的平均值也不随时间变化．
...
\end{theorem}
证明： 令能量的所有本征态为 $\{\ket{u_i}\}$， 系统状态随时间的演化可以记为（$C_i$ 为常数， 由初始状态决定）
\begin{equation}
\ket{v(t)} = \sum_i C_i \E^{-\I E_i t} \ket{u_i}
\end{equation}
令能量 $E_j$ 对应的所有本征态的角标为 $D_j$， 那么测得 $E_j$ 的概率为
\begin{equation}
...
， 有限深势阱\upref{FiSph}， 阶梯势能\upref{StpPot}等． 这些系统中， 将任意时刻的波函数投影到能量的本征态上， 得到的系数的模方都是相同的．
\end{example}
...
我们还是从有限维空间的简单情况来证明这个定理， 且假设哈密顿算符不含时\footnote{如果含时， 可以将时间分割成许多小区间， 假设每个小区间中不随时间变化}． 首先由 2 证明 1， 若两算符对称， 则它们必有一组完备的共同本征矢 $\{\ket{u_i}\}$， 满足
\begin{equation}
\leftgroup{
...
\ket{v(t)} = \sum_i C_i \E^{-\I E_i t} \ket{u_i}
\end{equation}
令任意 $q_m$ 子空间的一组基底为 $\{\ket{v_k}\}$， 那么任意时刻测得 $q_m$ 的概率为
\begin{equation}
\begin{aligned}

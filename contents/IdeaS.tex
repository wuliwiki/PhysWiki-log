...
设某理想气体由 $N $ 个原子组成，体积为 $V$，能量为 $U$，现在，我们利用玻尔兹曼熵公式计算它的熵（可以加上一个常量）。假设 $N $ 为定值，但是 $U $ 和 $V $ 可以变化。我们要求的量是 $S(U,V)$。为了计算熵，我们利用玻尔兹曼公式\footnote{具体的内容参考\enref{熵的微观定义与玻尔兹曼公式}{entro2}。}转化为求系统的微观状态数 $\Omega(U,V)$，再求 $S=k\ln \Omega$。
...
具体地求解微观状态数需要利用 $3N$ 维空间中球的体积公式。为了便于计算，我们舍去前面引入的 $a,b$ 变量，而是将 $6N$ 维\enref{相空间}{PhSpace}中每个体积为 $h^{3N}$ 的相格看作是一个微观状态，$h$ 是普朗克常数，将相空间划分为一个个相格可以理解为，在量子力学中坐标和动量的不确定性原理 $[x,p]=i\hbar$ 使得系统的能级是分立的。那么微观状态数密度\footnote{具体定义的细节参考\enref{理想气体的状态密度（相空间）}{IdSDp}。}就可以利用 \autoref{eq_PhSpace_2}~\upref{PhSpace}表达为：
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
其中 $\theta(x)=1$ 当且仅当 $x>0$，$\theta$ 被称为阶跃函数。最后一行的推导利用了多维球体的体积公式\footnote{参考\enref{多维球体的体积公式}{NSphV}}。代入熵公式可以得到
\begin{equation}
\begin{aligned}

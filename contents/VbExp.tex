...
\pentry{二阶常系数齐次微分方程\upref{Ode2}}{nod_68d8}
...
\subsection{振动的叠加}
这里举另一个例子说明使用指数函数表示振动比三角函数方便。 假设有若干个频率相同但振幅和初相位各不相同的振动 $x_i(t) = A_i \cos(\omega t + \varphi_{0i})$， 现在我们来计算它们叠加的结果， 即 $\sum_i x_i(t)$。 若用两角和公式（\autoref{eq_TriEqv_2}~\upref{TriEqv}）直接计算， 得
\begin{equation}\ali{

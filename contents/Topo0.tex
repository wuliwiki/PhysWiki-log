% 点集的内部、外部和边界
% keys 点集|内部|外部|边界|拓扑|开集|孤立点|闭集
% license Xiao
% type Tutor

\pentry{拓扑空间\nref{nod_Topol}}{nod_335c}
...
注意，这里定义的邻域必须是开集。在\enref{紧致性}{Topo2}一节中，我们会提到一个容易混淆的概念，“紧邻域”，它并不一定是开集，因此不一定是邻域。紧邻域的名字是有些混淆，但是没有办法，约定俗成这么叫了\footnote{事实上，一种常见的表达方式是在“邻域”两字前加其它修饰，这样表达的往往不是邻域（即不是开集），而是包含某个邻域的集合。“紧邻域”表示包含一个邻域的紧集，“闭邻域”表示包含一个邻域的闭集，它们往往都不是开集，从而不是邻域。}。

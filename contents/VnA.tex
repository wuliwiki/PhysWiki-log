...
\pentry{速度、加速度（一维）\nref{nod_VnA1}， 牛顿第二定律的矢量形式\nref{nod_New2}， 切线\nref{nod_TanL}，矢量的导数\nref{nod_DerV}， 矢量积分}{nod_b2ba} % 未完成
...
质点在运动时，其\enref{位矢}{Disp} $\bvec r$ 是时间 $t$ 的函数。假定质点在 $t_1$ 时刻的位矢为 $\bvec r(t_1)$，经过时间 $\Delta t$后， 位矢为 $\bvec r(t_1 + \Delta t)$， 所以物体在 $\Delta t$ 时间内的\enref{位移}{Disp}为
\begin{equation}
...
\end{equation}
即速度是位置矢量关于时间的\enref{导数}{DerV}， 同样是时间的矢量函数。

\begin{theorem}{}
速度方向总是质点运动轨迹的\enref{切线}{TanL}方向。
\end{theorem}

简单的例子： 匀速\enref{圆周运动的速度（求导法）}{CMVD}。
...
加速度可以有垂直于速度的分量，与平行于速度的分量，详见\enref{曲线运动的加速度}{PCuvMo}。

\textbf{简单的例子}： 圆周运动的加速度（\autoref{sub_CMAD_1}~\upref{CMAD}）。

\subsection{由速度或加速度计算位矢}

如果已知速度关于时间的函数 $\bvec v(t)$， 以及初始时间 $t_0$ 和位置 $\bvec r_0$， 该如何得到位移—时间函数 $\bvec r(t)$ 呢？ 类比\enref{一维的情况}{VnA1}， 我们也可以通过矢量函数的定积分\upref{IntV}（见\autoref{ex_IntV_1}） 来求出速度—时间函数进而求出位移—时间函数
...
\textbf{简单的例子}： \enref{匀加速运动}{ConstA}。

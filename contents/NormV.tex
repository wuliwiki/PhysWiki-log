...
在范数\upref{norm}一文中，我们定义了范数和赋范空间。赋范空间很自然的是一个度量空间\upref{Metric}，度量为 $d(x,y) := \norm{x-y}$，作为度量空间时完备的赋范空间称为\textbf{巴拿赫空间（Banach space）}\upref{banach}。特别的，赋范空间都是拓扑向量空间\upref{tvs}。
...
$$
其中 $\pi:x\to x+\text{Ker}(T)$ 是商投影， $T_1:x+\text{Ker}(T)\to T(x)$ 是商映射， 它是到像空间 $\text{Ran}(T)$ 的一一映射， 但逆映射却不一定有界。 关于有界算子的像空间的性质， 详见巴拿赫定理\upref{BanThm}。 %\addTODO{链接}

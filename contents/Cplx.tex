...
\end{equation}
只是这里的除法变为复数的除法（\autoref{eq_CplxNo_7}~\upref{CplxNo}）。 讨论一般复变函数的导数较为复杂， 因为它不但取决于 $z$ 的值， 还取决于求导的方向（即 $\Delta z$ 趋近于 $0$ 的方向）， 这有点类似于\enref{方向导数}{DerDir}。 但我们通常研究的解析函数有一个非常好的性质： 如果复变函数在某个区域上解析， 那么在该区域上其导数与求导方向无关。 详见 “\enref{柯西—黎曼条件}{CauRie}”。

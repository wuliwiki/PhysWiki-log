...
\end{aligned}
\end{equation}
最后两步可以看作对一个表格求和， 第 $i$ 行第 $j$ 列的值为 $C_a^i C_b^j x^{i+j}$， 求和的方式有两种， 一种是每行每列求和， 另一种是延斜线求和， 每条斜线 $n = i+j$ 的值相同。 注意每条斜线中 $i$ 的范围未必相同， 满足\autoref{eq_ChExpn_2}。

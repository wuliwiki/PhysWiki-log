...
\pentry{内积、内积空间\upref{InerPd}}{nod_844e}

在带有内积的矢量空间（实数域 $\mathbb R$ 或复数域 $\mathbb C$ 上的）中，矢量的模的性质可用来定义度量，使得矢量空间可看成一个度量空间。反过来，满足一定性质的度量也可用来定义矢量的模。

先列出矢量\textbf{模} $\norm{v}=\sqrt{\braket{v}{v}}$ 的性质（设矢量空间为 $V$）（\autoref{eq_InerPd_8}~\upref{InerPd}）：
\begin{enumerate}
\item 
...
而\textbf{度量}是满足以下三个条件的函数 $d:E\times E\rightarrow\mathbb R$（ $E$ 为任意的集合）（\autoref{def_Metric_2}~\upref{Metric}）：
\begin{enumerate}
...
\begin{example}{}
在 $V=C_2(a,b)$ （\autoref{ex_HVorUV_1}~\upref{HVorUV}）中，
\begin{equation}

...
先列出矢量\textbf{模} $\norm{v}=\sqrt{\braket{v}{v}}$ 的性质(设矢量空间为 $V$)（\autoref{InerPd_eq8}~\upref{InerPd}）：
\begin{enumerate}
\item 
...
而\textbf{度量}是满足以下三个条件的函数 $d:E\times E\rightarrow\mathbb R$（ $E$ 为任意的集合）（\autoref{Metric_def2}~\upref{Metric}）：
\begin{enumerate}

...
在二阶微扰公式\autoref{eq_TIPT2_2}~\upref{TIPT2} 中， 类氢原子的 $E_n^0 \propto Z^2$， 分子中的\enref{跃迁矩阵元}{HDipM} $\propto 1/Z$， 所以二阶能量修正 $E_n^2 \propto 1/Z^4$（参考 \cite{bookAMO}）。
...
\end{equation}
和电场成正比和 $Z$ 成反比，而二阶能量修正（\autoref{eq_TIPT2_2}~\upref{TIPT2}）
...
\end{equation}
\autoref{eq_TIPTc_1}~\upref{TIPTc} 在 Stark 效应中就是
...
\end{equation}
其中 $\ket{n,\alpha,m}$ 不过是 $\ket{n,l,m}$ 的线性组合。 第一个矩阵元为（\autoref{eq_HDipM_1}~\upref{HDipM}）
...
而第二个也是大同小异（\autoref{eq_HyIon2_3}~\upref{HyIon2}），
\begin{equation}\ali{ % 已验证：氢原子 |1,0,0> |2,0,0>polarizability

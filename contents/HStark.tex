...
\pentry{不含时微扰理论\nref{nod_TIPT}，含连续态的不含时微扰理论（量子力学）\nref{nod_TIPTc}，氢原子的跃迁偶极子矩阵元计算（束缚态之间）\nref{nod_HDipM}}{nod_8e0d}
...
其中第一项就是 $-E_n^1/\mathcal{E}_z$。 第二项第三项分别是二阶和三阶小量，所以第三项可忽略。 根据\autoref{eq_TIPT2_4}， $\mel{\psi_n^0}{z}{\psi_n^1} = E_n^2/\mathcal{E}_z$ 对应二阶能量修正，所以
\begin{equation}

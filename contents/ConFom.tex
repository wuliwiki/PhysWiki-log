...
$M$是黎曼流形，$\nabla$是丛上的联络。
\begin{itemize}
\item 若联络与度规兼容，且 $\{\uvec{e}_i\}$ 是 $\mathfrak{X}(U)$ 上的任意\textbf{标准正交基}，那么在这组基下，联络形式矩阵 $\omega^j_i$ 必是\textbf{反对称}，即 $\omega^j_i=-\omega^i_j$。
...
证明第二条定理如下：设$X,Y$是流形上的任意两个光滑切向量场，则我们可以把$Y$表示为：$Y=\bvec \theta^i(Y)\uvec{e}_i$，那么我们有：
\begin{equation}
\begin{aligned}
...
\begin{corollary}{}
令$M$是黎曼流形，$\{\uvec e_i\}$是$\mathfrak{X}(U)$上的一组标准正交基。令$\{\uvec\theta^i\}$是1形式的对偶基。那么存在\textbf{唯一的}\textbf{反对称}联络形式矩阵使得
\begin{equation}
d \theta^{i}+\omega_{j}^{i} \wedge \theta^{j}=0 \quad \text { for all } i=1, \ldots, n .~.
\end{equation}
\end{corollary}
\textbf{证明：}由\autoref{the_ConFom_1} 可知，联络形式反对称意味着该联络对度规兼容。那么这条推论实际上是\autoref{the_RieCon_1}~\upref{RieCon}，即黎曼联络的存在性和唯一性。

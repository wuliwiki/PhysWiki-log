...
仿射联络的定义是高度抽象的，并不涉及具体的运算。我们在本节所介绍的是将来进行计算时非常关键的理论基础。计算的实例请参见\enref{庞加莱半平面（微分几何计算实例）}{PoiHP}。
...
实际上，联络和联络形式并不一定是只能定义在流形的切丛上，它也可以定义在流形上的任何向量丛上。具体拓展请参见\enref{联络（向量丛）}{VecCon}文章。
...
关于对偶基的更多细节，请参见\enref{对偶空间}{DualSp}。
\begin{exercise}{}
...
\addTODO{证明有些长，待补充。}
证明第二条定理如下：设$X,Y$是流形上的任意两个光滑切向量场，则我们可以把$Y$表示为：$Y=\bvec \theta^i(Y)\bvec e_i$，那么我们有：
\begin{equation}
\begin{aligned}
\nabla_{X} Y & =\nabla_{X}\left(\bvec\theta^{j}(Y) \bvec e_{j}\right) \\
& =\left(X \bvec\theta^{j}(Y)\right) \bvec e_{j}+\bvec\theta^{j}(Y) \nabla_{X} \bvec e_{j} \\
& =\left(X \bvec \theta^{j}(Y)\right) \bvec e_{j}+\bvec\theta^{j}(Y) \omega_{j}^{i}(X) \bvec e_{i}
\end{aligned}~.
\end{equation}
根据对称性，可以得到$\nabla_{Y} X =\left(Y \bvec \theta^{j}(X)\right) \bvec e_{j}+\bvec\theta^{j}(X) \omega_{j}^{i}(Y) \bvec e_{i}$。
于是我们可以根据挠率的定义，证明定理中的形式：
\begin{equation}
\begin{aligned}
T(X, Y) & =\nabla_{X} Y-\nabla_{Y} X-[X, Y] \\
& =\left(\left(\left(X \bvec\theta^{i}(Y)-Y \bvec\theta^{i}(X)-\bvec\theta^{i}([X, Y])\right)+\left(\omega_{j}^{i}(X) \bvec\theta^{j}(Y)-\omega_{j}^{i}(Y) \bvec\theta^{j}(X)\right)\right) \bvec e_{i}\right. \\
& =\left(d \bvec \theta^{i}+\omega_{j}^{i} \wedge \bvec \theta^{j}\right)(X, Y) \bvec e_{i}.
\end{aligned}~
\end{equation}

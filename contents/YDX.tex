...
$$\mathbf{V}_P = \left[ \dot{\mathbf{T}}(t) \right] \mathbf{p} = \begin{Bmatrix} \mathbf{V}_P \\ 0 \end{Bmatrix} = \begin{bmatrix} \mathbf{A}(t) & \dot{\mathbf{d}}(t) \\ 0 & 1 \end{bmatrix} \begin{Bmatrix} \mathbf{p} \\1 \end{Bmatrix} = \begin{bmatrix} \dot{\mathbf{A}}(t) & \dot{\mathbf{d}}(t) \\ 0 & 0 \end{bmatrix} \begin{Bmatrix} \mathbf{p} \\ 1 \end{Bmatrix}~.$$
点表示相对于时间的导数；因为 $\mathbf{p}$ 是常数，它的导数为零。

这个公式可以通过对在固定框架中测得的 p(t) 的轨迹进行运算来获得 p 的速度。将 p 的逆变换代入速度方程得到：
\begin{equation}
\mathbf{V}_P = \left[ \dot{\mathbf{T}}(t) \right] \left[ \mathbf{T}(t) \right]^{-1} \mathbf{p}(t) =
\begin{Bmatrix}
\mathbf{V}_P \\
0
\end{Bmatrix} 
...
= 
\begin{bmatrix}
\dot{A} A^{-1} & -\dot{A} A^{-1} \mathbf{d} + \dot{\mathbf{d}} \\
0 & 0
\end{bmatrix}
\begin{Bmatrix}
\mathbf{P}(t) \\
...
=
\begin{bmatrix}
\dot{A} A^T & -\dot{A} A^T \mathbf{d} + \dot{\mathbf{d}} \\
0 & 0
\end{bmatrix}
\begin{Bmatrix}
\mathbf{P}(t) \\
...
这个方程可以首先通过计算来扩展
$$\dot{[S]} = \begin{bmatrix}
\dot{\Omega} & -\dot\Omega {d}  -\Omega \dot{d}+ \ddot{d} \\
0 & 0
\end{bmatrix} = \begin{bmatrix}
\dot{\Omega} & -\dot\Omega {d} - \Omega V_O + A_O \\
0 & 0
\end{bmatrix}~$$
和
$$[S]^2 = \begin{bmatrix}

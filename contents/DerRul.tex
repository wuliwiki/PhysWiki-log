% 求导法则（简明微积分）
% keys 微积分|导数|求导法则|线性运算
% license Xiao
% type Tutor

\pentry{基本初等函数的导数\nref{nod_FunDer}}{nod_4914}
...
f[g(x)]' = f'[g(x)]g'(x)~.
\end{equation}
详见“\enref{一元复合函数求导（链式法则）}{ChainR}”

\subsubsection{反函数}
\begin{equation}
[f^{-1}(x)]' = \frac{1}{f'[f^{-1}(x)]} ~,
\end{equation}
详见“\enref{反函数求导}{InvDer}”。
...
\subsection{乘积法则}
现在证明\autoref{eq_DerRul_6}， 令两函数分别为 $f(x)$ 和 $g(x)$， 现在求 $f(x) g(x)$ 的导函数。由\enref{导数的定义}{Der}得
...
\begin{figure}[ht]
\centering
\includegraphics[width=6.0cm]{./figures/ca4b452ff673be76.pdf}

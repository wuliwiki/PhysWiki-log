% 引力波和测试质量的相互作用
% keys 引力波|测地线|偏移方程|局域惯性系|自由落体参考系
% license Xiao
% type Tutor
...
\begin{equation}\label{eq_intera_1}
c^2 d \tau^2  = - ds^2 = - g_{\mu\nu} dx^\mu dx^\nu ~. 
\end{equation}
我们可以直接使用 $\tau$ 来作为参数 $\lambda$. 因此 $x^\mu = x^\mu(\tau)$. 从\autoref{eq_intera_1} 我们可以导出
\begin{equation}\label{eq_intera_2}
...
\end{equation}
这给出了测地线方程
\begin{equation}\label{eq_intera_3}
...
\end{equation}
现在考虑隔得很近的两个测地线，一个是 $x^\mu(\tau)$，另一个是 $x^\mu(\tau)+\xi^\mu(\tau)$. $x^\mu(\tau)$ 满足\autoref{eq_intera_3}，$x^\mu(\tau) + \xi^\mu(\tau)$ 满足
\begin{equation}\label{eq_intera_4}
\frac{d^2(x^\mu+\xi^\mu)}{d\tau^2} + \Gamma^\mu_{\nu\rho} (x+\xi) \frac{d(x^\nu+\xi^\nu)}{d\tau} \frac{d x^\rho+\xi^\rho}{d \tau} = 0~.
\end{equation}
\autoref{eq_intera_4} - \autoref{eq_intera_3} 并且展开到 $\xi$ 的一阶，我们有
\begin{equation}\label{eq_intera_5}
...
首先，从 $\{e_{\alpha}\}$ 的定义我们可以自然推导出 $g_{\mu\nu}(P) = \eta_{\mu\nu}$ 这样一个事实。为了证明 $\Gamma^\mu_{\nu\rho}(P)$ 也成立，考虑测地线方程\autoref{eq_intera_3}。依照定义，坐标对于固有时是线性的，这说明 $d^2 x^\mu/d \tau^2 = 0$, $dx^\mu/d\tau = n^\nu$. 因此测地线方程变成了如下形式
\begin{equation}
\Gamma^\mu_{\nu\rho} (P) n^\nu n^\rho = 0 ~.
\end{equation}
...
\subsubsection{TT参考系}
在TT参考系下，引力波可以呈现出特别简单的形式。从\autoref{eq_intera_3} 可以看出
\begin{equation}

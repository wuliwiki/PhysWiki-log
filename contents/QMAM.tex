...
很容易证明，经典的角动量 $\vec L = \vec r\times \vec p$ 对应的算符是一个很个性的算符：它在直角坐标中的三个分量都不对易，而且它们三个都是平权的． 我们知道，能够同时确定的物理量必须满足：它们有共同的本征态，换句话说，它们彼此对易． 因此，我们在这三个分量里面只能有一个能确定． 既然平权，我们不妨取为 $z$ 方向分量． 计算仍然给出这三个方向的角动量分量算符，和总角动量的平方这个标量算符也是对易的\footnote{这一段里的结论的验算可以参考后面的练习．}．

写出来就是
\begin{align}
[\hat L_z, \hat{\vec L}^2] &= 0 \label{QMAM_eq1}\\
...
\begin{align}
\hat{\vec L}^2|l,m\rangle &= l(l+1)|\hbar^2l,m\rangle\\
...
考虑到角动量算符的定义，
\begin{equation}
{\hat{\vec L}} = \displaystyle\frac{1}{2}\left({\hat{\vec p}}\times {\hat{\vec r}} - {\hat{\vec r}}\times {\hat{\vec p}}\right)
\end{equation}
...
这个的推导很简单，可以自己完成． 我们要讲的，是 $L_{\pm}|l,m\rangle$ 这个东西，它也是 $L_z, {\vec L}^2$ 的本征态． 前者很简单，我们只需要计算下面这个式子：
\begin{equation}
...
其中 $c$ 是一个待确定的数，可以明确的知道它和 $l,m$ 有关． 只需要确定它是不是 ${\vec L}^2$ 的本征值为 $l$ 的本征态就行了． 我们可以去计算
\begin{equation}
{\vec L}^2L_{\pm}|l,m\rangle
\end{equation}

这次，我们得先做点工作：我们直接重新写 ${\vec L}^2$ 的表达形式：
\begin{equation}
{\vec L}^2 = L_z(L_z+1)+L_-L_+
\end{equation}

由\autoref{QMAM_eq10}，很容易证明这个写法满足
\begin{equation}\ali{
{\vec L}^2L_{\pm}|l,m\rangle &=  L_z(L_z+1)L_{\pm}|l,m\rangle + L_-L_+L_{\pm}|l,m\rangle\\
...
从而
\begin{equation}\ali{
&\quad\,\, {\vec L}^2L_{+}|l,m\rangle \\
&= (m+1)(m+2)L_{+}|l,m\rangle +(L_+ (L_-L_+) - 2L_zL_+)|l,m\rangle \\
&= (m+1)(m+2)L_{+}|l,m\rangle +\left(L_+ ({\vec L}^2 - L_z(L_z+1)) -2(m+1)L_+\right)|l,m\rangle \\
...
&= m(m-1)L_{-}|l,m\rangle +\left(L_- ({\vec L}^2 - L_z(L_z+1)) + 2mL_-\right)|l,m\rangle \\
...
\end{equation}
这一下就简单了，因为我们注意到，在 ${\vec L}^2$ 里面也有类似的项． 分开来看：
\begin{equation}
L_-L_+ = {\vec L}^2 - L_z(L_z+1)
\end{equation}
于是
\begin{equation}
...
类似的，
\begin{equation}
L_+L_- = L_-L_+ +2L_z = {\vec L^2} -L_z(L_z-1)
\end{equation}
于是
\begin{equation}

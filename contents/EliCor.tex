...
令椭圆坐标轴 $\xi$、 $\eta$ 对应的单位矢量分别为 $\uvec{\xi}$、 $\uvec{\eta}$，由\autoref{eq_CurCor_8}~\upref{CurCor}
\begin{equation}
\left\{
    \begin{aligned}
...
显然，\autoref{eq_EliCor_4} 、\autoref{eq_EliCor_5} 成立。现在情况变成： $\xi$ 的等值曲面为一组共焦椭圆柱面族，而 $\eta$ 的等值曲面为一组共焦的双曲柱面族， $z$ 的等值面为椭圆坐标面。
...
结合\autoref{eq_CVecOp_7}~\upref{CVecOp} 到\autoref{eq_CVecOp_6}~\upref{CVecOp}，注意这里，$u=\xi,v=\eta,w=z,H_\xi=f,H_\eta=g,H_z=h$，得
...
%\addTODO{写出梯度散度旋度， 引用 “\enref{正交曲线坐标系中的矢量算符}{CVecOp}”}

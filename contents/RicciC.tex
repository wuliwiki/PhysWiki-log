...
\pentry{黎曼联络\nref{nod_RieCon}}{nod_38e0}

本节中默认 $(M, \nabla)$ 是一个配备了仿射联络 $\nabla$ 的流形 $M$。

\subsection{黎曼曲率张量}

我们在这里重新誊写一遍\enref{仿射联络（切丛）}{affcon}中定义的\textbf{曲率}映射。

\begin{definition}{曲率}
定义 $R:\mathfrak{X}(M)\times\mathfrak{X}(M)\to\opn{End}(\mathfrak{X}(M))$ 为：对于任意 $X, Y\in\mathfrak{X}(M)$，有 $R(X, Y)=\nabla_X\nabla_Y-\nabla_Y\nabla_X-\nabla_{[X, Y]}$，称该映射为 $(M, \nabla)$ 的\textbf{曲率（curvature）}。
\end{definition}

为什么要这么定义曲率呢？平坦流形和弯曲流形有一个关键的区别，那就是平行移动是否依赖路径。平坦的时空中，对一个向量进行平行移动，不管它沿着什么路径走，只要回到原点，它就和出发时是同一个向量；但是不平坦的流形上，如\enref{仿射联络（切丛）}{affcon}中地球表面的例子，平行移动后回到原点时是哪个向量，取决于走的是什么道路。
...
\begin{theorem}{反对称性}\label{the_RicciC_4}
...
由\autoref{eq_RicciC_7} 得：$\partial_iR^i{}_{j}=\frac{1}{2}\partial_jR$，也等价于更为常见的：$\partial^iR_{ij}=\frac{1}{2}\partial_{j}R$。
\end{corollary}
\textbf{Proof.}
...
\end{aligned}
\end{equation}
第三个等号处“$R^c{}_{bde;c}=-R_b{}^c{}_{de;c}$”源自\autoref{the_RicciC_4} 的反对称性，两边乘以“$g_{ck}$”可证。
证毕。

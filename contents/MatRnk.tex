...
给定一组$n\times m$的矩阵$\mathcal{V}_1, \mathcal{V}_2, \cdots, \mathcal{V}_n$，如果存在\textbf{不全为零}的系数$a_1, a_2, \cdots, a_n$使得$a_1\mathcal{V}_1+a_2\mathcal{V}_2+\cdots+a_n\mathcal{V}_n$是\textbf{零矩阵}，那么称这组矩阵\textbf{线性相关（linearly dependent）}；否则，称其\textbf{线性无关（linearly independent）}。
...
线性相关性的概念源自向量坐标的唯一性。以几何向量为例，当给定一组向量$\bvec{v}_1, \bvec{v}_2, \cdots, \bvec{v}_n$，其线性组合$a_1\bvec{v}_1+a_2\bvec{v}_2+\cdots+a_n\bvec{v}_n$也是一个向量，我们可以称列矩阵$\pmat{a_1&a_2&\cdots&a_n}^{\mathrm{T}}$为该向量的\textbf{坐标}。两个向量相加后的坐标，就是两个向量的坐标矩阵直接相加。零向量显然有一个坐标，即零矩阵$\pmat{0&0&\cdots&0}^{\mathrm{T}}$；如果零向量还有一个\textbf{非零}的坐标$\pmat{x_1&x_2&\cdots&x_n}^{\mathrm{T}}$，那么任何向量也都会有无穷多个坐标。比如说，有一个坐标为$\pmat{1&2&\cdots&n}^{\mathrm{T}}$的向量，其坐标值也可以是$\pmat{1+x_1&2+x_2&\cdots&n+x_n}^{\mathrm{T}}$，乃至$\pmat{1+2x_1&2+2x_2&\cdots&n+nx_n}^{\mathrm{T}}$。各向量的坐标唯一，当且仅当零向量的坐标唯一，而零向量的坐标唯一等价于“对于不全为零的系数$a_1, a_2, \cdots, a_n$，线性组合$a_1\bvec{v}_1+a_2\bvec{v}_2+\cdots+a_n\bvec{v}_n$不可能是零向量”，这便是线性无关的定义。
...
判断一个方阵是否为满秩矩阵的一种常见方法是计算方阵的\enref{行列式}{Deter}，若结果不为零， 则矩阵是满秩的， 否则不是（\autoref{the_DetPro_2}）。 注意非满秩的情况下行列式并不能判断秩具体是多少。 另一种更简单的方法是使用下面的高斯消元法。

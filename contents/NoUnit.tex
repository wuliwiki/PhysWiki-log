...
\pentry{万有引力\nref{nod_Gravty}， 物理量和单位转换\nref{nod_Units}}{nod_e9ef}

与数学公式不同，物理公式中的变量（这里姑且称为\textbf{物理量}）通常既包含数值又包含单位，如 $x = 10\Si{cm}$。 一个形象的比喻是把所谓 “物理量” 比作一个\enref{几何矢量}{GVec}， 它本身不是一个单纯的数字， 而是一个基底乘以一个数字。 但有时候我们需要将含有单位的公式转换为不含单位的物理公式。 例如数值计算时， 或者为了书写简单（常见于量子力学， 见\enref{原子单位制}{AU}）。 我们通过几个例子来说明转换过程。

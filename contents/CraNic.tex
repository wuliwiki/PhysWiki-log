...
\pentry{薛定谔方程（单粒子一维）\upref{TDSE11}}{nod_4e84}
...
\subsection{等间距网格}
对于等间距坐标网格 $x_1,x_2,\dots$， 可以用差分法（\autoref{eq_DerDif_5}~\upref{DerDif}）计算二阶导数， 表示为矩阵有
\begin{equation}
...
\subsection{虚时间法}
\pentry{虚时间法求基态波函数\upref{ImgT}}{nod_f168}
...
\subsection{代码}
\pentry{Matlab 的稀疏矩阵\upref{MatSpa}}{nod_6a5b}
Matlab 代码如下， 使用\autoref{eq_CraNic_5}， 以及 Matlab 的稀疏矩阵\upref{MatSpa}。 势能函数可以在 \verb|V_fun| 中设置， 我们以方势垒为例， 所有参数和 “高斯波包的方势垒散射数值计算（Matlab）\upref{FSBsct}” 相同。 不同的是， 由于我们使用迪利克雷边界条件， 波函数到达边界后会发生全反射。 要避免反射， 可以用开放边界条件（未完成）以及吸收势能（未完成）。

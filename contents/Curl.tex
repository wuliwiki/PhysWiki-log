...
现在来定义该矢量场的\textbf{旋度（curl）}。 旋度是一个矢量， 记为 $\curl \bvec F$。 如\autoref{fig_Curl_3} ， 在空间某点 $(x,y,z)$ 处选取一个小面元 $\bvec S$（模长为面元的面积， 方向为面元的一个法向量）， 令面元边界构成的回路为 $\mathcal L$， 正方向由\enref{右手定则}{RHRul} 判断。 要定义旋度 $\curl \bvec F$ 在 $x$ 方向的分量， 就取 $\bvec S$ 与 $x$ 轴单位矢量 $\uvec x$ 同向， 再计算环流量除以面积的极限， 即
\begin{equation}

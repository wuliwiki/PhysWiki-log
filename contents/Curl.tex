...
\end{equation}
下面我们来定义\bb{旋度}， 旋度是一个矢量， 记为 $\curl \bvec F$． 在空间某点 $(x,y,z)$ 处选取一个小面元 $\bvec S$（模长为面元的面积， 方向为面元的一个法向量）， 令面元边界构成的回路为 $\mathcal L$， 正方向由右手定则\upref{RHRul} 判断． 要定义其 $x$ 方向的分量， 就取 $\bvec S$ 与 $\uvec x$ 同向， 即
\begin{equation}
...
如\autoref{Curl_fig2}， 我们选取一块曲面， 并规定一个正方向． 使用右手定则\upref{RHRul}， 我们也可以定义曲面边界的正方向． 空间中存在连续光滑的矢量场 $\bvec F(\bvec r)$， 则\bb{斯托克斯定理}可以将矢量场在曲面边界上的环流量和矢量场的旋度在曲面上通量等同起来
\begin{equation}

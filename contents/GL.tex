...
$\opn{GL}(n, \mathbb{C})$ 构成一个群，同时也是拓扑空间 $M_n(\mathbb{C})$ 的一个开集合（因此是个子流形\autoref{def_SubMnf_1}，同时也是一个李群\autoref{def_LieGrp_1}）。
...
% \begin{enumerate}
% \item $\mathbb{R}$ 和 $\mathbb{C}$ 都是（Hausdorff\autoref{def_Topo5_2}）拓扑空间：因此 $\opn{GL}(n; \mathbb{R})$ 是 $\opn{GL}(n; \mathbb{C})$ 是拓扑群；

...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Error_function}{相关页面}．}\textbf{误差函数（error function）}的定义为%未完成：函数图像如图 N， 误差函数是一个奇函数．
\begin{equation}\label{Erf_eq1}
\erf(x) = \frac{1}{\sqrt{\pi}} \int_{-x}^x \E^{-t^2} \dd{t}
= \frac{2}{\sqrt{\pi}} \int_{0}^x \E^{-t^2} \dd{t}
\end{equation}
这是一个奇函数． 根据\autoref{GsInt_eq1}~\upref{GsInt}有
\begin{equation}
\erf(\pm\infty) = \pm 1
\end{equation}
由\autoref{NLeib_eq10}~\upref{NLeib}， 误差函数的导数为
\begin{equation}

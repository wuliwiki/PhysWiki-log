% Jordan 引理
% keys 复变函数|围道积分|约尔当引理|约当引理|若尔当引理|若当引理|Jordan's lemma|小圆弧引理|大圆弧引理|瑕积分
% license Xiao
% type Tutor

\pentry{留数定理\nref{nod_ResThe}}{nod_cb7b}

Jordan 引理可以结合\enref{留数定理}{ResThe}，利用围道积分来处理一些复杂的实函数定积分。
...
\begin{example}{}\label{ex_JdLem_1}

定义 \enref{$\sinc$ 函数}{sinc}为
\begin{equation}
\sinc x = 
\leftgroup{
&\frac{\sin x}{x} &\quad & (x \ne 0)\\
...
\begin{figure}[ht]
\centering
\includegraphics[width=12cm]{./figures/90a775aceb3086b9.pdf}
...
\autoref{ex_JdLem_1} 是非常重要的例题，在我们严格讨论\enref{狄拉克 delta 函数}{Delta}时有很大作用。

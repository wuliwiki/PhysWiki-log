% 集合的测度（实变函数）
% keys 测度|实变函数|measure|集合|广义函数|泛函|可测集|Borel集
% license Xiao
% type Tutor

\pentry{集合\nref{nod_Set}，微积分或数学分析}{nod_7a95}
...
$\mathbb{R}^n$ 的任意开集都能表示为形如\autoref{def_SetMet_1} 中 $I$ 的“开方块”的\textbf{不交并}，因此开集的体积就可以直接定义为这些开方块的体积之和。
...
最后，我们要再讨论一个引理，在下一节\enref{可测集合}{MsbSet}中会很有用。
...
但是由\enref{集合的极限}{SetLim}可知，$A_0=\lim\limits_{i\to\infty} A_i$，因此使得 $A_i\cap S=\varnothing$ 恒成立的 $S$ 必须是空集 $\varnothing$，而这和 $\opn{m^*}S=x_0>0$ 矛盾。
...
\begin{example}{不具有可加性的集合}\label{ex_SetMet_1}
...
所以，为了能正常研究积分，我们只好对集合的性质进行限制了，详见\enref{可测集合}{MsbSet}。

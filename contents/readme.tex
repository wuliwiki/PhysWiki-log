...
百科\link{网页版}{http://littleshi.cn/online/}中的所有词条是一个 $\LaTeX$ 的 document 环境， 每个部分是一个 \textbackslash part， 每章是一个 \textbackslash chapter， 词条是 \textbackslash section， 蓝色的小标题是 \textbackslash subsubsection， 黑色的小标题是 \textbackslash subsubsection。 编辑器打开的一个词条文件就是一个 subsection。 用 texlive 编译 pdf 的时候所有词条文件都会通过 textbackslash input 插入到主文件 PhysWiki.tex 中。 PhysWiki.tex 打开并编辑。

...
\subsection{简介}
\begin{itemize}
\item 在线编辑器网址为 \link{littleshi.cn/editor}{http://littleshi.cn/editor}， 也可从网站首页的链接进入
...
\item 我们在模板中加入了许多自定义命令， 但不会影响 LaTeX 原有的命令（除了 \lstinline|\bvec|）
\item 要查看支持的所有命令和自定义命令， 请在编辑器中打开 “词条示例（Sample）\upref{Sample}”
...
\item 我们把 \lstinline|\bvec| 命令重新定义为正体和粗体（以符合常见物理学教材）， 如 \lstinline|\bvec a| 被显示为 $\bvec a$ ．
\item 行内公式只能用美元符号

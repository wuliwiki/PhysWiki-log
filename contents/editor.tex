...
\item 用光标选中一串字符后按下加粗按钮， 这串字符会自动插入 \lstinline|\textbf{}| 中． 同样， 选中字符串后输入 \lstinline|(| 等括号， 这个字符串会自动插入 \lstinline|()| 中． 表示行内公式的 \lstinline|$$| 也支持该操作．
\end{itemize}
...
设置面板中的也可以添加 “符号替换” 规则． “符号替换” 功能是指， 在 LaTeX 公式中输入一些命令时， 编辑器会自动将其显示为对应的符号， 例如 \lstinline|\alpha| 显示为 $\alpha$， \lstinline|\sum| 显示为 $\sum$ 等． 这样做是为了增加源码的可读性， 注意这只是一种视觉效果， 不会影响源码本身．

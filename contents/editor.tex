...
\subsection{简介}
\begin{itemize}
\item 在线编辑器网址为 \link{wuli.wiki/editor}{http://wuli.wiki/editor}， 也可从网站首页的链接进入
...
\item 一个简单的公式编辑器见\link{这里}{https://www.codecogs.com/latex/eqneditor.php}
\item 一个简单的 TeX/LaTeX 入门教程见\link{超理论坛}{https://chaoli.club/index.php/211}
...
网页版的词条目录（\link{wuli.wiki/online/}{http://wuli.wiki/online/}）由 PhysWiki.tex 生成， 所以发布词条后必须修改该文件并发布才能在目录中显示．

每个注册用户默认有编辑词条的权限， 但却不能直接发布词条， 而是需要由发布权限的用户审核后发布． 所有编辑中的词条页面都在 \link{wuli.wiki/changed/}{http://wuli.wiki/changed/} 目录下． 若要查看所有编辑中词条的目录， 可以用 \link{wuli.wiki/changed/changed.html}{http://wuli.wiki/changed/changed.html}（见\autoref{editor_fig1}）．

\begin{figure}[ht]
\centering
\includegraphics[width=9cm]{./figures/editor1.png}
\caption{从 \link{wuli.wiki/online}{http://wuli.wiki/online} 查看编辑中的词条} \label{editor_fig1}
\end{figure}

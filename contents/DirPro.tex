...
＊ $\Q A \otimes \Q B$ 的本征值共有 $m \times n$ 个， $m$ 和 $n$ 分别是 $A$ 和 $B$ 的维度， 若 $a_1, a_2,\dots, a_m$ 和 $b_1, b_2, \dots, b_n$ 分别是 $A$ 和 $B$ 的本征值， 那么 $A \otimes B$ 的本征值分别为 $a_i b_j$ 本征矢为 $\ket{u_i} \otimes \ket{v_j}$ （$0 \les i \les m,  0 \les j \les n$）。

＊ $A \otimes I + I \otimes B$ 的本征值分别为 $a_i + b_j$，  本征矢同样为 $\ket{u_i}\ket{v_j}$． 

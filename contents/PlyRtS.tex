...
对于$\pmat{1&2}\pmat{3&4}$，求其共轭元素，发现一共有$3$个，因此集合$\{\pmat{1&2}\pmat{3&4}\}$的\textbf{中心}（\autoref{NormSG_ex1}~\upref{NormSG}）$C$有$8$个元素．容易验证，
\begin{equation}\label{PlyRtS_eq10}
C = V_4\cup\{\pmat{1&2}, \pmat{3&4}, \pmat{1&3&2&4}, \pmat{1&4&2&3}\}
\end{equation}

由于$8=2^3$，且$2\not\mid 24/8=3$，可知$C$是$S_4$的一个Sylow-$2$子群（\autoref{Sylow_def1}~\upref{Sylow}）．类似地，其余Sylow子群分别是$\pmat{1&3}\pmat{2&4}$和$\pmat{1&4}\pmat{2&3}$的中心．
...
\addTODO{完成\textbf{Sylow定理}\upref{Sylow}中相应的Sylow定理后，在此引用．}
...
由\autoref{GExt_the8}~\upref{GExt}

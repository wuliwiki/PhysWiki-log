% 气体输运过程（微观理论）
% keys 传递过程|输运过程|粘滞定律|菲克定律
% license Xiao
% type Tutor

\pentry{热传导定律与传递过程\nref{nod_heatc}，分子平均碰壁数\nref{nod_AvgHit}}{nod_658b}
...
\textbf{假设 1}. 微观上有大量的分子在作无规则运动，且可以运用\textbf{局部平衡假设}：每个小单元内的温度、粒子数密度、平均动量可以确定；小单元的状态量随时间与空间的变化分别为 $T(z,t),n(z,t),\bar v(z,t)$。粒子服从\enref{麦克斯韦——玻尔兹曼分布}{MxwBzm}，热运动将使得 $\Delta t$ 内穿过 $\Delta S$ 的平均分子数为 $\frac{1}{4}n\bar v \Delta S\Delta t$。\textbf{由于物理量在 $z$ 方向上分布不同，热运动将使得粒子可以把界面一侧的物理量带到界面另一侧}。

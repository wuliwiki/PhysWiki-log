...
\begin{definition}{Levi-Civita符号\footnote{另见\enref{列维—奇维塔符号}{LeviCi}}。}
设$\sigma\in S_n$，即$\sigma$是正整数$1, 2, \cdots, n$之间的一个\textbf{置换}，或者说集合$\{1, 2, \cdots n\}$到自身的双射。$\opn{sgn}\sigma$是$\sigma$的\enref{逆序数}{InvNum}。
\begin{equation}
...
\enref{量子力学中的基本算符}{OprQM}的对易关系列举如下\footnote{计算过程中不要忘了，要代入辅助函数，利用映射的复合来定义算子的乘法。}：
...
\end{equation}
其中$i, j\in\{x, y, z\}$，$\delta_{ij}$是\enref{克罗内克 delta 函数}{Kronec}。
\end{corollary}

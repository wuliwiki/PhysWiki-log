...
在正 $N$ 边形中， 记任意连续 4 点的位置矢量（从坐标原点指向某点的矢量\upref{Disp}）为 $\vec r_1, \vec r_2, \vec r_3, \vec r_4$， 我们希望能用 $\vec r_1, \vec r_2, \vec r_3$ 表示 $\vec r_4$． 令 $\vec v_i = \vec r_{i+1} - \vec r_i$， 从\autoref{FootBl_fig3} 中不难发现
\begin{equation}

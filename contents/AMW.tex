...
\pentry{有限深方势阱\nref{nod_FSW}}{nod_c066}
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/63ac995e092453f2.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=9cm]{./figures/127c8b99fc9a406f.png}
\caption{\autoref{eq_FSW_3}~\upref{FSW} 左右两式对应的函数图像（上层浅红色代表右式，下层浅橘色代表左式），其中 $\frac{\hbar}{\sqrt{2mV_1}}=0.5,\frac{\hbar}{\sqrt{2mV_2}}=0.1,n=1$} \label{fig_AMW_2}~.
\end{figure}
...
\end{equation}
给出了阱中至少存在一个能级的条件。由此可见，给定了不相等的 $V_1$ 和 $V_2$ 后，总可以找到一个很窄的阱宽 $L$，使得该阱中不能存在离散能级，\autoref{fig_AMW_2} 中可看出在 $0<L<0.5$ 处\autoref{eq_FSW_3}~\upref{FSW} 无解。对 $V_1=V_2$ 而言，\autoref{eq_AMW_2} 总能得到满足。

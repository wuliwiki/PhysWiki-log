...
(\laplacian + k^2)f = 0~,
\end{equation}
球坐标系中的求解过程与\enref{拉普拉斯方程}{SphLap}中的过程类似。 将拉普拉斯算子分解为径向和角向两部分（\autoref{eq_SphNab_3}）
\begin{equation}
...
\end{equation}
我们已知角向的解为\enref{球谐函数}{SphHar} $Y_{l,m}(\uvec r)$。 而径向方程比普拉斯方程中的（\autoref{eq_SphLap_4}）多出了一项 $k^2r^2$， 使用变量代换 $\rho = kr$ 得
\begin{equation}

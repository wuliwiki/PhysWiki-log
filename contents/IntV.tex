...
\subsection{单变量定积分}
类比一元标量函数\enref{定积分}{DefInt}的定义，要计算一元矢量函数 $\bvec f(t)$ 从 $t_1$ 到 $t_2$ 的定积分， 就先把区间 $[t_1, t_2]$ 分为 $N$ 个小区间， 长度分别为 $\Delta t_i$， 且令 $t_i$ 为第 $i$ 个区间内的任意一点。 当我们取极限令所有区间长度 $\Delta t_i$ 都趋近于 $0$ （这时 $N\to\infty$） 时， 如果以下极限存在， 得到的矢量就是定积分的结果。
\begin{equation}
...
所以 $\bvec f(t)$ 的定积分就是把直角坐标的各个分量分别进行定积分。现在对三个定积分分别运用\enref{牛顿—莱布尼兹公式}{NLeib}， $\bvec f(t)$ 的原函数为 $\bvec F(t)$， 各分量的原函数为 $F_x(t), F_y(t), F_z(t)$， 则上式等于

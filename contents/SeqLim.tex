...
\begin{issues}
\issueOther{内容有重复， 需要删减。 单调收敛定理的内容与别的文章重叠， 需要删减。}
\end{issues}

\pentry{极限\nref{nod_Lim}， 实数集的拓扑\nref{nod_ReTop}}{nod_4567}

\subsection{基本定义解析}

点列的极限是分析数学中最基本的定义。 文章 \enref{数列的极限（简明微积分）}{Lim0}和 \enref{极限}{Lim} 已经给出了一些序列极限的例子，它的形式定义以及背后的直观解释\footnote{点列趋于一个极限点的直观含义，就是无论你要求有多靠近极限点，都存在一个编号$N$，使得$N$之后的点全都满足你的要求。}。为方便阅读，这里再次重复极限的定义：
...
\end{theorem}
下面我们要介绍的是单调收敛原理。从直观上看，如果一个序列单调递增且不会趋于无穷大，那么它应该是有上确界的。其证明用到了\enref{确界存在定理}{SupInf}。

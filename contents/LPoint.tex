...
提取\autoref{eq_JacCon_1}~\upref{JacCon}中的势能部分并除以质量，记为等效势 $\widetilde{U}$ 
\begin{equation}
...
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/4d23d239ddec366a.pdf}
...
% $L_1, L_2$ 有\autoref{eq_LPoint_5} 得


% 可见方程\autoref{eq_LPoint_1}，\autoref{eq_LPoint_2} ，\autoref{eq_LPoint_3} 等价于等效势能的一阶偏导数为零

% \addTODO{这段的论述是错误的， 考虑到科里奥利力的话， L4, L5 可能的确是稳定点， 参考 wikipedia
% 一般而言，势能的极小值点为稳定点。 通过观察\autoref{fig_LPoint_2} 可见，不存在稳定点： 天体A、B连线上的 $L_1,L_2,L_3$ 三点附近的等势线呈形似“马鞍”的形状，处于这三点的质点C如果发生偏向A、B两者中任意一个天体的微小扰动，都会导致势能的快速减小，最终掉入“势能陷阱”。而 $L_4,L_5$ 虽然是势能的极大值点，但是由于其附近的等势线呈环形，并且势能变化平缓，处于此处的航天器发生微小扰动时，势能变化较小，只需消耗少量的燃料便可以遏制扰动的发散，因此 $L_4,L_5$ 是相对稳定的拉格朗日点。  }

\subsection{使用拉格朗日方程推导}
\addTODO{该小节有待整理}

保持与天体A和B的相对位置不变，即要求第三质点在旋转坐标系中满足 $\dot{x}=\dot{y}=\dot{z}=\ddot{x}=\ddot{y}=\ddot{z}=0$，代入第三质点的动力学微分方程组（\autoref{eq_JConst_2}~\upref{JConst}，\autoref{eq_JConst_3}~\upref{JConst} ，\autoref{eq_JConst_4}~\upref{JConst}）可得方程组
...
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/51cc69cf2e29a4a0.pdf}

...
\end{align}
这就是 $N$ 元函数的傅里叶变换和反变换。 这个过程有点类似于\enref{多元函数的傅里叶级数}{NdFuri}，事实上 $N$ 维平面波就是许多一维平面波的乘积， 令 $\bvec k = (k_1, k_2, \dots, k_N)$， 有
\begin{equation}
...
\end{equation}
满足正交归一化条件（见\enref{狄拉克 delta 函数}{Delta}）
\begin{equation}
...
多元傅里叶变换的证明可以类比\enref{傅里叶变换（指数）}{FTExp}，不再赘述。

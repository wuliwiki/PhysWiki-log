...
由于磁场 $\bvec B(\bvec r)$ 任何情况都是一个无源场\upref{MagGau}， 所以根据 “旋度的逆运算\upref{HlmPr2}” 的\autoref{HlmPr2_the1}， 必定存在一个矢量场 $\bvec A(\bvec r)$ 使得
...
\end{aligned}
\end{equation}
其中 $\grad'$ 代表求关于 $\bvec r'$ 的梯度。 接下来使用多维分部积分\autoref{IntBP2_eq1}~\upref{IntBP2}， 令 $f(\bvec r') = 1/\abs{\bvec r - \bvec r'}$， $\bvec A(\bvec r') = \bvec j(\bvec r')$。 面积分取无穷大的球面， 积分为零； 最后一项中由于 $\grad' \vdot \bvec j(\bvec r') = 0$， 积分同样为零。 证毕。
...
\div \bvec A = \bvec 0
\end{equation}
根据\autoref{HlmPr2_eq4}~\upref{HlmPr2}， 我们只需要令 $\bvec H(\bvec r)$ 是一个调和场即可， 事实上库仑规范直接规定 $\bvec H(\bvec r) = 0$。

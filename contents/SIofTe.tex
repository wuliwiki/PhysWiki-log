...
本节将引入对称化映射和交错化映射，并证明在这两映射作用下，张量将变为对称张量和斜对称张量。张量的对称性和斜对称性的概念事实上已在对称/\enref{反对称多线性映射}{SASmap}中指出。
\subsection{张量的对称化}
...
\end{equation}
而 $S_p$ 是作用在指标集合 $\{1,2,\cdots,p\}$ 上的 $p$ 阶\enref{置换群}{Perm}。
...
若设 $f_\pi(\alpha T+\beta T')=\alpha f(T)+\beta f_\pi(T')$，则任意 $\pi \in S_p$ 引导出非退化的\enref{线性算子}{LiOper} $f_\pi:\mathbb{T}_p^0\rightarrow\mathbb{T}_p^0$ 。

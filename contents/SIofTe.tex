...
由于 $\mathbb{T}_p^0$ 型张量共 $p$ 个指标，每个指标所在处需放一个数字，这些数字在 $\{1,\cdots,n\}$ 中选取。对 $\mathbb{T}_p^0$ 的对称张量，独立分量的个数相当于从 $n$ 个数字中一次选 $p$ 个的重复组合（\autoref{APC_def4}~\upref{APC}），该个数为 $\qty(\begin{aligned}
&n+p-1\\&\qquad p
\end{aligned})$。
\subsection{张量的交错化}
...
\begin{definition}{张量的交错化}\label{SIofTe_def1}
称映射
...
\end{equation}
 ...
 \begin{equation}\label{SIofTe_eq11}
...
\begin{theorem}{}\label{SIofTe_the2}
...
\end{equation}
 ...
 \end{aligned}\right.
 \end{equation}
 那么，$f_\pi(Q)\otimes R=f_{\tilde\pi}(Q\otimes R)$，从而由\autoref{SIofTe_the1} ，\autoref{SIofTe_eq10} 可写成（注意 $\epsilon_{\tilde\pi}=\epsilon_\pi$）
 \begin{equation}
 \begin{aligned}
...
这里， $\Lambda^p(V^*)$ 的元素叫作 “p次外形式”，表示的是它是 $V$ 的 $p$ 次函数，而“函数”通常也称作“型”（\autoref{MulMap_def2}~\upref{MulMap}），“型” 或 “形式”都是对应英文“form”。而 $\Lambda^p(V)$ 的元素为称为 $p$ 矢量，可理解成它是 $p$ 个 $V$ 上矢量的张量积。只不过这些专门的术语是对斜对称来说的。

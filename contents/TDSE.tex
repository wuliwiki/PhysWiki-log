...
\issueDraft
\issueAbstract
\issueOther{考虑\enref{合并}{QMndim}}
\end{issues}

\pentry{多维空间中的量子力学\nref{nod_QMndim}， 定态薛定谔方程\nref{nod_SchEq}， 分离变量法\nref{nod_SepVar}}{nod_8cd2}
...
波函数是\enref{位置矢量}{Disp}的函数 $\Psi(\bvec r, t)$
\begin{equation}
...
\I \hbar\pdv{t}T = ET~.
\end{equation}
其中\autoref{eq_TDSE_1} 就是\enref{定态薛定谔方程}{SchEq}， 即哈密顿算符的本征方程。 \autoref{eq_TDSE_2} 有简单的解
\begin{equation}
T(t) = \E^{-\I E t/\hbar}~.
\end{equation}
...
\end{equation}
其中 $C_n$ 为待定系数， 由初始条件决定。 一个简单的例子见 “\enref{无限深势阱中的高斯波包}{wvISW}”。
...
一个经典的例子见 “\enref{一维自由粒子（量子）}{FreeP1}” 。

...
考虑任意 $\mat H^0$ 的两个 $E_m\ne E_n$ 不同的简并空间在任何时间都具有不同能量且能使\autoref{eq_AdiaQM_11} 忽略的情况。 此时 $\tilde{\mat H}$ 是块对角厄米矩阵\upref{BHeig}，每个对角块代表一个本征子空间（或者若干个可能在某时刻本征值相同的本征子空间张成的空间），不同子空间之间不存在耦合。 $\mat U$ 也是结构相同的分块酉矩阵，且每个对角块都分别是一个酉矩阵。每个本征子空间独立演化，投影概率保持不变。
\addTODO{链接未完成： 一般含时薛定谔方程中 $\mat H(t)$ 是块对角矩阵的情况}

...
\addTODO{我猜如果时刻选择让 $\mel*{\psi_m}{\dot H}{\psi_n}$ 对角化的基底（好本征态），仍然可以使 $\mat A = \mat 0$！ 也就是说好本征态之间没有 coupling}
...
\addTODO{？？？，每个时刻 $t$ 都对分块厄米矩阵 $\mat A$ 的每个块分别对角化不就行了吗？？？ 如果不是分块矩阵，就直接整个对角化不就好了吗？ 但是！但是！这样一来 $t$ 改变时难道要不断的对角化？一种特殊情况：假设 $H=H^0 + \lambda(t)H^1$ 就不存在这个问题了！ $\mat A$ 关于时间的变化就只是乘以 $\lambda(t)$ 而已。 只需要对 $H^1$ 做一次对角化。}
\addTODO{有没有可能即使 $E_m=E_n$，$\braket*{\psi_m}{\dot\psi_n}$ 也恒为零呢？什么时候？例如缓慢增加氢原子的核电荷时 $\psi_{n,l,m}$ 之间会耦合嘛？ stark 的好本征态之间呢？}
\addTODO{或许可以搞个矩阵来数值验证一下，例如氢原子的 stark 效应是否真的可以这么搞？以及氢原子的 $Z$ 慢慢变化。}

...
\end{figure}
两个线性无关的解分别为第一类球贝塞尔函数 $j_l(x)$ 和第二类球贝塞尔函数 $y_l(x)$， 见\autoref{SphBsl_fig1}。 它们可以通过贝塞尔函数\upref{Bessel} $J_l$， $Y_l$ 来定义
\begin{equation}
...
\subsection{积分性质}
由渐进形式可得径向归一化积分为（$\delta$ 是狄拉克 $\delta$ 函数\upref{Delta}， 推导见\autoref{Delta_eq9}~\upref{Delta}）

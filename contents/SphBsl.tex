...
\end{figure}
两个线性无关的解分别为第一类球贝塞尔函数 $j_l(x)$ 和第二类球贝塞尔函数 $y_l(x)$， 见\autoref{fig_SphBsl_1}。 它们可以通过\enref{贝塞尔函数}{Bessel} $J_l$， $Y_l$ 来定义
\begin{equation}
...
\end{equation}
分解（$[\dots]$ 表示 \enref{Clebsch–Gordan 系数}{SphCup}）
\begin{equation}
...
\subsection{积分性质}
由渐进形式可得径向归一化积分为（$\delta$ 是\enref{狄拉克 $\delta$ 函数}{Delta}， 推导见\autoref{eq_Delta_9}~\upref{Delta}）

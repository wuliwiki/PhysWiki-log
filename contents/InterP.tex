...
\end{equation}
现在来讨论什么时候该方程有唯一解。 事实上系数矩阵就是\enref{范德蒙矩阵}{VandDe}，当 $x_i$ 互不相等时， 它的秩等于 $\min\qty{m, n}$。 所以当 $m > n$ 时， 存在无穷个解； 当 $n > m$ 时一般无解（见\enref{超定方程组}{OvrDet}）； 当 $n = m$ 时能确保有且仅有唯一解， 因为此时系数矩阵是满秩矩阵。

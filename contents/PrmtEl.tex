% 本原元定理
% keys primitive element|可分扩张|域|多项式|本原元定理|单扩张|有限扩张|代数扩张|域扩张
% license Xiao
% type Tutor

%primitive element
\pentry{域的扩张\upref{FldExp}}{nod_c1ff}
...
\begin{corollary}{}\label{cor_PrmtEl_1}
...
$\mathbb{K}/\mathbb{F}$是单扩张 $\iff$ $\mathbb{K}/\mathbb{F}$的中间域\footnote{见\autoref{the_FldExp_3}~\upref{FldExp}证明后的一句讨论。}只有有限多个。

\end{theorem}

\textbf{证明}：

$\mathbb{F}$是有限域时，由\autoref{cor_FntFld_1}~\upref{FntFld}，两个条件恒成立。因此下面只考虑$\mathbb{F}$是无限域的情况。

$\Leftarrow$：

有限扩张一定是代数扩张（\autoref{cor_FldExp_1}~\upref{FldExp}），即存在正整数$n$和$n$个$a_i\in\mathbb{K}$，使得$\mathbb{K}=\mathbb{F}(a_1, \cdots, a_n)$。于是，由\autoref{cor_PrmtEl_1} 立刻得证。
...
由\autoref{the_FldExp_1}~\upref{FldExp}，$[\mathbb{K}:\mathbb{M}]=\opn{deg}f_\mathbb{M}=n$，$[\mathbb{K}:\mathbb{M}']=\opn{deg}f_\mathbb{M}=n$，因此由\autoref{the_FldExp_3}~\upref{FldExp}，
\begin{equation}
...
\pentry{可分扩张\upref{SprbEx}}{nod_e227}

本原元定理\autoref{the_PrmtEl_1} 给出了判断有限扩张是否为单扩张的充要条件，但实际应用中有一个麻烦：证明中间域的数量是很难的。下面举一个例子来体现这一点，和\autoref{ex_SprbEx_1}~\upref{SprbEx}构造的思路一样：
...
显然，由于$x^p$的$p$次方根不在$\mathbb{Z}_p(x^p, y^p)$中，根据\autoref{the_SprbEx_4}~\upref{SprbEx}，$\mathbb{Z}_p(x^p, y^p)$不是完美域。这就提醒我们，这个扩张可能不是可分的。
...
\begin{example}{$\mathbb{Z}_p(x, y)$中若干元素的表示}\label{ex_PrmtEl_1}
...
由\autoref{cor_FntFld_1}~\upref{FntFld}，$\mathbb{F}$为有限域时定理天然成立。因此下设$\mathbb{F}$是无限域。
...
取$\alpha\in\mathbb{K}$，使$\opn{deg}\opn{Irr}(\alpha:\mathbb{F})=n$。由\autoref{the_FldExp_1}~\upref{FldExp}，$[\mathbb{F}(\alpha):\mathbb{F}]=n$。

任取$\beta\in\mathbb{K}$，则由Artin本原性\autoref{the_PrmtEl_2} ，存在$\gamma\in\mathbb{K}$使得$\mathbb{F}(\alpha, \beta)=\mathbb{F}(\gamma)$。由题设和\autoref{the_FldExp_1}~\upref{FldExp}，$[\mathbb{F}(\gamma):\mathbb{F}]\leq n$，但$\mathbb{F}(\alpha)\subseteq\mathbb{F}(\gamma)$，所以必有$\mathbb{F}(\alpha)=\mathbb{F}(\gamma)$。

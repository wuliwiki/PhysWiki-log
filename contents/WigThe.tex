...
\pentry{量子力学的基本原理（量子力学）\upref{QMPrcp}}{nod_9e65}
...
量子力学认为量子态构成一个\textbf{希尔伯特空间}\upref{Hilber}，其中互为倍数的量子态是等价的（\autoref{def_QMPrcp_2}~\upref{QMPrcp}）。为了方便，我们可以限定只讨论模为$1$的那些量子态，也可以说我们讨论的不是矢量，而是\textbf{射线（ray）}。当然，如果觉得射线的语言不好理解，也可以只考虑归一化的态矢，不影响对定理的理解和表述。

希尔伯特空间$H$中的射线构成$H$的一个\textbf{商集}\footnote{见\autoref{sub_Relat_1}~\upref{Relat}中的“商集”概念。}$\mathbb{P}H$，定义为${\displaystyle \mathbb {P} H=H\setminus \{0\}/{\approx }}$。其中等价关系$\approx$就是量子态等价的定义，即$\ket{s_1}\approx\ket{s_2}\iff \exists c\in\mathbb{C}, \ket{s_1}=c\ket{s_2}$。
...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/a00f646df4957ec0.pdf}
...
任取三个基右矢$\ket{s_1}$、$\ket{s_2}$和$\ket{s_3}$，构造态右矢$\ket{s} = \sum_{i=1, 2, 3}C_i\ket{s_i}$。按\autoref{eq_WigThe_7} 定义映射后的展开系数$C'_i$。

...
质点运动学量描述了质点的\enref{位矢}{Disp}、\enref{速度、加速度}{VnA}等。
\begin{table}[ht]
\centering
...
\subsubsection{牛顿定律}
\enref{牛顿定律}{New3}是经典物理中最伟大的发现（\textsl{可能没有之一}）。由于太重要了，因此我不得不在这里复读一遍：
...
牛顿第二定律描述了力对物体运动状态的影响(将牛二写为$\bvec a = \frac{\bvec F}{m}$时更直观地体现了这一点)\footnote{拓展一步：力对物体形状的影响是个复杂得多的问题，包括\enref{弹性力学}{YoungM} 、塑性力学、流体力学...}，联系了运动学与动力学。
...
\end{figure}
力由\enref{矢量}{GVec}描述，并且符合矢量的加减法。我们通过合成或分解力来处理合力或力平衡的问题。\upref{Fdecom}
...
\hline
\enref{牛二}{New3} & $\bvec F = m \bvec a$  & $\bvec a$是质点的加速度& $\bvec F=\sum \bvec F_i$ 是质点所受合力 & $\bvec F = \bvec 0 \Rightarrow \bvec a = \bvec 0 \Rightarrow \bvec v = \bvec v_0$ 质点所受合力为零时，保持静止或匀速运动。这就是判断质点受力平衡的重要依据。
\\
\hline
\enref{动量定理}{PLaw1} & $\bvec F = \dv{\bvec p}{t}$  & $\bvec p = m \bvec v$ 质点动量是质点质量与速度的乘积 & $\bvec F=\sum \bvec F_i$ 是质点所受的合力。 & 若$\bvec F = \bvec 0$，则$\dv{\bvec P}{t} = 0$，即合力为零时，质点动量守恒。\\
\hline
\enref{角动量定理}{AMLaw1} & $\bvec \tau = \dv{\bvec L}{t}$ & $\bvec L = \bvec r \times \bvec p$ 质点角动量是质点动量与位矢的叉乘 & $\bvec \tau = \sum \bvec \tau_i $是质点所受力矩和；$\bvec \tau_i= \bvec r \times \bvec F_i$ \enref{力矩}{Torque}是力与位矢的叉乘；& 力矩和为零时，质点角动量守恒\\
\hline
\enref{动能定理}{KELaw1}& $W = \Delta E_k$  & $E_k = \frac{1}{2} m v^2$ 质点动能=1/2*质点质量*速度的平方 & 
$W = \sum W_i$是各力做的功之和；
$W_i = \bvec F_i \cdot \Delta \bvec r$ \enref{功}{Fwork}是力在质点位移上的“作用量”。 & 功之和为零时，质点动能守恒\\
\hline
\enref{保守力的功-能关系}{V} & $W_{CON} = -\Delta E_p$&$E_P$是势能&$W_{CON}$是保守力的功之和&$W_{C} = 0 \Rightarrow \Delta E_{P} = 0$\\
\hline
\enref{机械能定理}{ECnst} & $W_{NC} = \Delta E_{mech}$ & $E_{mech} = E_k+E_P$ 机械能是动能、势能之和 & $W_{NC}$是非保守力的功之和& $W_{NC} = 0 \Rightarrow \Delta E_{mech} = 0$
\\
\hline
\end{tabular}
\end{table}
...
有时我们需要改变参考系来观察同一质点。\enref{伽利略变换}{GaliTr}告诉我们应该如何在不同参考系中相应地转换物理量（在工科力学中这类问题被称为“点的复合运动问题”）。我们先处理惯性系之间的相互转换问题，非惯性系问题将在文末简要说明。
\begin{table}[ht]
\centering
...
物理量 & 公式 \\
\hline
\enref{位矢}{GaliTr} & $\bvec r_{K1} = \bvec r_{K2} + \bvec r_r$ \\
\hline
\enref{速度}{Vtrans} & $\bvec v_{K1} = \bvec v_{K2} + \bvec v_r$ \\
\hline
\enref{加速度}{Iner}& $\bvec a_{K1} = \bvec a_{K2}$\\
\hline
\end{tabular}
\end{table}
注意到在任意惯性参考系中质点的加速度都一致。根据牛顿第二定律$\bvec F = m \bvec a$，这暗示了质点所受合力无关惯性参考系。\autoref{eq_Iner_3}~\upref{Iner} \footnote{拓展一步：实际问题比这个看似平凡的结论微妙得多。一个经典的问题是，“如果在S系中观察到一电荷在磁场中运动，那么S系的观察者会认为电荷受磁场力；但在随电荷一同运动的S‘参考系中的观察者看来，电荷静止不动，则不应受磁场力！为什么两个参考系中的观察者会“看到”电荷不同的受力情况？那电荷到底如何运动？”对于这类问题的思考启发了\enref{狭义相对论}{SpeRel}与\enref{相对论性电动力学}{EMRef}。简要的回答是，根据洛伦兹变换，在S’系中的观察者其实看到了磁场“变成了”电场，因此电荷受电场力，他的方向与大小恰好等效于S系观察者所认为的磁场力。尽管两个参考系的人对于力的来源意见不一，但力的作用效果是完全相同的。}
...
\enref{速度}{Vtrans} & $\bvec v_{K1} = \bvec v_{K2} + \bvec v_r$ \\
\hline
\enref{加速度（参考系间仅平动）}{AccTra}& $\bvec a_{K1} = \bvec a_{K2} + \bvec a_r$ \\
\hline
\enref{加速度（参考系间平动+转动）}{AccTra}& $\bvec a_{K1} = \bvec a_{K2} + \bvec a_r +2 \bvec \omega \times \bvec v_{K2}$ \\
\hline
\end{tabular}
\end{table}
除了加速度，其余项完全相同；但加速度变换的物理含义非同一般：再次根据牛二$\bvec F = m \bvec a$，物体的加速度与其受力直接相关。因此，在非惯性系中看来，物体似乎受额外的假想力，例如惯性力$\bvec F_\text{惯} = -m \bvec a_r$\upref{Iner}、\enref{离心力}{Centri}、科氏力$\bvec F_\text{科} = -2 m \bvec v_{K2}\times \bvec \omega$ \upref{Corio}。
...
\enref{引力}{Gravty}：$\bvec G = \frac{GMm}{r^2} \hat{\bvec r}$ 方向是两物体相互吸引的方向。

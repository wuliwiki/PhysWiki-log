...
\pentry{不定积分\upref{Int}， 复合函数求导\upref{ChainR}}
\subsection{第一类换元积分法}
...
\end{equation}
所以如果某个积分可以看成 $\int f[u(x)]u'(x) \dd{x}$ 的形式，且 $F(x)$ 较容易求出，即可根据\autoref{IntCV_eq2} 写出结果．这种方法叫做\bb{第一类换元积分法}．这类换元积分法的技巧就在于如何看出被积函数的的结构是 $\int f[u(x)]u'(x) \dd{x}$， 只有多练习才能熟能生巧． 

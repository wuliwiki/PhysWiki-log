...
\int a\sin(ax + b) \D x
\end{equation}
令 $f(x) = \sin(x)$， $u(x) = ax + b$， 则上式刚好是 $\int f[u(x)]u'(x)\D x$ 的形式．从基本初等函数积分表 %未完成
...
总结到更一般的情况，根据换元积分法，若已知 $\int f(x)\D x  = F(x) + C$， 则对于任意常数 $a$ 和 $b$， 必有 $\int a\,f(ax + b)\D x  = F(ax + b)$． 根据积分的基本性质，%未完成
两边同除 $a$， 得
\begin{equation}

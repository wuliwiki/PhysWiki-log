...
\subsection{应用到定积分}
\pentry{定积分（极简微积分）\nref{nod_DefInt}}{nod_60fb}
\autoref{eq_IntCV_2} 应用到定积分，有
\begin{equation}
\int_{x_1}^{x_2} f(u(x))u'(x) \dd{x} = \int_{u(x_1)}^{u(x_2)} f(x) \dd{x}~.
\end{equation}
典型的应用就是
\begin{equation}
\int_{x_1}^{x_2} f(ax+b) \dd{x} = \frac{1}{a}\int_{ax_1+b}^{ax_2+b} f(x) \dd{x}~.
\end{equation}

...
\end{cases}~.
\end{equation}
其中 $\opn{Arctan}$ 是\enref{四象限反正切函数}{Arctan}，也记为 $\opn{atan2}$。 注意根据\autoref{eq_CyCar_1}， 同一个直角坐标可以对应不同的极坐标， 例如将 $\theta$ 增加 $2\pi$ 的整数倍， 直角坐标不变。 但根据\autoref{eq_CyCar_2}， 我们可以找到两种坐标间的\enref{一一对应}{Set}关系。
...
\end{cases}~.
\end{equation}
其中 $\mat R$ 是关于角度 $\theta$ 的\enref{三维旋转矩阵}{Rot3D}
\begin{equation}
\mat R = \pmat{
    \cos\theta & \sin\theta & 0\\
    -\sin\theta & \cos\theta & 0\\
...
\uvec z = \uvec z~.
\end{equation}
将基底变换\autoref{eq_CyCar_3}  和\autoref{eq_CyCar_4} 分别代入\autoref{eq_CyCar_5}  和\autoref{eq_CyCar_6} 得坐标变换\autoref{eq_CyCar_7} 和\autoref{eq_CyCar_8}，详见 “\enref{三维旋转矩阵}{Rot3D}”。

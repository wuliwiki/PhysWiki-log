...
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/curvat_1.pdf}
\caption{密切圆和曲率半径}} \label{curvat_fig1}
\end{figure}

我们先来看一个半径为 $R$ 的圆的一小段圆弧， 令其长度为 $\Delta l$． 作这段圆弧两端的切线， 令它们的夹角为 $\Delta \theta$， 那么显然满足 $R \Delta\theta = \Delta l$． 同理对于\autoref{curvat_fig1} 中光滑曲线上一点， 取该点附近长度为 $\Delta l$ 的一段， 在其两端分别作切线， 令夹角为 $\Delta\theta$ 并分别作切线的垂直线， 那么当 $\Delta l \to 0$ 时两个垂直线的交点就是密切圆的圆心． 此时密切圆的半径， 即曲率半径 $\rho$ 就是

...
\begin{enumerate}
\item $C_k^{ij}=-C_k^{ji}$．
\item $C_l^{ij}C_m^{lk}+C_l^{jk}C_m^{li}+C_l^{ki}C_m^{lj}=0$\footnote{注意使用爱因斯坦求和约定．}．
\end{enumerate}
\end{theorem}

\autoref{LieAlg_the1} 的证明思路简述如下，细节读者可自行补充：对于第一条，直接应用反交换性；对于第二条，考虑 $[\bvec{x}^i, [\bvec{x}^j, \bvec{x}^k]]=[\bvec{x}^i, C^{jk}_l \bvec{x}^l=C^{il}_mC^{jk}_l \bvec{x}^m$，再应用反交换性就能直接得到式中左边的第二项，以此类推，应用Jacobi结合性即可得到整个式子．

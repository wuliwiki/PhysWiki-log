...
由于$Y_{l,-m}^*(\uvec r_1)Y_{l,-m}(\uvec r_2) = [Y_{lm}^*(\uvec r_1)Y_{lm}(\uvec r_2)]^*$， 矩阵 $\mel{\mathcal{Y}_{l_1 l_2}^{LM}}{Y_{lm}^*(\uvec r_1)Y_{lm}(\uvec r_2)}{\mathcal{Y}_{l_1' l_2'}^{L'M'}}$ 中将 $m$ 变为 $-m$ 会使矩阵取厄米共轭，在这里就是转置． 所以这是一个对称实矩阵， 且我们只需要求 $m \ges 0$ 的项即可．
...
\subsection{波函数演化}
弄清是不是可以像氢原子演化那样分别传播每个空间基底 $\ket{r_i}\ket{r_j}$ 的子空间。

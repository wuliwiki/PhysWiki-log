...
\subsection{波函数的对称性}
我们一般假设电子自旋态恒为 singlet （反对称）， 所以总波函数满足粒子交换对称 $\Psi(\vec r_1, \vec r_2) = \Psi(\vec r_2, \vec r_1)$． 由\autoref{GenYlm_eq5}\upref{GenYlm} 可以导出分波的对称性
\begin{equation}
\psi_{l_1, l_2}^{L, M}(\vec r_1, \vec r_2) = (-1)^{l_1 + l_2 - L} \psi_{l_2, l_1}^{L, M}(\vec r_2, \vec r_1)
\end{equation}

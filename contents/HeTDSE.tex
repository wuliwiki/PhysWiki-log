...
对于任何演化算法， 一个最简单的方法就是每隔一段时间， 使用 \verb|dt/N| （$N = 2$ 或 $4$） 为步长， 演化 $N$ 次然后和 \verb|dt/2| 一次的结果做对比， 计算 $\abs{\Psi'-\Psi}^2$， 用于衡量误差．

对于 Lanzos， 有标准的误差公式， 也可以和 $\abs{\Psi'-\Psi}^2$ 对比看看哪个更准．

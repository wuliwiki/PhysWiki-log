...
对偏微分方程， 把以上的 $f(x), y(x)$ 变为多元函数 $f(\bvec r), y(\bvec r)$， 算符 $\Q Q$ 变为线性偏微分算符， $\delta$ 函数变为\enref{多元狄拉克 $\delta$ 函数}{deltaN} $\delta(\bvec r - \bvec r')$， 积分变为重积分即可。

\subsection{例子： 弦的受力平衡}
\pentry{分部积分法\nref{nod_IntBP}， 方程 $y^{(N)}=f(x)$\nref{nod_ynfx}}{nod_df15}

一根两端固定的弦两端固定在 $x$ 轴上， 区间为 $[0, L]$， 张力为 $T$， 形状为 $y(x)$， 边界条件为 $y(0) = y(L) = 0$。 在弦上有 $y$ 方向的连续受力分布， 若弦的受力密度函数为 $f(x)$， 即单位长度受到的 $y$ 方向的力， 那么当 $\abs{f(x)} \ll T$ 时有方程（推导过程类比 “\enref{一维波动方程}{WEq1D}”）
...
\end{equation}
方程右边是\enref{狄拉克 $\delta$ 函数}{Delta}。且同样有边界条件 $G(x', a) = G(x', b) = 0$。 这相当于弦上只有一点 $x'$ 受大小为 $F = 1$ 的力。
...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/08f2968ba2cb4cb5.pdf}
...
\end{equation}
由\enref{分部积分法}{IntBP} （对 $f(x')$ 积分，剩下部分求导）可得\autoref{eq_GreenF_14}。 分部积分留做习题， 其中 $C_1$ 和 $C_2$ 的取值需要保证边界条件 $y(0) = y(L) = 0$。

\subsubsection{不用受力分析求解格林函数}
% 对\autoref{eq_GreenF_2} 在 $(0, x')$ 做两次定积分可以得到一条以 $(0,0)$ 为起点的斜线， 同理， 在 $(x', L)$ 两次定积分可以得到以 $(L, 0)$ 为终点的斜线。 而在 $(x-\epsilon, x+\epsilon)$ 积分 并取 $\epsilon\to 0$ 可以得到斜率 $y'$ 在 $x'$ 左侧到右侧的变化
% \begin{equation}
% -T\Delta y' = 1 \Longrightarrow \Delta y' = -\frac{1}{T}
% \end{equation}
% 解三角形可得\autoref{eq_GreenF_11}。

...
\pentry{狄拉克 delta 函数\upref{Delta}}{nod_48a4}
...
\pentry{分部积分法\upref{IntBP}， 方程 $y^{(N)}=f(x)$\upref{ynfx}}{nod_df15}
...
\end{equation}
由分部积分法\upref{IntBP} （对 $f(x')$ 积分， 剩下部分求导）可得\autoref{eq_GreenF_14}。 分部积分留做习题， 其中 $C_1$ 和 $C_2$ 的取值需要保证边界条件 $y(0) = y(L) = 0$。

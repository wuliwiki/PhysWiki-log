...
对于轨迹上任意一点，容易求出质点在该处的速度矢量．可将速度矢量分解为切向速度 $v_\tau$ 与法向速度 $v_n$．在极坐标系中，切向速度可表示为 $v_\tau =r\vdot{\theta}$，角动量为 $\vec L  = mr^2\dot{\theta}\uvec z$，因此，切向速度为
\begin{equation}\label{OribP_eq2} 
v_\tau =\frac{L}{mr}
\end{equation}

另有法向速度 $v_n=\dot{\vec r}_1$，对\autoref{OribP_eq1} 求导可得
...
\subsection{初值条件}
由轨道方程\autoref{OribP_eq1} 可知，轨道上 $\theta=0$ 的点距离坐标原点最近，并且由\autoref{OribP_eq2} 和 \autoref{OribP_eq3} 可以发现，该点处的切向速度最大，法向速度为零．在恒星-行星系统中，称这个最近点为“\bb{近日点}”，通常取改点的运动状态为初值条件．记这个最近距离为 $r_0$，该点处速度为 $v_0$．由此初值条件可得
\begin{align}
L &= mv_0r_0\\

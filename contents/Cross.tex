...
\end{equation}
由\autoref{Cross_eq1} 可知， 对于两个模场相同的矢量， 当它们平行时， 叉乘为零矢量； 当它们垂直， 叉乘的模长等于两模长直接相乘．

\subsubsection{平行四边形的矢量面积}
事实上\autoref{Cross_eq1} 就是平行四边形的面积公式， 这个平行四边形就是用平行四边形法则\upref{GVecOp}计算 $\bvec A + \bvec B$ 时的平行四边形．

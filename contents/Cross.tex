...
向量叉乘在物理定律中十分常见， 例如在讨论力学中的\enref{力矩}{Torque}，\enref{角动量}{AngMom}，以及电磁学中的\enref{洛伦兹力}{Lorenz}，\enref{安培力}{FAmp}时都会使用。以下我们讨论的向量都是三维中的\enref{空间向量}{GVec}；在讨论它们的坐标时， 我们默认取正交归一基底。
...
结合\autoref{fig_Cross_2} 与\autoref{eq_Cross_1}，可看出$\bvec C$的大小就是$\bvec A, \bvec B$所围成的平行四边形的面积， 亦即用\enref{平行四边形法则}{GVecOp}计算 $\bvec A + \bvec B$ 时的平行四边形。
...
从几何的角度理解，这个结论并不显然（见\enref{向量叉乘分配律的几何证明}{CrossP}）。
...
\end{cases}~
\end{equation}
\autoref{eq_Cross_2} 可以用\enref{三阶行列式}{Deter}表示为
...
%叉乘在物理中的应用如： 圆周运动的线速度（\autoref{eq_CMVD_5}）， 洛伦兹力（\autoref{eq_Lorenz_1}）， 安培力（\autoref{eq_FAmp_1}）。
%已经在引言中提及
...
根据叉乘的几何定义， $\bvec v = (1,-1,2)$ 就是三角形的法向量，进行\enref{归一化}{GVecOp}
得单位法向量为
 \begin{equation}

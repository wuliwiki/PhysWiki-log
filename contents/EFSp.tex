...
\pentry{正定二次型\nref{nod_DeQua}，矢量空间\nref{nod_LSpace}}{nod_ba4d}
带有内积的矢量空间是指在\enref{矢量空间}{LSpace} $V$ 上配备一个固定的\enref{二次型}{QuaFor}
\begin{equation}
...
\end{equation}
 ...
 \end{definition}
定义中的“指数”是指惯性指数（或正惯性指数）（\autoref{def_RQuaF_1}~\upref{RQuaF}）。
...
\end{equation}
事实上，每个二次型 $q$ 都有一配极的双线性型 $f$（\autoref{def_QuaFor_2}~\upref{QuaFor}），$f$ 是个对称的。二次型非退化意味着对称双线性 $f$ 非退化，而 $f$ 对称性保证了它可对角化（\autoref{the_GuaOQu_1}~\upref{GuaOQu}）， 进而可选择基底使得 $q$ 标准化为\autoref{eq_EFSp_1} （\autoref{the_RQuaF_1}~\upref{RQuaF}）。
...
\begin{definition}{指数有限度量空间}
 ...
 \end{definition}
 和\enref{欧几里得空间}{EucSp}完全类似，可建立一系列的概念。同样也把 $\rho^2(\dot p,\dot q)$ 称为仿射空间$\mathbb E$ 上的\textbf{度量型}，并且为了停留在实数域，只讨论 $\rho^2(\dot p,\dot q)$。

\begin{definition}{伪欧几里得空间、闵可夫斯基空间}\label{def_EFSp_1}

...
\item Win 11 装系统的时候会要求登录在线账号，若不想用本地账号用 \verb`Shift + F10` 打开 cmd， 输入 \verb`OOBE\BYPASSNRO` 然后按回车，自动重启后就可以选择没有网络。
...
\subsubsection{隐私}
\begin{itemize}
\item 如果要分享屏幕，可能就会希望不要让最近用过的文件和文件夹在各种地方显示，这可以随意打开一个文件浏览器窗口，在 Folder Options 的 Privacy 里面取消勾选并清除历史。
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/6cda4b6d945fa23f.png}
\caption{} \label{fig_WinNt_3}
\end{figure}
\item \verb`Setting -> Personalization -> Start` 里面可以把开始菜单中的最近文件和 app 关闭。
\end{itemize}
...
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/3d2015f06612199e.png}
...
\subsection{环境变量}
\begin{itemize}
\item \verb`Settings -> System -> About -> Advanced System Settings -> Environment Variables`。 也可以在系统菜单直接搜索 \verb`Environment Variables`，选择 \verb`Edit environment variables for you account`。 环境变量有 User 和 System 两种类型。
\end{itemize}

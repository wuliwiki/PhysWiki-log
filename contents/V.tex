...
\pentry{矢量场\nref{nod_Vfield}， 功\nref{nod_Fwork}， 牛顿—莱布尼兹公式\nref{nod_NLeib}}{nod_d8a7}
...
总结到一般情况，\textbf{力场}可以用场对质点施加的力（矢量）关于质点位置（\enref{位矢}{Disp}）的矢量函数表示， 所以力场是一种矢量场。
...
若质点从场的一点移动到另一点的过程中， \enref{力场对质点做的功}{Fwork} 只与初末位置有关，而与质点移动的路径无关， 那么这个力场就是一个\textbf{保守场}。 这时我们可以给该质点定义一个\textbf{势能函数}， 势能函数是一个关于位矢的标量函数，一般记为 $V(\bvec r)$， 具有能量量纲。 当质点从一点以任意路径移动到另一点时，场对质点做的功等于质点初位置的势能减末位置的势能， 即
...
= \int_a^b F(x) \dd{x}~.
\end{equation}
根据势能的定义， 对任意的 $a$ 和 $b$， 上式应该等于 $V(a) - V(b)$。 根据\enref{牛顿—莱布尼兹公式}{NLeib}， 势能函数恰好就是 $F(x)$ 的负原函数， 所以 $F(x)$ 是 $V(x)$ 负导函数。
...
\subsection{二维和三维函数}
\pentry{梯度定理\nref{nod_Grad}， 斯托克斯定理\nref{nod_Stokes}}{nod_f592}
在二维（平面）或三维空间中， 矢量场不一定是保守场， 那么应该如何判断一个场是否是保守场？ 事实上 “线积分结果与路径无关” 有一个充要条件就是 “延任意闭合曲线的环积分结果为零”。 而根据\enref{斯托克斯定理}{Stokes}， 这就要求矢量场的旋度处处为零， 即它是一个\textbf{无旋场}。
\addTODO{画图说明 “线积分结果与路径无关” 的等效条件是 “延任意闭合曲线的环积分结果为零”}

假设力场 $\bvec F(\bvec r)$ 是平面或三维空间中的保守场， 对应势能为 $V(\bvec r)$， 初始点为 $\bvec r_i$， 终点为 $\bvec r_f$。 对 $-V(\bvec r)$ 使用\enref{梯度定理}{Grad} 得
\begin{equation}

...
\end{equation}
其中 $\bvec \mu$ 是\textbf{磁矩（megnetic moment）}， 等于电流 $I$ 乘以以这个环路为边界的任意曲面的面积矢量， 曲面法向量的方向通过\enref{右手定则}{RHRul}判断。也就是说，该力矩的方向使磁矩矢量往磁场的方向偏转。
\begin{equation}
\bvec \mu = I \bvec A~,
\end{equation}
...
其中
\begin{equation}
\oint \bvec r \vdot \dd{\bvec r}  = \oint r \uvec r \vdot \dd{\bvec r}  = \oint r \dd{r}  = \eval{\frac{r^2}{2}}_{r_0}^{r_0}  = 0~, % \addTODO{这个符号有介绍吗？}
\end{equation}
...
对第一项进行分析，剩下两项类推即可。 由\enref{斯托克斯定理}{Stokes}得
\begin{equation}
\begin{aligned} 

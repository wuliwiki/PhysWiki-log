...
\footnote{本文参考 \cite{Bransden}}本文使用\enref{原子单位制}{AU}。 在长度规范\upref{LenGau}下， 对于某种势能 $V$ 的束缚态 $\ket{\psi_a}$ 和 $\ket{\psi_b}$， 可以证明以下三种形式的\textbf{跃迁偶极子矩阵（transition dipole matrix）}相等。
...
\end{equation}
这三种形式分别称为偶极子矩阵的\textbf{长度形式（length form）}、\textbf{速度形式（velocity form）}和\textbf{加速度形式（acceleration form）}。 注意他们都使用长度规范， 不要和速度规范\upref{LVgaug}和加速度规范混淆。 % \addTODO{链接}

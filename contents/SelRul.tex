...
相比于算符对易的方法\footnote{参考\cite{GriffQ}。}， 3j 符号的好处是不仅能得到选择定则，还可以直接算出偶极子矩阵元角向积分的具体值而无需手动积分\footnote{当然， 手动 3j 符号也比较繁琐， 可以借助 Wolfram Alpha 或 Mathematica， Matlab 中我也写了一个程序（同样可以符号计算）， 还没放进百科。}。 \enref{长度规范}{LenGau}中电场哈密顿量为 $\bvec{\mathcal{E}}\vdot \bvec r$ （\enref{长度规范}{LenGau}）。 其中 $\bvec r$ 的三个分量可以用球谐函数表示为（\autoref{eq_SphHar_4}）
...
令 $\Delta m = m - m'$， $\Delta l = l - l'$。 使用 3j 的选择定则（\autoref{eq_ThreeJ_1}） 规定第二行的三个 $m$ 相加为零， 所以只有\autoref{eq_SelRul_1} 满足
...
\addTODO{需核实 “满足了也由少数情况可能为零”， 需引用相关文献。 至少在\autoref{tab_HDipM_1}  中符合选择定则的都是非零。}

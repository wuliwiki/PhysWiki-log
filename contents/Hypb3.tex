...
显然，直接使用三角函数来表示并不如使用双曲函数那样方便，因为参数的变化范围在这里并不是连续的实数。同时，也可以联想到，双曲线的两条渐近线，其实正对应着正切函数或正割函数中的渐近线。
...
双曲线的反射性质与椭圆非常相似，都是指反射光线所在的直线经过另一个焦点。它们之间的区别仅仅在于：在椭圆中，反射光线能够实际到达另一个焦点；而在双曲线中，反射光线本身不会直接到达另一个焦点，而是反射线的延长线穿过另一个焦点。

类似地，也可以从几何角度解释双曲线为何具备这样的反射性质。

\begin{lemma}{对于在直线$l$两侧的点$A,B$满足二者到$l$的距离不相等，以及直线上一点$P$，当且仅当$l$平分$\angle APB$时，$||PA|-|PB||$最大。}
...
设平面上有两个定点 $M, N$，若希望找到所有从 $M$ 出发、经直线反射后沿直线方向离开，并满足入射光线与反射光线的延长线经过 $N$，那么反射点的集合正好构成双曲线。

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/40fbb0a544803b89.png}
\caption{证明反射性质} \label{fig_Hypb3_11}
\end{figure}

从物理直觉出发：
设 $l$ 是过 $P$ 的切线，要求 $\angle (F_1P, l) = \angle (PF_2, l)$，这意味着 $PF_1$ 和 $PF_2$ 关于切线对称。而由双曲线的定义，是定值，这与反射条件自然吻合。

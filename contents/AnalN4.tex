...
\item 4.4 ……就是说，$H$ 中的每一个柯西序列都收敛，那么 $H$ 就称为希尔伯特空间.
...
\item \textbf{等距映射}是保持距离的一种简单映射. 对所有 $X_{0}$ 中的点 $x_{1}, x_{2}, f\left(x_{1}\right)$ 与 $f\left(x_{2}\right)$ 在 $Y$ 中的距离刚好等于 $x_{1}$ 和 $x_{2}$ 在 $X$ 中的距离.
...
\item 4.18 设 $\left\{u_{a}: \alpha \in A\right\}$ 是 $H$ 中的规范正交集. 下面关于 $\left\{u_{a}\right\}$ 的四个条件中的每一个都蕴含着另外三个： (i) $\left\{u_{a}\right\}$ 是 $H$ 的极大规范正交集. (ii) $\left\{u_{a}\right\}$ 的全体有限线性组合 $P$ 在 $H$ 中稠密. (iii) 对每个 $x \in H$, 有 $\sum_{a \in A}|\hat{x}(\alpha)|^{2}=\|x\|^{2}$. (iv) 若 $x \in H, y \in H$, 则 $\sum_{a \in A} \hat{x}(\alpha) \overline{\hat{y}(\alpha)}=(x, y)$. 最后一个公式就是熟知的\textbf{帕塞瓦尔恒等式（Parseval's identity）}. 注意 $\hat{x},\hat{y} \in \ell^{2}(A)$, 所以 $\hat x\bar{\hat y} \in \ell^1(A)$. 从而 (iv) 中的和式的意义是明确的. 当然, (iii) 是(iv)当 $x=y$ 时的特例. 极大规范正交集通常叫做\textbf{完备规范正交集}或者\textbf{规范正交基}.

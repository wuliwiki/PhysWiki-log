% 平面波的的正交归一化
% keys 波函数|归一化|delta 函数|完备性|薛定谔方程
% license Xiao
% type Tutor

\pentry{一维自由粒子（量子）\nref{nod_FreeP1}}{nod_15dd}

本文使用\enref{原子单位制}{AU}。 回顾\enref{一维自由粒子}{FreeP1}中的分析，我们需要把能量本征态即平面波进行\textbf{动量归一化}或者\textbf{能量归一化}% \addTODO{链接}
...
\end{equation}
其中 $*$ 表示复共轭， 满足该式的 $\psi_k$ 就是动量归一化的平面波：% \addTODO{链接}
\begin{equation}\label{eq_EngNor_2}
\psi_k(x) = \frac{\E^{\I kx}}{\sqrt{2\pi}}~.
\end{equation}
但什么样的归一化系数能满足能量归一化条件呢？

我们可以令定态薛定谔方程的两个线性无关解 $\psi_{E,1}(x), \psi_{E,2}(x)$ 分别为 $-k$ 和 $k$ 的平面波（暂时规定 $k = \sqrt{2mE} > 0$），只是归一化系数和\autoref{eq_EngNor_2} 不同。 只需要使用 $\delta$ 函数的性质\autoref{eq_Delta_12}~\upref{Delta}， 把 $E'-E$ 看成 $k$ 的函数（$k'$ 视为常数）\footnote{反之亦然： $k'$ 看成变量， $k$ 看成常数}， 唯一的正根为 $k'$
\begin{equation}
...
现在， 任意波函数 $\psi(x)$ 就可以用能量归一化的基底展开为% \addTODO{链接}
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
他们满足\autoref{eq_EngNor_4} （习题）。 这个变换使用了矩阵 $\pmat{1 & 1\\ -\I & \I}/\sqrt{2}$， 可以证明使用任何 2 乘 2 的\enref{酉矩阵}{UniMat}做变换都可以。
...
\qquad (k > 0)~.
\end{equation}
这就是\enref{三角傅里叶变换}{FTTri}中使用的基底。他们满足归一化条件

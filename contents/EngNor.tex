...
本文使用\enref{原子单位制}{AU}。 回顾一维自由粒子\upref{FreeP1}中的分析， 我们需要把能量本征态即平面波进行\textbf{动量归一化}或者\textbf{能量归一化}% \addTODO{链接}
...
\end{aligned}
\end{equation}
他们满足\autoref{eq_EngNor_4} （习题）。 这个变换使用了矩阵 $\pmat{1 & 1\\ -\I & \I}/\sqrt{2}$， 可以证明使用任何 2 乘 2 的酉矩阵\upref{UniMat}做变换都可以。
...
\qquad (k > 0)~.
\end{equation}
这就是三角傅里叶变换\upref{FTTri}中使用的基底。他们满足归一化条件

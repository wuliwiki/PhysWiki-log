...
\item sshfs 相当于把 sftp 的某个文件夹挂载到某个本地目录。
...
\begin{itemize}
\item \verb|SMB| 对 Windows 更友好， \verb|NFS| 对 linux 更友好。
...
\item 安装客户端： \verb|sudo apt install cifs-utils -y|
\item 编辑文件（可以放到任何路径，用任何文件名）： \verb|sudo vim /root/.SMBcredentials|
\begin{lstlisting}[language=none]
username=smb用户名
password=smb密码
\end{lstlisting}

...
c(q) = \braket{q}{\psi}
\end{equation}
而 $\abs{c(q)}^2$ 则是测量得到 $q$ 的概率密度\upref{RandF}， 满足归一化条件（链接到测量理论\upref{QM1} 相关公式）
\begin{equation}
...
动量的算符为 $P = -\I\pdv*{x}$， 本征值记为 $k = p$， 因为在原子单位中波数等于动量\upref{AU}． 归一化的本征函数为
\begin{equation}
...
特殊地， 若 $\psi(x)$ 是一个实函数（或者乘以一个任意相位因子 $\E^{\I \theta}$）， 那么 $\abs{c(p)}^2$ 必定是一个偶函数（\autoref{FTExp_eq5}~\upref{FTExp}）， 可得动量平均值为零． 所以
\begin{theorem}{}
...
同时具有离散和连续本征值的典型的例子就是一个有限深势阱的哈密顿算符（能量算符）$H$， 如有限深方势阱\upref{FSW}．

...
本文使用\enref{原子单位制}{AU}， 使用\enref{狄拉克符号}{braket}。 我们先来回顾测量理论。 假设某个测量量 $q$ 对应的算符为 $Q$， 有离散的本征态 $\ket{\phi_i}$ （$i = 1,2\dots$）， 数量可以是有限或无穷多个， 对应的本征值为 $q_i$， 满足
...
谐振子内容详见 “\enref{量子简谐振子（升降算符法）}{QSHOop}”。 频率为 $\omega$ 的一维线性谐振子， 若状态为
\begin{equation}
...
\end{equation}
其中 $\delta$ 是\enref{狄拉克 delta 函数}{Delta}。 那么归一化波函数的展开变为
...
c(q) = \braket{q}{\psi}~.
\end{equation}
而 $\abs{c(q)}^2$ 则是测量得到 $q$ 的\enref{概率密度}{RandF}， 满足归一化条件
\addTODO{链接到\enref{测量理论}{QM1} 相关公式}
\begin{equation}
\int \abs{c(q)}^2 \dd{q} = 1~.
\end{equation}

这时可以按\autoref{eq_RandF_7} 定义平均值为
...
在\enref{位置表象}{moTDSE}下， 虽然我们往往直接认为 $\abs{\psi(x)}^2$ 就是位置分布的概率密度函数， 但这同样可以用上述过程推导出来。 位置的算符就是 $X = x$， 正交归一化的本征态矢为 $\ket{x}$ 也就是位于 $x$ 处的\enref{狄拉克 $\delta$ 函数}{Delta}， 满足正交归一化条件（\autoref{eq_QMavg_9}）
\begin{equation}
...
一维动量的算符为 $P = -\I\pdv*{x}$， 本征值记为 $k = p$， 因为在\enref{原子单位制}{AU}中波数等于动量。 归一化的本征函数为
\begin{equation}
...
\end{equation}
若要计算函数 $\ket{\psi} = \psi(x)$ 的动量平均值， 那么可以先计算展开系数， 即\enref{傅里叶变换}{FTExp}
\begin{equation}
c(p) = \braket{p}{\psi} = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty} \psi(x)\E^{-\I kx} \dd{x}~.
\end{equation}
事实上， $c(p)$ 也叫做\enref{动量表象波函数}{moTDSE}， 也就是 $\ket{\psi}$ 在动量基底 $\ket{p}$ 的展开系数。 计算平均值得
\begin{equation}
...
\begin{corollary}{}
\enref{定态薛定谔方程}{SchEq}中束缚态的动量平均值必为零。
\end{corollary}
...
同时具有离散和连续本征值的典型的例子就是一维有限深势阱的哈密顿算符（能量算符）$H$， 如\enref{有限深方势阱}{FSW}。

...
虽然我们可以直接使用 \lstinline|std::vector|， 但是通过创建一个简单的矢量类我们可以了解写一个类的基本思路， 在这之后就可以按照同样的思路创建各种容器了．
...
首先这是一个类模板， 而不是类本身， 例如如果用户声明了一个变量如 \lstinline|Vector<Doub>|， 那么编译器会根据模板生成一个定义， 将模板中的所有 \lstinline|T| 替换为 \lstinline|Doub|． 如果声明了多种类型的 \lstinline|Vector<T>|， 那么编译器也会相应生成多个不同版本的 \lstinline|Vector| 的定义．

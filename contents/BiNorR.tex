...
\end{aligned}~
\end{equation}
容易看出，当 $u$ 为整数时，$i>u$ 的所有项为 $0$，得到整数指数的\enref{二项式定理}{BiNor}。

\subsection{数值验证}

在学习微积分之前，这里只给出一个数值验证的方法（而不是证明）。 在微积分中， 这个定理可以用\enref{泰勒展开}{Taylor}推导出来。
...
\end{equation}
即可。接下来，可以用计算器或程序对\autoref{eq_BiNorR_1} 的前 $N$ 项进行求和。如果增加 $N$ 使结果趋近精确值，则验证成功（见下表）。 这里给出计算下表的 \enref{Matlab 程序}{BiNorM}。

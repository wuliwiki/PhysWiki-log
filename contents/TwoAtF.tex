...
三维散射问题的一般解法是分波法，也就是将波函数按照动能算符$\nabla^2$在球坐标下，角向部分的本征态展开

\begin{equation}
\Psi(r) = \sum_{l=0}^{+\infty}\frac{\chi_{kl}(r)}{kr}P_l(\cos\theta) ~,
\end{equation}
其中呢，$\chi_{kl}$ 是分波之后的成分，需要进一步求解，而 $P_l(\cos\theta)$ 则是勒让德函数。 $k$ 是受能量调控的。 $E=\hbar^2 k^2/(2m)$。 很容易的写出径向本征态薛定谔方程：
\begin{equation}
...
根据对short range相互作用势的一般假设，我们假定当$r>r_0$时，$V(r)$几乎等于0。对于一个散射态本征态来说，$E>0$,这个的解在大于我们的截止半径 $r_0$ 的时候一目了然，就是简单地 $\sin(kr + \delta_k)$ 嘛；然而这个 $\delta$ 究竟是多少呢？这就要取决于在小于截止半径内的具体势能形式了。 不过要注意，这里的这个截止半径确实是很关键的一个要求；但是，有的时候即使并不是 $\E^{-kr}$ 而是 $r^{-\alpha}$ 的形式的势能，在某些情况下我们的分析也是正确的，在本文的后半部分会涉及这一点。
...
这个表达式可以在球方势阱的例子中通过势阱边界两端的连续性条件得到。其中，这个 $a_s$ 是人为定义的一个参数，叫做散射长度；其一个很直观的物理意义就是：在远处波函数的行为可以认为与一个半径为 $a_s$ 的硬球模型一样（其势能为 $V(r) = (1-\theta(r-a_s)) \times +\infty$）。
...
\subsection{质心系的坐标变换}
细心的读者会注意到，虽然我们在讨论两个原子间的相互作用，但在计算中我们在计算单原子的薛定谔方程。这是因为我们的计算过程中做了一次坐标变换，自考虑相对坐标而不关心质心坐标。一个简单的说明如下：考虑一个两粒子体系的哈密顿量为
\begin{equation}
\hat{H} = \frac{\hat{p}_1^2}{2m} + \frac{\hat{p}_2^2}{2m} + V(|r_1-r_2|)~,
\end{equation}
其中我们假设两个原子间的相互作用只跟他们的相对距离有关，同时也假定两个粒子的质量相同。注意到关系式$(x+y)^2 +(x-y)^2 = 2x^2 + 2y^2$,我们令$\hat{P} = (\hat{p}_1 + \hat{p}_2)/\sqrt{2}$, $\hat{p} = (\hat{p}_1 - \hat{p}_2)/\sqrt{2}$,此时就有
\begin{equation}
\frac{\hat{p}_1^2}{2m} + \frac{\hat{p}_2^2}{2m} = \frac{\hat{P}^2}{2m} + \frac{\hat{p}^2}{2m}~.
\end{equation}
另外，可以证明，对动量的变换等价于对坐标的变换$R = (r_1+r_2)/\sqrt{2},r=(r_1-r_2)/\sqrt{2}$，或$r_1 = (R+r)/\sqrt{2},r_2 = (R-r)/\sqrt{2}$
\begin{equation}
\frac{\partial \psi}{\partial r} = \frac{\partial \psi}{\partial r_1} \frac{\partial r_1}{\partial r} + \frac{\partial \psi}{\partial r_2} \frac{\partial r_2}{\partial r} = \frac{\partial \psi}{\sqrt{2}\partial r_1} -\frac{\partial \psi}{\sqrt{2}\partial r_2}~.
\end{equation}
对于$R$的导数类似，可以证得
\begin{equation}
\frac{\partial^2 \psi}{\partial r^2} + \frac{\partial^2 \psi}{\partial R^2} =  \frac{\partial^2 \psi}{\partial r_1^2} + \frac{\partial^2 \psi}{\partial r_2^2} ~.
\end{equation}
于是两粒子体系的哈密顿量就可以写为
\begin{equation}
\hat{H} = \frac{\hat{P}^2}{2m} + \frac{\hat{p}^2}{2m} + V(r)~.
\end{equation}
质心部分参数是自由粒子的哈密顿量，所以一般忽略不计。注意到，我们将两粒子问题化为单粒子问题后，谈到的势场实际上是原子间相互作用势，而两粒子之外没有外势场。
\subsection{三维球方势阱}

之前我们也提到过，只要有相同的 $\delta_k$，那么对于不同的势能形式，其原子远处的性质也是一样的。 于是我们来研究一个很有趣的toy model，三维球方势阱：
...
\end{cases}
\end{equation}
于是，对于s波的 Schrödinger 方程就是

\begin{align}
\chi'' + \frac{2m}{\hbar^2}(E+V_0)\chi &= 0\quad (r < r_0)~,\\
\chi'' + \frac{2m}{\hbar^2}(E)\chi &= 0\quad (r \geqslant r_0)~.
\end{align}
对于散射态，$E>0$,因此方程的解必定是$\sin (\omega x+\phi)$或$\cos(\omega x+\phi)$这样的形式。但考虑到我们在最初变量代换时有$\Psi(r) \sim \frac{\chi(r)}{r}$，因此为确保波函数在$r=0$时不发散，就要求有$\chi(0) = 0$。因此这个方程的解就确定了，
\begin{align}
\chi(r) = A\sin k' r \quad (k' = \sqrt{\frac{2m(E+V_0)}{\hbar^2}},0 \leq r \leq r_0)~,\\
\chi(r) = B\sin (k r+ \delta_k)\quad  (k = \sqrt{\frac{2mE}{\hbar^2}}, r\geq r_0)~.
\end{align}


其中，$E$ 表示由于是低能散射，所以能量很小,与球方势阱的深度$V_0$相比可以忽略，因此在势阱内可以近似写为$k'\approx \sqrt{2mV_0/\hbar^2}$。 如此我们就可以写出连接条件
\begin{equation}

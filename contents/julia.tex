...
Juno 和 JuliaPro 都已经停止更新， 当前（2023-3）最热门的 IDE 环境是 Julia 的 \href{https://www.julia-vscode.org}{VScode 拓展}， 应该支持逐行调试。
...
\item \verb|println("$var1 some words $var2")| 中 \verb|var1, var2| 可以是数字、字符串等， 这和 \verb|println(var1, " some words ", var2)| 等效。 其实这是字符串的功能而不是 \verb|println| 特有的。 要避免混淆用 \verb|$(var)|。
\end{itemize}
...
\item 一维数组 \verb|Array{类型,1}| 也叫 \verb|Vector{类型}|， 不区分行和列； \verb|Array{类型,2}| 也叫 \verb|Matrix{类型}|， 区分行向量和列向量。
\item \verb|[1, 2]| 和 \verb|[1; 2]| 的类型是 \verb|Array{Int64,1}|， 但 \verb|[1 2]| 的类型是 \verb|Array{Int64,2}|
...
\item \verb|a| 作为参数传给函数是 pass by reference， 相当于 C 语言传递指针（有例外， 见 Julia 的函数\upref{JuFunc}）
...
\subsection{线性代数}
\begin{itemize}
\item 迹 \verb|tr(A)|， 行列式 \verb|det(A)|， 逆 \verb|inv(A)|， 本征值 \verb|eigvals(A)|， 本征矢 \verb|eigvecs(A)|， 本征对 \verb|tmp = eigen(A)|（\verb|tmp.vectors, tmp.values|）， LU 分解（链接未完成） \verb|factorize(A)|
...
\item 见 “Julia 调用 C 语言\upref{JuliaC}”
\end{itemize}

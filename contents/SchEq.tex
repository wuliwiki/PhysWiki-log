...
\end{equation}
由于哈密顿算符 $H$ 是线性的， 但从方程来看， 把 $\psi$ 乘一个任意复常数仍然是解． 但由于 $\psi$ 需要满足一定的归一化条件， 所以 $\psi$ 乘以复相位 $\E^{\I\theta}$ 仍然是解． 事实上任何测量量算符的本征函数都是如此． 下面会看到 $H$ 是一个实算符\footnote{实算符这里指其把实值函数映射到实值函数， 虚值函数映射到虚值函数．}， 所以它的解 $\psi(\bvec r)$ 也总可以取实值函数．
...
对一维或二维的有限深势阱， 如果各方向无穷远处势能为零， 那么都至少有一个束缚态\footnote{一维证明参考 \cite{Teschl} 的第九章．}． 若不满足则未必存在束缚态（例如有限深不对称方势阱\upref{AMW}）． 三维或以上的有限深势阱也未必存在束缚态\footnote{详见 \cite{Landau} 45 节．}．
\end{theorem}
一个不存在束缚态的一维势阱的例子见不对称方势阱\upref{AMW}， 它不符合定理中 “各方向无穷远处势能为零” 的条件．
...
容易看出对于自由粒子， 该积分就是反傅里叶变换， 用还原积分法把积分变量改为 $k$， 用第一个积分表示 $k\ge 0$， 第二个表示 $k<0$． 例如自由粒子（$V \equiv 0$）的高斯波包\upref{GausPk} 就可以看作由平面波线性叠加而来， 即反傅里叶变换．
...
\subsection{波函数的对称性}
\pentry{宇称算符\upref{Parity}， 矩阵对易与共同本征矢\upref{OpComu}}
对任意维薛定谔方程， 若哈密顿算符和宇称算符 $\Pi$ 对易， 则它们具有一组共同的本征波函数\upref{OpComu}， 其中每个都具有奇宇称或者偶宇称．

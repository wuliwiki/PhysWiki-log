...
该定理在物理中可应用于电场和电荷的关系， 定理中的 $\bvec F$ 可看做电场 $\bvec E$， 而 $V(\bvec r)$ 可看做电荷密度 $\rho(\bvec r)$ 除以真空介电常数 $\epsilon_0$。 于是\autoref{DivInv_eq3} 就是电场的高斯定律（\autoref{EGauss_eq1}~\upref{EGauss}）， \autoref{DivInv_eq1} 就是库仑定律（\autoref{Efield_eq9}~\upref{Efield}）。
...
\end{equation}
由于 $V(\bvec r')$ 是 $x', y', z'$ 而不是 $x, y, z$ 的函数， 右边第一项中 $\grad V(\bvec r') = 0$。 第二项中（\autoref{CEfDiv_eq2}~\upref{CEfDiv}）
\begin{equation}

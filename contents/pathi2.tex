...
这里初态和末态以及 $\hat x(t_1)\cdots \hat x(t_n)$ 中的时间指标代表它们是\textbf{海森堡绘景}\footnote{\enref{薛定谔绘景和海森堡绘景}{HsbPic}。}下的态矢量和算符。因此作绘景变换后，利用 $\hat x(t)=e^{iHt}\hat x e^{-iHt}$ 和 $\ket{x_i,t_i}=e^{iHt_i} \ket{x_i}$，可以得到：\textbf{（不妨设 $t_1>t_2>\cdots >t_n$）}
\begin{equation}

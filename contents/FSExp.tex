...
根据欧拉公式（\autoref{CExp_eq2}~\upref{CExp}）， 可以写出正余弦函数和复指数函数的关系
\begin{equation}
...
\end{equation}
证明： 用狄拉克符号\upref{braket}记为 $\ket{f} = \sum_n c_n \ket{n}$， 利用基底的正交性（\autoref{FSExp_eq4}）
\begin{equation}
...
\subsection{任意区间的展开}
类比\autoref{FSTri_sub2}~\upref{FSTri}中的讨论， 要用指数傅里叶级数展开 $[a, b]$ 区间的函数 $f(x)$， 一种方法是令 $l = (b - a)/2$， 并把\autoref{FSExp_eq2} 的积分区间改为 $[a, b]$ 即可． 或者也可以取 $l > (b - a)/2$ 的任意实数．

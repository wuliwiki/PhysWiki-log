...
\pentry{定轴旋转矩阵\upref{RotA}， Matlab 的函数\upref{MatFun}}

这里给出一个 Matlab 函数 \verb|turn()| 可以一次把三维空间中若干点绕非零矢量 $\bvec A$ 按照右手定则\upref{RHRul}旋转角度 $\theta$。 在程序中， 这些点可以用任意形状的 \verb|X, Y, Z| 数组来表示， 也可以用单个三列的矩阵 $P$ 来表示（每列分别为 $x, y, z$ 坐标）。 $\bvec A$ 和 $\theta$ 既可以直接指定， 也可以通过指定两个矢量 $\bvec D_1, \bvec D_2$： $\bvec A$ 垂直于 $\bvec D_1, \bvec D_2$， 且把 $\bvec D_1$ 方向转到 $\bvec D_2$ 方向。

使用示例： 转动一个正方形
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/4881e6039f19ffc5.pdf}
\caption{运行结果 1： 绕 $z$ 轴转动} \label{fig_turnM_1}
\end{figure}
\begin{figure}[ht]
\centering
\includegraphics[width=7cm]{./figures/186892291c5ae99d.png}

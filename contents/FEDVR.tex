...
现在以一维 FEDVR 为例，把整个区间划分成 $N_e$ 个\bb{有限元}，第 $i$ 个有限元的区间为 $[x_i,x_{i+1}]$．每个有限元内进一步加入格点，令 $x_{ij}\ (j = 1\dots N)$ 为第 $i$ 有限元的 $N$ 阶 Gauss-Lobatto 数值积分 %链接未完成
...
易证， 若要对整个区间积分， 所需权重为
\begin{equation}
w_{ij} = \frac{1}{u_{ij}^2(x_{ij})} = \leftgroup{
&a_i w_{0j} \quad &&( 1 < j < N) \\
&(a_i + a_{i+1}) w_{0N} \quad &&( j = N)\\
} \end{equation}
...
可以看出 $\mat K$ 是实对称矩阵。 这个积分可以精确地用求和表示， 而且只有同一个 FE 里面的 $u_i(x), u_j(x)$ 才能使积分不为零（bridge function 属于两个 FE）， 所以就得到了几乎是 block diagonallized 的矩阵， 只是每一个 block 左上角和右下角的一个矩阵元与其他 block 重叠。

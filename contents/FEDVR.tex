...
现在来定义 FEDVR 基底 $u_{ij}(x)$． 令 $a_i$ 是第 $i$ 个 FE 长度的一半， $b_i$ 是第 $i$ 个 FE 的中点坐标， 把\autoref{GLquad_eq3}~\upref{GLquad} 中的 $f_n(x)$ （$x$ 超出 $[-1,1]$ 时令 $f_n(x) = 0$） 记为 $f_n(t)$， 并把 $t \in [-1,1]$ 线性依次映射到每个有限元的区间 $x \in [x_i, x_{i+1}]$ 上， 有
\begin{equation}
x_{ij} = a_i t_j + b_i
\end{equation}
\begin{equation}

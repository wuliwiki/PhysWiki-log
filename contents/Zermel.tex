...
\item 莱维（Levy, Azriel）（2002年）。《基础集合论》（Basic Set Theory）。多佛出版社（Dover Publications）。ISBN 048642079-5。
\item 林克（Link, Godehard）（2014年）。《形式主义与超越：论数学话语的本质》（Formalism and Beyond: On the Nature of Mathematical Discourse）。德古意特出版社（Walter de Gruyter GmbH & Co KG）。ISBN 978-1-61451-829-7。
\item 蒙塔古（Montague, Richard）（1961年）。《语义封闭性与非有限公理化》（"Semantical closure and non-finite axiomatizability"）。载于《无穷主义方法》（Infinistic Methods）。伦敦：培格曼出版社（Pergamon Press）。第45–69页。
\item 蒯因（Quine, Willard van Orman）（1969年）。《集合论及其逻辑（修订版）》（Set Theory and Its Logic，Revised ed.）。马萨诸塞州剑桥与英国伦敦：哈佛大学出版社贝尔纳普分社（The Belknap Press of Harvard University Press）。ISBN 0-674-80207-1。
\item 舍恩菲尔德（Shoenfield, Joseph R.）（1977年）。《集合论公理》（"Axioms of set theory"）。载于巴威斯（Barwise, K. J.）编。《数理逻辑手册》（Handbook of Mathematical Logic）。北荷兰出版公司（North-Holland Publishing Company）。ISBN 0-7204-2285-X。
\item 舍恩菲尔德（Shoenfield, Joseph R.）（2001年）[初版1967年]。《数理逻辑（第2版）》（Mathematical Logic，2nd ed.）。A K Peters出版社。ISBN 978-1-56881-135-2。
\item 苏佩斯（Suppes, Patrick）（1972年）[初版1960年]。《公理集合论》（Axiomatic Set Theory）。多佛再版（Dover reprint）。
\item 武藤夏生（Takeuti, Gaisi）；扎林（Zaring, W M）（1971年）。《公理集合论导论》（Introduction to Axiomatic Set Theory）。斯普林格出版社（Springer-Verlag）。
\item 武藤夏生（Takeuti, Gaisi）；扎林（Zaring, W M）（1982年）。《公理集合论导论》（Introduction to Axiomatic Set Theory）。斯普林格出版社（Springer）。ISBN 9780387906836。
\item 塔斯基（Tarski, Alfred）（1939年）。《论任意集合的良序子集》（"On well-ordered subsets of any set"）。《数学基础》（Fundamenta Mathematicae），32：176–183。doi:10.4064/fm-32-1-176-783。
\item 泰尔斯（Tiles, Mary）（1989年）。《集合论哲学》（The Philosophy of Set Theory）。多佛再版（Dover reprint）。
\item 图拉基斯（Tourlakis, George）（2003年）。《逻辑与集合论讲义（第二卷）》（Lectures in Logic and Set Theory, Vol. 2）。剑桥大学出版社（Cambridge University Press）。
\item 沃尔乔弗（Wolchover, Natalie）（2013年）。《为平息无穷之争，一条新的逻辑法则》（"To Settil Infinity Dispute, a New Law of Logic"）。《量子杂志》（Quanta Magazine）。
\item 策梅洛（Zermelo, Ernst）（1908年）。《集合论基础研究I》（"Untersuchungen über die Grundlagen der Mengenlehre I"）。《数学年刊》（Mathematische Annalen），65（2）：261–281。doi:10.1007/BF01449999。S2CID 120085563。英译版见范·海恩诺特（Heijenoort, Jean van）（1967年）。《集合论基础研究》（"Investigations in the foundations of set theory"）。《从弗雷格到哥德尔：数理逻辑原始文献集（1879–1931）》（From Frege to Gödel: A Source Book in Mathematical Logic, 1879–1931）。科学史原始文献丛书（Source Books in the History of the Sciences）。哈佛大学出版社（Harvard University Press）。第199–215页。ISBN 978-0-674-32449-7。
\item 策梅洛（Zermelo, Ernst）（1930年）。《论极限数与集合域》（"Über Grenzzahlen und Mengenbereiche"）。《数学基础》（Fundamenta Mathematicae），16：29–47。doi:10.4064/fm-16-1-29-47。ISSN 0016-2736。
\end{itemize}
\subsection{外部链接}
\begin{itemize}
\item Axioms of Set Theory - Lec 02 - Frederic Schuller 在 YouTube 上  
\item "ZFC"，*数学百科全书，EMS 出版社，2001 [1994]  
\item 斯坦福哲学百科全书由 Joan Bagaria 编写的文章：  
\item Bagaria, Joan (2023年1月31日). "Set Theory". 在 Zalta, Edward N. (编). 斯坦福哲学百科全书。  
\item — (2023年1月31日). "Axioms of Zermelo–Fraenkel Set Theory". 在 — (编). 斯坦福哲学百科全书。  
\item Metamath 版本的 ZFC 公理 — 一种简洁且无冗余的公理化。背景的一阶逻辑特别定义，以便于机器验证证明。  
\item 在 Metamath 中，从替换公理的一个版本推导出分离公理模式的一个版本。  
\item Weisstein, Eric W. "Zermelo-Fraenkel Set Theory". *MathWorld*。
\end{itemize}
...
\item 要改变 swap 文件的尺寸： \verb|sudo swapoff /swapfile|， \verb|sudo dd if=/dev/zero of=/swapfile bs=1M count=1024 oflag=append conv=notrunc|， \verb|sudo mkswap /swapfile|， \verb|sudo swapon /swapfile| 一般无需重启， 但如果系统不允许关闭 swap， 也可以考虑用多个 swap 文件。
...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/LinNt_2.png}
...
\item \verb|egrep| 是支持正则表达式\upref{regex}的 grep， 例如 \verb`lsb_release -a 2>&1 | egrep -o -m 1 [0-9]{2}\.[0-9]{2}` 可以获取 ubuntu 系统的版本号（相当于 \verb|lsb_release -rs|）。 其中 \verb|-m 1| 只匹配第一个结果， \verb|-o| 只输出匹配部分。

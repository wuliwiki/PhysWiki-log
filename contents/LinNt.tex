...
\item \verb`useradd [-m] [-g 用户组] [-U] [-s /bin/bash] 用户名`。 其中 \verb|-m| 给新用户创建 home 目录， \verb|-U| 选项创建和用户名相同的用户组， \verb|-s| 指定 shell。
...
\item \verb|/etc/group| 文件记录了所有用户组的信息， 格式为 \verb|用户名:密码:组ID:组员|
...
\item 注意 \verb|cd| 到某个文件夹中必须要有 \verb|-x| 权限， 即使不执行任何东西。

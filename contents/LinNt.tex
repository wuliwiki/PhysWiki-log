...
\item 要改变 swap 文件的尺寸： \verb|sudo swapoff /swapfile|， \verb|sudo dd if=/dev/zero of=/swapfile bs=1M count=1024|， \verb|sudo mkswap /swapfile|， \verb|sudo swapon /swapfile| 一般无需重启， 但如果系统不允许关闭 swap， 也可以考虑用多个 swap 文件。有时候 \verb|/swapfile| 也叫做 \verb|/swap|。
...
\item 要开启 SetGID， 用 \verb|chmod +s|。 在显示权限时， 文件夹会显示为 \verb|rws|。 \verb|s| 的编号是 \verb|2000|， 例如可以用 \verb|chmod 2775 文件夹|。

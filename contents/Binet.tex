...
我们来看 “中心力场问题中\upref{CenFrc}”得到的两条运动方程（\autoref{CenFrc_eq5} 和\autoref{CenFrc_eq4}）
\begin{align}
\ddot{r} - r \dot\theta^2 &= F(r) \label{Binet_eq1}\\
mr^2\dot \theta &= L \label{Binet_eq2}
\end{align}
为了得到极坐标中 $r(\theta)$ 的微分方程（\bb{轨道方程}）， 我们以下用\autoref{Binet_eq2} 消去 \autoref{Binet_eq1} 中的 $t$． 首先可以把 $r$ 看做复合函数 $r[\theta(t)]$， 再用链式法则\upref{ChainR}把\autoref{Binet_eq1} 的第一项用 $\dv*{\theta}{t}$ 表示

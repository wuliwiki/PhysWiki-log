...
\pentry{中心力场问题\nref{nod_CenFrc}， 二阶常系数非齐次微分方程\nref{nod_Ode2N}}{nod_1435}
\subsection{比耐公式}
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Binet_equation}{相关页面}。}我们来看 “中心力场问题\upref{CenFrc}” 中得到的两条运动方程（\autoref{eq_CenFrc_5} 和\autoref{eq_CenFrc_4}）
\begin{equation}
...
\end{equation}
为了得到极坐标中 $r(\theta)$ 的微分方程（\textbf{轨道方程}）， 我们以下用\autoref{eq_Binet_2} 消去\autoref{eq_Binet_1} 中的 $t$。 首先可以把 $r$ 看做复合函数 $r[\theta(t)]$， 再用\enref{链式法则}{ChainR}处理\autoref{eq_Binet_1} 的第一项
...
\end{equation}
这就是轨道方程。 这个方程比较复杂， 但可以通过换元法 % \addTODO{介绍微分方程的换元}
化为十分简洁的形式。令
...
\end{equation}
这是一个\enref{阶常系数非齐次微分方程}{Ode2N}，被称为\textbf{比耐公式（Binet equation）}。

在\enref{开普勒问题}{CelBd}中，相互作用势为 $V(\rho)=-GMm/r$， 可以证明此时轨道的形状是圆锥曲线的一种， 详见 “\enref{开普勒第一定律的证明}{Keple1}”。

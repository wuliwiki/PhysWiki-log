...
\begin{figure}[ht]
\centering
\includegraphics[width=13cm]{./figures/376a587ddc7e9250.pdf}
...
\subsection{波函数}
$\bvec E, \bvec B$均为\enref{矢量场}{Vfield}，即电场$\bvec E$或磁场$\bvec B$均是关于空间与时间的矢量函数 $\bvec E = \bvec E(\bvec r, t), \bvec B = \bvec B(\bvec r, t)$。

完全类似于机械波，电磁波的波函数的通解可以写为一组平面简谐波的线性组合。

\subsubsection{三角形式}
平面电磁波为
\begin{equation}\label{eq_VcPlWv_3}
\begin{aligned}
...
\subsubsection{复指数形式}
或写为\enref{复数形式}{PWave}。复数形式的波函数更便于求导、相乘等计算，并在某些情况下是必不可少的数学工具（但也更难懂）。最终“实际存在的场”是他的实数部分 $\bvec E = \Re (\tilde {\bvec E})$
\begin{equation}
\begin{aligned}
...
$\tilde {\bvec E_0}$表明$\tilde {\bvec E_0}$是一个复矢量，即它的各分量是一个复数。$\tilde {\bvec E}_0$包括了初相位信息，写为分量形式即为$
\tilde {\bvec E_0} = 
\begin{pmatrix}
...
\begin{equation}
\tilde {\bvec B}_0 = \frac{1}{\omega} \bvec k \times \tilde{\bvec E}_0 ~.
\end{equation}
由此，可推导出以下结论：
\begin{itemize}
...
部分推导可见 “\enref{电场波动方程}{EWEq}”

\subsubsection{能量密度}
\pentry{电场的能量\nref{nod_EEng}， 磁场的能量\nref{nod_BEng}}{nod_3047}
任意一点的瞬时能量密度为
...
\end{equation}
另一个基于坡印廷矢量的推导见\autoref{ex_EBS_1} %， 也可以认为瞬时能流密度等于能量密度乘以波速 $c$， 对于简谐波，需要除以二得平均值。

% 泰勒展开（简明微积分）
% keys 微积分|导数|高阶导数|泰勒展开|泰勒级数
% license Xiao
% type Tutor

\pentry{阶乘\nref{nod_factor}， 高阶导数\nref{nod_HigDer}， 分部积分法\nref{nod_IntBP}，幂级数（简明微积分）\nref{nod_powerS}}{nod_f40e}
...
\begin{figure}[ht]
\centering
\includegraphics[width=14cm]{./figures/52cb72427ddf7f2e.pdf}
...
\subsection{收敛半径}
我们把形如 $\sum_{n=0}^\infty c_n (x-x_0)^n$ 的表达式叫做\enref{幂级数（power series）}{powerS}。 泰勒展开就是在用幂级数表示函数。 如果某幂级数收敛的点不止 $x_0$ 一点， 那么必定存在一个\textbf{收敛半径（radius of convergence）} $0 < r < 1$， 使得当 $\abs{x-x_0} < r$ 时级数必定收敛， 而 $\abs{x-x_0} > r$ 是必定发散（不收敛）。

...
Matlab 中的所有变量都可以理解为\textbf{矩阵}，单值变量（\x{标量}，scalar）可以理解为 \x{1×1} 的矩阵，只有一行或一列的矩阵叫做\textbf{行矢量（row vector）}和\textbf{列矢量（column vector）}．一些简单的矩阵操作如下
\begin{Command}
>> a = [1,2,3] \\
a = 1\ \ 2\ \ 3
\end{Command}
...
1\ \ 2\ \ 3\ \ 1\ \ 2\ \ 3
\end{Command}
用 \x{size} 函数获取矩阵尺寸，用 \x{numel} 函数获取矩阵元个数
\begin{Command}
>> size(d) \\
ans = 2\ \ 3 \\
>> size(d,1) \\
...
5\ \ 5\ \ 5
\end{Command}
用 \x{eye(N)} 生成 $N\times N$ 的单位矩阵，用 \x{rand(M,N)} 生成随机矩阵，矩阵元从 0 到 1 均匀分布．用 \x{M:step:N} 生成等差数列（行矢量），例如
\begin{Command}
>> 1:2:10 \\
ans = 1\ \ 3\ \ 5\ \ 7\ \ 9 \\
>> 0:pi/3:pi*2 \\
...
1.5000\ \ -2.0000
\end{Command}
单引号“\x{'}”可以使实数矩阵转置，或使复矩阵取厄米共轭．若需要对复矩阵转置，用“\x{.'}”即可．
\begin{Command}
>> c = a + 1i*b \\
c = \par
...
5\ \ 6
\end{Command}
其中 \x{end} 表示某维度的最大索引． 以上的格式可以归结为“在小括号中用逗号把行矢量隔开”．注意索引不仅可以用来取值，还可以放在等号左边赋值．
\begin{Command}
>> b = a; b(1:3) = a(2:4) \\
b = \par
4\ \ 2\ \ 3 \par
...
ans = 24
\end{Command}
把双精度变类型变为字符串可以用 \x{num2str} 函数（注意 \x{2} 的英文读音与“to”相同， \x{num} 代表“number”， \x{str} 代表字符串“string”），通常用于与其他字符矩阵合并，如
\begin{Command}
...
逻辑变量只能具有 \x{0} 或 \x{1} 两个值，分别代表\textbf{假（false）}和\textbf{真（true）}．最常用的地方是判断语句 \x{if} 的后面以及获取矩阵元．逻辑算符有
\begin{Command}
>，>=（大于等于），<，<=，==（等于）
\end{Command}
可用于比较双精度数组，返回逻辑型数组
...
\end{Command}
在控制行中， 输出的 \x{\color{blue}\underline{logical}} 与 \x{0} 各占一行， 这里为了节约空间将其写成一行． 逻辑“与”，“或”，“非”算符分别为（仅用于逻辑标量）\x{\&\&，||，\texttilde}． 当算符两边都为真时，与运算才能为真， 若至少有一边为真，或运算就为真． 非运算用于把真假互换． 例如
\begin{Command}
>> 1 > 0 \&\& 2 > 1 \\
...
\x{find} 函数可以用于寻找逻辑矩阵中所有值为 1 的矩阵元的位置． 如果只用一个输出变量（或不提供输出变量）， 函数返回单索引， 如果提供两个输出变量， 函数返回行标和列标索引． 注意输出变量仍然是列矢量， 为了节约空间以下记为行矢量． 例如
\begin{Command}
>> a = [1, 0; 0, 1]\\
a =\par
1\ \ 0\par

...
逐个元素运算也可以用于一些尺寸不同的情况， 例如对 \x{2×3} 的矩阵与 \x{2×1} 的矢量进行逐个元素运算， 相当于将后者复制成 \x{2×3} 的矩阵， 再与前者逐个元素运算。 又例如 \x{1×3} 的矢量与 \x{2×1} 的矢量进行逐个元素运算， 相当于把两个矢量都复制成 \x{2×3} 的矩阵， 再进行逐个元素运算．
\begin{Command}
...
用 \x{det} 函数可以求方阵的行列式\upref{Deter}， 用 “\x{\textbackslash}” 算符可以解线性方程组% 链接未完成
 $\mat A \vec x = \vec b$． 如\footnote{若 det(\mat A) = 0， 即方程组存在无数个解的时候， \x{\textbackslash} 算符只能得到一个特解． }
\begin{Command}
>> A = [1,2; 3,4]; det(A)\\
ans = -2\\
>> x = [-1; 1]; b = A*x\\

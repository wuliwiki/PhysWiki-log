...
\footnote{本文参考 Wikipedia \href{https://en.wikipedia.org/wiki/Gauss's_law}{相关页面}．}\textbf{电场的高斯定律（Gauss's law for electric field）}是麦克斯韦方程组\upref{MWEq}中四条方程中的一条， 是经典电磁学的基本假设之一． 用电场线来形象地描述该定理， 就是正电荷会产生电场线， 负电荷会消除电场线． 产生和消除的电场线和电荷量成正比．
...
\subsubsection{积分形式}
在空间中任意选取一个闭合曲面 $\mathcal S$，电场在这个曲面上从内向外的通量\upref{SurInt}等于被曲面包围的总电荷量除以真空中的介电常数 $\epsilon_0$．
...
\begin{figure}[ht]
\centering
\includegraphics[width=2.5cm]{./figures/EGauss_5.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/EGauss_6.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=7.5cm]{./figures/EGauss_7.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=3.5cm]{./figures/EGauss_9.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/EGauss_10.pdf}
...
现在，我们直接从\autoref{Efield_eq9}~\upref{Efield} 来证明微分式\autoref{EGauss_eq1} ．
\begin{equation}\label{EGauss_eq6}
...
\begin{equation}\label{EGauss_eq7}
...
\end{equation}
由散度的定义式
\begin{equation}\label{EGauss_eq3}
\begin{aligned}
...
\end{aligned}
\end{equation}
\autoref{EGauss_eq3} 代入\autoref{EGauss_eq4} ，得
\begin{equation}\label{EGauss_eq8}
\int\rho(\bvec r')\bvec\nabla_{\bvec r}\vdot\frac{\bvec r-\bvec r'}{\abs{\bvec r-\bvec r'}^3}\dd[3]{r'}=4\pi\rho(\bvec r)
\end{equation}
结合\autoref{EGauss_eq6} 、\autoref{EGauss_eq7} 和\autoref{EGauss_eq8} ，得
\begin{equation}
\div \bvec E=\frac{\rho}{\epsilon_0}
\end{equation}

综上，微分式\autoref{EGauss_eq1} 成立．

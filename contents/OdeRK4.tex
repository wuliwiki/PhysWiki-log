...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Runge\%E2\%80\%93Kutta_methods}{相关页面}。}\textbf{龙格库塔法（Runge-Kutta）}是一类数值解微分方程\upref{ODE}或方程组的算法， 其中较常见的是\textbf{四阶龙格库塔法}。 这里不进行推导， 仅仅给出公式如下（$y_n, t_n, h$ 的定义类比\autoref{eq_OdeNum_5}~\upref{OdeNum}）

...
\textbf{基本不等式}是基于$(a-b)^2\geq0$得到的一组不等关系。由于$(a-b)^2\geq0$恒成立，因此由此推广得到的不等式也是恒成立的。推广的过程是通过向$a,b$赋值实现的，而推广后由于不等号两边的表达式均为某种均值的形式，因此基本不等式也称为\textbf{均值不等式}，而由于存在多种均值，也有人称其为\textbf{（均值）不等式链}。
...
\subsubsection{不等式链}
这四种均值都有广泛的应用\footnote{此处只给出这四种，其实有很多种均值的定义，它们也都可纳入下面的均值不等式中。}。事实上，上面的四种平均数都可以扩展至多个数，对于一列数$a_1,a_2,\cdots,a_n$，称
\begin{equation}
\begin{array}{c} 

...
\pentry{惯性力\upref{Iner}，离心力\upref{Centri}，平面旋转矩阵，矢量的叉乘\upref{Cross}}%未完成
...
这里首先给出一个较符合直觉的结论（暂时不证）． 对任意一个随时间变化的矢量（假设一阶导数存在）， 我们把它在 $xyz$ 和 $abc$ 系中的时间导数分别记为 $(\dv*{\vec A}{t})_{xyz}$ 和 $(\dv*{\vec A}{t})_{abc}$， 则有
...
我们先令 $\vec A$ 为质点的位矢 $\vec r$， 得量参考系中质点的速度关系为
...
\end{equation}
其中第一项是已知的离心力\upref{Centri}， 我们将第二项定义为科里奥利力 $\vec F_c$． 科里奥利力可以用叉乘记为
\begin{equation}
\vec F_c = 2m \vec v_{abc} \cross \vec \omega
\end{equation}

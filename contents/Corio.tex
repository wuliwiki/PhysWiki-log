...
\end{equation}
其中 $\bvec v_{S'}$ 是质点相对于旋转参考系 $S'$ 的瞬时速度， $\bvec\omega$ 是旋转系相对于某惯性系 $S$ 转动的角\enref{速度矢量}{CMVD}。
式中的乘法是\enref{叉乘}{Cross}。
在匀速转动参考系（属于非惯性系）中，若质点保持相对静止，则惯性力只有离心力。然而当质点与转动参考系有相对速度时，惯性力中还会增加一个与速度垂直的力，这就是科里奥利力。地理中的地转偏向力就是科里奥利力，可用上式计算（见“\enref{地球表面的科里奥利力}{ErthCf}”）。
...
\addTODO{移动到 “\enref{加速度的参考系变换}{AccTra}” 中}
...
\end{equation}
最后一项参考\autoref{eq_CMVD_5}。 注意该式中的矢量为\enref{几何矢量}{GVec}而不是坐标列矢量， 若要将该式记为坐标形式， 应该使用同一\enref{坐标系}{Vtrans}。

我们先将 $\bvec A$ 替换为质点的位矢 $\bvec r$， 得参考系中质点的速度关系为（即\autoref{eq_Vtrans_1}）
...
\end{equation}
将\autoref{eq_Corio_5} 和\autoref{eq_Corio_7} 代入\autoref{eq_Corio_6}， 得（参考 “\enref{连续叉乘的化简}{TriCro}”）
\begin{equation}

...
\end{equation}
另外薛定谔方程中 $H_F(\bvec r, t) = \bvec{\mathcal E}(t) \vdot \bvec r = \mathcal E(t) z$， 进而可以用球谐函数表示（\autoref{eq_SphHar_4}~\upref{SphHar}）
\begin{equation}
...
\end{equation}
电场作用为 $H_F(\bvec r, t) = \bvec{\mathcal E}(t) \vdot \bvec r  = E_x x + E_y y + E_z z$。 其中 $x,y,z$ 可以用球谐函数表示为（\autoref{eq_SphHar_4}~\upref{SphHar}）
\begin{equation}

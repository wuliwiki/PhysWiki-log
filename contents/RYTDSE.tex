...
\begin{equation}
\Psi(\vec r, t) = \sum_{l,m} R_l (r, t) Y_{l,m} (\uvec r) = \sum_{l,m} \frac{1}{r} \psi_{l,m} (r, t) Y_{l,m} (\uvec r)
\end{equation}
我们可以将 $\psi_l (r, t) Y_{l,m} (\uvec r)$ 看做径向空间和角向空间中态矢的直积． 我们将 $l, m$ 的组合进行排序并给每个组合一个全局下标 $i$ 或 $j$．
\begin{equation}
\Psi(\vec r, t) = \sum_j R_j (r, t) Y_j (\uvec r) = \sum_j \frac{1}{r} \psi_j (r, t) Y_j (\uvec r)
\end{equation}
...
但是如果我们有一个任意方向的外场， 哈密顿算符就会新增一项 $V_f = \vec E \vdot \vec r$． 显然这个哈密顿算符不是对角矩阵
\begin{equation}
\mel{Y_i}{\vec E \vdot \vec r}{Y_j}
\end{equation}

...
V(r)=q\phi(r)~.
\end{equation}
我们继续电磁场中的\enref{狄拉克方程}{DiracE} 的讨论，在 $\gamma$ 矩阵的标准表示下，令波函数 $\psi=\pmat{\varphi\\\chi}e^{-imt}$，并令 $A_\mu(\bvec r)=(\phi(r),\bvec 0)$，那么可以由\autoref{eq_DiracE_5} 得到
\begin{equation}
\begin{aligned}
...
\end{equation}
注意这里比文章“\enref{狄拉克方程的非相对论近似}{DiracB}”中求解泡利方程的\autoref{eq_DiracB_4} 的推导多保留了一阶近似。让我们继续推导前进，将\autoref{eq_socpl_1} 代入\autoref{eq_socpl_2} 的第一行，可以得到
\begin{equation}
...
\end{equation}
可以将这两项的修正结果与文章\enref{氢原子的精细能级结构}{HfineS}中的\autoref{eq_HfineS_2}  和\autoref{eq_HfineS_3}  进行对比（注意单位制的区别）。而且在\enref{氢原子的精细能级结构}{HfineS}中采取的是一种唯象的推导，可以看出自旋轨道耦合项所对应的一个经典图像。

...
由实验可知，对银原子磁矩、或者说电子自旋的\textbf{测量}，只有两个本征值，$\pm\hbar/2$。由于$\hbar$是常用的常数，因此\enref{自然单位制}{NatUni}下我们可以设$\hbar=1$，而将电子自旋描述为$\pm 1/2$。不过为了避免混淆，我们接下来还是带着$\hbar$进行讨论。根据\autoref{the_QMPrcp_7}，这意味着刻画一个电子自旋的态空间可以是二维的。这是一个抽象的矢量空间，不可和几何的二维空间相混淆。

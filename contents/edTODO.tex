...
\item 如果用户 A 编辑以后没有关闭页面， 那么用户 B （如 admin）再去编辑， 用户 A 似乎还是会持续备份（但内容的确是 B 新修改的内容）． 建议解决办法： 同一个词条如果同时被第二个电脑打开（包括 A 使用两个浏览器的情况）， 第一个浏览器中自动变为只读模式并且停止备份．
...
\item 在加载页面显示 “建议使用 Chrome 或 Chromium 内核的浏览器”

\item 替换模式下上传图片后不需要再提示 “该图片已存在， 是否替换？”

\item 上传图片的时候允许同时上传一个其他格式的文件（例如源码文件）， 重命名为和图片文件相同．

...
\item 【待部署】重命名按钮改为 🛈 图标， 名字改为 “修改文章信息”。
...
\item 【待部署】editor 对 \verb|PhysWikiScan| 的所有调用都要检查是有 \verb|cerr| 输出， 以及 exit code 是否为零。 如果有错误应该在编辑器前端提示。
...
\item 【待部署】删除词条以前运行 \verb|./PhysWikiScan --delete 词条id|。 运行成功后再修改 \verb|main.tex| （scan 会删除词条 tex 文件）， 否则提示 \verb`cerr` 中的错误， 不删除词条。
...
\subsubsection{新备份算法}
\begin{itemize}
\item 【进行中】editor 备份步骤： 更新 \verb|contents/文件名.tex|， 然后运行 \verb|PhysWikiScan --backup 文件名|，不需要修改数据库（具体行为见 \verb|README.md|）。 注意输出中 \verb|exist| 不应该出现，因为编辑器前端应该检测到内容改变才进行备份。
\item 【进行中】编辑器出现任何内容改动（即使不手动保存）都必须做一次自动备份（不使用 scan 更新 html， 只有手动按保存才更新）。 两次备份之间不少于 20s （这个参数需要以后可以调整）。
\item 【进行中】手动保存或关闭词条时， 如果内容有改变， 也备份一次。

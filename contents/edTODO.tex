...
\item 更新数据库时， 需要尽量开启 \verb|PRAGMA foreign_keys = ON;|
...
\item 词条备份文件确保换行符使用 LF 而不是 CRLF， 保存前先计算文件 SHA1 的前 16 位， 确保 \verb|history.hash| 不存在（否则不备份）。 然后保存到 \verb|PhysWiki-backup/YYYYMMDDHHMM_authorID_entry.tex|， 其中 \verb|authorID| 是数据库的 \verb|authors.id|。 然后运行 \verb|./PhysWikiScan --backup YYYYMMDDHHMM_authorID_entry.tex| 更新数据库（scan 会更新 \verb|add, del| 等所有备份信息）。 如果输出了 \verb|cerr| 就说明运行失败（详见错误信息）。
\item  \verb|YYYYMMDDHHMM_authorID_entry.tex|， 
\item 打开词条后哪怕有一点改变都需要备份以及标记占用。 如果不产生新的备份（新的文件 hash）则不要占用。
\item 标记占用（也就是产生新的备份）后更新 \item \verb|authors.recent_entries|。 “打开文件” 按钮中的 “我的编辑” 使用这里的数据。
...
\item 【先不做】关闭词条时， 如果内容有改变， 也备份一次。
\item 【先不做】删除词条以前运行 \verb|./PhysWikiScan --delete 词条id|。 运行成功后再执行， 否则提示 \verb`cerr` 中的错误。

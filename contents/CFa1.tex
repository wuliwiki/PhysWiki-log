...
\pentry{纤维丛\nref{nod_Fibre}，哈密顿正则方程\nref{nod_HamCan}}{nod_acc9}
...
考虑一个粒子在三维空间中的运动。粒子的位置由一个三维向量函数$\bvec{r}(t)$描述，它是时间$t$的函数。定义\enref{拉格朗日函数}{Lagrng}
\begin{equation}
...
\eta^{\mu\nu}\partial_\mu\partial_\nu\phi + m^2\phi = 0
 ~.
\end{equation}
这正是Klein-Gordan方程（\enref{Klein-Gordon 方程}{KGeq}，简称为K-G方程）。
...
粒子的哈密顿函数$H(t, q_r, p_r)$是对拉格朗日函数$\mathcal{L}(t, q_r, \dot{q}_r)$作\enref{勒让德变换}{TrLgdr}，将自变量$\dot{q}_r$都替换为$p_r=\frac{\partial \mathcal{L}}{\partial \dot{q}_r}$得来的：
\begin{equation}

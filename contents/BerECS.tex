...
\end{equation}
类比\autoref{LipSch_eq2}~\upref{LipSch}， 形式上这相当于计算
\begin{equation}
...
\end{aligned}
\end{equation}
假设 $\varphi_{l_1,l_2}^{L,M}(r_1,r_2)$ 中 $r_1$ 是束缚态， 那么上面第一个求和为零． 第二个求和中， $r_1 \ll R_2$， 所以 $\varphi_{l_1,l_2}^{L,M}(r_1,R_2)$ 可以用乘积来替代． \autoref{BerECS_eq3} 中第二项的推导类似．

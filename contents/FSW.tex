...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Finite_potential_well}{相关页面} 以及\cite{GriffQ}。}本文使用\enref{原子单位制}{AU}。 一维定态薛定谔方程\autoref{eq_SchEq_1} 为
\begin{equation}
...
设 $z=k L/2$，我们可以在同一坐标系中绘制 $-\cot(z)$ 和 $\sqrt{(z_0/z)^2-1}$ 的图像（$z_0=L\sqrt{2mV_0}/2$，表征了势阱“大小”），两个图像的每一个交汇点都对应着一个奇函数解。\autoref{fig_FSW_2} 显示了 $z_0=7$ 的情况，可以看到两个图像共有 $2$ 个交点。
...
\end{equation}
设 $z=k L/2$，我们可以在同一坐标系中绘制 $\tan(z)$ 和 $\sqrt{(z_0/z)^2-1}$ 的图像（$z_0=L\sqrt{2mV_0}/2$，表征了势阱“大小”），两个图像的每一个交汇点都对应着一个偶函数解。\autoref{fig_FSW_3} 显示了 $z_0=7$ 的情况，可以看到两个图像共有 $3$ 个交点。

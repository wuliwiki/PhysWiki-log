...
\pentry{指数函数（复数）\upref{CExp}，线性微分方程的一般理论\upref{ODEb1}}{nod_13e3}

本节讨论的是齐次方程的解法，是常系数线性方程中最简单的情况。非齐次方程的几种解法请参见\textbf{拉普拉斯变换与常系数线性微分方程}\upref{ODELap}词条。

\addTODO{引用所有常系数非齐次方程解法的词条。}
...
如果从\autoref{eq_ODEb2_3} 中解出 $\lambda$\footnote{$\lambda$ 可以是复数，这就是为什么我们要先介绍复值函数的概念。}，那么 $x(t)=\E^{\lambda t}$ 就是\autoref{eq_ODEb2_1} 的一个解。根据预备知识中的\autoref{the_ODEb1_1}~\upref{ODEb1}，如果\autoref{eq_ODEb2_3} 有 $n$ 个互不相同的解 $\lambda_i$，那么 $\{\E^{\lambda_i t}\}$ 就构成了\autoref{eq_ODEb2_1} 的一组基解。这样，我们就把解\textbf{常系数齐次线性微分方程}化为解\textbf{代数方程}了。

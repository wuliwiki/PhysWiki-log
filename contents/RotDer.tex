...
证明绕轴旋转矩阵\autoref{RotA_eq3}~\upref{RotA}符合\autoref{RotDer_eq4}．
\end{exercise}
...
\end{equation}
角速度和速度之间有 $\bvec v = \bvec \omega \cross (\mat R \bvec r')$（\autoref{CMVD_eq5}~\upref{CMVD}）． 我们可以把叉乘用矩阵乘法表示为（\autoref{CrosMt_eq3}~\upref{CrosMt}）

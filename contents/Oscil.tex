% 小振动
% keys 小振动|拉格朗日方程|本征值问题|拉格朗日量|受迫振动|久期方程
% license Xiao
% type Tutor

\pentry{拉格朗日方程\nref{nod_Lagrng}，对称矩阵的本征值问题\nref{nod_SymEig}}{nod_2d64}
...
我们前面提到 $[V]_{\alpha\beta},[T]_{\alpha\beta}$ 是实对称矩阵。设 $x=\omega^2$，则该方程一共有 $s$ 个实数解（$s$ 是广义坐标的个数）。我们只考虑其中 $x>0$ 的解，设该解为 $\omega_\gamma^2=x_\gamma>0$，可求得本征方程的本征矢量 $[b]_\gamma=(b_{1\gamma},\cdots,b_{s\gamma})^T$。不同的\enref{本征矢量相互正交}{SymEig}，即 $[b_2]_{\alpha}[T]_{\alpha\beta} [b_1]_{\beta}=0$。最终小振动的解可以写为不同试探解的线性叠加：
\begin{equation}

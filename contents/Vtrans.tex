...
注意\autoref{eq_Vtrans_1} 中的矢量都是\enref{几何矢量}{GVec}， 不能将 $\bvec v_S$ 和 $\bvec v_S'$ 等同于点 $P$ 在 $S$ 系和 $S'$ 系中的三个直角坐标的求导。 如果要将\autoref{eq_Vtrans_1} 写成坐标矢量的形式， 三个矢量必须使用同一坐标系（见\autoref{ex_Vtrans_2}）。

小时百科中， 粗体正体的字母既可以用于表示几何矢量本身， 又可以表示其坐标\footnote{\enref{小时百科符号与规范}{Conven}}。 对于后者， 我们可以声明坐标所使用的坐标系
...
对于更一般的情况， 两参考系中矢量的坐标变换需要使用\enref{三维旋转矩阵}{Rot3D} 。
\end{example}

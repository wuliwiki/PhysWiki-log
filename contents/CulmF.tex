...
\subsection{定义}
\footnote{参考 \href{https://dlmf.nist.gov/33.2}{NIST 相关页面}。}在球坐标系中解库仑势场中的\enref{定态薛定谔方程}{RadSE}， 当能量 $E > 0$ 时\footnote{当能量 $E < 0$ 时将得到束缚态， 见 “\enref{类氢原子的定态波函数}{HWF}”}，会得到径向方程（\autoref{eq_CulmWf_5}）
...
\begin{figure}[ht]
\centering
\includegraphics[width=14cm]{./figures/489647cc41ba6751.pdf}
\caption{第一类库仑函数（$Z=1,k=1$）} \label{fig_CulmF_2}
\end{figure}

第一类库仑函数（\autoref{fig_CulmF_2}）的解析式为（式中两套正负号是等价的）
...
F_l(\eta, \rho) \rightarrow A_l(\eta) r^{l+1} + \order{r^{l+2}}~.
\end{equation}

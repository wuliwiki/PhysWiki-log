% QED 重整化理论—电子自能和光子自能的单圈修正
% keys 重整化|单圈修正|量子电动力学|电子自能|光子自能
% license Xiao
% type Tutor

\pentry{QED的费曼规则\nref{nod_qedfey}}{nod_839f}
...
\end{aligned}
\end{equation}
可以看到自能函数中的 $1/\epsilon$ 发散被抵消项消去。但是这里潜藏着一个问题，当我们将光子质量项 $m_\gamma^2$ 取为 $0$，积分将是发散的。这个问题也被称为\textbf{红外发散}，我们将在红外发散的文章中具体讨论这个问题。
\subsubsection{光子自能单圈修正}
\pentry{Ward-Takahashi 等式\nref{nod_ward}}{nod_776f}
\addTODO{需要引用 电荷重整化和光子自能 的文章}
根据 Ward 等式（\autoref{eq_ward_4}~\upref{ward}），即根据局域规范对称性的要求，$q_\mu\Pi^{\mu\nu}=q_\nu \Pi^{\mu\nu}=0$。因此根据 $\Pi^{\mu\nu}$ 的洛伦兹结构进行张量分解以后，它只能是以下的函数形式：
\begin{equation}
\begin{aligned}
i\Pi^{\mu\nu}=(q^2g^{\mu\nu}-q^\mu q^\nu)i\Pi(q^2)~.
\end{aligned}
...
利用 Feynman 参数化
\begin{equation}\label{eq_qedlop_2}
\begin{aligned}
I^\mu 
...
\begin{equation}\label{eq_qedlop_3}
\begin{aligned}
...
\begin{equation}\label{eq_qedlop_5}
\text{Tr}[(\not k+m)\gamma^\mu (\not q+\not k+m)\gamma^\nu]

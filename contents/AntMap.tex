...
\begin{issues}
\issueDraft
\issueOther{可以对照对称/反对称多线性映射\upref{SASmap}进行阅读}
\end{issues}

\pentry{矢量空间\upref{LSpace}，线性映射\upref{LinMap}}{nod_bdcd}
...
\end{equation}
使得 $i_{\pi1}<\cdots< i_{\pi k}$。由于任意置换都和通过依次进行两两交换\upref{permu}得到（即先交换某两个数再交换某两个数）。假如\autoref{eq_AntMap_1} 到\autoref{eq_AntMap_2} 进行这样两两交换的次数为 $n$，

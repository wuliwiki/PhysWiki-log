...
\begin{definition}{群作用}\label{Group3_def1}
...
设群$G$作用在集合$X$上。给定$x\in X$，定义$F_x=\{g\in G\mid g\cdot x=x\}$。证明 $F_x$ 构成群（\autoref{GroupP_ex2}~\upref{GroupP}）。
\end{exercise}
...
\begin{corollary}{轨道元素数与迷向子群指数}\label{Group3_cor2}
设群$G$作用在集合$X$上。任取$x\in X$，则其轨道中的元素数目，等于$x$的迷向子群的在$G$中的\textbf{指数}（\autoref{coset_def2}~\upref{coset}）。
\end{corollary}
...
\begin{theorem}{}\label{Group3_the5}
...
由拉格朗日定理（\autoref{coset_the2}~\upref{coset}），易知$G$的子群在$G$中的指数都形如$p^n$，其中各$n$是\textbf{非负整数}。又由\autoref{Group3_cor2} ，$X$中各轨道的元素数量都形如$p^n$，其中元素数量是$1$的即不动点集合$\opn{Fix}_G(X)$。

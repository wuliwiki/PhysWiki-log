...
\pentry{量子力学简介\nref{nod_QM0}， 重积分\nref{nod_IntN}， 厄米矩阵本征值问题\nref{nod_HerEig}， 狄拉克符号\nref{nod_braket}}{nod_e318}

在\enref{量子力学简介}{QM0} 中， 我们知道粒子的状态由波函数描述。 许多量子力学教材会从波函数和薛定谔方程讲起（如\enref{无限深势阱}{ISW}， \enref{简谐振子}{QSHOop}）， 而另一些教材会从矩阵的形式讲起。 前者由于涉及到求解（偏）微分方程， 数学要求会相对较高， 而后者只需涉及一些有限维的线性代数。 二者乍看起来不同， 但实际上几乎是等效的\footnote{最大的区别和难点是前者是无穷维而后者是有限维}。 为了让读者先对量子力学的理论结构由一个总体的了解， 避免迷失在数学计算中， 我们先介绍矩阵的形式。

以下将\autoref{ex_QM0_2}  展开讨论， 使用线性代数的语言， 继续完善量子力学的基本假设并应用到例子中。 该例中的角动量本征态 $\ket{x+}$， $\ket{x-}$ 和 $\ket{z+}$， $\ket{z-}$ 之间的关系是线性的。 这很容易让我们联想到\enref{矢量空间}{LSpace}的基底。 根据矢量空间的定义， 这四个本征态符合作为矢量的要求： 可以给它们乘以常数， 可以把它们相加（见\autoref{eq_QM0_2}）， 等等（参考\autoref{ex_LSpace_1}）。 正是由于这个原因， 我们把量子力学中的态（即波函数）称为\textbf{状态矢量（state vector）}， 简称\textbf{态矢}。

当明确了它们是矢量后， 我们就可以对这些态（波函数）定义\enref{内积}{InerPd}。 以（某个时刻的）一维（波）函数 $f(x)$， $g(x)$ 为例（可以用狄拉克符号记为 $\ket{f}$， $\ket{g}$）， 我们将其内积定义为（注意这里的内积是有顺序的， 星号代表对函数值取复共轭）% 未完成： 这个概念是否应该已经在傅里叶级数中讲过？
\begin{equation}
...
\addTODO{介绍归一化和正交； 厄米矩阵}


\subsection{角动量算符}
在 “\enref{量子力学简介}{QM0}” 中， 由于数学工具上的不足， 我们并没有提及某个物理量的本征态是怎么得到的， 这里来简单介绍。
...
\end{equation}
注意它们都是\enref{厄米矩阵}{HerMat}， 即满足 $\mat L_i\Her = \mat L_i$。% \addTODO{链接}
...
\addTODO{投影即可得到系数}
\addTODO{正交归一性见\autoref{the_QMPrcp_3} .}

%\subsection{回收的内容}
%
%如果我们对处于 $\ket{z+}$ 或者 $\ket{z-}$ 态的粒子测量 $z$ 方向角动量， 显然也会分别得到确定的值 $\hbar/2$ 和 $-\hbar/2$。 如果对 $\ket{z+}$ 或者 $\ket{z-}$ 测量 $x$ 方向的角动量， 就需要先将 $\ket{z+}$ 和 $\ket{z-}$ 用 $\ket{x+}$ 和 $\ket{x-}$ 表示（将\autoref{eq_QM0_2}  中的两式相加或相减即可）。

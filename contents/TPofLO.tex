...
\end{equation}
显然， $\varphi$ 是个线性算子，只需利用 $\mathcal A,\mathcal B$ 是\enref{线性算子}{LiOper}和张量积的性质（\autoref{the_TsrPrd_1}）即可。

由张量积的定义\autoref{def_vecTsr_1}，存在唯一的线性算子 $\mathcal C$，使得
\begin{equation}
...
\end{aligned}
\end{equation}
其中第一式用到了算子张量积的性质1和算子运算与矩阵运算的对应关系（\autoref{the_LiOper_1}）.

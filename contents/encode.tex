...
如果你使用 \enref{Linux 命令行}{Linux}，那么 \verb`dos2unix` 命令也常用于检测换行符和 BOM 等信息，以及是文本文件还是二进制文件。 用法是 \verb`dos2unix -ih 文件1 文件2...`， 输出结果每个文件一行，格式为 \verb`DOS UNIX MAC BOM TXTBIN FILE`，分别是 \verb`CRLF的个数 LF的个数 CR的个数 是否有BOM 文本还是二进制 文件名`。 \verb`dos2unix 文件1 文件2...` 还可以自动删除 BOM。

\subsection{判断文本文件和二进制文件}
如果一个文件中出现了任意 \verb`NUL` 字符，那么它很有可能是二进制文件。 如果出现了许多 \verb`NUL` 字符，那么几乎肯定是二进制文件。 \verb`NUL` 字符是 \aref{ascii}{sub_encode_2} 表中编号为 0 的字符，也就是说在该子节中，所有的比特都为零。 \verb`NUL` 字符通常用于 C 语言的字符串的最后代表字符串结束，但几乎不用于文本文件中（无论使用哪种编码）。

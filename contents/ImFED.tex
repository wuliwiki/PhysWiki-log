...
\end{equation}
沿着通过 $A_0,B_0$ 的两条水平直线，得到两个 $x$ 的函数：$F(x,y+\Delta'),F(x,y-\Delta')$。由\autoref{eq_ImFED_1} 已看到，第一个函数有正值，第二个有负值。按条件1：这两函数是连续的。因此必有点 $x_0$ 的某一邻域 $(x_0-\delta,x_0+\delta),\quad(0<\delta\leq\Delta)$ ，使得这两函数保持自己的符号（连续函数的保号性，需编辑文章并引用）。于是当 $x\in(x_0-\delta,x_0+\delta)$ 时，
\begin{equation}
...
\end{equation}
由连续性（布尔查诺-柯西第一定理（or连续函数介值定理），编辑文章并引用），必有 $\overline{A}\overline{B}$ 间一点 $y=\overline{y}$，使得

...
假定 $f(x)$ 在区间 $[x_0,x_0+H]\;(H>0)$ 内定义着，并在该区间内有直至 $n$ 阶导数存在并连续，此外，至少在开区间 $(x_0,x_0+H)$ 内 $n+1$ 阶导数 $f^{(n+1)}(x)$ 存在且有限．函数在区间 $[x_0-H,x_0]$ 内定义时的情形，可类似说明．(注：之所以这样假定，是为了应用柯西微分中值定理\autoref{MeanTh_eq1}~\upref{MeanTh})
...
任取一函数 $\psi(z)$，其在区间 $[x_0,x]$ 内连续，且至少在开区间 $(x_0,x)$ 内有不等于0的导数 $\psi'(z)$．应用柯西微分中值定理\autoref{MeanTh_the3}~\upref{MeanTh}
...
\autoref{SchRo_eq1} 中令 $p=n+1$，即得\textbf{拉格朗日余项公式}\autoref{Tayl_eq3}~\upref{Tayl}
\begin{equation}

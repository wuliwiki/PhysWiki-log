% 一维波动方程
% keys 波动方程|机械波|张力|偏微分方程|横波|纵波
% license Xiao
% type Tutor

\pentry{导数与差分\upref{DerDif}， 平面波\upref{PWave}， 偏导数\upref{ParDer}}{nod_ba04}
...
\begin{figure}[ht]
\centering
\includegraphics[width=5cm]{./figures/e722c8e66a971ab5.pdf}
...
\end{equation}
注意到 $h$ 很小， 由\autoref{eq_DerDif_5}~\upref{DerDif} 可得等式右边为 $T\pdv*[2]{y}{x}$。 于是我们得到波动方程为
...
由于这个方程中未知函数 $y(x,t)$ 是一个多元函数， 且出现了偏微分\upref{ParDer}， 我们把它叫做\textbf{偏微分方程}。

\subsection{纵波}

\begin{figure}[ht]
\centering
\includegraphics[width=8.3cm]{./figures/3dd68b0166eda7f2.pdf}
...
\end{equation}
同样利用\autoref{eq_DerDif_5}~\upref{DerDif} 得波动方程为
\begin{equation}
\pdv[2]{\xi}{x} - \frac{\lambda}{k}\pdv[2]{\xi}{t} = 0~.
\end{equation}

\subsection{方程的解}
\pentry{偏微分算符\upref{ParOp}}{nod_5b1b}

% 跃迁概率（一阶微扰）
% keys 一阶微扰理论|电磁波|含时微扰|跃迁|能级|长度规范|速度规范
% license Xiao
% type Tutor

\pentry{电磁波包的频谱\nref{nod_WpEng}， 含时微扰理论\nref{nod_TDPTc}， 速度规范\nref{nod_LVgaug}}{nod_60d3}

本文使用\enref{原子单位制}{AU}。一阶微扰理论就是单光子电离， 即两能级之间的能量等于单个光子的能量。

\subsection{长度规范下的微扰跃迁理论}
含时微扰理论（\autoref{eq_TDPTc_1}~\upref{TDPTc}） 为
\begin{equation}\label{eq_HionCr_1}
c_i(+\infty) = -\I \int_{-\infty}^{+\infty} \mel{i}{H'(t)}{j} \E^{\I\omega_{ij} t} \dd{t}~.
\end{equation}
$\ket{i},\ket{j}$ 可以是束缚态或者散射态， 需要\enref{正交归一化}{ScaNrm}。长度规范中， 使用偶极子近似后电磁波哈密顿为（\autoref{eq_LenGau_7}~\upref{LenGau}）
\begin{equation}
...
\end{equation}
令电场的傅里叶变换（\autoref{eq_FTExp_6}~\upref{FTExp}）为
\begin{equation}
...
\end{equation}
使用能量面密度的频率分布（\autoref{eq_WpEng_3}~\upref{WpEng}）
...
\end{equation}
对于束缚态 $\ket{i}$， $P_{ij}$ 是从 $\ket{j}$ 跃迁到 $\ket{i}$ 的概率； 而对于连续态的 $\ket{i}$ （如原子电离）， 若 $\ket{i}$ 对应出射方向的渐进动量 $\bvec k$， 那么 $P_{ij}$ 是 $\bvec k$ 空间的\enref{三维概率密度分布函数}{TDPTc}。

另外注意只有电磁波包中频率为 $\omega_{ij}$ 的平面波分量对跃迁有贡献， 所以我们也可以直接将能量差 $\omega_{ij}$ 替换为 $\omega$。

对氢原子的具体计算见\autoref{eq_HyIon2_1}~\upref{HyIon2}。

\subsection{速度规范下的微扰跃迁理论}
\footnote{参考\cite{Merzbacher} 含时微扰相关章节。}\enref{速度规范}{LVgaug}中，
\begin{equation}
...
\end{equation}
结合波包的频谱公式（\autoref{eq_WpEng_5}~\upref{WpEng} 变为原子单位）
\begin{equation}
...
注意 $\ket{i}, \ket{j}$ 是没有电磁场时的能量本征态， 波函数与规范无关。 把\autoref{eq_DipEle_3}~\upref{DipEle} 和\autoref{eq_WpEng_4}~\upref{WpEng} 带入\autoref{eq_HionCr_4} 可以证明两种规范等效（\autoref{eq_HionCr_7}  等于\autoref{eq_HionCr_4}）。 但是如果例如 $\ket{i}$ 是平面波， 则不同规范结果不同。

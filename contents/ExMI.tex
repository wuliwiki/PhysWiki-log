% 常见几何体的转动惯量
% keys 刚体|转动惯量|圆环|球壳|球体|圆柱体|长方体
% license Xiao
% type Tutor

\pentry{刚体定轴转动、转动惯量\nref{nod_RigRot}}{nod_3f78}

\begin{figure}[ht]
\centering
\includegraphics[width=12cm]{./figures/77830476010a2a68.pdf}
...
\end{equation}
也可以在\enref{极坐标}{Polar}中直接根据定义写出积分
\begin{equation}
...
\subsection{薄圆盘（共面轴）}
使用垂直轴定理\autoref{eq_MIthm_2}~\upref{MIthm} 可得， 薄圆盘（垂直轴）的转动惯量是薄圆盘（共面轴）的两倍， 所以
\begin{equation}
I = \frac{1}{4}MR^2~.
\end{equation}
...
&= 2\pi \sigma R^4 \int \sin^3 \theta_i \dd{\theta}  = 2\pi \sigma R^4 \int_0^\pi \sin^3 \theta \dd{\theta}~, % \addTODO{积分有例题吗？}
}\end{equation}
也可以在球坐标中直接写出球面积分
\begin{equation}
\ali{
...
其中对 $\theta$ 的积分使用了换元积分法。%\addTODO{链接，最好有链接到例题}
...
其中对 $\theta$ 的积分使用了换元积分法。% \addTODO{最好有链接到例题}

\subsection{薄长方体（垂直轴）}
由“薄长方体（共面轴）”可知两个共面方向的转动惯量分别为（\autoref{eq_ExMI_8} ） $MR_1^2/3$ 和 $MR_2^2/3$，使用垂直轴定理\autoref{eq_MIthm_2}~\upref{MIthm} 可得关于垂直轴的转动惯量为二者之和

...
\pentry{哈密顿正则方程\upref{HamCan}， 偏导与差分\upref{ParDf}}{nod_68ce}
...
\end{equation}
这样， 二阶导数只存在于左边的 $\ddot q_j$， 其他项都只是 $q,\dot q, t$ 的函数。 该式中的偏微最好计算出解析表达式， 但也可以通过差分来计算\upref{ParDf}（会引入更多误差）。 数值解线性方程\footnote{如果 $N$ 不大可以直接用克拉默法则\upref{kramer}， 甚至写出解析解。}， 就可以得到（令 $v_i = \dot q_i$）（\autoref{the_GO2SOD_2}~\upref{GO2SOD}）
\begin{equation}
\leftgroup{
&\dot v_i = f_i(q, v_i, t)\\
&\dot q_i = v_i
...
代码依赖 “偏导与差分\upref{ParDf}” 中的 \verb|D2_ij.m|， 以及 “导数与差分\upref{DerDif}” 的 \verb|D_i.m|。

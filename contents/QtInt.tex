...
\subsection{Ubuntu 22.04 尝试 Qt5 成功}
\begin{figure}[ht]
\centering
...
\item 第一个编译成功的例子： 使用 \verb|sudo apt install qtbase5-dev build-essential|。 如无意外会有 broken package， 把 \verb|apt| 换成 \verb|aptitude|， 然后选一个合适的解决方案。
\item 只需要创建两个文件：

...
\item 所有的变精度计算会追踪误差， 运算结果的有效数字一般比输入的有效数字要少， 但一定会精确到最后一位。 这估计和 \enref{Arb}{ArbLib} 的算法一样。
...
\item Mathematica 的矩阵是\enref{行主序}{MatSto}的，可以把三维矩阵/数组第一个指标看作列， 第二个看作行， 第三个看作指向屏幕内的方向。 元素写成一行的顺序是先增加最右边（最内层）的指标， 再增加右边第二个…… 越左边的指标是最外层的 list。
...
\begin{figure}[ht]
\centering
\includegraphics[width=12cm]{./figures/fd6a248719af1a39.png}
...
\item 一些系统定义的字符串以 \verb|$| 开头， 例如 \verb|$UserBaseDirectory| 表示用户路径， 更多详见“\enref{Mathematica 文件操作}{mmaio}”。
...
\subsection{“突然不能用”的错误修复}
如果 MMA \textbf{某天突然}卡在这个界面：
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/9e683cc1265ad3b0.png}

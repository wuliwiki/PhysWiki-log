% 自由群
% keys 字母|词|运算|群|字母的逆|自由生成群|群同态基本定理
% license Xiao
% type Tutor

\pentry{群同态\upref{Group2}}{nod_4373}
%基本完成
...
证明见\autoref{ex_GroupP_4}~\upref{GroupP}。

考虑集合 $G$ 到群 $G$ 上的映射 $f:f(g)=g, \forall g\in G$，那么由 $f$ 拓展而来的同态 $\varphi$ 就是 $F(G)$ 到 $G$ 的满射。结合\textbf{群同态基本定理}（\autoref{exe_Group2_1}~\upref{Group2}）可推知，这意味着 $G$ 是 $f(G)$ 的商群。

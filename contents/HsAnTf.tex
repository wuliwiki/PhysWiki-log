...
同角的不同三角函数间的恒等关系，包括\aref{倒数关系}{eq_HsTrFu_1}、\aref{乘积关系}{eq_HsTrFu_2}以及\aref{平方关系}{eq_HsTrFu_3}，在三角函数时就已介绍过。他们是很重要的恒等关系，但很多时候更关注的是不同角或三角函数的不同幂次之间的关系。本文予以介绍并提供证明。注意公式很多，先了解，然后可以通过做题来记忆，本文也会提供一些方法帮助记忆。当然，过多的公式是难以记忆的，所以需要注意各个公式推导的基本条件，实际使用中，选择自己熟练的来记忆就好，剩下的公式，很多时候是可以通过熟悉的推导的，使用熟练的公式，尽管速度可能没有直接使用对应的公式容易，但不会出现混淆或错误。
...
\subsubsection{证明}
这一证法是先通过旋转法求出余弦的加法公式\autoref{eq_HsAnTf_4}，然后进行简单变换得到剩下的加法公式。思路来自П. М. Котельников的学位论文\footnote{据刘培杰工作室出版的《世界著名三角学经典著作钩沉 平面三角卷I》第22节。笔者按此名字搜索，并未找到出处，特此声明。}。
...
\autoref{eq_HsAnTf_1} 和\autoref{eq_HsAnTf_2} 应相等，比较它们的最后一步即可得\autoref{eq_HsAnTf_4} 的减法形式，计算$\cos\left(\alpha-(-\beta)\right)$即可得到加法形式。将$\sin x=\cos(x-\pi/2)$代入这两个余弦加法公式，即可得到正弦加法公式\autoref{eq_HsAnTf_5}。再代入$\tan x=\sin x/\cos x$即可得正切的加法公式\autoref{eq_HsAnTf_7}。

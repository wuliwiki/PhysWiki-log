...
\pentry{行列式\nref{nod_Deter}}{nod_dadb}

\addTODO{应该并入\enref{行列式}{Deter}}

以下是行列式常见的性质， 我们从定义出发容易一一证明。 由于行列式的绝对值表示\enref{体积}{DetVol}，我们也可以从几何上理解这些定理。 这些定理对复数元素的行列式同样适用，只是复数行列式不再具有直观的几何意义。
...
矩阵\enref{转置}{Mat}（将所有 $a_{i,j}$ 与 $a_{j,i}$ 交换）后行其列式的值不变。
\end{theorem}
...
行列式的代数性质可以抽象为\textbf{外代数}（见\autoref{ex_AlgFie_1}~\upref{AlgFie} 以及例子后的解释），进而用于定义\enref{外微分}{ExtDer}，描述微分形式的代数性质。

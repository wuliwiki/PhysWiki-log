...
\pentry{三维旋转矩阵\nref{nod_Rot3D}}{nod_20f5}

\subsection{zyz 欧拉角与旋转矩阵}

为了书写方便， 把 $\sin x, \cos x$ 记为 $C_x, S_x$。 则 $z$-$y$-$z$ 欧拉角 $\psi,\theta,\phi$ 对应的\enref{旋转矩阵}{Rot3D}就是
...
\end{aligned}~
\end{equation}
其中 $\theta,\phi$ 就是\enref{球坐标系}{Sph}中的两个角。

\subsection{角速度和时间导数}
\pentry{刚体的瞬时转轴、角速度的矢量相加\nref{nod_InsAx}}{nod_1cfe}
由于\enref{角速度可以矢量叠加}{InsAx}，角速度矢量为

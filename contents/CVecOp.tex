...
\pentry{正交曲线坐标系中的重积分\upref{CrIntN}， 旋度\upref{Curl}， 拉普拉斯算符\upref{Laplac}}

\footnote{参考 \cite{GriffE} 附录．}我们以三维正交曲线坐标系\upref{CurCor}为例讨论． 该坐标系中， 位置矢量\upref{Disp} $\bvec r$ 是三个坐标 $u, v, w$ 的（矢量）函数． 那么位置矢量的全微分\upref{TDiff}为
\begin{equation}\label{CVecOp_eq5}
\dd{\bvec r} = \pdv{\bvec r}{u}\dd{u} + \pdv{\bvec r}{v}\dd{v} + \pdv{\bvec r}{w}\dd{w}
\end{equation}
根据正交曲线坐标系中单位矢量的定义（\autoref{CurCor_eq8}~\upref{CurCor}）， 上式中三个偏微分分别指向 $\uvec u, \uvec v, \uvec w$ 方向， 令他们的模长为
\begin{equation}

...
\footnote{参考 \cite{GriffE} 附录。}我们以\enref{三维正交曲线坐标系}{CurCor}为例讨论。 该坐标系中，\enref{位置矢量}{Disp} $\bvec r$ 是三个坐标 $u, v, w$ 的（矢量）函数。 那么位置矢量的\enref{全微分}{TDiff}为
...
\subsection{梯度的推导}
球坐标和柱坐标的具体推导见\autoref{sub_Grad_1}~\upref{Grad}。 由于正交曲线坐标系中任意一点处 $\uvec u, \uvec v, \uvec w$ 都互相垂直， 所以求该点处梯度的方法和直角坐标系类似： 求出三个方向的\enref{方向导数}{DerDir}， 并把他们作为梯度的三个分量。 和直角坐标系不同的是， $u$ 坐标增加 $\dd{u}$ 时， 位矢 $\bvec r$ 并不是沿 $\uvec u$ 移动 $\dd{u}$ 而是移动 $f \dd{u}$， 所以 $\uvec u$ 方向的方向导数是 $(\pdv*{s}{u})/f$。
...
\end{equation}
这是错误的， 因为在\enref{直角坐标系的推导过程}{Divgnc}中， 我们假设小长方体的相对两个面的表面积完全相等， 而在正交曲线坐标系中这不成立。 例如球坐标中的体积元 $r\dd{r} \cdot r\dd{\theta} \cdot r\sin\theta \dd{\phi}$ 中， 垂直于 $\uvec r$ 的两个面的表面积为 $r\dd{\theta} \cdot r\sin\theta \dd{\phi}$， 显然与 $r$ 有关， 即对 $r$ 求导不为零。

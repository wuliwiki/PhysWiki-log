...
\begin{issues}
\issueDraft
\issueOther{可能可以合并：\enref{结合绝热近似（量子力学）}{AdiaQM} 和\enref{一阶不含时微扰理论（量子力学）}{TIPT}更新的内容整理本文}
\end{issues}
...
\enref{绝热近似（adiabatic approximation）}{AdiaQM}告诉我们， 如果 $\Psi(0)$ 是一个非简并本征态（或好量子态） $\psi_n$， 那么该过程中 $\Psi(t)$ 将始终近似保持 $H+\lambda(t) H'$ 的本征态（好量子态） $\psi_n(\lambda)$ 乘以一个额外的相位因子 $\E^{\I\alpha_n(t)}$。 其中 $\psi_n(\lambda)$ 是实数函数且满足本征方程

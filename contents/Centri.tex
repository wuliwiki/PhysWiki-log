...
\pentry{匀速圆周运动的加速度\nref{nod_CMAD}，惯性力\nref{nod_Iner}， 连续叉乘的化简\nref{nod_TriCro}}{nod_e2e8}
...
\bvec a_{abc} = \bvec 0~.
\end{equation}
所以由\enref{惯性力}{Iner}中的结论，惯性力为
\begin{equation}\label{eq_Centri_3}
\bvec f = m(\bvec a_{abc} - \bvec a_{xyz}) = \omega^2\bvec r_\bot = m\omega ^2\pmat{a\\b\\0}_{abc}~.
\end{equation}
注意离心力向外，与直觉相符。 注意这个结论只适用于质点相对于 $abc$ 系静止的情况，若有相对运动，则惯性力除了离心力，还会有一项\enref{科里奥利力}{Corio}。

若转轴取任意方向 $\uvec\omega$， 由\autoref{eq_CMAD_9}~\upref{CMAD} 得（这里使用了\enref{连续叉乘的化简}{TriCro}）
\begin{equation}

...
\pentry{线性算子代数\upref{LiOper}，范数、赋范空间\upref{NormV}}{nod_93bb}

线性算子是矢量空间上的线性映射。如果矢量空间本身还是一个赋范空间，那么线性算子空间不仅能在加法和数乘的定义下构成矢量空间（见线性算子代数\upref{LiOper}），而且能在其上定义范数。下面我们约定 $V^* \equiv V - \{0\}$，其中 $0$ 为矢量空间 $V$ 的零矢量。
...
\begin{example}{}
试证明$B(V)$是一个赋范空间（\autoref{def_NormV_1}~\upref{NormV}），即：
\begin{enumerate}

...
\end{equation}
有以下常用性质：
$$\ln n = \sum_{d|n} \varLambda(d) ~,$$
$$\varLambda(n) = \sum_{d|n}  \mu\left(\frac nd\right) \ln(d) ~,$$
$$\varLambda(n) = - \sum_{d|n} \mu(d) \ln d ~.$$
...
\item $\vartheta(x)$ 函数
\begin{equation}
\vartheta(x) = \sum_{1 \le p \le x} \ln p = \ln\left(\prod_{1 \le p \le x} p\right) ~.
\end{equation}

\item $\psi(x)$ 函数
\begin{equation}
\psi(x) = \sum_{1 \le p^m \le x} \ln p = \sum_{1 \le n \le x} \varLambda(n)~,
\end{equation}

...
\end{equation}
如果两个矩阵 $\mat P, \mat Q$ 对易\upref{Commu}， 就有
\begin{equation}
...
\end{equation}
其中 $\mat H$ 是厄米矩阵（哈密顿矩阵）。 若把两边同除以 $\I$，则系数矩阵 $-\I\mat H$ 就是一个反厄米矩阵。 如果 $\mat H$ 不随时间变化，那么 $\E^{\I \mat H t}$ （$t\in \mathbb R$）是一个酉矩阵（幺正矩阵）\upref{UniMat}（为什么？链接未完成）。 所以 $\bvec v(t)$ 的模 $\norm{\bvec v(t)} = \sum \abs{v_i}^2$ 将不随时间变化。

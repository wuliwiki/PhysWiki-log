...
\item \verb`git log -i --all --grep="关键词"` 可以搜索所有分支所有 commit 中的关键词。
...
\item 注意 \verb`--squash` 会使用合并者作为新增内容的作者。且在分支图上面不会显示两个分支合并。 若需要显示，在 commit 以后还需要额外使用 \verb`git merge -m "合并新分支" 新分支 --no-ff`。

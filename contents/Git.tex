...
Git for Windows 的 Git bash 是不支持 symlink 的， symlink clone 下来以后都会变成文本文件（文本就是 symlink 指向的路径）， 所以还是建议在 WSL\upref{WSLnt} 中安装 Git 使用。 但就算使用 wsl， checkout 出的 symlink 也可能只是一个文本文件， 内容是目标的路径。 此时可以手动使用 \verb|ln -sf $(cat 文件) 文件| 去修复， 也可以写一个脚本（见文末）。

但新的问题是，WSL 用 git 会有讨厌的权限问题\upref{WSLnt}。 所以 git bash 和 WSL 可以结合起来用。
...
\item \verb|git log -- 文件| 可以只显示一个文件的历史， \verb|-p| 和 \verb|--word-diff-regex| 同样有效。
...
\item \verb|git grep| 可以在仓库中快速搜索关键词， \verb|grep --exclude-dir='.git'| 则往往较慢。 注意 \verb|git grep| 不能搜到没有 \verb|add| 的文件， 除非加了 \verb|--no-index|。 \verb|git grep -l| 可以仅列出文件名（有些文件一行很长）。
...
\item 要检查编码类型， 或转换编码类型，用 Visual Studio Code\upref{encode}.  不要用 Windows 的 Notepad。

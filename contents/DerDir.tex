% 方向导数
% keys 多元微积分|矢量|全微分|正交归一基|方向导数|Del 算符
% license Xiao
% type Tutor

\pentry{全微分\nref{nod_TDiff}， 正交归一基\nref{nod_OrNrB}}{nod_fce5}
先来看一幅等高线图（\autoref{fig_DerDir_1}）。 令高度 $z$ 为位置的函数$z = f(\bvec r)$。 这里 $\bvec r$ 是二维平面上的\enref{位矢}{Disp}， 即 $f(\bvec r) = f(x,y)$。 当位矢沿着等高线移动时， $z$ 不变，而当位矢沿垂直于等高线的方向移动时， $z$ 变化得最快。位置沿其他方向运动， $z$ 的变化速度介于两者之间。
...
\end{equation}
如果使用平面的\enref{正交归一基}{OrNrB} $\uvec x, \uvec y$ 写成矢量\enref{内积}{Dot} 的形式，就是
\begin{equation}

...
\textbf{离散傅里叶变换（Discrete Fourier Transform）（DFT）}是一个复数域的\enref{线性变换}{LTrans}。 对两组有序数列 $f_0, f_1, \dots, f_{N-1}$ 和 $g_0,g_2,\dots, g_{N-1}$，正变换和逆变换分别为\footnote{工程上的定义常常是正变换没有 $1/\sqrt{N}$ 因子，逆变换的 $1/\sqrt{N}$ 因子变为 $1/N$。 这样的好处是节省运算量。 我们使用的定义好处是变换为幺正变换，有保持归一化的特点。}
...
在详细分析 DFT 的性质之前， 我们先看看它与解析的傅里叶变换如何对应。 函数 $f(x)$ 和 $g(k)$ 间的\enref{傅里叶变换}{FTExp}为
...
要用 \enref{DFT 计算傅里叶级数}{FSExp} 的系数（\autoref{eq_FSExp_2}~\upref{FSExp}）， 只需要把\autoref{eq_DFT_3} 乘以 $\sqrt{2\pi}/L_x$ 即可， 所以
...
\end{equation}
下面证明， $\mat F$ 是一个\enref{酉矩阵}{UniMat}， 所以逆变换矩阵就是 $\mat F$ 的厄米共轭。
\begin{equation}
\mat F^{-1} = \mat F\Her~,
\end{equation}

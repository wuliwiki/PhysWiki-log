% 离散傅里叶变换
% DFT|FFT|快速傅里叶变换|离散傅里叶变换

\textbf{离散傅里叶变换（Discrete Fourier Transform）（DFT）}是一个复数域的线性变换\upref{LTrans}． 对两组有序数列 $f_0, f_1, \dots, f_{N-1}$ 和 $g_0,g_2,\dots, g_{N-1}$，正变换和逆变换分别为\footnote{工程上的定义常常是正变换没有 $1/\sqrt{N}$ 因子，逆变换的 $1/\sqrt{N}$ 因子变为 $1/N$． 这样的好处是节省运算量．本书中使用的定义好处是变换为幺正变换，有保持归一化的特点．}
\begin{align}
...
一个更常见的名词是\textbf{快速傅里叶变换（Fast Fourier Transform）（FFT）}， 其定义与离散傅里叶变换一样，只是优化了算法使程序运行更快\footnote{参考 Numerical Recipes 3ed}．
...
g(k) = \leftgroup{
\frac{1}{\sqrt{2\pi}} &\sum_{n = -\infty}^{\infty} f(x_n) \E^{-\I kx_n} \Delta x \qquad &(\abs{k} \leqslant L_k/2)\\
&0 & (\abs{k} > L_k/2)
}
\end{equation}
...
\end{equation}
我们分别取 $x, k$ 的区间为 $[-5, 5]$ 和 $[90, 110]$ （区间外的函数值不超过 $1.4\times 10^{-11}$）． 所以我们采用\autoref{DFT_eq21} 的傅里叶变换， 令 $x_0 = 0$， $k_0 = 100$， 则 $L_x \geqslant 10$， $L_k \geqslant 20$． 为了满足 $L_x L_k = 2\pi N$ （见\autoref{DFT_eq9}）， $N$ 取最小值 32． 所以不妨令 $L_x = 10, L_k = 2\pi N/L_x = 20.106...$．

...
\pentry{Python 简介与安装\nref{nod_Python}}{nod_3ac6}

\subsection{作为计算器}
请在 Jupyter Notebook 中尝试输入以下命令并执行（运行结果略）。 Python 程序使用 \verb`#` 注释一行，  或者用两个 \verb`"""` 或 \verb`'''` 注释多行\footnote{事实上 \lstinline|"""..."""| 或 \lstinline|'''...'''| 是一个\enref{字符串}{PyStr}而不是注释， 但单独出现的字符串并不会对程序运行造成影响， 所以可以用作注释}。 注释是为了辅助人阅读代码， 不会被执行。
...
1 + 2 + 3 + 4 + \
    5 + 6
\end{lstlisting}
各种常见的数学函数都在 \enref{math 模块}{Module}中， 需要加载。
\begin{lstlisting}[language=python]
import math
\end{lstlisting}

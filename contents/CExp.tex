...
% \addTODO{复数文章中， 要提如何求辐角， 如何把分母变为实数， 如何计算复数相除}

\pentry{指数函数， 复数\nref{nod_CplxNo}， 基本初等函数的导数\nref{nod_FunDer}}{nod_cf6c}
...
令 $z = x + \I y$ 为任意\enref{复数}{CplxNo}， 那么复数域中的指数函数的一种定义就是
...
\E^{\I z} = \cos z + \I\sin z~.
\end{equation}
将“\enref{三角函数（复数）}{CTrig}”中的\autoref{eq_CTrig_1} 和\autoref{eq_CTrig_2} 代入即可证明。
...
但乍看之下， 拓展有许多方式， 为什么一定是\autoref{eq_CExp_1} 呢？ 因为在复变函数中， 我们一般研究的是解析函数。 若要求 $\E^x$ 拓展到复数域后是解析函数\autoref{eq_CExp_1} 是唯一的定义。 详见 “\enref{柯西—黎曼条件}{CauRie}”。

\subsection{导数}
虽然我们只学了实函数的\enref{导数}{FunDer}，还没有系统地学习复变函数求导， 但我们可以根据\autoref{eq_CExp_2} 求出一个常见的导数公式。 令 $a, x$ 为实数， $a$ 为常数，
\begin{equation}\ali{
...
这只是对实数 $x$ 求导的特殊情况， 一般来说， 复变函数是可以对复数求导的， 详见 “复变函数的导数、\enref{柯西—黎曼条件}{CauRie}”。

...
根据\autoref{CExp_eq1} 的定义结合两角和公式（\autoref{TriEqv_eq1}~\upref{TriEqv}）， 容易证明（留做习题） $\E^z$ 同样满足恒等式
\begin{equation}
...
\subsection{导数}
虽然我们只学了实函数的导数\upref{Der}， 还没有系统地学习复变函数求导， 但我们可以根据\autoref{CExp_eq2} 求出一个常见的导数公式。 令 $a, x$ 为实数， $a$ 为常数，
\begin{equation}\ali{

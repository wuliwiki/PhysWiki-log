% 黑体辐射定律
% keys 黑体辐射|普朗克|电磁波|stefan-Boltzmann 定律
% license Xiao
% type Tutor
\pentry{热平衡、热力学第零定律\nref{nod_TherEq}}{nod_42d0}

在非绝对零度的温度下，任何物体都能辐射出电磁波（热辐射），同时也能吸收外来电磁波。假想一种黑体，它能 $100\%$ 地吸收所有辐射在其上的电磁波。并且为了能够达到热平衡，黑体也不断地辐射出能量。黑体的能量密度函数（关于频率 $\nu$ 的函数）以及辐射规律只和温度 $T$ 有关，这样才能保证热平衡定律（即\enref{热力学第零定律}{TherEq}）的满足。
...
\end{equation}
如果要计算波长的分布， 根据\enref{随机变量的变换}{RandCV}， 由 $\abs{S_\lambda(\lambda) \dd{\lambda}} = \abs{S_\nu(\nu)\dd{\nu}}$ 得
...
由能量密度公式可以推出黑体在单位面积上的辐射功率。由于球面\enref{立体角}{SolAng}为 $4\pi$，单位面积单位频率单位立体角的功率为
...
\end{aligned}
\end{equation}
这就是著名的 \enref{斯特藩—玻尔兹曼（Stefan-Boltzmann）定律}{SteBol}，它指出了单位面积上辐射功率与辐射场能量密度之间的关系。对频率积分就可以得到总的辐射功率和辐射场能量密度之间的关系
\begin{equation}
J=\frac{c}{4} \frac{U}{V}=\frac{c}{4} \int S_\nu(\nu,T) d\nu\propto T^4~.
\end{equation}
利用光子气体的计算结果\autoref{eq_PhoGas_2}~\upref{PhoGas}，可以最终得到
\begin{equation}
...
根据平衡态统计理论，能量为 $\epsilon=nh\nu$ 振子数目的相对值是 $e^{-nh\nu/kT}$，于是频率为 $\nu$ 的振子的平均能量为\footnote{或者我们也可以从\enref{玻色爱因斯坦分布}{MBsta}进行推导}（令 $\beta = 1/(k_BT)$）

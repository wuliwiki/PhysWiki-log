% 量子力学的基本原理（量子力学）
% keys 量子力学|态矢|算符|算符对易|动量算符|能量算符|角动量算符|算符运算|算符乘积|量子力学的基本假设|量子力学公理
% license Xiao
% type Tutor

\pentry{线性代数，微积分，经典力学，量子力学的基本原理（科普）\nref{nod_QM0}}{nod_3347}
...
% \addTODO{建议使用\enref{Stern-Gerlach实验}{SGExp}文章作为例子来理解本文列举的概念。}

量子力学中基本算符的导出，请参见\enref{量子力学中的基本算符}{OprQM}。算符间的对易关系，请参见\enref{算符对易性（量子力学）}{ComOpQ}。
...
% \item 对易子-\autoref{def_QMPrcp_13} 
% \item 相容算符-\autoref{def_QMPrcp_17} 
...
量子态的演化规律满足薛定谔方程，参见\enref{薛定谔方程（单粒子一维）}{TDSE11}、\enref{定态薛定谔方程（单粒子一维）}{SchEq}、\enref{薛定谔方程（单粒子多维）}{QMndim}、\enref{薛定谔方程 2（单粒子多维）}{TDSE}和\enref{多体薛定谔方程}{NbdQM}等文章，或在百科中搜索关键词“薛定谔方程”。
...
以一维单粒子的连续情况为例：位置表象下的波函数$\psi(x)$，可以视为取位置算符的本征矢量（在位置表象下的波函数表示为$\delta(x-x_0)$\footnote{这是\enref{狄拉克 delta 函数}{Delta}。}，在动量表象下则为$\exp(x_0p/\I\hbar)$。）线性组合而成；动量表象下的波函数$\phi(p)$，可以视为取动量算符的本征矢量（在位置表象下表为$\exp{(\I p_0 x/\hbar)}$，在动量表象下则为$\delta(p-p_0)$。）线性组合而成。

你可以理解为，$\psi(x_0)$就是线性组合中基向量$\delta(x-x_0)$的系数，即
\begin{equation}\label{eq_QMPrcp_7}
\psi(x) = \int \psi(x_0)\delta(x-x_0)\dd x_0~.
\end{equation}
这也确实符合\enref{狄拉克 delta 函数}{Delta}的性质，以及离散到连续的推广。

\begin{definition}{量子态（狄拉克符号）}\label{def_QMPrcp_4}
一个系统所处的\textbf{量子态}是\enref{复希尔伯特空间}{Hilber}上的一个矢量，用\textbf{狄拉克符号}\footnote{见\enref{狄拉克符号}{braket}或\textbf{对偶空间}\autoref{sub_DualSp_1}~\upref{DualSp}。}表示。
...
如果你了解线性代数中\enref{对偶空间}{DualSp}的概念，可能会猜想这两个希尔伯特空间之间是不是对偶关系，同一个量子态的对偶表示是不是就是对偶矢量。这种理解是\textbf{不恰当的}。对偶矢量是指互为对方的线性函数，单给一个矢量是无法确定其对偶矢量的。量子态的对偶表示则如\autoref{def_QMPrcp_4} 所说，是规定了一个同构，用这个同构来联系两个空间的矢量。在术语的使用上，应注意“对偶矢量”和“量子态的对偶表示”的区别，后者我们尽量避免使用，以尽可能减少理解负担。
...
一个左矢$\bra{a}$和一个右矢$\ket{b}$可以相乘得到一个复数，记为$\braket{a}{b}$。该运算称为“\textbf{内积（inner product）}”，定义为“满足\enref{埃尔米特矢量空间（酉空间）}{HVorUV}内积的性质\footnote{注意\enref{埃尔米特矢量空间（酉空间）}{HVorUV}中描述的是同一个空间中的两个矢量相乘，而左矢和右矢是在不同的空间中。}”的运算，即
\begin{equation}
\leftgroup{
...
\addTODO{有了讨论时间反演算符的文章或相关内容后，在此引用。}
...
Jacobi恒等式建议自行证明，也可直接参考\enref{李代数}{LieAlg}文章中李代数的定义（要求Jacobi恒等式成立）以及\autoref{the_LieAlg_2}~\upref{LieAlg}（该定理适用于对易子）。
...
% 设$X$在给定本征基下的矩阵表示为$\mathcal{M}$，其第$j$行$i$列的元素是$m^j_i$。由\autoref{the_QMPrcp_1} ，$m^j_i=\bra{s_i}$
...
设可观测量$X$有一个本征矢构成的基$\{\ket{s_\alpha}\}$。\textbf{离散}情况下，指标$\alpha$只有最多可数个\footnote{“可数”的概念见\autoref{sub_CardiN_1}~\upref{CardiN}。}可能性，此时设$\ket{a}=\sum_{\alpha}\ket{s_\alpha}$；\textbf{连续}情况下，指标$\alpha$的取值范围是一个实数区间。
...
\begin{example}{光的偏振态}\label{ex_QMPrcp_3}
考虑光的偏振实验。

...
\caption{氢原子波函数的概率密度 $\abs{\psi_{nlm}(\bvec r)}^2$ 的 $x$-$z$ 截面， 每个图中的三个数字分别是 $n, l, m$ 角量子数（来自维基百科）} \label{HWF_fig2}
\end{figure}
...
$Z = 1$ 时 $r R_{l,m}(r)$ 的函数图\footnote{我们以后会看到 $r R_{l,m}(r)$ 比 $R_{l,m}(r)$ 更常用}如\autoref{HWF_fig1}
\begin{figure}[ht]
\centering

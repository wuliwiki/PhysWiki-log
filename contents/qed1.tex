...
\begin{enumerate}
\item proper orthochronous$\quad$($\Lambda_{+}^{\uparrow}/SO^+(1,3)$)，对应 $\det\Lambda=1\quad \Lambda_{0}^{0}\geqslant1$。
\item proper non-orthochronous$\quad$($\Lambda_{+}^{\downarrow}$)，对应 $\det\Lambda=1\quad \Lambda_{0}^{0}\leqslant-1~.$
\item improper orthochronous$\quad$($\Lambda_{-}^{\uparrow}$)，对应 $\det\Lambda=-1 \quad\Lambda_{0}^{0}\geqslant1~.$
\item improper non-orthochronous$\quad$($\Lambda_{+}^{\downarrow}$)，对应 $\det\Lambda=-1 \quad\Lambda_{0}^{0}\leqslant-1~.$
\end{enumerate}

其实，$SO^+(1,3)$的性质棒呆了！首先，容易证明任意洛伦兹变换都可以写成该分支与时间反演变换 $ \Lambda_{T}$ 以及宇称变换 $\Lambda_{P}$ 的乘积。即
$$\mathcal{O}(3,1)=\left\{SO^+(1,3), \Lambda_{P} SO^+(1,3), \Lambda_{T} SO^+(1,3), \Lambda_{P} \Lambda_{T} SO^+(1,3)\right\}~.
$$

实际上，这不仅是“看起来”大大减轻了对洛伦兹变换的研究负担。在进入为何$SO^+(1,3)$为何这么特殊这一话题之前，我们先来看一些例子。
\subsection{举例}
\begin{enumerate}

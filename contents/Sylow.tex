...
\pentry{群作用\upref{Group3}}{nod_3dcb}

拉格朗日定理（\autoref{the_coset_2}~\upref{coset}）揭示了子群阶数的特点。可惜的是，其逆命题“如果$n\mid \abs{G}$则$G$总有阶数为$n$的子群”是不成立的。比如说，交错群$A_4$就没有$6$阶的子群——你可以动手验证这一点。
...
如果$p\not\mid \abs{Z(G)}$，那么根据共轭类等式（The Class Equation\autoref{the_Group3_4}~\upref{Group3}），
\begin{equation}
\abs{G} = \abs{Z(G)} + \sum_{C\in O, \abs{C}>1} \abs{C}~,
\end{equation}
其中$O$是$G$在伴随作用下全体轨道构成的集合。

由于$p\mid \abs{G}$，$p\not\mid \abs{Z(G)}$，故存在$\abs{C}$使得$p\not\mid \abs{C}$，即存在$x\in G$使得$p\not\mid \abs{C_x}$，故$\abs{C_x}\mid m$。由\autoref{cor_Group3_3}~\upref{Group3}，得$\abs{C_G(x)}=p^km/\abs{C_x}=p^ks<\abs{G}$\footnote{$C_G(x)$即$x$的中心化子，或者说$x$在伴随作用下的迷向子群。}，其中$s>1$。因此由归纳假设，$C_G(x)$有一个$p^k$阶子群$A$，从而$G$有个Sylow $p$-子群$A$。
...
\begin{theorem}{Sylow第二定理}\label{the_Sylow_1}
...
据\autoref{the_Group3_5}~\upref{Group3}，
\begin{equation}
...
由\autoref{the_Group3_5}~\upref{Group3}，注意$\abs{P}=p^k$，
...
考虑整个$G$按\textbf{共轭}作用，作用在$\opn{Syl}_p(G)$上。由\autoref{the_Sylow_1} ，这是一个可递作用，因此只有\textbf{一个}轨道，因此任意元素的轨道中元素数量是$n_p$。由\autoref{cor_Group3_2}~\upref{Group3}，$n_p$等于$\opn{Syl}_p(G)$中任意元素的迷向子群\textbf{指数}。又由\textbf{拉格朗日定理}（\autoref{the_coset_2}~\upref{coset}）可知子群指数整除群的阶，即$n_p\mid p^km$。
...
\autoref{the_Sylow_2} 中第二个性质的证明过程中也蕴含了这一关系：$n_p=[G:\opn{Syl}_p(G)]$（子群的指数，见\autoref{def_coset_2}~\upref{coset}）。


\begin{theorem}{}
取\textbf{有限群}$G$，令$P$是$G$的一个Sylow $p$-子群，$N(P)$是$P$的正规化子（\autoref{def_NormSG_2}~\upref{NormSG}）。

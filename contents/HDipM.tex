...
\pentry{氢原子的跃迁偶极子矩阵元和选择定则\upref{SelRul}}{nod_4d67}

\subsection{$z$ 方向跃迁偶极子矩阵元}
\autoref{tab_HDipM_1} 给出了核电荷数 $Z=1$ 时的 $\mel{\psi_{n',l',m'}}{z}{\psi_{n,l,m}}$， 由于这\textbf{是一个实对称矩阵}\upref{SymMat}， 只给出矩阵的下半三角。 当 $Z > 1$ 时把表中每个矩阵元除以 $Z$ 即可。 这是因为 $\psi_{n,l,m}$ 与 $Z$ 成反比进行缩放（保持归一化）， 导致 $\abs{\psi_{n,l,m}}^2$ 和 $z$ 的平均值也是如此。
\begin{table}[ht]
\centering

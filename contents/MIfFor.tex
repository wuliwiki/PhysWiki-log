...
\subsection{脚本文件}
在讲解更复杂的程序结构前，我们先来看脚本文件．\bb{脚本（script）文件} 是包含若干个指令的文件，文件后缀名为 “.m”．脚本文件可以单独执行，也在其他文件或 Command Window 中被调用（注意需要将所在文件夹添加到搜索路径）． 后者当于把被调用脚本的代码直接插入到调用指令处，调用指令就是脚本文件的文件名．脚本中的每条命令后面应该加分号以隐藏输出结果，若需要输出，用 \x{disp} 函数．
\begin{Command}
>> disp({\color{string}'good'}); a = 3; disp([{\color{string}'a = '},num2str(a)]) \\
good \\
a = 3
\end{Command}
在脚本文件中，可以在行首或命令后用百分号 \x{\%} 进行\bb{注释（comment）}\footnote{截止到 Matlab 2017b， 在英文版 Matlab IDE 中， 任何中文注释都会在 Matlab 重启后变为乱码． 若要使用中文注释， 建议使用中文操作系统和中文 Matlab．}．注释是程序的说明，使程序更易读，但在执行程序时会被忽略（\autoref{Matlab_fig1}）．
...
ii\^{}2 = 9 
\end{Command}
容易看出这段代码被执行了 3 次，\bb{循环变量} \x{ii} 按顺序取 \x{1:3} 中的一个矩阵元．注意选取 \x{ii} 作为变量名是为了与虚数单位区分，当然也可以选择其他变量名．再来看一个稍复杂的循环
\input{./codes/MIfFor4}
在循环开始前 \x{x(1)} 被赋值为 2，在循环中，第 \x{ii} 个矩阵元 依次被赋值为第 \x{ii-1} 个矩阵元的平方．运行结果为
\begin{Command}
x = 2\ \ 4\ \ 16\ \ 256\ \ 65536
\end{Command}
注意在循环前用 \x{zero} 对矩阵进行了\bb{预赋值（preallocation）}．预赋值不是必须的，但如果不进行预赋值，每次循环矩阵的尺寸都要改变，会导致程序运行变慢．另外注意循环中不允许给循环变量赋值．

再来看另一种循环叫做 \x{while} 循环． 下面来看一个例程， 输出 100 以内的斐波那契数列（$a_1 = 1, a_2 = 1, a_{n+1} = a_{n} + a_{n-1}$）．

\subsubsection{fibonacci.m}
\input{./codes/fibonacci}
...
\input{./codes/fibonacciOdd}
先来看第 4 行， \x{double} 类型的非零数在这里会自动转换为 \x{logical} 类型的 1 （\x{true}）， 只有 \x{double} 类型的 0 才会转换为 \x{logical} 类型的 0 （\x{false}）． 乍看之下， \x{while} 循环将永远执行下去（称为\bb{死循环}）， 然而第 9 行的 \x{break} 在 \x{a3 > 100} 时就会使程序跳出循环． 如果 \x{a3 <= 100} 且为偶数， 则第 10 行的 \x{elseif} 判断为真， \x{continue} 命令被执行， 程序将直接跳过之后的 \x{disp} 函数直接进入下一个循环， 所以数列的偶数项都不会被输出． 程序的运行结果为（每个数占一行）
\begin{Command}
1\ \ 1\ \ 3\ \ 5\ \ 13\ \ 21\ \ 55\ \ 89
\end{Command}

...
\end{equation}
其中 $\delta$ 理解为 \enref{$\delta$ 函数列}{Delta}， $z$ 是\enref{零函数列}{F0}， 以避免积分不收敛的问题。
...
若我们已经解得两个不正交的线性无关解 $\psi'_{k,1}, \psi'_{k,2}$， 可以使用施密特正交化（\autoref{sub_OrNrB_1}）： 先把 $\psi'_{k,1}$ 按照\autoref{eq_ScaNrm_4} 即\autoref{eq_ScaNrm_7} 归一化得 $\psi_{k,1}$， 那么

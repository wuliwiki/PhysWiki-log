...
\begin{issues}
\issueOther{需要补充习题}
\issueOther{或许和 \enref{包络和奇解}{EnvSol} 重复？这里写的做法与证明更多一些。}
\end{issues}

\pentry{包络和奇解\nref{nod_EnvSol}，一阶隐式常微分方程的存在唯一性定理\nref{nod_ODEa5}}{nod_ae23}
一般的一阶隐式常微分方程，往往可能会出现对于\enref{一阶隐式常微分方程的存在唯一性定理}{ODEa5}的判定中，条件 $(3)$ 的不满足。也就是可能出现 $F(x,y,y'), F'_y(x,y,y'), F'_{y'}(x,y,y')$ 连续，但在某处 $F(x,y,y')=0$ 的情况。在这一点处解的唯一性\textbf{可能}不成立，从而 $F(x,y,y')=0$ 可能有奇解产生。

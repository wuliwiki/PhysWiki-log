% 渐近展开
% keys 泰勒级数|收敛|发散|渐近级数
% license Xiao
% type Tutor

\pentry{泰勒级数\nref{nod_Taylor}}{nod_ad59}
...
\subsection{基本例子}
\begin{example}{泰勒展开}
\enref{泰勒展开}{Taylor}就是渐近展开的例子。 泰勒级数不必收敛， 即便收敛也不必收敛到函数本身。 在函数 $\E^{-1/x^2}$ 的例子中， 原点处的泰勒级数与函数本身的差永远是函数本身， 但显然这个差在 $x\to0$ 时衰减得比 $x$ 的任意正幂都快。
\end{example}

...
\end{cases}~,
\end{equation}
这样的两组坐标之间的变换，称为\textbf{伽利略变换}。它符合我们天生的直觉：两个参考系中同一个物体的长度仍然是一样的，时间独立于空间自由流动，所以事件发生的时间和参考系的选取无关。对伽利略变换两边求导，就能得到速度叠加原理（\enref{速度的参考系变换}{Vtrans} ）。
...
实际的问题比想象中的复杂。例如，由于相对转动的参考系无可避免的涉及（含时）基底转换问题，这就导致了科氏加速度（\enref{加速度的参考系变换}{AccTra} ）。

...
\item 如果想在本地使用 Jupyter Notebook 中的 Cling （推荐）， 用 \verb|conda|\upref{CondaN} 安装即可（不支持\footnote{最后测试时间 2023/8/13} Windows 版的 conda， 可以用 WSL\footnote{用 WSL 时，用 Windows 中的浏览器访问 Jupyter 给出的链接即可。 这是因为 WSL 和 Windows 公用网卡，并不是虚拟机的虚拟网卡。} 或者 WSL2 安装 conda。

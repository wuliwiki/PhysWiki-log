...
\item 如果想在本地使用 jupyter notebook 中的 cling， 用 \verb|conda|\upref{CondaN} 安装即可（不支持 windows： 找不到包， 可以用 WSL2， 但最好用真正的 linux）。 \verb|conda create -n cling|， \verb|conda install xeus-cling -c conda-forge|。 \verb|conda install notebook|。 运行： \verb|conda activate cling|， \verb|jupyter notebook|。

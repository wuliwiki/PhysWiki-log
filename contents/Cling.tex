...
\item 如果想在本地使用 Jupyter Notebook 中的 Cling， 用 \verb|conda|\upref{CondaN} 安装即可（不支持 windows\footnote{找不到包， 可以用 WSL2， 但最好用真正的 Linux}）。 \verb|conda create -n cling|， \verb|conda install xeus-cling -c conda-forge|。 \verb|conda install notebook|。 运行： \verb|conda activate cling|， \verb|jupyter notebook|。

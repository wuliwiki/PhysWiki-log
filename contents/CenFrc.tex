...
我们已知\enref{二体系统}{TwoBD}的运动可以等效为单个质点的中心力场问题， 所以在质点的中心力场问题的讨论中， 只需把质点质量 $m$ 和位矢 $\bvec r$ 分别替换成约化质量 $\mu$ 和相对矢量 $\bvec R$ 即可拓展到二体系统。
...
由于\autoref{eq_CenFrc_1} 中的 $F(r)$ 与位置矢量 $\bvec r$ 的方向无关， 在\enref{极坐标系}{Polar}中处理中心力场问题通常比较简单。 极坐标中质点的速度和加速度\upref{PolA} 分别为
...
\end{equation}
其中 $\uvec z$ 是垂直于极坐标平面的单位矢量（这个符号来自\enref{柱坐标系}{Cylin}）。 
...
\begin{example}{万有引力}
对万有引力， $V(r) = -GMm/r$， 等效势能的大致图像如\autoref{fig_CenFrc_1}。注意 $V'(r)$ 的形状还取决于常数 $L$， 但根据“\enref{用极值点确定函数图像}{DerImg}”，曲线总存在一个最小值， 且 $\lim\limits_{r\to\infty}V'(r) = 0$， $\lim\limits_{r\to 0} V'(r) = +\infty$。

若质点具有能量 $E_2 > 0$， 由图可得这个质点不可能一直绕力心旋转， 而是从无穷远处入射， 在距离 $r_0$ 时开始远离力心， 最终回到无穷远。 若质点具有能量 $E_1 < 0$， 由图可知 $r$ 始终在 $[r_1, r_2]$ 区间内往返变动（在“\enref{开普勒问题}{CelBd}”中，我们将会知道 $E_1$ 和 $E_2$ 分别对应椭圆轨道和双曲线轨道）。 特殊地， 当质点能量等于 $V'(r)$ 的最小值时， 它与力心的距离将保持不变， 即轨道为圆形。 若给处于圆形轨道的质点一个扰动， $r$ 将在曲线最低点附近振动， 且振动频率由最低点处曲线的二阶导数决定 % \addTODO{链接到一维势能的应用}

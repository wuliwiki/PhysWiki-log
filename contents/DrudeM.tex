...
\end{equation}
不过我们注意到该结果还是和\autoref{DrudeM_eq47} 有差异的．德鲁德模型还在室温附近将电子比热 $c_v$ 估算大了两个数量级，不过巧合的是 $v^2$ 又估算小了两个数量级，错上加错结果补偿抵消了．后续我们会在德鲁德-索默菲尔德模型（金属自由电子气体模型）\upref{DruSom}介绍一个更加精确的描述．

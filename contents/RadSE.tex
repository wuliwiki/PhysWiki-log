...
\pentry{定态薛定谔方程\nref{nod_SchEq}， 球坐标系中的亥姆霍兹方程\nref{nod_SphHHz}， 球坐标系中的角动量算符\nref{nod_SphAM}}{nod_1e8f}

本文使用\enref{原子单位制}{AU}。 我们希望在球坐标中求解定态薛定谔方程
\begin{equation}\label{eq_RadSE_2}
-\frac{1}{2m}\laplacian \psi(\bvec r) + V(\bvec r)\psi(\bvec r) = E\psi(\bvec r)~.
\end{equation}
当势能 $V(\bvec r)$ 处处为零时， 这就变成了球坐标系中的\enref{亥姆霍兹方程}{SphHHz}。以下考虑球对称势能 $V(\bvec r) = V(r)$， 即势能只与到原点的距离有关。 该方程在球坐标中的求解过程也和求解亥姆霍兹方程类似\upref{SphHHz}。

我们知道一维定态薛定谔方程的解可分为两种情况， 一种是 $E < V(\pm\infty)$ 时可能存在离散的束缚态， 另一种是 $E > V(\pm\infty)$ 时存在散射态（引用未完成）。 这在三维空间中同样成立， 当 $E < V(\infty)$ 时， 若存在束缚态， 那么波函数可以用\enref{球谐函数}{SphHar}展开为
...
若势能 $V(r)$ 在整个空间都是有限值， 对于每个较小的 $l$， 可能不存在束缚态， 也可能存在有限个束缚态， 即有限个 $n$。 束缚态能量 $E_{n,l}$ 由这两个量子数共同决定。 随着 $l$ 不断增加， 方括号中的等效势能越来越浅， 最终会导致径向方程不存在束缚态解。 一个简单的例子是\enref{有限深球势阱}{FiSph}。

一个重要的例子是氢原子： 由于 $1/r$ 势能的特殊性， 它不但有无穷个束缚态， 而且束缚态能量 $E_n$ 和 $l$ 无关， 详见 “\enref{类氢原子的定态波函数}{HWF}”。 另一些例子 “\enref{无限深球势阱}{ISphW}” 以及 “\enref{三维量子简谐振子（球坐标系）}{SHOSph}”，由于简谐振的 $V(r\to \infty)$ 为无穷大， 所以同样存在无穷个束缚态。
...
\end{equation}
一个例子见\enref{库仑函数}{CulmF}，库仑函数是类氢原子散射态的径向波函数， $V(r) = -Z/r$。

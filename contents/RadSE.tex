...
\pentry{球坐标中的拉普拉斯方程\upref{SphLap}}

\subsection{球坐标}

使用球坐标的拉普拉斯算子\autoref{SphNab_eq4} 可以将单粒子的哈密顿算符表示为
\begin{equation}
H = -\frac{\hbar^2}{2m} \laplacian + V(\vec r) =  K_r + \frac{L^2}{2mr^2} + V(\vec r)
\end{equation}
其中径向动量算符为
\begin{equation}
K_r =-\frac{\hbar^2}{2m} \laplacian_r =  - \frac{\hbar^2}{2m} \qty(\pdv[2]{r} + \frac2r \pdv{r}) = -\frac{\hbar^2}{2m}\frac{1}{r^2} \dv{r} \qty(r^2\pdv{r})
\end{equation}
角动量平方算符为%未完成： 在 wikipedia 上， \vec L 也是有极坐标的表达式的． 研究一下． https://en.wikipedia.org/wiki/Angular_momentum_operator
...
定态薛定谔方程为
\begin{equation}
\qty(K_r + \frac{L^2}{2mr^2} + V - E)\Psi(\vec r) = 0
\end{equation}

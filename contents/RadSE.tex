...
\pentry{定态薛定谔方程\upref{SchEq}， 球坐标系中的亥姆霍兹方程\upref{SphHHz}}

\subsection{球坐标}

本文使用原子单位制\upref{AU}． 我们希望在球坐标和柱坐标中求解定态薛定谔方程
\begin{equation}
...
L^2 = -r^2 \laplacian_\Omega = -\qty[ \frac{1}{\sin\theta} \pdv{\theta} \qty(\sin \theta \pdv{u}{\theta}) + \frac{1}{\sin^2 \theta} \pdv[2]{u}{\phi}]
\end{equation}
注意角动量算符不含 $r$．
...
\begin{equation}
K_r R_l =  - \frac{1}{2m} \qty(\dv[2]{R_l}{r} + \frac2r \dv{R_l}{r}) = -\frac{1}{2mr^2} \dv{r} \qty(r^2\dv{R_l}{r}) =  -\frac{1}{2mr} \dv[2]{u_l}{r}
\end{equation}
所以\autoref{RadSE_eq6} 化简为
\begin{equation}\label{RadSE_eq1}
-\frac{1}{2m} \dv[2]{u_l}{r} + \qty[V(r) + \frac{1}{2m}\frac{l(l + 1)}{r^2}]u_l = Eu_l
\end{equation}

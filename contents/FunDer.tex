...
\pentry{导数\nref{nod_Der}，小角极限（极简微积分）\nref{nod_LimArc}}{nod_6142}
...
\end{equation}
由于 $h \to 0$，  $h/x \to 0$。 令 $\varepsilon  = h/x$， 由非整数\enref{二项式定理}{BiNor}，
\begin{equation}
...
\subsection{正切函数求导}
根据\enref{求导法则}{DerRul}%\addTODO{文章}
...
\subsection{指数函数求导}
先证明 $(\E^x)' = \E^x$。 
由于上面已经证明了 $ \ln'x = 1/x$， 而 $\E^x$ 是 $\ln x$ 的反函数。 所以令 $f(x) = \ln x$， $f'(x) = 1/x$，  $f^{ - 1} (x) = \E^x$， 代入\enref{反函数的求导法则}{InvDer}%\addTODO{文章}
\begin{equation}
...
\end{equation}
再证明 $(a^x)' = a^x \ln a$。  $(a^x)' = \qty[ \qty( \E^{\ln a} )^x ]' = \qty( \E^{(\ln a) x} )'$。 把 $\E^{(\ln a) x}$ 看成是 $\E^u$ 和 $u = (\ln a) x$ 的复合函数，根据复合函数的\enref{求导法则}{DerRul} ，$(a^x)' = (\ln a) a^x$ 

\subsection{反三角函数求导}
以对 $\arctan x$ 为例，设 $x=f(y)=\tan y$，$y=f^{-1}(x)=\arctan x$。利用\enref{反函数的求导法则}{InvDer}
\begin{equation}
[f^{-1} (x)]' = \frac{1}{f'[f^{ - 1}(x)]}~.
\end{equation} 

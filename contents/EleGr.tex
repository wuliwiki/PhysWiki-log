...
\pentry{静电边值条件与唯一性定理\upref{empoi}}
...
不难看出，为了得到 $V$ 内部的 $\phi(\bvec r)$，我们需要同时代入 $\phi$ 和 $\partial \phi/\partial n$ 在边界上的取值．但一般情况下我们只知道其中之一，如果同时给定 $\phi$ 和 $\partial \phi/\partial n$ 在边界上的取值会因过约束而导致矛盾．根据静电势的唯一性定理（\autoref{empoi_the1}~\upref{empoi}），只要知道 $\phi$ 或 $\partial \phi/\partial n$ 其中之一在边界上的取值，就可以得出唯一的电势分布．下面我们分别对它们进行讨论．

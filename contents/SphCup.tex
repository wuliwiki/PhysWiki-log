% Clebsch–Gordan 系数
% keys 线性代数|角动量加法|基底|归一化|本征值|本征基底|CG系数|选择定则|对称性|正交归一性|球谐函数
% license Xiao
% type Tutor
...
\subsection{基本选择定则}
从“\enref{角动量加法}{AMAdd}” 中可以得到
\begin{equation}\label{eq_SphCup_3}
...
\subsection{对称性}
由于 \enref{3j 符号}{ThreeJ}具有更简洁的对称性， 我们可以用其推导 CG 系数的对称性\footnote{下式中如果 $L$ 是整数， 那么前面取正负号都行，但如果是半整数， 则只能取负号， 推导时要注意。}
...
\subsection{与球谐函数的关系}
三个球谐函数之积的积分可以表示成两个 CG 系数或 \enref{3j 符号}{ThreeJ}相乘\footnote{见 Bransden 附录 A4， 以及 Wikipedia 的 3j/CG coefficients 页面}

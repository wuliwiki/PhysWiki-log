...
令 $\vec E_{XUV}$ 为 XUV 的电场， 而 IR 电场用矢势来表示 $\vec E_{IR} = -\pdv*{\vec A}{t}$． 令末态为
\begin{equation}
...
\end{equation}
其中 $\vec d_{\vec v} = \mel{\vec v}{\vec r}{0}$， $\ket{\vec v}$ 为速度为 $\vec v$ 的平面波， 由于经典力学中 $\vec v - \vec A$ 是一个守恒量\footnote{我实在不明白为什么许多论文上都写 $\vec p = \vec v + \vec A$！}，% 未完成： p = v+A 中的 p 是广义动量！ 就是量子力学中的动量算符！ 到底还是我这里弄错了， 想想怎么改．
\begin{equation}

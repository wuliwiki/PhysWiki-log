...
本词条仅供内部参考， 请使用\link{在线编辑器}{http://wuli.wiki/editor}编辑百科（编辑器说明\upref{editor}）．

\subsection{文件结构}

本文主要介绍如何使用 TeXlive 2017 软件编写小时物理百科． 假设你已经安装了软件， 并且获得项目文件夹 PhysWiki． 本文默认使用 TeXlive 自带的 TeXworks 编辑器．

网站（\link{wuli.wiki}{http://wuli.wiki}）上可以下载的 pdf 是由 PhysWiki.tex 文件编译的， 为确保软件安装成功， 先打开 PhysWiki.tex， 选择 XeLaTeX 编译器， 按绿色的箭头编译． 编译完成后会出现 PhysWiki.pdf， 就是网站上提供下载的 pdf． 注意一次编译是不彻底的（公式引用等超链接会显示问号）， 需要编译两次．

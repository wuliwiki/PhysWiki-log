...
\pentry{全微分\nref{nod_TDiff}， 复合函数求导\nref{nod_ChainR}}{nod_94b7}
...
z(x,y) = f[u(x,y),v(x,y)]~.
\end{equation}
那如何求 $z(x,y)$ 的偏导数呢？我们先来看\enref{全微分关系}{TDiff}
\begin{equation}
...
\begin{example}{}
一个质点延着一条静止轨道 $y = 0$ 运动， 它的\enref{动能}{KELaw1}为 $E(v_x) = m v_x^2$。 虽然速度 $v_x$ 是时间 $t$ 的函数， 但我们说 $E(v_x)$ 不显含时间。
...
\subsubsection{全导数}
特殊地， 当所有的 $u_i$ 都只是一个变量 $x$ 的函数时， 我们可以求\enref{全导数}{TotDer} $\dv*{f}{x}$。 这时可以分为 $f$ 隐含 $x$ 和显含 $x$ 来讨论。

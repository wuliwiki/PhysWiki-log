...
该定理表明，$\dot p$ 完全确定了 $\Pi$ 到 $U$ 的双射（\autoref{AfSp_sub1}~\upref{AfSp} 中性质1）。于是 $U=\{\overrightarrow{pq}|q\in \Pi\}$ ，即方向子空间 $U$ 完全由平面 $\Pi$ 确定。
...
\end{equation}
于是， $u'=u''\in U'\cap U''$。即 $\Pi$ 是所有形如 $\dot p+u,u\in U'\cap U''$ 的点组成。而矢量子空间的交仍是个矢量子空间（\autoref{SubSpc_the1}~\upref{SubSpc}），故 $\Pi$ 是与 $U=U'\cap U''$ 相伴的平面。
...
\end{definition}
\begin{theorem}{}
平行的平面都可以通过相互平移得到。即若 $(\Pi,U),(\Pi',U)$ 是仿射空间 $(\mathbb A,V)$ 的两个平行的平面，则存在 $v\in V$，使得 $\Pi=t_{v}\Pi'$，其中 $t_v$ 是平移映射（\autoref{AfSp_eq3}~\upref{AfSp}）。
\end{theorem}
\textbf{证明：}设
\begin{equation}

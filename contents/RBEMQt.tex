...
\autoref{QuatN_eq1}~\upref{QuatN} 中已经给出了 4 条（$q$ 关于时间的导数）
\begin{equation}
\dot{q} = \frac12 \omega q~,
\end{equation}

而之前的\autoref{RBEqM_eq6}~\upref{RBEqM} 给出了另外 3 条（$\bvec\omega$ 关于时间的导数）。 注意现在我们可以用四元数 $q$ 表示 $\mat R$ （\autoref{QuatN_eq2}~\upref{QuatN}）。
\begin{equation}
\begin{aligned}
...
\end{equation}
以及欧拉方程（\autoref{RBEqM_eq9}~\upref{RBEqM} \autoref{RBEqM_eq16}~\upref{RBEqM}）。

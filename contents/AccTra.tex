% 加速度的参考系变换
% keys 加速度|科里奥利加速度|相对加速度
% license Xiao
% type Tutor

\pentry{速度的参考系变换\upref{Vtran2}}{nod_5fde}

\subsection{无相对转动}
若两个参考系之间只有平移没有转动， 某时刻两个参考系中任意两个固定点之间的加速度都是相等的（类比\autoref{eq_Vtrans_1}~\upref{Vtrans}）。 令某时刻点 $P$ 相对于 $S$ 系和 $S'$ 系的加速度分别为 $\bvec a_S$ 和 $\bvec a_{S'}$， 再令两坐标系之间的加速度为 $\bvec a_r$， 那么有
\begin{equation}\label{eq_AccTra_1}
\bvec a_S = \bvec a_{S'} + \bvec a_r~.
\end{equation}
同样地， 如果要将该式写成分量的形式， 三个矢量必须使用同一坐标系（见\autoref{ex_Vtrans_2}~\upref{Vtrans}）。

\begin{example}{}\label{ex_AccTra_1}
如\autoref{fig_AccTra_1} 一个圆盘绕着圆形轨道的外侧无摩擦滚动， 圆盘中心的相对于轨道中心的角速度为 $\omega$， 求圆盘边缘上一点在任意时刻的加速度。
\begin{figure}[ht]
\centering
\includegraphics[width=4cm]{./figures/3add897bf36ceefe.pdf}
...
\subsection{有相对转动}
类比\autoref{eq_Vtran2_2}~\upref{Vtran2}， 若两参考系之间有可能存在转动， 牵连加速度 $\bvec a_{r}$ 的定义会变得比牵连速度 $\bvec v_r$ 更微妙， 因为牵连速度与参考系的选取无关， 而牵连加速度却有关！ 我们举例解释。

\begin{example}{牵连加速度}
令 $S'$ 系原点沿着 $S$ 系的 $x$ 轴匀速运动， 且相对 $S$ 系以恒定角速度矢量（\autoref{sub_CMVD_1}~\upref{CMVD}） $\bvec \omega = 2\uvec y$ 转动。 令 $t = 0$ 时两系完全重合。 我们来讨论此刻原点处的牵连速度。
...
若我们把 $S'$ 相对于 $S$ 的运动分解为原点的平移加绕原点的转动， 那么牵连加速度 $\bvec a_r$ 也可以分解为\textbf{平移加速度}和\textbf{旋转加速度}， 而旋转加速度又可以分为\textbf{向心加速度}（\autoref{eq_CMAD_1}~\upref{CMAD}）和\textbf{角加速度项}。
...
\begin{exercise}{}\label{exe_AccTra_1}
\begin{enumerate}
\item 请使用\autoref{ex_Vtran2_1}~\upref{Vtran2} 的情景验证\autoref{eq_AccTra_2}。
...
\pentry{旋转矩阵的导数\upref{RotDer}}{nod_f430}
...
再来看\autoref{eq_AccTra_3} 最后一项， 将\autoref{eq_RotDer_4}~\upref{RotDer} 代入， 得
\begin{equation}
...
\addTODO{把\autoref{sub_Corio_1}~\upref{Corio} 的内容移过来。}

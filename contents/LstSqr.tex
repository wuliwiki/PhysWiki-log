...
\addTODO{应该先列出\enref{超定线性方程组}{OvrDet}， 系数就直接是范德蒙矩阵， 在上面开一小节讲解如何用最小二乘法得到近似解（搬运）。 参考 Wikipedia \href{https://en.wikipedia.org/wiki/Linear_least_squares}{相关页面}。}
...
\autoref{eq_LstSqr_2} 也可以用\enref{范德蒙矩阵}{VandDe} $\mat X$ 表示为
\begin{equation}
...
此外， 根据\enref{魏尔施特拉斯逼近定理}{Weiers}，闭区间上的连续函数都可以使用多项式逼近， 随着阶数增加， 多项式会\enref{一致收敛}{UniCnv}于要拟合的函数。
...
该方程存在的一个问题是， 各个 $c_i$ 偏导为零， 就一定会是全局最小值吗？ 我们不能证明任意线性拟合中系数行列式都不为零， 那么会不会解出的是局部最小值，最大值，甚至\enref{鞍点}{DerMax}呢？ 首先要明确的是方差 $S_2$ 是一个关于 $c_i$ 的二次函数， 处处可导， 所以全局最小值必定满足各阶偏导为零。 再来考虑有多个解的情况， 根据\enref{线性方程组解的结构}{LinEq}， 若线性方程组存在多于一个解， 那么这些解必定不可能是离散的而是会存在无穷个解构成一个连续的平面， 在该平面上由于各方向偏导处处为零， $S_2$ 必定处处相等， 所以所有解都是全局最优解。

...
\addTODO{$\gamma^{\mu}A_{\mu}$与狄拉克旋量对洛伦兹不变的证明。}
\addTODO{讨论$u,v$的一般解。}
...
对 $x$ 分量进行计算，有：
\begin{equation}
\left[H, \Sigma_{x}\right]=\left[\alpha_{x} p_{x}+\alpha_{y} p_{y}+\alpha_{z} p_{z}, \Sigma_{x}\right]=p_{x}\left[\alpha_{x}, \Sigma_{x}\right]+p_{y}\left[\alpha_{y}, \Sigma_{x}\right]+p_{z}\left[\alpha_{z}, \Sigma_{x}\right]~.
\end{equation}
分别代入两个算符，得：

...
\subsection{特征线}
考虑一个一阶\textbf{拟线性}方程的柯西问题，一般形式为 
\begin{equation}\left\{
\begin{aligned}
...
在点 $P_0$ 处的切向量为 
\begin{equation}
\vec \tau = \left.\left(\dv{x}{s}, \dv{t}{s}, \dv{u}{s}\right) \right|_{P_0} ~.
\end{equation}
...
\begin{equation}\left\{
\begin{aligned}
    \dv{x}{s} &= a(x, t, u), ~ x(0) = \alpha \\
    \dv{t}{s} &= b(x, t,u), ~ t(0) = 0 \\
    \dv{u}{s} &= c(x, t, u), ~ u(0) = \varphi(\alpha) ~
\end{aligned}\right. ~~
\end{equation}

其中 $\alpha$ 是曲线的参数，这组微分方程的解就被称为\textbf{特征线}。这条曲线在 $(x, t)$ 平面的投影就是一般意义的特征线，给出原微分方程的解曲面。

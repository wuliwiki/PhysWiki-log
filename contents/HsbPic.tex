...
% 根据\textbf{转移矩阵}\upref{TransM}的\autoref{TransM_sub1}~\upref{TransM}，如果固定$\ket{s}$不变，而基根据转移矩阵$\mathcal{\Omega}(t)$改变，那么$\ket{s}$的系数变化是$c^a \to \sum_{j}\omega^a_j(t)c^j$．

% 因此，薛定谔绘景和海森堡绘景的统一性要求，在能量本征态为基时有
% \begin{equation}\label{HsbPic_eq1}
% \E^{-\I at}c^a = \sum_{j}\omega^a_j(t)c^j
% \end{equation}
% 由于能量本征态随时间的演化依旧是能量本征态，因此$\mathcal{\Omega}(t)$在能量本征态下的矩阵是对角矩阵，于是\autoref{HsbPic_eq1} 简化为
% \begin{equation}
% \E^{-\I at} = \omega^a_a(t)
% \end{equation}

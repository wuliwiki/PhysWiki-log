...
\end{equation}
由于 $\rho, V$ 都是实函数， 有（\autoref{eq_SphHar_13}）
\begin{equation}
C_{l,m} = (-1)^m C_{l,-m}^*~.
\end{equation}
这其实相当于使用\enref{实球谐函数}{RYlm}作为基底。

式中 $l = 0$ 的项就是\textbf{电单极子（electric monopole）} 具有球对称的电荷分布和势能分布， $l = 1$ 的项都是\enref{电偶极子（dipole）}{eleDP2}（$\propto\cos \theta$）， $l = 2$ 的项都是\textbf{电四极子（electric quadrupole）}……  $l = N$ 的项叫做电 $2^N$ 极子。

当电荷分布关于极轴对称时， 求和中只需要 $m=0$ 的项，也就是说只有 $C_{l,0}$ 不为零，且为实数。

% \addTODO{什么样的分布满足只有某个 $l$ 不为零？例如只有电偶极子}
% \addTODO{用球谐函数电二级展开，系数之间有什么规律？一定是 z 方向的偶极子通过 wwigner D 矩阵旋转得到的吗？}
...
首先我们给出单个点电荷势能的\enref{球谐展开公式}{PChYlm}

...
\end{figure}
理论上虽然随着项数变多三角级数会收敛于方波， 但是容易发现在函数不连续点的附近， 收敛会比较慢， 而且会出现高频振荡。 这被称为\textbf{龙格现象}， 在\enref{多项式插值}{InterP}中同样会遇到。
\end{example}
...
在\enref{泰勒级数}{Taylor}中，我们通过求 $n$ 阶导数的方式来“过滤”出第 $n$ 阶系数。这里我们用积分的方式来“ 过滤” 系数。我们在推导中还要介绍一个重要的类比来讲解，即把级数中的 $\sin$ 和 $\cos$ 的性质对比\enref{几何矢量}{GVec}在正交（不归一）基底上的展开（\autoref{eq_OrNrB_1}~\upref{OrNrB}）。

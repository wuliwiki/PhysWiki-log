...
\begin{figure}[ht]
\centering
\includegraphics[width=14.25cm]{./figures/5b5c1e5a072fb776.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=14.25cm]{./figures/b7c55f5c63403c6d.pdf}
...
我们通过求 $n$ 阶导数的方式来“过滤”出第 $n$ 阶系数。这里我们用积分的方式来“ 过滤” 系数。我们在推导中还要介绍一个重要的类比来讲解，即把级数中的 $\sin$ 和 $\cos$ 的性质对比几何矢量\upref{GVec}在正交（不归一）基底上的展开（\autoref{eq_OrNrB_1}~\upref{OrNrB}）。
...
\begin{figure}[ht]
\centering
\includegraphics[width=4cm]{./figures/b88b60cde78786e4.pdf}
...
首先假想 $f(x)$ 为奇函数，原则上可以直接使用\autoref{eq_FSTri_3} 计算展开系数，但由于被积函数 $f(x)\sin(x)$ 为偶函数， 可以先把\autoref{eq_FSTri_3} 化简为%未完成：介绍定积分的时候也讲讲偶函数奇函数的化简呗
\begin{equation}

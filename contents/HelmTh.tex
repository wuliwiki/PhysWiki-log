...
\pentry{散度的逆运算\upref{DivInv}， 旋度的逆运算\upref{HlmPr2}， 调和场\upref{HarmF}}{nod_116c}
...
\autoref{eq_HelmTh_1} 和\autoref{eq_HelmTh_2} 可以分别类比静电学中的库仑定律（\autoref{eq_Efield_9}~\upref{Efield}）和静磁学中的比奥萨法尔定律（\autoref{eq_BioSav_3}~\upref{BioSav}）： 把 $\bvec F_c$ 看作静电场， $\rho(\bvec r)$ 看作电荷密度， 满足高斯定律（\autoref{eq_EGauss_1}~\upref{EGauss}）； 把 $\bvec F_c$ 看作静磁场， $\bvec j(\bvec r)$ 看作电流密度， 满足安培环路定律（\autoref{eq_AmpLaw_2}~\upref{AmpLaw}）。 也可以把 $\bvec j$ 看成磁场的负时间偏导 $-\pdv*{\bvec B}{t}$， $\bvec F_c$ 看成感生电场， 满足电磁感应定律（\autoref{eq_FaraEB_3}~\upref{FaraEB}）。

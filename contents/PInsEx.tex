...
\pentry{可分元素的单扩张是可分扩张\nref{nod_SprbE2}}{nod_32c8}
%GTM 242 V.6
...
留意\autoref{the_SprbEx_4}~\upref{SprbEx}，可以猜测可分性质与$p$次根是否在给定域中息息相关，其中$p$是这个域的特征。由此提示，我们可以定义“纯不可分元素”：
...
\begin{theorem}{}\label{the_PInsEx_3}
...
由\textbf{可分元素的封闭性}（\autoref{cor_SprbE2_4}~\upref{SprbE2}），可知$\mathbb{S}$是$\mathbb{F}$的可分闭包与$\mathbb{K}$的交集，显然是个域。

反设$\mathbb{K}-\mathbb{S}$中存在$\mathbb{S}$的可分元素$a$，那么$\mathbb{S}(a)/\mathbb{S}$是可分扩张。由可分扩张的传递性（\autoref{cor_SprbE2_1}~\upref{SprbE2}），可知$\mathbb{S}(a)/\mathbb{F}$是可分扩张，这与可分闭包的定义矛盾。因此反设不成立，即$\mathbb{K}-\mathbb{S}$中全是$\mathbb{S}$的不可分元素。
...
设$\beta_i=\alpha_i^{p^k}$，$\opn{irr}(x)(\beta_1, \mathbb{F})=g$，那么
\begin{equation}
...
设$\mathbb{S}$是$\mathbb{F}(\alpha)$上$\mathbb{F}$的可分闭包。由可分元的封闭性（\autoref{cor_SprbE2_4}~\upref{SprbE2}）可知，可分元必是可分元运算所得结果。又考虑到$\mathbb{S}$的元素都是由各$\alpha_i$组合而来，又不能包含不可分元素，于是
...
由于特征为$0$的域都是\textbf{完美域}（定义见\autoref{the_SprbEx_5}~\upref{SprbEx}），不存在不可分元素，因此纯不可分扩张的讨论集中在特征为素数$p$的域上。
...
% 由于\autoref{the_SprbEx_4}~\upref{SprbEx}，$\mathbb{F}$是完美域当且仅当任意元素$a\in\mathbb{F}$在$\mathbb{F}$中都有$p$次方根。

受此启发，我们可以得到一个纯不可分扩张的例子：

\begin{example}{}\label{ex_PInsEx_1}
...
\begin{exercise}{}
设域$\mathbb{F}$的特征为素数$p$。证明$\mathbb{F}$是完美域（\autoref{def_SprbEx_3}~\upref{SprbEx}），当且仅当$\mathbb{F}^{1/p^\infty}=\mathbb{F}$。
\end{exercise}
...
只需要考虑$\alpha\in\mathbb{K}-\mathbb{F}$的情况。记$\opn{Irr}(\alpha, \mathbb{F})=f$，则$f$不可分、不可约，故由\autoref{def_SprbEx_2}~\upref{SprbEx}和\autoref{cor_SprbEx_2}~\upref{SprbEx} 知，可以把$f$中次数不是$p$整数倍的单项式挖掉。

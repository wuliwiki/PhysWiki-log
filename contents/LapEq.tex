...
\pentry{梯度\nref{nod_Grad}， 散度\nref{nod_Divgnc}， 分离变量法解偏微分方程\nref{nod_SepVar}}{nod_b355}
...
从物理上看， 二元函数的拉普拉斯方程可以理解为一片静止的， 不受外力的\enref{薄膜}{Wv2D}所满足的方程。 要得到方程的解， 我们需要规定一些边界条件。 常见的条件是给定一个区域， 然后给出 $u(\bvec r)$ 在边界上的函数值。
...
\end{equation}
拉普拉斯方程还可化为\enref{柱坐标系中的拉普拉斯方程}{CylLap}与\enref{球坐标系中的拉普拉斯方程}{SphLap}。

\subsection{一些例子}
\begin{example}{}
%这个例子移动到 “球坐标系中的拉普拉斯方程” 条目更好吧（
在静电学问题中， 电势能满足泊松方程， 在没有电荷分布的区域， 泊松方程变为\enref{拉普拉斯方程}{EPoiEQ}。
\begin{equation}
\Delta V=0~.
\end{equation}
我们来计算 $V$ 仅依赖于 $r$（$r$ 为到原点的距离）的情况。采用球坐标，拉普拉斯方程使用\autoref{eq_SphLap_5}~\upref{SphLap} 写为
\begin{equation}
...
\end{equation}
由\autoref{eq_LapEq_3} ，$k_y=\I k_x$，代入上式，并由\autoref{eq_CTrig_5}~\upref{CTrig} 和\autoref{eq_CTrig_6}~\upref{CTrig}，得
\begin{equation}

...
接下来我们只需要证明，对于$G_{n}=G_{n-1}/\ev x_n$，若满足$G_{n}$为循环群，则$G_{n-1}$必是商群即可。这其实是从商群列逆向推导出$G$是循环群。
...
因此在$r\ev x_n$中存在唯一的$r'$使得$r'^d=x_n$，把陪集表示为$r'^i\ev r'^d,i=1,2...d$。可见$G_{n-1}$确实是循环群，证毕。
...
\end{equation}
描述的系统中， 假设系统中没有含时约束\footnote{例如\autoref{Lagrng_ex3}~\upref{Lagrng}和\autoref{Lagrng_ex4}~\upref{Lagrng}中具有含时约束， 所以虚功原理不适用．}， 即每个质点的位置 $\bvec r_i$ 不显含时， 且 $V$ 不显含 $\dot q$． 若系统不随时间变化（即每个广义坐标 $q_i$ 不随时间变化）， 那么每个广义力（包括势能产生的）都恒为零

...
\pentry{平均值\upref{QMavg}， 柯西不等式\upref{CSNeq}， 高斯波包\upref{GausPk}，量子力学的基本原理（量子力学）\upref{QMPrcp}}{nod_9416}


\subsection{相容性与不确定性关系}
\pentry{可观测量的相容性\upref{MsrCmp}}{nod_e574}
...
根据\textbf{标准差与方差}\upref{StDevi}的\autoref{eq_Uncert_4} ，可见\autoref{eq_Uncert_6} 体现的正是大量测量后所得测量值的\textbf{方差}。测量结果不确定度越大，方差也就越大，因此我们可以用方差来衡量不确定性。
...
首先，注意到一个事实：如果算符$A=-A^\dagger$（称之为反厄米算符），则其本征值是纯虚数。证明可以类比厄米算符的\autoref{the_QMPrcp_2}~\upref{QMPrcp}。

考虑Schwartz不等式以及\autoref{the_QMPrcp_2}~\upref{QMPrcp}，可推出
\begin{equation}\label{eq_Uncert_7}
\langle (\Delta X)\rangle^2\langle (\Delta Y)\rangle^2\geq \abs{\langle \Delta X \Delta Y \rangle}^2~,
\end{equation}

现在求出\autoref{eq_Uncert_7} 右边。显然有
\begin{equation}
...
%\autoref{the_QMPrcp_5}~\upref{QMPrcp}
...
\begin{example}{高斯波包}\label{ex_Uncert_1}
%（未完成）证明高斯波包可以使\autoref{eq_Uncert_2} 取等号。
已知高斯波包\autoref{eq_GausPk_1}~\upref{GausPk} 形为
\begin{equation}
...
% \subsection{不确定原理的拓展}
% 任意两个物理量 $A$ 和 $B$ 都满足
% \begin{equation}\label{eq_Uncert_5}
% \sigma_a \sigma_b \geqslant \frac{1}{2\I}\mel{v}{[A, B]}{v}
% \end{equation}
% \autoref{eq_Uncert_2} 可以看成是该式的特例： 令 $A = x, B = p$， 根据正则对易关系
% \begin{equation}
% [A, B] = [x, p] = \I\hbar
% \end{equation}
% 代入\autoref{eq_Uncert_5} 得\autoref{eq_Uncert_2}。 

% \subsection{证明}
% 令 $f = (A-a)v$， $g = (B-b)v$， 使用柯西不等式\upref{CSNeq} 有
% \begin{equation}\label{eq_Uncert_1}
...
% \end{aligned}
% \end{equation}
% 忽略\autoref{eq_Uncert_1} 中的 $\Re$ 项（大于等于 0）， 得到\autoref{eq_Uncert_5}。 容易证明， $\mel{v}{[A, B]}{v}$ 必然是一个纯虚数， 所以\autoref{eq_Uncert_5} 右边必为实数。
...
\subsection{最小不确定波包}
\autoref{ex_Uncert_1} 中我们展示了高斯波包为最小不确定波包，现在我们来回答这样的问题：什么是最一般的最小不确定波包？%即使\autoref{eq_Uncert_5} 取等号。我们将证明对于坐标-动量的最一般的最小不确定波包是一个高斯波包。

% 回望对不确定原理\autoref{eq_Uncert_5} 的证明，我们注意到在两个地方出现不等式：\autoref{eq_Uncert_1} 中 $\braket{f}{f} \braket{g}{g} \geqslant \abs{\braket{f}{g}}^2$ 和最后舍去实部 $\Re\braket{f}{g}$。假若波函数使得这两个地方都成为等式，则这样的波函数便是最小不确定波包。
% 注释原因：所引用的eq5也被注释掉了

...
本词条需要与 LaTeX 源码对照阅读， 请使用\link{在线编辑器}{http://littleshi.cn/editor}中打开．
...
梯度散度旋度拉普拉斯如 $\grad V$，$\div\bvec A$，$\curl\bvec A$，  $\laplacian V$． 单独一个粗体的 $\nabla$ 用 $\Nabla$．
...
表格中若用 dfrac， 需要在行首加上 \lstinline|\dfracH| 命令．% 未完成： 为什么？
...
\begin{definition}{}\label{Sample_def1}
 定义导数为
\begin{equation}
...
\begin{lemma}{}\label{Sample_lem1}
三角形内角和为 $\pi$
\end{lemma}

定理如\autoref{Sample_the1}．
\begin{theorem}{}\label{Sample_the1}
内错角相等
\end{theorem}

推论如\autoref{Sample_cor1}
\begin{corollary}{}\label{Sample_cor1}
1 + 2 = 3
\end{corollary}

例子如\autoref{Sample_ex1}． 
\begin{example}{名称}\label{Sample_ex1}
...
\begin{exercise}{名称}\label{Sample_exe1}

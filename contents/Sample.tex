...
引用公式和图表都统一使用 \verb|\autoref| 命令， 注意前面不加空格后面要加空格（后面是标点符号除外）， 例如\autoref{Sample_eq1}． 如果要引用其他词条中的公式，可以引用 “其他词条\upref{BiNor}” 的\autoref{BiNor_eq1} 也可以用 “\autoref{BiNor_eq1}~\upref{BiNor}”， 为了方便在纸质书上使用， 词条页码是不能忽略的．

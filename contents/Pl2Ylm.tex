% 平面波的球谐展开
% keys 复数|平面波|球谐波|归一化|球谐展开函数|傅里叶变换
% license Xiao
% type Tutor

\pentry{球谐函数\nref{nod_SphHar}， 径向函数的归一化\nref{nod_FrNorm}}{nod_5cf3}

\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Plane_wave_expansion}{相关页面}及参考文献。}复数形式的归一化平面波可以展开为\autoref{eq_SphHar_9}~\upref{SphHar} 的形式：
\begin{equation}\label{eq_Pl2Ylm_1}
...
可以证明， 一组正交归一的球面波基底为（见 “\enref{径向函数的归一化}{FrNorm}”）
\begin{equation}
\ket{s_{l,m}(k)} = s_{l,m}(k,\bvec r) = \frac{1}{r}\sqrt{\frac{2}{\pi}}kr j_l(kr) Y_{l,m}(\uvec r)~.
\end{equation}
平面波\autoref{eq_Pl2Ylm_1} 可以表示为相同能量球谐波的线性组合
\begin{equation}\label{eq_Pl2Ylm_3}
\ket{\bvec k} = \sum_{l,m}\frac{\I^l}{k} Y_{l,m}^*(\uvec k)\ket{s_{l,m}(k)} ~.
\end{equation}

\subsubsection{其他形式}
根据\autoref{eq_SphHar_10}~\upref{SphHar}， 令 $\alpha$ 为 $\uvec k, \uvec r$ 的夹角， \autoref{eq_Pl2Ylm_1} 也可以记为
\begin{equation}\label{eq_Pl2Ylm_2}
...
= \frac{1}{\pi} \sum_{l=0}^{\infty} \I^l \sqrt{l+\frac12}\ j_l(kr) Y_{l,0}(\uvec r)~.
\end{equation}

\subsection{球谐展开函数的傅里叶变换}
\pentry{多元傅里叶变换\nref{nod_NFTran}}{nod_f10e}
...
\begin{example}{类氢原子基态的动量谱}\label{ex_Pl2Ylm_1}
类氢原子基态的波函数为（见\autoref{eq_HWF_3}~\upref{HWF}， 使用原子单位）
\begin{equation}
...
\subsection{推导}
理论上， \autoref{eq_Pl2Ylm_1} 可以通过计算\autoref{eq_SphHar_7}~\upref{SphHar} 得到， 但这个积分较为复杂， 可能需要借助 Mathematica 等工具\footnote{其实 Mathematica 也只能算给定的 $l, m$。}
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
但从\autoref{eq_Pl2Ylm_2} 可以知道我们只需要证明 $\bvec k$ 延 $\uvec z$ 方向的情况， 令 $\theta = \alpha$ 即可， 也就是上式中 $k_x = 0, k_y = 0, k_z = k$。 由对称性， $m \ne 0$ 时积分都是 0， 所以
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
由\autoref{eq_Legen_3}~\upref{Legen} 得
\begin{equation}
\int Y_{l,0}^*(\uvec r)\E^{\I \bvec k \vdot \bvec r} \dd{\Omega}
...
\end{equation}
的解。 这也是\enref{定态薛定谔方程}{SchEq}势能为零的一个解。 在\enref{球坐标系中解亥姆霍兹方程}{SphHHz}得通解为
\begin{equation}
f(\bvec r) = \sum_{l,m} [A_{l,m} j_l(kr) + B_{l,m} y_l(kr)] Y_{l,m}(\uvec r)~,
\end{equation}
由于\enref{第二类球贝塞尔函数}{SphBsl} $y_l(kr)$ 在 $r\to 0$ 时有奇点， 所以 $B_{l,m} = 0$。 而第一类球贝塞尔函数 $j_l$ 在 $r\to \infty$ 时有渐进形式（\autoref{eq_SphBsl_2}~\upref{SphBsl}）
\begin{equation}

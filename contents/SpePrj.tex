...
有如下定理:
\begin{theorem}{}
如果$(z-T)^{-1}$以$z=\lambda_0$为$m$阶极点, 也就是说它的洛朗展开式的负幂项只到$A_{-m}(z-\lambda_0)^{-m}$, 那么$\lambda_0$是$T$的特征值, 特征子空间的维数为$m$. 此时空间$X$可以分解为$T$的不变子空间的直和:
$$
X=\text{Ker}(\lambda_0-T)\oplus \text{Ran}P_{\lambda_0}.
$$
\end{theorem}
...
\pentry{长度规范\upref{LenGau}}{nod_5928}

本文使用原子单位制\upref{AU}。 和长度规范中的思路一样， 我们只在使用偶极子近似时讨论\textbf{速度规范（velocity gauge）}。 用角标 $V$ 表示速度规范， 先从规范不变的哈密顿算符（\autoref{eq_LenGau_2}~\upref{LenGau}）出发
...
\end{equation}
长度规范的思路是把上式中的 $\bvec A_V^2$ 消去。 对库仑规范使用规范变换（\autoref{eq_QMEM_5}~\upref{QMEM}）
...
\end{equation}
可见\textbf{速度规范下的矢势和库仑规范的相同}， 以下统一记为 $\bvec A_C$。 这使得广义动量（\autoref{eq_QMEM_6}~\upref{QMEM}）也和库仑规范的相同， 
\begin{equation}

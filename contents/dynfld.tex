...
结合电磁感应定律与原有的静场方程，我们就得到了如雷贯耳的 \enref{Maxwell 方程组}{MWEq}：
\begin{table}[ht]
\centering
...
\end{tabular}
\end{table}
\enref{洛伦兹规范 }{LoGaug}：取 $\div \bvec A = -\mu_0 \epsilon_0 \pdv{\varphi}{t}~$。
...
由此，我们推广了势的含义，从而得到了\enref{动态问题中的势 }{EMPot}。
...
同样地，我们可以做规范变换；不过与静场问题不同，我们一般不选取 $\div \bvec A =0$ （\enref{库伦规范 }{Cgauge}），反而选取$\div \bvec A = -\mu_0 \epsilon_0 \pdv{\varphi}{t}~$（\enref{洛伦兹规范 }{LoGaug}）。尽管洛伦兹规范形式上看起来更复杂，但它使势的达朗贝尔方程高度对称、简明（见下）。

\subsubsection{达朗贝尔方程；推迟势}
将势的表达形式代回Maxwell方程组、选取洛伦兹规范，并运用一些数学技巧，就能得到\enref{达朗贝尔方程组 }{LoGaug}。达朗贝尔方程组之于势，犹如Maxwell方程组之于场。或许你已经注意到，达朗贝尔方程形式上是一个波动方程。
...
举个例子，$t=t_0$时刻的场点，能够感受到$R$距离之外场点$t=t_0$时的及时信息吗？不能，因为信息传递是需要时间的、$t=t_0$时刻场源的信息还没被发送到场点。场点能知道的，只是$\frac{R}{c}$前，即$t=t_0-\frac{R}{c}$时刻场源的信息。这就引出了\enref{推迟势 }{RetPt0} 的概念。有时定义推迟时刻 $t_r = t - \frac{R}{c}$。
...
最后，我们能不能像处理静场问题中的点电荷一样，根据这些知识处理一个运动点电荷的电磁场？答案当然是可以，但是由于晦涩难懂的推迟势问题，即使是最简单的匀速运动电荷的电场也异常繁杂，此处不再\textsl{Ctrl+V}。具体请参考 \enref{李纳-维谢尔势}{LWP} 与 \enref{带电粒子的辐射}{chgrad}。
...
具体可参考 电磁场的动量守恒、\enref{动量流密度张量}{EBP}。

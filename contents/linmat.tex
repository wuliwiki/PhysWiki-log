...
\verb|写在前面：|在本页中，我们将所有 $m$ 行 $n$ 列的实矩阵组成的集合记为 $\mathbb{R}^{m\times n}.$
...
\pentry{线性空间、线性映射、单射、满射、可逆映射、矩阵乘法、范数}
\subsection{1。矩阵的秩与线性变换的联系}

\begin{definition}{(矩阵的秩)}\label{linmat_def2}
...
(参见《范数、赋范空间》的 \autoref{NormV_def1}~\upref{NormV}). 对$\mathbb{R}^n$ 中的一个向量 $x$, 我们一般将其看成列向量， 并用 $|x|$ 来表示它的 \textbf{欧氏范数} (也称为 \textbf{模}), 即
...
矩阵按 （ \autoref{linmat_eq1} ）  定义的范数称为矩阵的标准范数。 除此之外， 矩阵 $A\in\mathbb{R}^{m\times n}$ 常用以下范数：
\[

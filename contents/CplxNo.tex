...
\pentry{集合（高中）\upref{HsSet}}{nod_abec}
...
最后我们把虚部为零的复数 $(x, 0)$ 等同于实数 $x$。 把所有复数的集合\upref{Set}记为 $\mathbb C$， 那么全体实数的集合 $\mathbb R$ 就是 $\mathbb C$ 的一个真子集\upref{Set}， 即 $\mathbb R \subset \mathbb C$。
\end{definition}
...
\subsection{复平面}
\pentry{几何矢量的运算\upref{GVecOp}， 四象限 Arctan 函数\upref{Arctan}}{nod_2998} % 未完成：Arctan 只需要第一个 node
\begin{figure}[ht]
\centering
...
\pentry{三角恒等式\upref{TriEqv}}{nod_84fd}

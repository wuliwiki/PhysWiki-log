...
\begin{equation}\label{CplxNo_eq10}
...
\end{equation}
要证明该式， 把\autoref{CplxNo_eq8} 代入\autoref{CplxNo_eq9}， 再使用\autoref{CplxNo_eq2} 即可． 要证明逆元的唯一性， 假设 $z$ 的任意两个逆元分别为 $z_1, z_2$， 有 $zz_1 = 1 = zz_2$, 两边左乘 $z^{-1}$， 使用结合律得 $z_1 = z_2$． 证毕．
...
\end{aligned}
\end{equation}
在复平面中， 该式可以表示余弦定理（\autoref{CosThe_eq2}~\upref{CosThe}）， 即计算两矢量之和的模． 上式右边第三项就是两矢量的点乘．

...
可以发现， $h_1(k, x)$ 和 $h_2(k, x)$ 分别是两个\bb{入射波（incoming wave）}， 即从所有方向入射， 一个方向出射．

我们只需把 $h_1(k, x)$ 和 $h_2(k, x)$ 取复共轭， 即可得到两个方向的\bb{出射波（outgoing wave）}， 即从一个方向入射， 各个方向出射． 相对于驻波， 可以称入射波和出射波为\bb{行波（travelling wave）}．

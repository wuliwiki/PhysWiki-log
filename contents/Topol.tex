...
给定集合$X$上的一个拓扑$\mathcal{T}$，就是$X$上所有开集的集合．也就是说，一个拓扑是$X$的幂集\footnote{幂集的概念请见集合\upref{Set}中脚注．}的集合：$\mathcal{T}\subset 2^X$．给定拓扑，相当于规定了哪些集合是开集．任何集合可以是开集，也可以是闭集；可以既开又闭，还可以既不开也不闭．

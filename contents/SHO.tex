...
满足这种形式的运动叫做\bb{简谐运动（或简谐震动）}．其中 $A$ 为\bb{振幅}， $\omega t + \varphi _0$ 为\bb{相位}， $\varphi _0$ 为\bb{初相位}（即 $t = 0$ 时刻的相位）． 但是如何决定 $A$ 和 $\varphi _0$ 呢？ 根据上面给出的条件还不能判断． 由于有两个待定常数， 我们需要两个额外条件才能解出． 常见的情况是给出初始时刻 $t = 0$ 时质点的位置 $x(0)$ 和速度 $\dot x(0)$ ， 这就叫做\bb{初值条件}．

例如给出 $x(0) = 0$，  $\dot x(0) = v_0$， 把方程的通解代入， 得 $A\cos \varphi_0 = 0$，  $ - A\omega \sin \varphi _0 = v_0$， 解得 $\varphi _0 = \pi /2$，  $A =  -v_0\omega $． 所以
\begin{equation}

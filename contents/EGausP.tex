...
\pentry{电场的高斯定律\nref{nod_EGauss}， 球坐标系中的梯度散度\nref{nod_SphNab}}{nod_b292}

以下我们用使用库仑定律和\enref{散度定理}{Divgnc} 严谨地证明\enref{电场的高斯定律}{EGauss}。
...
\end{equation}
然而根据散度定理（\autoref{eq_Divgnc_1}~\upref{Divgnc}）， 如果 $\div \bvec E$ 在球内处处为零， 应该有 $\Phi = 0$ 才对。 所以问题应该出在原点， 由于散度定理要求矢量场在闭合曲面内部处处可偏导， % \addTODO{链接}
 所以\autoref{eq_EGausP_2} 的结果并不严格适用于散度定理\footnote{但有办法可以弥补， 见 “点电荷电场的散度”}。 % \addTODO{链接}

% 度量空间中的概念
% keys 度量空间|去心领域|内点|离散点|极限点|序列|极限|开集|闭集|非开|拓扑空间
% license Xiao
% type Tutor

\pentry{度量空间\nref{nod_Metric}}{nod_9455}
...
一个点是否是集合 $A$ 的内点取决于 $X$ 的定义。 例如令 $A$ 为 $(-1, 1)$ 中的有理数， $X$ 为有理数集， 则 $A$ 中的任意点都是内点。 但如果令 $X$ 为实数集， 那么 $A$ 中的任意点都不是内点， 因为可以证明任意两个有理数之间都存在实数。% \addTODO{链接}
若令 $X = A$， 那么所有点都是内点。
\end{example}
...
\begin{definition}{}\label{def_Metri2_4}
...
\subsubsection{拓展}
即使讨论的不是度量空间（没有距离的概念）， 我们也可以通过更广义的方式定义开集， 见 “\enref{拓扑空间}{Topol}”。 度量空间是拓扑空间的一种， 可以证明上述定义的开集同样满足拓扑空间中对开集的要求， 即 “有限交任意并” 也都是开集。

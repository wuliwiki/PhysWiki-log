...
\subsection{任意势能}
如果需要加上任意势能项 $V'(\bvec r, t)$， 首先需要进行分解
\begin{equation}\label{HyTDSE_eq6}
V'(\bvec r, t) = \sum_{l,m} V'_{l,m}(r, t) Y_{l,m}(\uvec r)
\end{equation}
那么 coupling 矩阵元为（\autoref{SphHar_eq5}~\upref{SphHar}）
...
对 $m = 0$ 的对称情况， $\mel{Y_{l'',m''}}{Y_{l,m}}{Y_{l',m'}}$ 在 $l = 0$ 时是一个对角矩阵， $l = 1$ 时只有两个 1-副对角线不为零， $l = 2$ 时只有对角线和两个 2-副对角线不为零， 以此类推． 左上角的三角形也会等于零， 见\autoref{AMAdd_fig2}~\upref{AMAdd}．

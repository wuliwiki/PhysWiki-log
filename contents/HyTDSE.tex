...
\end{equation}
虽然最直观的方法是使用直角坐标， 但计算效率很低． 实际中一般使用球坐标系， 用球谐函数展开波函数（参考球坐标系中的定态薛定谔方程\upref{RadSE}）．
\begin{equation}\label{HyTDSE_eq3}
\Psi(\bvec r, t) = \frac{1}{r}\sum_{l,m} \psi_{l,m}(r) Y_{l,m}(\bvec r)
\end{equation}
其中 $\psi_{l,m}(r)$ 是\textbf{约化径向波函数（scaled radial wave function）}， 式中每一项叫做一个\textbf{分波（partial wave）}． 如果哈密顿算符是轴对称的（例如线偏振电场）， 那么只需要 $m = 0$ 的球谐函数， 即勒让德多项式\upref{Legen}．
...
\begin{equation}
H_0 \psi_{l}(r) + E(t)r\sum_{l' = 0}^{\infty} F_{l, l'} \psi_{l'}(r) + \sum_{l'=0}^\infty F'_{l,l'}(r, t)\psi_{l'}(r) = \I \pdv{\psi_{l}}{t} \quad (l=0,1,\dots)
\end{equation}

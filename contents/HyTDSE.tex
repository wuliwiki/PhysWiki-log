...
事实上，注意到 $\exp(\I q\bvec{\mathcal E}\vdot\bvec r\dd t)$ 不过是一个普通的平面波函数而不是微分算符， 所以我们只需要把它和波函数相乘： $\exp(\I q\bvec{\mathcal E}\vdot\bvec r\dd t)\Psi$。 为了使相乘后的函数仍然具有\autoref{eq_HyTDSE_3} 的形式， 可以先根据\autoref{eq_Pl2Ylm_1}  对其进行分波展开（$\exp(\I q\bvec{\mathcal E}\vdot\bvec r\dd t) = \sum_{l'',m''}R_{l'',m''}Y_{l'',m''}$）。 所以演化后的每个分波的径向波函数就是
...
\end{aligned}
\end{equation}
现在和上文一样，可以用\autoref{eq_SphHar_5}  把三个球谐函数的积分变为两个 \enref{3j 符号}{ThreeJ}的乘积，再根据选择定则，排除两个求和中等于零的项。 另外如果电场只沿 $z$ 方向， 那么式中所有 $m$ 为零。

% 量子力学的算符和本征问题
% keys 算符|本征值|测量理论|厄密算符|厄密矩阵|动量算符
% license Xiao
% type Tutor

% 未完成： 考虑将 “量子力学” 文章中与基本假设无关的内容分到另一篇文章中。

\pentry{概率密度函数\nref{nod_RandF}， 狄拉克 delta 函数\nref{nod_Delta}， 平面简谐波\nref{nod_PWave}， 量子力学与矩阵\nref{nod_QMmat}}{nod_ac84}

“\enref{量子力学}{QM0}” 中我们已经简单介绍了量子力学的基本假设。 这里我们来进行更详细的说明， 注意我们仍然只讨论做一维直线运动的单个微观粒子。 首先来做一个阶段性总结
\begin{itemize}
...
之前提到， 位置的本征函数是一些无穷窄的函数， 叫做\enref{狄拉克 $\delta$ 函数}{Delta}， 对应的位置本征值则是这些 $\delta$ 函数所在的位置。 动量的本征函数是一些平面波， 对应的动量本征值就是平面波的空间频率乘以一个常数。 然而我们并没有说明某个物理量的本征波函数是怎么得到的， 以下将进一步介绍。
...
我们姑且认为\footnote{事实上各算符的定义是完全从经典力学的对应概念导出的，具体请参考\enref{量子力学中的基本算符}{OprQM}。}， 量子力学的基本假设规定\footnote{严格来说这并不是基本假设的一部分， 但初学时这么认为并没有大碍。}一维情况下\textbf{位置的算符} $\Q x$ 是 $x$， \textbf{动量的算符} $\Q p$ 是（偏）微分算符 $-\I\hbar \pdv*{x}$。 其中 $\I$ 是虚数单位， $\hbar$ 是一个常数， 叫做\textbf{约化普朗克常数}， 即\enref{普朗克常数}{Consts} $h$ 除以 $2\pi$。

事实上， 同一个算符在不同的\textbf{表象（representation）}下具有不同的形式， 这可以类比同一个矢量用不同的基底可以得到不同的\enref{坐标}{Gvec2}， 同一个线性算符也表示为不同的矩阵。% \addTODO{链接}
这里使用的是最常见的\textbf{位置表象}， 另外有\enref{动量表象}{moTDSE}，现在先不用担心。

可以证明位于坐标 $x_0$ 处的\enref{狄拉克 $\delta$ 函数}{Delta}是 $\Q x$ 的广义上的本征函数， 且本征值为 $x_0$。 位于原点处的 $\delta$ 函数\upref{Delta}记为 $\delta(x)$， 那么 $x_0$ 处的 $\delta$ 函数就是 $\delta (x - x_0)$。 将本征函数和本征值代入本征方程， 得
\begin{equation}
...
\end{equation}
代入即可证明本征函数为\footnote{也可以通过解微分方程得到。} $\psi_p(x) = \exp(\I k x)$， 对应的动量本征值为 $p(k) = \hbar k$。 $\exp(\I k x)$ 是\enref{一维简谐波}{PWave}的复数形式， 再次提醒量子力学中习惯把简谐波或平面简谐波叫做平面波， 即使讨论的是一维问题。 量子力学中的平面波总是指 $\exp(\I k x)$ 而不是 $\sin(kx)$ 或 $\cos(kx)$。

利用波长和波数的关系（\autoref{eq_PWave_2}）
...
现在， 测量结果的概率就很自然地从离散的概率 $\abs{C_i}$ 变为\enref{概率密度函数}{RandF} $\abs{C(\lambda)}^2$ 了。 也就是说， 测量值落在某个区间 $\lambda \in [a, b]$ 的概率为
\begin{equation}
...
\end{equation}
该方程的解取决于 $V(x)$ 的形式， 我们留到 “\enref{定态薛定谔方程（单粒子一维）}{SchEq}” 详细讨论。

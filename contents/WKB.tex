...
在二者的转折点 $a$ 处， 连接函数为\enref{艾里函数}{AiryF} $\opn{Ai}$， 详见 “\enref{线性势能的定态薛定谔方程}{LinPot}”。在转折点处的连接公式为
...
\dv[2]{u}{\xi}-\xi u=0~,
\end{equation}
这是\enref{艾里函数}{AiryF}满足的微分方程\autoref{eq_AiryF_2}~\upref{AiryF}。显然，$u$ 应是 $\mathrm{Ai}(\xi)$ 和 $\mathrm{Bi}(\xi)$ 的线性组合
\begin{equation}
...
由 $\mathrm{Ai(x)}$ 函数的\enref{渐进形式}{AiryF} ，得\autoref{eq_WKB_18} 的渐进形式

...
现在我们来证明奇异值分解定理。
对于\textbf{任意复矩阵}$M$，我们可以分解为\autoref{lem_SVD_1} 的形式。那么对于\textbf{满秩}$N$矩阵，自然而然的，我们可以利用\autoref{the_SVD_1} 进行进一步的分解。设该分解表示为$N=T^{\dagger}D'F$，其中$D'$为可逆对角矩阵。

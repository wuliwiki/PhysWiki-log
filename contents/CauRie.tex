...
\pentry{复变函数\upref{Cplx}， 复合函数的偏导、链式法则（多元微积分）\upref{PChain}， 全微分\upref{TDiff}}{nod_2530}
...
对于一元实函数$y=f(x)$， $x$只能沿正负两个方向趋于零; 但对于复函数$w=f(z)=f(x+\I y)$，$z$可以沿复平面$x-y$上的任意路径趋于零，因此复变函数可导的条件更复杂。这（\autoref{eq_CauRie_4} ）会让我们联想到二元函数的方向导数\upref{DerDir}, 但由于\autoref{eq_CauRie_4} 中的除法是复数的除法， 所以它比方向导数\upref{DerDir}要更复杂一些。
...
\pentry{指数函数（复数）\upref{CExp}}{nod_5307}

% 抛物线坐标系
% keys 抛物线|坐标系|正交曲线坐标|方位角|梯度|散度|旋度
% license Xiao
% type Tutor

\pentry{抛物线\nref{nod_Para3}}{nod_34f2}

\footnote{参考 \cite{Bransden} Chap 3.5 和 Wikipedia \href{https://en.wikipedia.org/wiki/Parabolic_coordinates}{相关页面}。}抛物线的极坐标方程为（\autoref{eq_Para3_1}~\upref{Para3}）
...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/a192db3bf242bd2c.pdf}
...
\phi = \Arctan(y, x)~,
\end{equation}
其中 $\Arctan$ 见 “\enref{四象限 Arctan 函数}{Arctan}”。 可以解得
...
\subsection{正交曲线坐标系}
可以证明抛物线坐标系是一个\enref{正交曲线坐标系}{CurCor}， 即\autoref{fig_ParaCr_1} 中过任意一点的两条坐标曲线都垂直：
...
\subsection{矢量算符}
\pentry{正交曲线坐标系中的矢量算符\nref{nod_CVecOp}}{nod_1173}

结合\autoref{eq_ParaCr_4} 和\autoref{eq_CVecOp_4}~\upref{CVecOp}到\autoref{eq_CVecOp_6}~\upref{CVecOp}得
\begin{equation}
...
\end{aligned}
\end{equation}
\begin{equation} % \cite{Bransden}  eq (3.92)

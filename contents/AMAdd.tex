...
\pentry{轨道角动量， 直积空间\upref{DirPro}}
...
我们用这两个空间生成 $(2l_1+1)(2l_2+1)$ 维的直积空间， 基底为 $\ket{l_1, m_1} \ket{l_2, m_2}$ （或记为 $\ket{l_1, m_1, l_2, m2}$）． 直积空间中一组显然的 Complete Set of Commutable Operators （CSCO）为
\begin{equation}
\{L_{1z}, L_{2z}\}
\end{equation}
...
\{J^2, J_z\}
\end{equation}
即我们可以想得到直积空间的另一组基底． 令两个算符的本征值（量子数）分别为 $J$  和 $M$， 这组基底可记为 $\ket{J, M}$．  首先由对易关系， $\ket{l_1, m_1, l_2, m_2}$ 已经是 $J_z$ 的本征矢，每个本征值 $M$ 对应一个子空间（因为有简并）． 子空间的维数 $N_M$ 是 $m_1 + m_2 = M$ 的不同组合数（\autoref{AMAdd_fig1} 左）． 例如图中当 $M = l_1 + l_2 = 7/2$ 时 $N_M = 1$ （唯一的非简并情况）， $M = -1/2$ 时 $N_M = 4$．
\begin{figure}[ht]
\centering

...
\end{equation}
我们用这两个空间生成 $(2l_1+1)(2l_2+1)$ 维的张量积空间， 基底为 $\ket{l_1, m_1}\otimes\ket{l_2, m_2}$，以下记为 $\ket{l_1, m_1, l_2, m_2}$。 张量积空间中一组显然的 \enref{Complete Set of Commutable Operators （CSCO）}{Commut}为 % \addTODO{引用} （\cite{Sakurai} 1.4.36 有提到， 以及 \cite{Shankar}）
\begin{equation}
\{L_{1z}, L_{2z}\}~,
\end{equation}
...
\subsection{CG 系数}
既然每个 $M$ 子空间可以通过 $N_M$ 个正交归一的 $\ket{l_1, m_1, l_2, m_2}$ 基底或者 $\ket{L,M}$ 基底展开， 那么就有必要讨论它们之间的酉变换矩阵 $\mat U_M$。 其矩阵元 $\braket{l_1, m_1, l_2, m_2}{L, M}$ 被称为 \textbf{Clebsch–Gordan 系数}或者简称 \textbf{CG 系数}。 详见 “\enref{Clebsch–Gordan 系数}{SphCup}”。

...
本词条是专门用来证明下述\autoref{the_SprbE2_1} 的，其自然语言的描述即是本词条的标题。
...
% 据\autoref{cor_PrmtEl_2}~\upref{PrmtEl}，存在$\gamma\in\mathbb{F}(a_1, a_2, \cdots, a_n)\subseteq\mathbb{K}$，使得$\mathbb{F}(a_1, a_2, \cdots, a_n)=\mathbb{F}(\gamma)$。%孟道骥的做法，在这里显得累赘了，略去。

$\mathbb{L}$的保$\mathbb{F}$自同构数量，等于$\mathbb{L}$的保$\mathbb{K}$自同构数量\textbf{乘以}$\mathbb{K}$的保$\mathbb{F}$自同构数量。

于是据\autoref{cor_SprbE2_2} ，可知$\mathbb{L}$的保$\mathbb{F}$自同构数量等于$[\mathbb{L}:\mathbb{F}]$。这违反了\autoref{the_SprbE2_1} 第$2$条，因此$\mathbb{L}/\mathbb{F}$是可分扩张。

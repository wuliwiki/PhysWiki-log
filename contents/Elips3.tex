...
这启发人们进一步思考：$|O_1P|=|O_2P|=r$能不能构造成其他的表达方式，前面的构造是打开了第二个等号，如果打开第一个等号，那么会发生什么？也就是说，将 $|O_1P| = r, |O_2P| = r$ 作为一种初始设定，而不要求它们始终相等。一种比较自然的推广方式是考虑两者的和保持为定值，即 $|O_1P| + |O_2P| = 2r$，这意味着这两个距离始终满足一个固定的和 $m$\footnote{当然，也可以要求两者的乘积是一个定值 $m$，即 $|O_1P||O_2P| = m$，此时得到的图形称为\textbf{卡西尼曲线（Cassini curve）}。}。下面试着推导一下他的表达式。
...
另外，根据\aref{点斜式}{def_HsGeBa_1}，可以整理得到椭圆的切线方程为：
\begin{equation}
\frac{xx_0}{a^2} + \frac{yy_0}{b^2} = 1~.
\end{equation}
这个形式很有规律，就像是在椭圆的方程中，把一个 $x$ 和一个 $y$ 分别替换成了椭圆上某个固定点的坐标 $(x_0, y_0)$，就能直接写出该点处的切线方程。

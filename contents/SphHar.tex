...
\end{equation}
其中 $\uvec r$ 表示一个单位矢量\upref{GVec}， 指向球坐标中的点 $(1, \theta, \phi)$。 $l, m$ 为整数， $l \geqslant 0$， $-l \leqslant m \leqslant l$。 $A_{l,m}$ 是归一化系数， 使得 $\abs{Y_{l, m} (\uvec r)}^2$ 在单位球面上的面积分等于 1 （\autoref{SphHar_eq12}）。
...
\caption{$\abs{Y_{l,m}(\uvec r)}$ 在 $x$-$z$ 平面的极坐标曲线\upref{PolCrd}（$\phi$ 不会影响球谐函数的绝对值）。 红色代表 $Y_{l,m}(\uvec r) > 0$， 蓝色代表 $Y_{l,m}(\uvec r) < 0$。 第 1 行到第 4 行分别为 $l = 0$ 到 $3$， 每行从左到右分别为 $m = -l$ 到 $l$。 图中的右上角标明了 $r$ 的单位长度。} \label{SphHar_fig1}
\end{figure}
...
\end{equation}
另外根据\autoref{SphHar_eq1} 和\autoref{AsLgdr_eq2}~\upref{AsLgdr}可得
\begin{equation}
...
\end{equation}
是球坐标系拉普拉斯算子中与 $r$ 无关的部分（\autoref{SphNab_eq3}~\upref{SphNab}）。

\subsection{归一化系数}
由球谐函数的归一化条件， 对单位球面积分（\autoref{CrIntN_ex1}~\upref{CrIntN}）得
\begin{equation}\label{SphHar_eq12}\ali{
...
与连带勒让德多项式相同， 在定义球谐函数时我们也可以选择是否包含 \textbf{Condon–Shortley 相位} $(-1)^m$。 物理中一般选择包含， 包括本书。 如果包含， 我们可以选择将其包含在连带勒让德多项式中（如\autoref{AsLgdr_eq1}~\upref{AsLgdr}）， 或者包含在球谐函数的定义中。 如果不包含， 该相位在两个定义中都不出现。

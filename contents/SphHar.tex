...
\end{equation}
其中 $\uvec r$ 表示一个单位矢量\upref{GVec}， 指向球坐标中的点 $(1, \theta, \phi)$， $l, m$ 为整数， $l \geqslant 0$， $-l \leqslant m \leqslant -l$． $A_{l,m}$ 是归一化系数， 使得 $\abs{Y_{l, m} (\uvec r)}^2$ 在单位球面上的面积分等于 1．
...
\caption{$r(\theta) = \abs{Y_{l,m}(\theta, 0)}$ 的极坐标曲线\upref{PolCrd}（$\phi$ 并不会改变球谐函数的绝对值）． 注意 $Y_{l,m}(\theta, 0)$ 是实数． 红色代表 $Y_{l,m}(\theta, 0) > 0$， 蓝色代表 $Y_{l,m}(\theta, 0) < 0$． 第 1 行到第 4 行分别为 $l = 0$ 到 $3$， 每行从左到右分别为 $m = -l$ 到 $l$． 图中的右上角标明了 $r$ 的单位长度．} \label{SphHar_fig1}
\end{figure}
...
\subsection{归一化系数}
由球谐函数的归一化条件，
\addTODO{链接：哪里讲了立体角是单位球面上的面积元， 以及极坐标中的表达式？}
\begin{equation}\ali{

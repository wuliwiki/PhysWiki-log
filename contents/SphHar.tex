...
\caption{球谐函数 $Y_{l,m}(\theta, 0)$ 的极坐标曲线， 即 $r(\theta) = \abs{Y_{l,m}(\theta, 0)}$． 红色代表 $Y_{l,m}(\theta, 0) > 0$， 蓝色代表 $Y_{l,m}(\theta, 0) < 0$． 第 1 行到第 4 行分别为 $l = 0, \dots, 3$， 每行从做到右分别为 $m = -l, \dots, 0, \dots, l$． 这里的球谐函数使用了 Condon–Shortley 相位（见下文）．} \label{SphHar_fig1}
\end{figure}

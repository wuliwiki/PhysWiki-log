...
现在我们来定义一个矢量场在一个曲面上的\textbf{曲面积分}\footnote{简称\textbf{面积分}， 这时需要通过语境与 “\enref{二重积分}{IntN}” 区分开。}（或\textbf{通量}）。 假设矢量场与曲面都处处光滑， 令矢量场为 $\bvec F(\bvec r)$， 给曲面定义一个正方向， 并将曲面划分为许多面积为 $\Delta S_i$ 的小面元， 令其法向量为 $\uvec n_i$（与曲面正方向同侧）， 则一块面元可以表示为 $\Delta \bvec S_i = \Delta S_i \uvec n_i$。 当面元很小时可以假设其内部的矢量场为常矢量 $\bvec F(\bvec r_i)$， 则面积分被定义为

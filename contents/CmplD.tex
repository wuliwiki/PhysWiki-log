...
由\autoref{CmplD_cor3} 可知，特征为$0$的域都是完全域，因此我们接下来只讨论特征为素数$p$的情况。首先，我们给出两个相似的简单引理，对之后的定理证明有帮助：


\begin{lemma}{}\label{CmplD_lem2}
...
由于证明思路是完全一样的，因此省略\autoref{CmplD_lem2} 的证明，读者可参考下面\autoref{CmplD_lem1} 的证明。
...
\begin{theorem}{}\label{CmplD_the4}
...
\autoref{CmplD_eq1} 的最后两步，应用了\autoref{CmplD_lem1} 。这么一来，$f(x)$就可约了，和“不可约”的假设矛盾。故反设不成立，故不存在不可分的不可约多项式，也就是说$\mathbb{F}$是完全域。
...
显然，$f$有重根\footnote{你也可以计算$\opn{D}f$看看是否符合\autoref{CmplD_cor2} .}，因此不可分。这个不可分不可约多项式的存在意味着$\mathbb{F}$不完全，从而得证必要性。
...
\begin{corollary}{}\label{CmplD_cor5}
...
% 由于$\mathbb{F}$是完全域，所以可以取$b_i\in\mathbb{F}$使得$b_i^p=a_i$。定义$h(x) = \sum_{i=0}^n b_ix^i$。由于$\opn{ch}\mathbb{F}=p$，据\autoref{CmplD_lem1} 知
% \begin{equation}
% \begin{aligned}
...
% \textbf{如果}$g(x)=\sum_{i=0}^nb_ix^i$在$\mathbb{F}$上是可约的，即存在$c(x)=\sum_{i=0}^rc_ix^i$和$d(x)=\sum_{i=0}^{n-r}d_ix^i$使得$h(x)=c(x)d(x)$，那么据\autoref{CmplD_lem2} ，知
% \begin{equation}
...
考虑映射$\sigma:\mathbb{F}\to\mathbb{F}$，定义为$\sigma(a)=a^p$。由\autoref{CmplD_the4} ，
\begin{equation}
...
\begin{example}{\autoref{CmplD_cor5} 的正例}

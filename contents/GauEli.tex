...
与 “高斯消元法\upref{GAUSS}” 中的步骤略有不同的是， 该程序在处理每一行 \lstinline|A(ii,:)|  时都会试图做一个行交换使系数 \lstinline|A(ii,q(ii))| 的绝对值尽可能大． 这是为了减小数值误差： 试想如果 \lstinline|A(ii,q(ii))| 的解析值为 0， 但由于数值误差， 计算出来是一个很小的数（例如 \lstinline|1e-15|）， 那么用其消元时就可能需要将第 \lstinline|ii| 行乘以一个很大的系数（例如 \lstinline|1e15|）再加到另一行上， 导致程序不稳定．

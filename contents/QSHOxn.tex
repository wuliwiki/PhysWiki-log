...
\end{equation}
其中
\begin{equation}\label{QSHOxn_eq13}
...
如果记$\psi(x)=h(x)\exp(-m\omega x^2/2)$\footnote{指数上的符号是为了让波函数在无穷远处趋于0．}，那么代入\autoref{QSHOxn_eq7} 后得
\begin{equation}\label{QSHOxn_eq8}
h''(x)-2m\omega xh'(x)+(\varepsilon-m\omega)h=0
\end{equation}
只要能解出\autoref{QSHOxn_eq8} 即可．
...
对\autoref{QSHOxn_eq3} 求关于$t$的偏导数得：
\begin{equation}\label{QSHOxn_eq9}
\ali{
...
\end{equation}
等价于
\begin{equation}\label{QSHOxn_eq10}
H_{n}(x) = 2xH_{n-1}(x)-2(n-1)H_{n-2}(x)
\end{equation}

将\autoref{QSHOxn_eq10} 与\autoref{QSHOxn_eq5} 组合后得
\begin{equation}\label{QSHOxn_eq11}
\ali{
H_n''(x)&=2nH'_{n-1}(x)\\
&=2n\cdot 2(n-1)H_{n-2}(x)\\
&=2n\cdot(2xH_{n-1}(x)-H_n(x))\qquad\text{这一步代入\autoref{QSHOxn_eq10} }\\
&=2xH'_n(x)-2nH_n(x)
}
\end{equation}
整理一下\autoref{QSHOxn_eq11} 得
\begin{equation}\label{QSHOxn_eq12}
H_n''-2xH_n'-2nH_n=0
\end{equation}

\autoref{QSHOxn_eq12} 和\autoref{QSHOxn_eq8} 形式完全相同，系数的差异可以通过合适的变量代换或单位选择来消除．将$\hbar$添回\autoref{QSHOxn_eq7} ，进行合适的变量代换（\autoref{QSHOxn_eq13} ），即可从\autoref{QSHOxn_eq8} 得到\autoref{QSHOxn_eq1} ．

...
线性映射 $f:V\to U$ 的像 $M = f(V) \subseteq U$ 是一个线性空间， 即 $U$ 的子空间\upref{SubSpc}。 称为 $f$ 的\textbf{值空间（range space）}。 在不造成歧义的情况下我们也可以把该映射记为 $f: V\to M \subseteq U$。
...
首先注意 $A$ 未必把 $Y$ 中的每个元素都射中， 即值空间 $A(X) \subseteq Y$ 只是 $Y$ 的一个子空间。 所以只有 $b \in A(X)$ 时方程有解， 否则无解（解集为空集）。 用映射的语言， 解集 $X_s$ 就是集合 $\qty{b}$ 的逆像\upref{map} $A^{-1}(\qty{b})$。

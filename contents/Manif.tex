...
需要注意的是，光滑和解析不是等价的概念．解析函数一定光滑，但光滑函数不一定解析\footnote{考虑以下函数：$f:\mathbb{R}\rightarrow\mathbb{R}$，其中对于$x>0$有$f(x)=\exp(-1/x)$，对于$x\leq 0$有$f(x)=0$．容易验证这个函数具有任意阶导函数，因此处处光滑；但是对于$x\leq 0$，它的导数都是$0$，因此泰勒展开的结果是$0$，因此$f$的泰勒展开不等于$f$，故它不是解析函数．}． 可参考幂级数\upref{anal}.

...
\pentry{引力的弱场近似\nref{nod_WeakG}，尘埃云的能动张量\nref{nod_SRFld}，曲率张量场\nref{nod_RicciC}}{nod_3ffb}
...
% 爱因斯坦场方程的引出过程中，我们考虑的是最简单的宏观模型，即\enref{尘埃云的能动张量}{SRFld}中所介绍的\textbf{理想流体}。对于任何物质，其四动量的各分量，都会随着参考系的不同而有不同取值，因此这些量只能是流形上某种量在具体坐标系中的坐标分量而已。这样一来，在流形上描述能量质量分布的量，就不能是简单的光滑函数，或者说标量场，而只能是更高阶的张量场。能描述理想流体四动量分布的张量，可以是四动量本身，也可以是能动张量，而我们会选择能动张量，这样才能和Ricci曲率张量的阶数吻合。
...
\enref{引力的弱场近似}{WeakG}一节中，我们看到了作为平直时空的微小扰动，带曲率时空中的测地线确实能描述稳定、低速、弱场近似下的引力效应。我们现在希望利用这个原则，将它推广到任意情况下的时空中去。
...
在稳定、低速、弱场近似下，$T_{00}=\rho$，而 $h_{00}=-2\Phi$\footnote{见\enref{引力的弱场近似}{WeakG}最后的讨论。}。度量张量为 $g_{\mu\nu}=\eta_{\mu\nu}+h_{\mu\nu}$，其中 $h_{\mu\nu}\ll 1$。考虑到低速近似下，流体压强很小，因此 
...
\end{equation}
再代入\enref{曲率张量场}{RicciC}中的\autoref{eq_RicciC_9}~\upref{RicciC}，誊抄如下：
\begin{equation}
...
另外，现在Wikipedia的文章\href{https://en.wikipedia.org/wiki/Einstein_field_equations}{Einstein Field Equations}中默认的场方程形式和\autoref{eq_EinEqn_9} 一致，但其符号习惯有两点与本文不同：度规张量使用$+2$号差，而Ricci张量的定义式与\enref{曲率张量场}{RicciC}中差了一个负号，负负得正了。

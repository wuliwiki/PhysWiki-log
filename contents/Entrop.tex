...
在热力学和统计力学中， \textbf{熵（entropy）}用于描述系统的无序程度， 是一个状态量， 通常记为 $S$。 例如若已知理想气体的 $P, V, n, T$ 等状态量， 就可以确定它的\enref{熵}{MacroS}。

\subsection{回顾卡诺循环}

\enref{卡诺循环}{Carnot}由两个等温过程，两个绝热过程构成。在 $P$-$V$ 图上由两条等温线和绝热线组成。对于卡诺热机（卡诺制冷机有类似的结论），系统经历 $P$-$V$ 图上一个顺时针的过程。在这个过程中系统对外做功为 $P$-$V$ 图上曲线围成的面积，在等温膨胀过程中系统吸热 $Q_1$，在等温压缩过程中系统放热 $-Q_2$（注意这里我们取负号，来表示系统在放出热量），由\enref{热力学第一定律}{Th1Law}，有
\begin{equation}
Q_1+Q_2=W~.
\end{equation}
由卡诺循环的公式\autoref{eq_Carnot_4}~\upref{Carnot}，可得\footnote{这里与\enref{卡诺热机文章}{Carnot}中不同的地方在于，系统吸热用正号表示，放热用符号表示，所以符号要做相应的调整。}
\begin{equation}
...
根据这个定义，经过任一\enref{卡诺循环}{Carnot}，墒均回到初始值：

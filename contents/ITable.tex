...
\int f(ax+b) \dd{x} = \frac 1a F(ax+b)
\end{equation}
\subsection{基本积分表}
\begin{align}
...
\begin{align}
&\int \sin^2 x \dd{x} = \frac 12 (x - \sin x\cos x) + C \quad\text{（\autoref{ITable_ex3}）}\label{ITable_eq13}
\\
&\int \cos^2 x \dd{x} = \frac 12 (x + \sin x \cos x) + C \quad\text{（\autoref{ITable_ex4}）}
\\
&\int \sec x \dd{x} = \ln\abs{\tan x + \sec x} + C \quad\text{（\autoref{ITable_ex10}）}
\\
&\int \ln x \dd{x} = x\ln x - x + C \quad\text{（\autoref{ITable_ex6}）}
\\
&\int \sqrt{a^2 - x^2}\dd{x} = \frac 12 \qty(x\sqrt{a^2 - x^2} + a^2\arcsin\frac xa) + C \label{ITable_eq14}\quad\text{（\autoref{ITable_ex12}）}
\\
...
\begin{equation}\ali{
\int \sin^2 x \dd{x} &=  \int \frac 12 \dd{x} - \frac 12\int \cos 2x \dd{x} \\
...
\begin{equation}\ali{
\int \cos^2 x \dd{x} &=  \int \frac 12 \dd{x} + \frac 12\int \cos(2x) \dd{x} \\
&=  \frac {x}{2} + \frac 14\sin(2x) = \frac 12 (x + \sin x \cos x) + C
}\end{equation}
\end{exam}
...
\begin{equation}
\int \frac 1x \dd{x} = \ln\abs{x} + C
\end{equation}
事实上， 由于 $1/x$ 在 $x=0$ 没有定义， 更广义的原函数可以取
\begin{equation}
\int \frac 1x \dd{x} = \leftgroup{
&\ln x + C_1 \quad (x>0)\\
&\ln (-x) + C_2 \quad (x<0)
}
\end{equation}
...
\int \ln x \dd{x}
\end{equation}
\bb{方法一：} 使用第二类换元法\autoref{IntCV_eq6}\upref{IntCV}
\begin{equation}
...
\end{equation}
\bb{方法二：} 直接使用分部积分法\autoref{IntBP_eq1}\upref{IntBP}， 对常数 1 积分， 对 $\ln x$ 求导， 得
\begin{equation}
\int \ln x \dd{x} = x\ln x - \int x\cdot \frac 1x \dd{x} = x\ln x - x + C
\end{equation}
\end{exam}
...
\end{equation}
使用第二类换元法\autoref{IntCV_eq6}\upref{IntCV}， 令 $x = a\sin t$ 得
\begin{equation}
\int a\cos t \dd(a\sin t) = a^2 \int \cos^2 t \dd{t}
\end{equation}
将\autoref{ITable_ex4} 的结论代入得 $a^2(t + \sin t\cos t) + C$， 再将 $t = \arcsin(x/a)$ 代入得
\begin{equation}
\int \sqrt{a^2 - x^2}\dd{x} = \frac 12 \qty(x\sqrt{a^2 - x^2} + a^2\arcsin\frac xa) + C
\end{equation}
\end{exam}
...
\begin{equation}\ali{
\int \sec x \dd{x} &= \int \frac{\sec^2 x + \sec x\tan x}{\sec x + \tan x} \dd{x} = \int \frac{u'(x)}{u} \dd{x} = \int \frac 1u \dd{u} \\

...
\end{equation}
其中星号代表复共轭。 由于函数的内积\upref{InerPd}通常定义为 $\int f^*(x) g(x) \dd{x}$ 的形式， 该式说明两列简谐波（\autoref{sub_PWave_2}~\upref{PWave}）存在某种意义上的\textbf{正交归一}，详见 “连续正交归一基底与傅里叶变换\upref{COrNoB}”。

...
\pentry{狄拉克方程\upref{qed4}}{nod_415f}

狄拉克方程\upref{qed4}是描述相对论性自由电子的方程\autoref{eq_qed4_9}~\upref{qed4}：（这里我们没有采用自然单位制，所以需要带上 $\hbar,c$）
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
其中 $\bvec p=-i\hbar\nabla$；$\bvec \alpha,\beta$ 是四维矩阵代数中的元素，满足一定的反对易关系。更常见地，上式也可以写成\autoref{eq_qed4_22}~\upref{qed4} 的形式：
\begin{equation}
...
\end{equation}
将它代入狄拉克方程。这里不妨采用 $\bvec \alpha,\beta$ 的标准表示\autoref{eq_qed4_6}~\upref{qed4}，则有
...
\end{equation}
可以将此式与\autoref{eq_scheq2_3}~\upref{scheq2} 进行对比，可以发现两种方式推导出的非相对论性自旋 1/2 粒子的波函数是一致的。

\subsection{电磁场中狄拉克方程的非相对论近似}
\pentry{电磁场中的狄拉克方程\upref{DiracE}}{nod_bfab}
缓变外场中狄拉克方程可以写为\autoref{eq_DiracE_4}~\upref{DiracE} 的形式：
\begin{equation}

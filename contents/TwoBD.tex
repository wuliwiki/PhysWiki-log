...
\end{equation}
联立\autoref{TwoBD_eq1} 和\autoref{TwoBD_eq2} 可以发现在质心系中 $\vec R, \vec r_A, \vec r_B$ 间始终存在一一对应的关系（共线且模长呈固定比例）， 所以质心系中不受外力的二体系统只有三个自由度\upref{DoF}
...
再来看系统的动能． 令 $\vec v = \dot {\vec R}$， 使用\autoref{TwoBD_eq3} 把系统在质心系中的总动能用相对位矢表示得
\begin{equation}
E_k = \frac12 (m_A \dot{\vec r}_A^2 + m_B \dot{\vec r}_B^2) = \frac12 \frac{m_A m_B}{m_A + m_B} \dot{\vec  R}^2 = \frac12 \mu \dot{\vec  v}^2
\end{equation}
这恰好是等效质点动能．
...
\subsection{动量守恒}
在质心系中， 两质点的总动量恒为零， 动量守恒． 令它们的动量分别为 $\vec p_A$ 和 $\vec p_B$， 有
\begin{equation}\label{TwoBD_eq10}
\vec p_B = m_B \dot {\vec r}_B = \mu \vec v
\end{equation}
令等效质点的动量为 $\vec p = \mu \vec v$， 则
\begin{equation}\label{TwoBD_eq11}
\vec p = \vec p_B = -\vec p_A
\end{equation}
...
\begin{equation}
\vec L = \vec r_A \cross \vec p_A + \vec r_B \cross \vec p_B
= \vec R \cross \vec p
\end{equation}

...
我们现在考虑两个仅受相互作用的质点 $A$ 和 $B$， 它们的质量分别为 $m_A$ 和 $m_B$． 由于不受系统外力， 在任何惯性系中他们的质心都会做匀速直线运动．%（未完成，引用源）



现在定义他们的\textbf{相对位矢}（也叫\textbf{相对坐标}）为点 $A$ 指向点 $B$ 的矢量
\begin{equation}\label{TwoBD_eq1}
\vec R = \vec r_B - \vec r_A
\end{equation}
且定义\text{相对速度}和\textbf{相对加速度}分别为 $\vec R$ 的导数 $\dot{\vec R}$ 和二阶导数 $\ddot{\vec R}$．

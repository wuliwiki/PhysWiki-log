...
\pentry{Navier-Stokes 方程\nref{nod_NSeq}}{nod_89e1}

描述流体微元的物理量有 $\rho,p,T,e,\bvec u$ 等，而完整描述它们随时间的演化需要流体力学方程组。它们分别是质量守恒方程（\autoref{eq_fluidC_6}~\upref{fluidC}）、动量守恒方程（\autoref{eq_NSeq_2}~\upref{NSeq}）、动能守恒方程（\autoref{eq_fluidC_5}~\upref{fluidC}）、内能守恒方程（\autoref{eq_fluidC_7}~\upref{fluidC}）、本构方程（\autoref{eq_NSeq_1}~\upref{NSeq}）和状态方程 $\rho=\rho(p,T)$。
\begin{equation}
\begin{aligned}

...
\pentry{对称矩阵的本征问题\nref{nod_SymEig}，复合命题（高中）\nref{nod_HsCoPr}，矩阵与线性映射\nref{nod_MatLS}}{nod_3543}

\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Definite_matrix}{相关页面}。}\textbf{实数正定矩阵（positive definite matrix）}定义如下。 另见 “\enref{正定矩阵（复数）}{DefMat}”， 由于复数包含实数的情况，你也可以直接学习这篇而跳过本文。
\begin{definition}{}
...
\end{definition}
其中 $\bvec v\Tr$ 表示 $\bvec v$ 的\enref{转置}{Mat}。

它对应一个对称 2-线性函数， $q(v) = \bvec v\Tr \mat A \bvec v$ 是对应的\enref{二次型}{QuaFor}。

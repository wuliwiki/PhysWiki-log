...
给定一个复流形\autoref{CMani_def1}~\upref{CMani} $G$，若 $G$ 配合二元运算 $\cdot$ 构成一个群，并且群运算映射和逆运算映射都是全纯映射，那么我们说 $G$ 是一个\textbf{复李群}．
\end{definition}
简单可证，复李群都是（实）李群．
...
\begin{definition}{李群}\label{LieGrp_def3}
...
如果我们取上述 $m$ 附近的图 $(U_m, \varphi_m)$ 和 $n$ 附近的图 $(U^n, \varphi^n)$，令 $(U, \varphi)=(U_m, \varphi_m)\times(U^n, \varphi^n)$ （见\autoref{map_def2}~\upref{map}），那么由于欧几里得空间之间的光滑映射的任意分量都是光滑映射，可知在图 $(U_m, \varphi_m)$ 和图 $(U^n, \varphi^n)$ 中 $f_m, f^n$ 分别是光滑映射．由 $m, n$ 的任意性，必要性得证．
...
\begin{example}{李群的例子}
\begin{enumerate}
\item 实数轴 $\mathbb{R}$ 和单位圆 $S^1$ 都是流形，都构成李群．见\autoref{TopGrp_ex1}~\upref{TopGrp}．
...
\begin{example}{环面的稠密单参数子群\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Lie_group}{相关页面}．}}\label{LieGrp_ex1}

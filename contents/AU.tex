...
原子单位下的物理量和公式都可以看作是无量纲的数字。 沿用 “\enref{无单位的物理公式}{NoUnit}” 中的记号， 长度、质量、时间、能量、一维波函数的转换常数分别记为 $\beta_x$， $\beta_m$， $\beta_t$， $\beta_E$， $\beta_\Psi$。
...
计算原子单位下\enref{玻尔模型}{BohrMd}中氢原子基态的能量， 假设原子核固定不动（答案：$-1/2$）。
\end{exercise}

\subsubsection{电磁学相关}
把\enref{精细结构常数}{FinStr}记为 $\alpha$。 国际单位中有 $a_0 = \hbar/(\alpha c m_e)$。 原子单位中光速时精细结构常数的倒数
\begin{equation}
c = \frac{1}{\alpha} \approx 137.036~.
\end{equation}
另外 $\epsilon_0 = 1/(4\pi)$， $\mu_0 = 4\pi\alpha^2$， $\mu_0\epsilon_0 = \alpha^2$。 对平面电磁波 $E = cB$。

注意原子单位制和经典电磁学中的\enref{高斯单位制}{GaussU}是两种不兼容的单位制，不能混用。 高斯单位制中 $\hbar\ne 1$。
...
另一个常见的例子是\enref{简谐振子}{QSHOxn}， 若使用其振动的固有频率来定义 $\beta_t$， 其薛定谔方程为
...
\subsection{转换为国际单位制}
使用 “\enref{无量纲的物理公式}{NoUnit}” 中的方法， 容易把原子单位的物理量和公式换回到国际单位， 前提是要明确每个物理量的量纲。

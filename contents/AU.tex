...
在量子力学的许多理论或数值计算中，选用\textbf{原子单位（atomic unit）}会更方便． 需要特别注意的是， 原子单位下的物理公式在使用时和其他单位制有所不同， 例如会出现 $E = \omega$ （能量等于角频率）这种看似不符合量纲分析的公式． 为了解决这个问题， 本书中我们把使用原子单位的公式都看成无单位的物理公式\upref{NoUnit}．

若声明使用原子单位， 物理量的数值后面就不需要标记单位， 例如 “氢原子基态能量为 $E = -0.5$”． 有时候为了强调我们使用原子单位， 我们会在数值后面加上 “a.u.”， 如 “$E = 0.5 \Si{a.u.}$”． 这里的 “a.u.” 可等效为 “单位 1”， 类似弧度单位 “Rad”\footnote{例如在扇形面积公式 $S = \theta R^2/2$ 中的 $\theta$ 可以看作具有单位 “Rad”， 但面积的量纲却只是 “$\Si{m^2}$” 而无需记为 “$\Si{Rad\cdot m^2}$”．}．
...
\end{equation}
与“无量纲的物理公式\upref{NoUnit}” 中的方法类似， 我们需要给公式中出现的每个量纲定义一个常数， 分别记为 $\beta_x$， $\beta_m$， $\beta_t$， $\beta_E$， $\beta_\Psi$， 且令 $x = x_a\beta_x$， $m = m_a\beta_m$， $t = t_a\beta_t$， $V = V_a\beta_E$， $\Psi = \Psi_a \beta_\Psi$ （注意 $x_a, m_a$ 等都是不带单位的）．
...
由\autoref{AU_eq5} 和\autoref{AU_eq6} 可知我们只剩下两个自由变量， 例如只要确定 $\beta_x$ 和 $\beta_m$， 剩下的 $\beta$ 也就确定了．

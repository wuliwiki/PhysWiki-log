...
质点做匀速曲线运动时， 由于速度矢量模长 $\abs{\bvec v}$ 恒定， \enref{加速度}{VnA}矢量 $\bvec a$ 完全由速度矢量 $\bvec v$ 的方向改变而产生， 就像匀速圆周运动那样。 回顾加速度的定义（\autoref{eq_VnA_4}）
...
如果使用角速度矢量（\autoref{fig_CMVD_2}） $\bvec \omega$， 那么加速度矢量可以用\enref{矢量叉乘}{Cross}表示为
\begin{equation}
\bvec a = \bvec \omega \cross \bvec v~.
\end{equation}

那么当质点经过曲线某一点时，如何求 $\omega$ 呢？ 我们可以使用\enref{曲率}{curvat}的概念。 令质点所在位置的曲率半径为 $R$， 根据曲率半径的定义（\autoref{eq_curvat_3}）， $\Delta t$ 内质点在曲线上走过的长度为 $\Delta l = v \Delta t$， 所以切线的角度变化率为
\begin{equation}

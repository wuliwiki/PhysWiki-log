...
\pentry{曲率半径\nref{nod_curvat}， 圆周运动的加速度\nref{nod_CMAD}}{nod_9860} % 未完成： 这里引用的曲率半径只是平面曲线的，怎么办？
...
\subsection{匀速曲线运动}
在以下的推导中我们会发现匀速曲线运动的加速度总是垂直于速度的方向。 加速度的大小可以直接使用匀速圆周运动的\enref{向心加速度}{CMAD}公式来计算： 只要把半径替换成曲线在质点处的\enref{曲率半径}{curvat}即可； 加速度的方向指向密切圆的圆心。
\begin{figure}[ht]
\centering
...
质点做匀速曲线运动时， 由于速度矢量模长 $\abs{\bvec v}$ 恒定， \enref{加速度}{VnA}矢量 $\bvec a$ 完全由速度矢量 $\bvec v$ 的方向改变而产生， 就像匀速圆周运动那样。 回顾加速度的定义（\autoref{eq_VnA_4}~\upref{VnA}）
\begin{equation}\label{eq_PCuvMo_2}
\bvec a = \lim_{\Delta t \to 0} \frac{\Delta \bvec v}{\Delta t}~.
\end{equation}
类似用几何法推导匀速\enref{圆周运动的速度}{CMVD}那样， 我们可以近似认为\upref{LimArc}当速度矢量 $\bvec v$ 转过一个小角度 $\Delta \theta$ 时， 它的增量 $\Delta \bvec v$ 垂直于 $\bvec v$， 且大小为
...
如果使用角速度矢量（\autoref{fig_CMVD_2}~\upref{CMVD}） $\bvec \omega$， 那么加速度矢量可以用\enref{矢量叉乘}{Cross}表示为
\begin{equation}
\bvec a = \bvec \omega \cross \bvec v~.
\end{equation}

那么当质点经过曲线某一点时，如何求 $\omega$ 呢？ 我们可以使用\enref{曲率}{curvat}的概念。 令质点所在位置的曲率半径为 $R$， 根据曲率半径的定义（\autoref{eq_curvat_3}~\upref{curvat}）， $\Delta t$ 内质点在曲线上走过的长度为 $\Delta l = v \Delta t$， 所以切线的角度变化率为
\begin{equation}
...
\end{equation}
其中第一项$\bvec a_{\tau}$为切向加速度，平行于速度方向，反映质点速度大小的改变；第二项$\bvec a_n$为法向加速度，垂直于速度方向，反映质点运动方向的改变。此外可以证明，$a_n=\frac{v^2}{R}$, R为此处的\enref{曲率半径}{curvat} \footnote{赵凯华，《新概念物理教程-力学》}

% 三角函数（复数）
% keys 复数|三角函数|三角恒等式|欧拉公式
% license Xiao
% type Tutor

\pentry{指数函数（复数）\nref{nod_CExp}}{nod_d29d}
\subsection{定义}
定义复数域的正弦函数为
...
\end{equation}
为什么三角函数要这么定义？因为只有这么定义，才能既“兼容”实数范围内的三角函数，同时满足解析的要求， 即以后会学习的\enref{柯西—黎曼条件}{CauRie}。
...
\end{equation} 
根据\enref{复数域指数函数的定义}{CExp}， 在这里具体指欧拉公式， 得
\begin{equation}
\E^{\I x} = \cos x + \I\sin x~,
\end{equation} 
\begin{equation}
...
%\subsection{解析性}
%由于 ${\E^z}$ 是解析函数，而解析函数的线性组合也是解析函数，所以正余弦函数都是解析函数%\addTODO{文章}
%。但也可以根据柯西—黎曼公式%\addTODO{链接}
%直接证明。

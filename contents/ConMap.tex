...
设 $(X,d)$ 是\enref{完备度量空间}{cauchy}, $T:X\to X$ 是其上的\enref{压缩映射}{ComMap}，那么映射 $T$ 有唯一的不动点， 即满足 $x=Tx$ 的点。 而且， 对于任意 $x_0\in X$, 点列 $\{T^nx_0\}_{n\in\mathbb{N}}$ 都收敛到这个不动点。
\end{theorem}
...
$$
如图所示， 它的图形是\enref{双曲线}{Hypb3}的一支，以对角线 $y=x$ 为渐近线。 方程 $x=f(x)$ 当然无解。 另一方面， 通过微分中值定理， 却也容易证明不等式
$$

...
\end{equation}
其中的约等号使用了微分近似（\autoref{Diff_sub1}~\upref{Diff}）。 该式可以理解成，右图中的小矩形面积约等于左图中的小竖线长度，即原函数 $F(x)$ 在 $x_i$ 到 $x_{i+1}$ 间的增量。当取极限 $\Delta x_i \to 0$ 时，上式取等号。代回\autoref{NLeib_eq1}， 有
\begin{equation}
...
现在我们可以用\autoref{DefInt_ex2}~\upref{DefInt} 中列出的两个定积分计算圆的面积。 先看第一个定积分， 由积分表\autoref{ITable_eq14}~\upref{ITable} 得
\begin{equation}

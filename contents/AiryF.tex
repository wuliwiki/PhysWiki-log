...
\pentry{常微分方程\nref{nod_ODE}， 二阶线性微分方程}{nod_95c7} % \addTODO{链接}
...
\begin{figure}[ht]
\centering
\includegraphics[width=11cm]{./figures/fdd9bf35099b0a48.pdf}
\caption{艾里函数} \label{fig_AiryF_1}
\end{figure}

艾里函数可以用\enref{反常积分}{impro}定义 % \addTODO{推导}
\begin{equation}
% 已数值验证
...
\end{equation}
其中 $\Gamma$ 是 \enref{Gamma 函数}{Gamma}。
...
\begin{figure}[ht]
\centering
\includegraphics[width=14.25cm]{./figures/811730e3113182f4.pdf}
...
\end{equation}
这需要使用 \enref{$\delta$ 函数列}{Delta}来理解。

近似的证明： 当 $L\to \infty$ 时，
\begin{equation}
\int_{-n}^{+n} \opn{Ai}(x-x_1)\opn{Ai}(x-x_2)\dd x \to \frac{k_n}{\pi}\sinc[k_n (x_1 - x_2)]~.
\end{equation}
（为什么？）其中 $k_n$ 关于 $n$ 递增， 而右边是一个 $\delta$ 函数列。%\addTODO{链接}

\subsubsection{与贝塞尔函数关系}
\pentry{贝塞尔函数\nref{nod_Bessel}}{nod_bced}
对 $x>0$，有
\begin{equation}
...
\end{equation}
其中 $K$ 和 $I$ 分别为第一类、第二类修正贝塞尔函数（见\autoref{eq_Bessel_2}~\upref{Bessel}）.
对 $x<0$，有
\begin{equation}
...
\end{equation}
其中， $J$ 为第一类贝塞尔函数（\autoref{eq_Bessel_3}~\upref{Bessel}）.
...
在一维定态薛定谔方程%\addTODO{连接}
中， 若势能函数是线性的， 即 $V(x) = px + q$， 那么方程整理后可变为\autoref{eq_AiryF_13} 的形式， 所以波函数就是\autoref{eq_AiryF_1} 的形式。 详见 “\enref{线性势能的定态薛定谔方程}{LinPot}”。 该势能的一个应用是 \enref{WKB 近似}{WKB}， WKB 近似是量子力学中解定态薛定谔方程的一种近似方法。

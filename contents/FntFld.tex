% 有限域
% keys 素域|域的扩张|伽罗瓦域|伽罗华域|Galois Field|本原元素|primitive element
% license Xiao
% type Tutor

\pentry{分裂域\upref{SpltFd}}{nod_ed22}
...
回顾\textbf{环和域}\upref{field}中的讨论，可知$\mathbb{F}$的特征一定是一个素数$p$，其素域相应为$\mathbb{F}_p$。再由\textbf{域的扩张}\upref{FldExp}讨论可知，$\mathbb{F}$应该是$\mathbb{F}_p$的有限扩张，假设$[\mathbb{F}:\mathbb{F}_p]=k$，则$\abs{\mathbb{F}}=p^k$。
...
这一点也符合\autoref{cor_SprbEx_1}~\upref{SprbEx}。$\opn{D}f(x)=p^kx^{p^k-1}-1$的最低次项是零次的，而显然$\opn{D}f\not\mid f$，故得$(f, \opn{D}f)=1$，故$f$应无重根。
\end{example}


\begin{theorem}{}\label{the_FntFld_1}
有限域的乘法群，必是循环群。
\end{theorem}
...
沿用\autoref{def_FldExp_7}~\upref{FldExp}的设定。
...
该定义和\autoref{def_FldExp_7}~\upref{FldExp}一致。

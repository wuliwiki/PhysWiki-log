...
本文介绍如何使用 Linux 命令行\upref{Linux}用 g++ 编译器\upref{gpp}编译一个 C++\upref{Cpp0} 的 Hello World 程序。 阅读本文不需要任何基础（包括 C++ 和 Linux）。

如何安装 Linux 命令行？ 你可以直接在电脑上安装一个 Linux 系统\footnote{推荐 Ubuntu， 因为小时百科大部分 Linux 相关的教程都是以 Ubuntu 为环境的。}， 也可以在 Windows 10 或 11 中使用 WSL\upref{WSLnt}。 如果使用 Win7 系统， 可以安装一个 MinGW\upref{Mingw}。 本文以 Ubuntu 系统为例（Linux 有不同的版本， Ubuntu 是广泛使用的一个）。
...
\begin{figure}[ht]
\centering
\includegraphics[width=14cm]{./figures/fa46dc686c20e28f.png}

...
\pentry{矢量的叉乘\nref{nod_Cross}}{nod_f2ac}
我们定义以下运算
\begin{equation}
(\bvec A \cross \bvec B) \vdot \bvec C~.
\end{equation}
为矢量 $\bvec A, \bvec B, \bvec C$ 的\textbf{混合积}， 即 $\bvec A$ 和 $\bvec B$ 先做叉乘， 然后再把得到的矢量与 $\bvec C$ 做\enref{内积（点乘）}{InerPd}。 注意即使我们把括号去掉这个表达式也没有歧义， 因为叉乘只能在矢量之间进行， 而点乘的结果是标量。 混合积满足
...
\subsection{代数法证明}
\pentry{行列式\nref{nod_Deter}}{nod_e52c}
不难证明三矢矢积若展开成分量的形式，等于三个矢量组成的行列式， 另见 “\enref{行列式与体积}{DetVol}”
\begin{equation}

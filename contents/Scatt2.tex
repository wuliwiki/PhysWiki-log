...
边界条件为\footnote{满足该边界条件的波函数貌似也满足归一化条件 $\delta(\vec k - \vec k')$．}
\begin{equation}
\psi_{\vec k}(\vec r) \to \frac{1}{(2\pi)^{3/2}} \qty[\E^{\vec k \vdot \vec r} + f(\uvec r) \frac{\E^{\I kr}}{r}]
\end{equation}
其中 $f(\uvec r)$ 是一个只与方向有关的函数， 叫做\bb{散射幅}． 散射幅的模方就是微分截面
\begin{equation}
\pdv{\sigma}{\Omega} = \frac{\abs{\vec j_{out}(\vec r)}r^2}{\abs{\vec j_{in}(\vec r)}}
= \abs{f(\uvec r)}^2
\end{equation}
将各项球谐展开， 令
\begin{equation}

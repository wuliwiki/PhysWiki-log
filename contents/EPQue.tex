...
\pentry{极端曲线\nref{nod_ExtCur}}{nod_9b01}
\subsection{问题的提出}\label{sub_EPQue_1}在前面的问题中，总是取以两个固定点为端点的曲线作为\enref{可取曲线}{DesCur}。现在，我们研究以更广泛的曲线作为可取曲线的泛函极值问题。
...
注意到：若某一以 $A$ 和 $B$ 为端点的曲线 $\gamma_0$ 是本问题的解，即 $\gamma_0$ 给积分\autoref{eq_EPQue_1} 以极值，那么这一曲线 $\gamma_0$ 也在连接 $A$ 及 $B$ 的所有 $C_1$ 类曲线中，给 $J$ 以极值。由最简单问题时的\enref{欧拉定理}{ElueEV}，曲线 $\gamma_0$ 满足欧拉方程
\begin{equation}
F_y-\dv{}{x}F_{y'}=0~.
\end{equation}
...
\end{enumerate}
\subsection{问题的解决}
给出问题中泛函以极值的曲线的端点满足的方程称为\enref{斜截条件}{OCCond}，解该方程就能得到所要的曲线的端点，那么所要的曲线就是端点满足斜截条件的极端曲线。

% 表示的约化
% keys 等价表示|可约表示|不可约表示|完全可约表示|特征标
% license Xiao
% type Tutor

\pentry{群代数与正则表示\upref{gpalg}，不变子空间\upref{InvSP}，直和（线性空间）\upref{DirSum}}{nod_cf8b}
...
回忆共轭元素的概念\autoref{def_gpcon_2}~\upref{gpcon}，$g^{-1}g_\alpha g=g_\beta$，则有$D^{-1}(g)D(g_\alpha)D(g)=D(g_\beta)$，则不难看出：

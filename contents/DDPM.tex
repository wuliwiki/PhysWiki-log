...
\pentry{条件概率与事件的独立性（高中）\upref{HsCpMi}，高斯分布（正态分布）\upref{GausPD}，神经网络\upref{NN}}{nod_f688}
...
反向的逆扩散过程是一个参数化的马尔科夫过程，每一步转换概率的参数通过\textbf{神经网络}\upref{NN}模型学习而获得。时间序列为：$t=T$ -> $T-1$ -> ... -> $1$ -> $0$.逆过程的起点为时间步t=T时的带有噪音的数据，其数据分布为$p_\theta(\bvec x_T)=\mathcal{N}(\bvec x_T;0,I)$.整个过程的转换概率为：
\begin{equation}

...
\pentry{球谐函数\nref{nod_SphHar}， 正交矩阵、酉矩阵\nref{nod_UniMat}， 欧拉角\nref{nod_EulerA}}{nod_5f3e}
...
\end{equation}
由于旋转算符是正交算符， 所以 Wigner D 矩阵是\enref{酉矩阵}{UniMat}。 注意该矩阵一般是复数的，因为作为基底的球谐函数一般是复数的。

\subsection{表达式}
一种定义旋转的方法就是利用 $zyz$ \enref{欧拉角}{EulerA}, 即先将 $\ket{l, m}$ 绕 $z$ 轴逆时针旋转 $\gamma$ 角， 再绕 $y$ 轴逆时针旋转 $\beta$ 角， 最后绕 $z$ 轴旋转 $\alpha$ 角。 我们将旋转算符记为 $\mathcal R(\alpha, \beta, \gamma)$。 由球谐函数的定义\autoref{eq_SphHar_1}~\upref{SphHar}， 第一个旋转得到 $\exp(-\I m \gamma)\ket{l, m}$。

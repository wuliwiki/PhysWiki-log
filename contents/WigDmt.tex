...
\subsection{表达式}
一种定义旋转的方法就是利用 $zyz$ \enref{欧拉角}{EulerA}, 即先将 $\ket{l, m}$ 绕 $z$ 轴逆时针旋转 $\gamma$ 角， 再绕 $y$ 轴逆时针旋转 $\beta$ 角， 最后绕 $z$ 轴旋转 $\alpha$ 角。 我们将旋转算符记为 $\mathcal R(\alpha, \beta, \gamma)$。 由球谐函数的定义\autoref{eq_SphHar_1}， 第一个旋转得到 $\exp(-\I m \gamma)\ket{l, m}$。

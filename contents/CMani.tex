...
对于一个复值多复变函数 $f(z_1, z_2, \dots, z_n): \mathbb C^n \to C$ 是全纯函数当且仅当 $f$ 对每个变量 $z_i$ 而言都是全纯的。
\end{definition}

\begin{definition}{多复变全纯函数（向量值）}
对于一个向量值多复变函数 $f(z_1, z_2, \dots, z_n) = (f_1, f_2, \dots, f_m): \mathbb C^n \to \mathbb C^m$，称其是全纯的当且仅当对于每个复值多复变函数 $f_i$ 而言，其对每个变量 $z_i$ 都是全纯的。
\end{definition}

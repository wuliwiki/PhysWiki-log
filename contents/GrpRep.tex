...
\pentry{群作用\nref{nod_Group3}， 域上的代数\nref{nod_AlgFie}}{nod_573d}
\subsection{表示}
...
\addTODO{群表示的等价定义，参考\enref{群作用}{Group3}}
...
% Giacomo: 要不要移动到 \enref{群代数与正则表示}{gpalg} 里？
...
\begin{example}{平凡表示}
类似平凡作用\autoref{ex_Group3_5}~\upref{Group3}，对任何群 $G$，任何向量空间 $V$，我们可以定义\textbf{平凡表示} $\rho(g) := \text{id}_V$。
\end{example}
...
注：等变映射的英译为equivariant map，有时也称\textbf{交结映射}intertwing map。

...
最广义的情况下， $l,m$ 都可以是任意复数。 本文中我们只讨整数 $l,m$，且 $l\geq 0$， $\abs{m} \leq l$ 的情况， 且只讨论区间 $x\in [-1,1]$。 这些限制在实际应用中是最常见的。 此时方程的解是\textbf{连带勒让德函数（associated Legendre function）} $P_l^m(x)$ 可用勒让德多项式\upref{Legen} $P_l(x)$ 生成

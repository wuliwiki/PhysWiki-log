...
\int \frac{1}{\sqrt{1+x^2}} \dd{x}
\end{equation}
使用第二类换元法\autoref{IntCV_eq6}\upref{IntCV}， 令 $x = \tan t$， 再利用“三角恒等式\upref{TriEqv}” 的\autoref{TriEqv_eq13} 和\autoref{ITable_eq10} 得
\begin{equation}
...
\end{equation}
注意上式中 $\ln$ 后面的绝对值符号消失是因为 $x + \sqrt{1+x^2}\ges 0 $ 恒成立． 另外由“双曲函数\upref{TrigH}”中\autoref{TrigH_ex1} 可知上式又等于 $\sinh^{-1} x + C$．
\end{exam}
...
三维直角坐标系中的一点 $P$ 的位置可以用 $(r,\theta ,\phi )$ 这 3 个有序实数来表示，称为该点的\bb{球坐标}（\autoref{Sph_fig1}）．其中 $r$ 表示该点到原点的距离 （$r \ges 0$）， 即位矢\upref{Disp}的\bb{模长}； $\theta$ 表示该点的位矢与 $z$ 轴的夹角 （$\theta  \in [0,\pi]$）， 即\bb{极角}； $\phi$ 表示该点的位矢在 $x - y$ 平面上的投影与 $x$ 轴的夹角 （$\phi  \in [0,2\pi]\text{或}[- \pi,\pi]$）， 即\bb{方位角}．注意有些教材中用 $\theta $ 表示方位角， $\phi $ 表示极角，或者将 $\phi $ 记为 $\varphi $，  $r$ 记为 $\rho $ 等，需要通过上下文判断每个坐标符号的具体含义．

\begin{figure}[ht]
\centering
\includegraphics[width=9cm]{./figures/Sph1.pdf}
...
三个坐标按照 $(r, \theta , \phi )$ 排序，是为了使对应的单位矢量满足 $\uvec r \cross \uvec \theta  = \uvec \phi $ （类比直角坐标系的三个单位矢量必须满足 $\uvec x \cross \uvec y = \uvec z$， 见矢量的叉乘\upref{Cross}）． 这也是所有\bb{正交曲线坐标系}%未完成：词条
 的要求．
 
\subsection{球坐标系中矢量的两种表示方法}
球坐标系中，矢量可以用球坐标 $(r, \theta, \phi)$ 表示，即矢量以原点为起点，以终点的球坐标表示该矢量．

更常见的方法，是将矢量投影到 3 个单位矢量上（ 当然，要说明是关于哪个点的单位矢量）， 用单位矢量的线性组合来表示．在矢量分析中，这种方法常用于表示\bb{矢量场}．%未完成：词条
...
\subsection{两方向的夹角}
\pentry{点乘\upref{Dot}}
若已知球坐标系中两个方向分别为 $(1, \theta_1, \phi_1)$ 和 $(1, \theta_2, \phi_2)$ 如何求它们之间的夹角 $\alpha$ 呢？ 我们可以先计算两个单位矢量的直角坐标， 然后对它们进行点乘即可得到两矢量夹角的余弦值。 由%未完成
， 两矢量的直角坐标分别为
\begin{equation}
(\cos\theta_1\cos\phi_1, \cos\theta_1\sin\phi_1, \sin\theta_1)
\qquad
(\cos\theta_2\cos\phi_2, \cos\theta_2\sin\phi_2, \sin\theta_2)
\end{equation}
利用三角恒等式%未完成
， 得
\begin{equation}\ali{

...
\end{equation}
其中 $\alpha$ 叫做阶数． 两个线性无关的解分别是\bb{第一类贝赛尔函数} $J_\alpha(x)$ 和\bb{第二类贝赛尔函数} $Y_\alpha(x)$． 这里只讨论 $x > 0$ 且 $\alpha$ 为整数或半整数的情况．

\begin{figure}[ht]
\centering
\includegraphics[width=13cm]{./figures/Bessel1.pdf}
...
\subsection{汉克尔函数}
贝赛尔方程的两个线性无关解也可以用第一类和第二类汉克尔函数来表示\footnote{可类比欧拉公式 $\exp(\pm\I x) = \cos x \pm \sin x$ （\autoref{CExp_eq2}\upref{CExp}）．}
\begin{equation}

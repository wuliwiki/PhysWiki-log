...
\omega &= \frac{2\pi}{T}\\
x &= v t \\
\vec L &= m\vec r \cross \vec v \\
\mathcal{E} &= \frac{q}{r^2} \\
V &= \frac{q}{r} = \mathcal{E} x \\
E = qV = q\mathcal{E} x
\end{align}
薛定谔方程为
...
\end{equation}
注意当考察对象为电子时， 式中 $m = 1$， $m$ 可不写。


\begin{exam}{电场中电子的薛定谔方程}
令\autoref{AU_eq12} 中 $m = 1$， 再将以上的 $V = \mathcal{E} x$ 代入， 得
\begin{equation}
-\frac{1}{2} \pdv[2]{\Psi}{x} + \mathcal{E} x \Psi= \I\pdv{\Psi}{t}
\end{equation}
\end{exam}

\begin{exer}{氢原子的基态能量}
计算玻尔模型中氢原子基态的能量。 答案：$-1/2 a.u.$。
\end{exer}

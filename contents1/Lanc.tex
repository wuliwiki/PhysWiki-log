...
\end{equation}
这样，幺正算符 $\exp(-\I\Q H\Delta t/\hbar)$ 也可以表示成矩阵．根据定义 % 链接未完成
\begin{equation}
\exp(-\I\Q H\Delta t/\hbar) = 1+ (-\I\Q H\Delta t/\hbar) = 1+ (-\I\Q H\Delta t/\hbar) + \frac{1}{2!} (-\I\Q H\Delta t/\hbar)^2\dots
\end{equation}
此时若把 $\Q H$ 对角化（求解本征方程），得到本征矢列矩阵 $\mat P$ （单位正交阵）以及本征值矩阵 $\mat \Lambda$ （对角矩阵），则可进行基底变换变到 $\Q H$ 矩阵的本征空间求解上式 %（矩阵指数的对角化化简，未完成）
\begin{equation}\ali{
...
\item 把 $\Psi_1$ 进行施密特正交化， $\ket{\Psi_1}=\ket{\Psi_1}/\braket{\Psi(\vec r,t)}$．  % 未完成

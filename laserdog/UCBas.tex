...
\begin{equation}
\begin{split}
\hat{H}_{at}=&-\sum_i\left(\frac{\hbar^2\nabla_i^2}{2\mu_i}+V_{ei}({\vec r})\right) + \sum_{i<j}V_{ee}({\vec r}_i-{\vec r}_j)\\
&+\sum_i [\alpha_f {\vec S}_i\cdot{\vec L}_i+\alpha_{hf}({\vec S}_i+{\vec L}_i)\cdot {\vec I}_i]
\end{split}
\end{equation}
...
除此之外，第二项还能提供一个我们所知的Hund规则，单重态 $\psi_1({\vec r}_1)\psi_2({\vec r}_2)-\psi_1({\vec r}_2)\psi_2({\vec r}_1)$ 在离的很近的时候波函数是基本没有的，也就是提供了一个repulsive的能量；这也就是说三重态相比单重态能量更低．
...
\begin{equation}
E\propto -{\vec\mu}_s\cdot{\vec B}_{\text{eff}}=\frac{Ze^2\mu_0}{8\pi m_0^2 r^3}({\vec S\cdot L})
\end{equation}
...
$^{87}{\text{Rb}}, ^6{\text{Li}}$ 在磁场 $\vec B = B\uvec z$ 中，Hamiltonian可以写成
\begin{equation}
\hat{H} = B(\mu_B J_z+\mu_N I_z)+\alpha({\vec J\cdot I})
\end{equation}

这个的处理方法实际上和传统的 $LS$ 耦合基本一样：引入一个新的参数 ${\vec F = J+I}$，从而保证
\begin{equation}
{\vec J\cdot I} = \frac{1}{2}({\vec F}^2 - {\vec J}^2 - {\vec I}^2) = \frac{1}{2} (F(F+1) - J(J+1) - I(I+1))
\end{equation}
...
\begin{figure}[ht]
\centering
\includegraphics[width=15cm]{./figures/UCBas1.png}
...
光引入的附加Hamiltonian是：
\begin{equation}
\hat{H}_d = {\vec d\cdot E} = d_j E_j^0 \cos(\omega t-\phi_j)
\end{equation}

其中 ${\vec d} = \sum_\alpha (-e{\vec r}_\alpha)$，代表原子的偶极矩．
...
\begin{equation}
\mathcal{P}_g = \frac{{\vec L}^2}{2\hbar},\quad \mathcal{P}_e = 1 - \mathcal{P}_g
\end{equation}
...
我们前面分析了，无外场
\begin{equation}
\hat{H}_{at} = E_e\mathcal{P}_e + \frac{A_{f}}{\hbar^2}\vec L\cdot \vec S
\end{equation}
...
因为包含精细结构 $A_f\vec L\cdot \vec S/\hbar^2$ 的项已经很小了，其在这个Unitary变换下的变化可以忽略． 而另一方面，一个unitary的变换，如果是含时的，则会带来一个变化，是由于要保证Hamiltonian的定义正确\footnote{注意：这里我的这个note写的比较乱，但是我觉得可以阅读；我也懒得改了．}
\begin{equation}
...
\end{equation}
\begin{equation}
\hat{H}'_{at} = E_e \mathcal{P}_e - \hbar\omega\mathcal{P}_e + \frac{A_{f}}{\hbar^2}\vec L\cdot \vec S 
\end{equation}
...
$\vec 1$. 极端情况下，可以忽略精细结构，也就是说 $A_f = 0$． 这时候 $\mathcal{D}_{ij}$ 就不再是一个很复杂的算符，而是简单地
\begin{equation}
...
其中，很显然的可以得到
\begin{equation}
u_s = -\frac{1}{12\Delta_e} |\langle l=0|{\vec d}|l = 1\rangle|^2
\end{equation}

因此，有效Hamiltonian为
\begin{equation}
\hat{H}_{\text{eff}} = u_s {\vec E}^2
\end{equation}

这说明什么呢？这说明没有精细结构的话，\bb{光是没有办法改变电子的自旋的（Hamiltonian里面没有包含自旋与其它参数耦合的项）}；而且，这种作用并不依赖光的偏振，而仅仅和光的强度有关(${\vec E}^2$)；利用这点可以很好的进行实验上的\bb{光陷阱，从而捕捉原子}；而 $\Delta_e$ 实际上与原子的速度有关（Doppler效应），于是说给激光制冷提供了一个实验的理论指导．

{$\vec 2$. }考虑精细结构（当然很小的精细结构）的时候的能级分裂
...
\begin{equation}
\hat{H}_{at} = \frac{1}{\Delta_e}\mathcal{P}_e-\frac{1}{\Delta_e}\frac{A_f}{\hbar^2}{\vec L\cdot \vec S}\hat{H}_{at}^{-1} 
\end{equation}

从而，我们可以分出来和之前一样的 $\mathcal{D}_{ij}$ 部分，和精细结构带来的效应
\begin{equation}
\mathcal{D}_{ij} = \mathcal{P}_g d_i \frac{1}{\Delta_e} d_j \mathcal{P}_g - \alpha \mathcal{P}_g d_i {\vec L\cdot \vec S}d_j\mathcal{P}_g,\quad \alpha \equiv A_f/(\hbar^2\Delta_e)
\end{equation}
...
因此
\begin{equation}
d_i {\vec L\cdot \vec S}d_j = {\vec L\cdot \vec S}d_id_j + [{\vec L\cdot \vec S}, d_i]d_j
\end{equation}

第一项很显然等于0，因为这是作用在基态上的，而基态的话 $L=0, j = s, {\vec L\cdot \vec S} = j(j+1) - s(s+1) = 0$，因此我们有
\begin{equation}
...
化简可以得到
\begin{equation}
\hat{H}_{\text{eff}} = u_s|{\vec E}|^2 + \I \alpha\hbar u_s ({\vec E^*\times \vec E )\cdot S}
\end{equation}

可以注意到，这样的光会使得自旋的极化． 例如，圆偏振，${\vec E} = E_0 ({\vec e}_x + \I{\vec e}_y)$，我们会发现
\begin{equation}
{\vec E^*\times E} = 2\I E_0^2{\vec e}_z
\end{equation}
